...
def make_report(
    ...
    high_b2 = by_subset[
        ...
        & (by_subset["method"].isin(["peak_calibrated", "template_fit_calibrated", "ml_hgb"]))
...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/p04_amplitude_charge_regression.yaml")
    ...
    tmpl_scale = template_scales(meta, wave, templates, bins, [float(x) for x in config["template_shift_grid"]])
    ...
    methods["template_fit_calibrated"] = {
    ...
    best_trad_amp = float(amp_rows.loc[["peak_calibrated", "template_fit_calibrated"], "res68_abs_frac"].min())
    ml_charge = float(charge_rows.loc["ml_hgb", "res68_abs_frac"])
    best_trad_charge = float(charge_rows.loc[["integral_calibrated", "template_fit_calibrated"], "res68_abs_frac"].min())
    finding = (
        ...
        f"versus {best_trad_charge:.4f}. The duplicate readout is already very strongly predicted "
        "by calibrated peak/integral baselines, and the much smaller ML error is treated as a "
        "duplicate-readout closure result rather than an absolute true-energy calibration."
    )
    ...
    output_files = ["REPORT.md", "result.json", "benchmark.csv", "benchmark_by_subset.csv", "counts_by_run.csv"]
    manifest = {
        "study": "P04",
        "ticket_id": config["ticket_id"],
        "command": "/home/billy/anaconda3/bin/python scripts/p04_amplitude_charge_regression.py --config configs/p04_amplitude_charge_regression.yaml",
