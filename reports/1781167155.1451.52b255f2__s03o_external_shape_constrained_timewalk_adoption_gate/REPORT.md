# Study report: S03o - External-shape constrained timewalk adoption gate

- **Ticket:** 1781167155.1451.52b255f2
...
- **Config:** `configs/s03o_1781167155_1451_52b255f2_external_shape_timewalk_adoption_gate.yaml`
- **Winner named in `result.json`:** `traditional_hier_amp` with excluded-support sigma68 = 1.892 ns, run-block 95% CI [1.588, 2.335] ns.

## 0. Question

Does the S03n/S03m timewalk correction remain adoption-safe when the support is constrained by an external waveform-shape proxy rather than ordinary in-distribution residual performance? The atomic decision is whether a transparent analytic timewalk model, or one of several ML/NN residual correctors, has the smallest held-out pairwise timing width on the externally constrained support without leakage.
...
| ---------------------------------- | ------------ | ---------- | ----- | --------- | ---- |
| total selected B-stave pulses      | 640737       | 640737     | 0     | 0         | 1    |
| sample_ii_analysis selected_pulses | 125096       | 125096     | 0     | 0         | 1    |
| sample_ii_analysis B2              | 88213        | 88213      | 0     | 0         | 1    |
| sample_ii_analysis B4              | 21229        | 21229      | 0     | 0         | 1    |
| sample_ii_analysis B6              | 11148        | 11148      | 0     | 0         | 1    |
| sample_ii_analysis B8              | 4506         | 4506       | 0     | 0         | 1    |
...
The external shape support is predeclared from waveform observables only:

\[
I_i = 1[L_i > 5.85,\; p_i \ge 9,\; s_i > 0.25],
\]

where \(L_i=\sum_{j=9}^{17} w_{ij}/A_i\) is late normalized charge, \(p_i\) is peak sample, and \(s_i=\overline{w/A}_{10:14}-\overline{w/A}_{5:8}\). An event is in the blinded support if any downstream pulse satisfies \(I_i=1\). These quantities implement the external q-template/beamline-style support constraint requested by the ticket and do not use residual labels, same-event partner times, event order, or run identity.
...
```bash
.venv/bin/python scripts/s03o_1781167155_1451_52b255f2_external_shape_timewalk_adoption_gate.py --config configs/s03o_1781167155_1451_52b255f2_external_shape_timewalk_adoption_gate.yaml
```
