...
architecture for saturation-aware waveform recovery.  The worker is `testbeam-laptop-3`.  The
...
The traditional comparator is **analytic_clipped_template_sideband_traditional**, which stands in for the requested family of two-template least squares, sparse deconvolution, censored likelihood, clipped-template inversion, and analytic pile-up timing separation methods.
...
baseline for this short-window setting.  The new architecture is **saturation_residual_fusion_new**:
...
The table below exposes the ticket-requested pile-up detection AUC, timing bias, and recovery failure-rate intervals.  `pileup_miss_rate` is the failed injected-overlap recovery rate; the companion clean-control false split rate is included in the overall table above.

| method                                         |   detection_auc |   detection_auc_ci_low |   detection_auc_ci_high |   time_bias_ns |   time_bias_ns_ci_low |   time_bias_ns_ci_high |   pileup_miss_rate |   pileup_miss_rate_ci_low |   pileup_miss_rate_ci_high |
|:-----------------------------------------------|----------------:|-----------------------:|------------------------:|---------------:|----------------------:|-----------------------:|-------------------:|--------------------------:|---------------------------:|
| saturation_residual_fusion_new                 |          0.8398 |                 0.8189 |                  0.8611 |        -0.2155 |               -1.167  |                 0.5238 |             0.341  |                    0.2923 |                     0.3924 |
| gradient_boosted_trees                         |          0.8393 |                 0.8219 |                  0.8575 |        -0.3938 |               -0.9433 |                 0.1934 |             0.3538 |                    0.3103 |                     0.4026 |
| ridge                                          |          0.8493 |                 0.8228 |                  0.8741 |        -1.051  |               -2.372  |                -0.2593 |             0.3051 |                    0.2513 |                     0.359  |
| 1d_cnn                                         |          0.8162 |                 0.7619 |                  0.8554 |        -1.768  |               -3.039  |                -0.8385 |             0.2667 |                    0.2282 |                     0.3154 |
| analytic_clipped_template_sideband_traditional |          0.6226 |                 0.6001 |                  0.6475 |         0.5086 |               -0.6879 |                 1.893  |             0.5846 |                    0.559  |                     0.6103 |
| tiny_sequence_transformer                      |          0.7969 |                 0.743  |                  0.8397 |       -15.75   |              -17.8    |               -14.56   |             0.5949 |                    0.5487 |                     0.6385 |
| mlp                                            |          0.8493 |                 0.8081 |                  0.8835 |         0.3222 |               -0.4328 |                 1.327  |             0.3846 |                    0.3051 |                     0.4718 |
...
boundary movement under a shared run-held-out design.  These strata are
used to interpret pulse-shape deformation, timing movement, pile-up detection,
saturation severity, pedestal sensitivity, energy calibration drift, and PID
boundary movement under a shared run-held-out design.  These strata are
used to interpret pulse-shape deformation, timing movement, pile-up detection,
...
For ticket #2474, use `saturation_residual_fusion_new` as the preferred S54b controlled-overlay pile-up/saturation recovery method
...
Runtime was `60.9` s on `Linux-5.15.0-139-generic-x86_64-with-glibc2.35`.


## Queue Provenance

The required single claim command was run once as `tn-ticket claim testbeam-laptop-3 --project testbeam` and returned the known null pseudo-ticket output `null / # null / null`.  The testbeam queue was not empty and no issue was attached to this worker, so issue `#2474` was manually recovered without a second claim attempt using `gh issue edit 2474 --repo SzeChunYiu/factory-tickets --add-label factory:claimed --add-label worker:testbeam-laptop-3 --remove-label factory:open`.  Completion was recorded with `tn-ticket done 2474`.  No novel follow-up ticket was appended.
