...
tail-knockout 1D-CNN, and a phase-stability gated ensemble.  The target is the
CFD-aligned, amplitude-normalized waveform; peak-phase swaps, rising-edge
ablations, shuffled-phase controls, and too-good triggers test whether no-tail
q gains are physically carried by early waveform information.
"""
...
def add_strata(config: dict, table: pd.DataFrame, aligned: np.ndarray) -> pd.DataFrame:
    ...
    area_over_amp = work["area_adc_samples"].to_numpy(dtype=float) / np.maximum(work["amplitude_adc"].to_numpy(dtype=float), 1.0)
    work["rise_edge_proxy"] = area_over_amp
    work["rise_edge_bin"] = pd.qcut(pd.Series(area_over_amp).rank(method="first"), 4, labels=["edge_q1", "edge_q2", "edge_q3", "edge_q4"]).astype(str)
...
def build_traditional_templates(config: dict, table: pd.DataFrame, aligned: np.ndarray, train_idx: np.ndarray):
    ...
    templates: Dict[Tuple[str, int, str, str, str, str], np.ndarray] = {}
    fallback_phase: Dict[Tuple[str, int, str, str], np.ndarray] = {}
    ...
    for key, group in train.groupby(["stave", "amp_bin", "peak_phase_bin", "rise_edge_bin"], observed=True):
        if len(group) >= min_bin:
            fallback_phase[(str(key[0]), int(key[1]), str(key[2]), str(key[3]))] = np.nanmedian(aligned[group.index.to_numpy()], axis=0)
    for key, group in train.groupby(["stave", "amp_bin", "current_stratum", "saturation_bin", "peak_phase_bin", "rise_edge_bin"], observed=True):
        if len(group) >= min_bin:
            templates[(str(key[0]), int(key[1]), str(key[2]), str(key[3]), str(key[4]), str(key[5]))] = np.nanmedian(aligned[group.index.to_numpy()], axis=0)

    def predict(rows: pd.DataFrame) -> np.ndarray:
        pred = []
        for row in rows.itertuples():
            full = (str(row.stave), int(row.amp_bin), str(row.current_stratum), str(row.saturation_bin), str(row.peak_phase_bin), str(row.rise_edge_bin))
            phase_key = (str(row.stave), int(row.amp_bin), str(row.peak_phase_bin), str(row.rise_edge_bin))
            amp_key = (str(row.stave), int(row.amp_bin))
            if full in templates:
                pred.append(templates[full])
            elif phase_key in fallback_phase:
                pred.append(fallback_phase[phase_key])
...
def feature_matrix(config: dict, table: pd.DataFrame, mode: str) -> Tuple[np.ndarray, List[str]]:
    ...
    elif mode == "no_phase_edge":
        keep = [name not in {"area_over_amp", "peak_sample"} for name in names]
        names = [name for name, ok in zip(names, keep) if ok]
        arr = [col for col, ok in zip(arr, keep) if ok]
        for current in sorted(config["current_strata"]):
            names.append("current_{}".format(current))
            arr.append((table["current_stratum"].to_numpy() == current).astype(float))
        ...
        rows.append(
            {
                ...
                "rise_edge_bin": row["rise_edge_bin"],
...
def add_too_good_flags(rows: pd.DataFrame, config: dict) -> pd.DataFrame:
    base = rows[rows["method"] == "traditional_empirical_template"][
        ["row_id", "q_template_mse", "timing_abs_error_ns"]
    ].rename(columns={"q_template_mse": "trad_q_template_mse", "timing_abs_error_ns": "trad_timing_abs_error_ns"})
    out = rows.merge(base, on="row_id", how="left")
    q_frac = float(config.get("too_good_q_fraction", 0.5))
    timing_worse = float(config.get("too_good_timing_worse_ns", 0.5))
    out["too_good_trigger"] = (
        (out["method"] != "traditional_empirical_template")
        & (out["q_template_mse"] < q_frac * out["trad_q_template_mse"])
        & (out["timing_abs_error_ns"] > out["trad_timing_abs_error_ns"] + timing_worse)
    )
    out.loc[out["method"] == "traditional_empirical_template", "too_good_trigger"] = False
    return out


def run_summary(rows: pd.DataFrame, config: dict) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    metrics = ["q_template_mse", "tail_mse", "live10_abs_error_ns", "tau_eff_abs_error_ns", "timing_abs_error_ns", "secondary_abs_error", "too_good_trigger"]
    per_run = []
    for (method, run), group in rows.groupby(["method", "heldout_run"], observed=True):
        ...
        for metric in metrics:
            rec["too_good_trigger_rate" if metric == "too_good_trigger" else metric] = float(np.nanmean(group[metric]))
    ...
    summary = []
    value_cols = ["q_template_mse", "tail_mse", "live10_abs_error_ns", "tau_eff_abs_error_ns", "timing_abs_error_ns", "secondary_abs_error", "too_good_trigger_rate", "timing_sigma68_ns"]
...
def action_atlas(rows: pd.DataFrame, summary: pd.DataFrame, secondary: pd.DataFrame, config: dict) -> Tuple[pd.DataFrame, pd.DataFrame]:
    needed = {"trad_q_template_mse", "trad_tail_mse", "trad_live10_abs_error_ns", "trad_timing_abs_error_ns", "trad_secondary_abs_error"}
    if needed.issubset(set(rows.columns)):
        merged = rows.copy()
    else:
        partial_trad_cols = [col for col in rows.columns if col.startswith("trad_")]
        base_rows = rows.drop(columns=partial_trad_cols)
        merged = base_rows.merge(base_rows[base_rows["method"] == "traditional_empirical_template"][["row_id", "q_template_mse", "tail_mse", "live10_abs_error_ns", "timing_abs_error_ns", "secondary_abs_error"]].rename(columns={
...
def write_report(out_dir: Path, config: dict, repro: pd.DataFrame, summary: pd.DataFrame, deltas: pd.DataFrame, secondary: pd.DataFrame, atlas_support: pd.DataFrame, sentinels: pd.DataFrame, result: dict) -> None:
    ...
    winner = nonsentinel.sort_values(["q_template_mse", "tail_mse", "too_good_trigger_rate"]).iloc[0]
    head = nonsentinel[["method", "n_runs", "n_rows", "q_template_mse", "tail_mse", "timing_sigma68_ns", "live10_abs_error_ns", "tau_eff_abs_error_ns", "secondary_abs_error", "too_good_trigger_rate"]].copy()
    delta_tab = deltas[(deltas["metric"].isin(["q_template_mse", "tail_mse", "timing_sigma68_ns", "live10_abs_error_ns", "secondary_abs_error", "too_good_trigger_rate"])) & ~deltas["method"].str.startswith("sentinel")].copy()
    ...
    lines = [
        ...
        "",
        "The winner named in `result.json` is **{}** by the primary ordering: minimum q-template MSE among non-sentinel methods, then tail MSE, then too-good trigger rate. Its q-template MSE is {:.6g} with CI [{:.6g}, {:.6g}].".format(result["winner"], float(winner["q_template_mse"]), float(winner["q_template_mse_ci_low"]), float(winner["q_template_mse_ci_high"])),
        "",
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/p10n_1781078146_875_034f6846_peak_phase_counterfactual.json")
    ...
    X_no_phase_edge, no_phase_edge_names = feature_matrix(config, table, "no_phase_edge")
    ...
    for heldout in sorted(table.iloc[eval_idx_all]["run"].unique()):
        ...
        for family, X in [("ridge", X_nominal), ("gbt", X_nominal), ("mlp", X_nominal)]:
            ...
            name = {"ridge": "ridge_no_tail_phase_model", "gbt": "gradient_boosted_trees_no_tail_phase_model", "mlp": "mlp_no_tail_phase_model"}[family]
            method_preds[name] = pred
            score_predictions(pred_rows, table, aligned, eval_idx, name, pred, obs_metrics, rel_grid, sample_period)
            Xcf = X[eval_idx].copy()
            for cf_name in ["area_over_amp", "peak_sample"]:
                if cf_name in feature_names:
                    j = feature_names.index(cf_name)
                    Xcf[:, j] = rng.permutation(Xcf[:, j])
            score_predictions(pred_rows, table, aligned, eval_idx, "sentinel_{}_peak_phase_swapped".format(family), model.predict(Xcf).astype(np.float32), obs_metrics, rel_grid, sample_period)

            ab_model = fit_tabular(config, family, X_no_phase_edge[fit_idx], y_full[fit_idx])
            ab_model.fit(X_no_phase_edge[fit_idx], y_full[fit_idx])
            score_predictions(pred_rows, table, aligned, eval_idx, "sentinel_{}_rising_edge_ablation".format(family), ab_model.predict(X_no_phase_edge[eval_idx]).astype(np.float32), obs_metrics, rel_grid, sample_period)

        cnn_predict, cnn_device = train_cnn(config, aligned, X_nominal, fit_idx, rng)
        pred_cnn = cnn_predict(eval_idx)
        method_preds["cnn_tail_knockout_no_tail_phase_model"] = pred_cnn
        score_predictions(pred_rows, table, aligned, eval_idx, "cnn_tail_knockout_no_tail_phase_model", pred_cnn, obs_metrics, rel_grid, sample_period)

        pred_gbt = method_preds["gradient_boosted_trees_no_tail_phase_model"]
        ...
        score_predictions(pred_rows, table, aligned, eval_idx, "phase_stability_gated_cnn_gbt_ensemble", pred_gated, obs_metrics, rel_grid, sample_period)
    ...
    rows = add_too_good_flags(pd.DataFrame(pred_rows), config)
    ...
    nonsentinel = summary[~summary["method"].str.startswith("sentinel")].copy()
    winner_row = nonsentinel.sort_values(["q_template_mse", "tail_mse", "too_good_trigger_rate"]).iloc[0]
    winner = str(winner_row["method"])
    next_ticket = {
        "title": "P10o downstream timing closure for P10n phase-stable no-tail gains",
        "body": "Question: do P10n no-tail q-template gains that survive peak-phase swaps and rising-edge ablations improve independent downstream same-particle timing? Traditional: freeze the P10n phase-binned empirical template and S02/S03 timing baselines. ML/NN: freeze the P10n winner and phase-stability gated ensemble without refitting. Metric: downstream sigma68/full RMS, >5 ns tail fraction, and too-good trigger carryover with run-block bootstrap CIs. Expected information gain: tests whether phase-stable q gains survive a physics timing consumer rather than only waveform reconstruction."
    }
    ...
    result = {
        ...
        "winner": winner,
        "winner_rule": "lowest q_template_mse among non-sentinel methods, then tail_mse, then too_good_trigger_rate",
    ...
    manifest = {
        "ticket_id": config["ticket_id"],
        "study": config["study_id"],
        "command": "/home/billy/anaconda3/bin/python scripts/p10n_1781078146_875_034f6846_peak_phase_counterfactual.py --config {}".format(config_path),
        "git_commit": result["git_commit"],
        "platform": platform.platform(),
        "python": platform.python_version(),
        "config": str(config_path),
        "config_sha256": sha256_file(config_path),
        "script": "scripts/p10n_1781078146_875_034f6846_peak_phase_counterfactual.py",
        "script_sha256": sha256_file(ROOT / "scripts/p10n_1781078146_875_034f6846_peak_phase_counterfactual.py"),
        "support_scripts": [
