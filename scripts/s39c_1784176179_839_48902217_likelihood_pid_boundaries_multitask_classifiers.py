#!/usr/bin/env python3
"""S39c likelihood PID boundary versus multitask classifier benchmark.

The ticket asks where a traditional likelihood-ratio PID boundary built from
charge, timing, and template residuals disagrees with learned pulse
representations. This runner uses the keyed digitized GEANT4 native-join
benchmark as the fixed model panel, rescans raw ROOT for the selected-pulse
anchor, and adds S39c-specific nuisance residualization, boundary-disagreement
diagnostics, run/species bootstrap intervals, and stress-slice diagnostics.
"""
...
def boundary_disagreement_table(held: pd.DataFrame) -> pd.DataFrame:
    trad = REQUIRED_METHODS["strong_traditional"]
    index_cols = ["event_id", "source_run", "pid_name"]
    base = held[held["method"].eq(trad)][index_cols + ["pid_score", "pid_label_pred"]].rename(
        columns={"pid_score": "traditional_pid_score", "pid_label_pred": "traditional_pid_pred"}
    )
    rows = []
    dims = ["overall", "source_run", "pid_name", "energy_bin", "timing_bin", "pileup_bin", "saturation_bin", "pedestal_bin", "stave"]
    for method, group in held[~held["method"].eq(trad)].groupby("method", sort=True):
        joined = group.merge(base, on=index_cols, how="inner")
        joined["boundary_disagreement"] = joined["pid_label_pred"].ne(joined["traditional_pid_pred"])
        joined["traditional_boundary_margin"] = np.abs(joined["traditional_pid_score"] - 0.5)
        joined["method_boundary_margin"] = np.abs(joined["pid_score"] - 0.5)
        for dim in dims:
            iterator = [("all", joined)] if dim == "overall" else joined.groupby(dim, sort=True)
            for value, sub in iterator:
                if len(sub) < 12:
                    continue
                y = sub["pid_label"].to_numpy(dtype=int)
                disag = sub["boundary_disagreement"].to_numpy(dtype=bool)
                rows.append(
                    {
                        "method": method,
                        "slice": dim,
                        ...
                        "boundary_disagreement_rate": float(disag.mean()),
                        "disagreement_error_rate": float((sub.loc[disag, "pid_label_pred"] != sub.loc[disag, "pid_label"]).mean()) if disag.any() else 0.0,
                        "agreement_error_rate": float((sub.loc[~disag, "pid_label_pred"] != sub.loc[~disag, "pid_label"]).mean()) if (~disag).any() else float("nan"),
                        "traditional_auc": safe_auc(y, sub["traditional_pid_score"].to_numpy(dtype=float)),
                        "method_auc": safe_auc(y, sub["pid_score"].to_numpy(dtype=float)),
                        "method_minus_traditional_auc": safe_auc(y, sub["pid_score"].to_numpy(dtype=float)) - safe_auc(y, sub["traditional_pid_score"].to_numpy(dtype=float)),
                        "traditional_boundary_margin_median": float(np.median(sub["traditional_boundary_margin"])),
                        "method_boundary_margin_median": float(np.median(sub["method_boundary_margin"])),
                    }
                )
    return pd.DataFrame(rows)


def build_tables(pred: pd.DataFrame, config: dict) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    disagreement_df = boundary_disagreement_table(held)
    return summary, by_run_df, strata_df, audit_df, disagreement_df
...
def write_report(out: Path, config: dict, result: dict, source_result: dict) -> None:
    ...
    disagreement = pd.read_csv(out / "boundary_disagreement_metrics.csv")
    ...
    lines = [
        ...
        "",
        md_table(disagreement, ["method", "slice", "value", "n_events", "boundary_disagreement_rate", "disagreement_error_rate", "agreement_error_rate", "traditional_auc", "method_auc", "method_minus_traditional_auc", "traditional_boundary_margin_median", "method_boundary_margin_median"], n=140),
        "",
        ...
        "## Verdict",
        "",
        f"`{winner}` is the S39c winner. The residualization table shows whether the method keeps PID separation after timing, energy, pedestal, pile-up, saturation, dE/dx, depth, and gross shape controls; the disagreement and stress-strata tables show where likelihood PID boundaries diverge most from learned pulse representations.",
        "",
        "## Reproducibility",
        "",
        "```bash",
        f"/home/billy/anaconda3/bin/python scripts/s39c_1784176179_839_48902217_likelihood_pid_boundaries_multitask_classifiers.py --config configs/s39c_1784176179_839_48902217_likelihood_pid_boundaries_multitask_classifiers.json",
        "```",
    ]
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=ROOT / "configs/s39c_1784176179_839_48902217_likelihood_pid_boundaries_multitask_classifiers.json")
        ...
        [
            {
                ...
                "s39c_direct_raw_root_scan": True,
                "source_crosscheck": str((source / "reproduction_match_table.csv").relative_to(ROOT)),
            }
        ]
    )
    if selected != expected:
        raise RuntimeError(f"S39c direct raw ROOT reproduction failed: {selected} != {expected}")
    ...
    summary, by_run, strata, audit, disagreement = build_tables(pred, config)
    ...
    disagreement.to_csv(out / "boundary_disagreement_metrics.csv", index=False)
    ...
    result = {
        ...
        "winner": {
            ...
            "criterion": "minimum S39c residualized PID/stress composite score",
        ...
        "artifacts": {
            ...
            "boundary_disagreement_metrics": "boundary_disagreement_metrics.csv",
