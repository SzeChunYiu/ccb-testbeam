#!/usr/bin/env python3
"""S03g HGB timewalk feature monotonicity and leakage audit."""
...
def summarize_records(records: pd.DataFrame, rng: np.random.Generator, n_boot: int, by_run: bool) -> pd.DataFrame:
    ...
    for method, group in records.groupby("method"):
        ...
        for metric in ["sigma68_ns", "full_rms_ns", "tail_frac_abs_gt5ns", "bias_vs_log_amp_slope_ns"]:
...
def paired_delta_rows(records: pd.DataFrame, primary_ml: str, primary_trad: str, rng: np.random.Generator, n_boot: int) -> pd.DataFrame:
    ...
    comparisons = [
        ...
        (primary_ml, "signed_shared_shrinkage"),
...
def run_one_fold(
    ...
    return (
        ...
        pd.concat([signed_cv, binned_cv.assign(variant="monotone_residual_table")] + cv_parts, ignore_index=True),
    )
...
def write_report(
    ...
    lines = [
        ...
        f"Original HGB sigma68 is `{result['ml']['hgb_full_unconstrained']['sigma68_ns']:.3f} ns` versus S03a `{result['traditional']['s03a_amp_only']['sigma68_ns']:.3f} ns`, signed shrinkage `{result['traditional']['signed_shared_shrinkage']['sigma68_ns']:.3f} ns`, and monotone table `{result['traditional']['monotone_residual_table']['sigma68_ns']:.3f} ns`.",
        ...
        "",
        "```bash",
        f"{sys.executable} scripts/s03g_1781020297_470_075250d8_hgb_monotonicity_audit.py --config {config_path}",
        "```",
        "",
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/s03g_1781020297_470_075250d8_hgb_monotonicity_audit.yaml")
    ...
    for heldout_run in all_runs:
        print(f"S03g fold heldout_run={heldout_run}", flush=True)
    ...
    verdict = (
        "hgb_gain_survives_monotone_log_amp_and_shape_ablation_no_leakage_flag"
        if monotone_cost < 0.15 and shape["sigma68_ns"] < amp["sigma68_ns"] and not leakage_flag
        else "hgb_gain_partly_nonmonotone_or_control_limited"
    )
    ...
    write_report(out_dir, config, config_path, reproduction, reference_repro, per_run, pooled, deltas, leakage, result)
