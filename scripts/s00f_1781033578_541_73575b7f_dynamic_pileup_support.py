...
counts exactly, then compares baseline-excursion dynamic-only pulses to exact
matched S00 controls. The ML panel is deliberately diagnostic: run labels,
event identifiers, current labels, selector amplitudes, and the baseline
excursion cut variable are excluded from learned feature matrices.
"""
...
CONFIG = ROOT / "configs/s00f_1781033578_541_73575b7f_dynamic_pileup_support.json"
...
def write_report(out_dir: Path, config: dict, reproduction: pd.DataFrame, support: pd.DataFrame, metrics: pd.DataFrame, strata: pd.DataFrame, bench: pd.DataFrame, leakage: pd.DataFrame, decision: Dict[str, object], result: Dict[str, object]) -> None:
    winner = result["winner"]["method"]
    report = f"""# S00f: dynamic-only baseline-excursion pile-up support map
...
This study tests whether the dynamic-range-only selector excess, restricted to the dominant baseline-excursion morphology, is a useful pile-up support region or a selector artifact. The raw ROOT reproduction gate returns **{result['reproduction']['dynamic_only']:,}** dynamic-only pulses, exactly matching the S00a/S00d value of 65,636. After exact matching to S00 controls by run, current label, stave, dynamic-amplitude bin, and P02-style downstream topology proxy, the physics-facing verdict is **{decision['verdict']}**. The benchmark winner recorded in `result.json` is **{winner}**.
...
The topology is the raw-root downstream/stave multiplicity proxy used here for P02-style support matching. Exact matched coverage is **{decision['matched_coverage']:.3f}**: {decision['matched_dynamic_rows']:,} dynamic baseline-excursion pulses and {decision['matched_control_rows']:,} S00 controls.
...
def main() -> None:
    ...
    result = {
        ...
        "next_tickets": [
            {
                "title": "S00g: dynamic-only topology proxy replacement with joined P02 latent labels",
                "body": "Replace the S00f raw multiplicity topology proxy with a joined P02 latent/topology label for every dynamic-only row, then rerun the same exact run/current/amplitude/topology matching and pile-up proxy deltas to test whether the selector-artifact verdict is invariant to the topology definition."
            }
    ...
    manifest = {
        ...
        "script": "scripts/s00f_1781033578_541_73575b7f_dynamic_pileup_support.py",
    ...
    write_report(out_dir, config, reproduction, support, metrics, strata, bench, leakage, decision, result)
