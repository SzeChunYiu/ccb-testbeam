...
digitizes hibeam_g4 Sci_bar truth into four B-stave gain-transition ADC
waveforms. It reruns the S27b-style panel (Birks lookup, ridge,
gradient-boosted trees, MLP, 1D-CNN, transformer, physics-residual MLP, and a
gated residual CNN) on simulated ADC waveforms with known deposited energy and
compares the ordering to the real S27b held-out run reference. Real HRD events
are not event-aligned to GEANT4 truth, so the comparison is method-level rather
than event-level.
"""
...
class ResidualMLP(nn.Module):
    def __init__(self, n_tab: int):
        super().__init__()
        self.head = nn.Sequential(nn.Linear(n_tab, 64), nn.ReLU(), nn.Linear(64, 32), nn.ReLU(), nn.Linear(32, 1))
...
class WaveformTransformer(nn.Module):
    def __init__(self, n_tab: int):
        ...
        self.pos = nn.Parameter(torch.zeros(1, 18, 32))
        enc = nn.TransformerEncoderLayer(d_model=32, nhead=4, dim_feedforward=64, dropout=0.05, batch_first=True)
        self.encoder = nn.TransformerEncoder(enc, num_layers=2)
        self.head = nn.Sequential(nn.Linear(32 + n_tab, 64), nn.ReLU(), nn.Linear(64, 1))

    def forward(self, w, x):
        z = self.proj(w.transpose(1, 2)) + self.pos
...
def benchmark(cfg: dict, meta: pd.DataFrame, truth: np.ndarray, wave: np.ndarray, x: np.ndarray) -> Tuple[pd.DataFrame, pd.DataFrame, Dict[str, np.ndarray]]:
    ...
    birks_raw = (charge_by * (1.0 + kb * dedx) / max(alpha, 1e-9)).sum(axis=1)
    birks_scale = float(np.median(y[train] / np.maximum(birks_raw[train], 1e-6)))
    birks = birks_raw * birks_scale
    ...
    trans, tsc = fit_torch(WaveformTransformer(x.shape[1]), wave, x, ylog, train, 5, 19)
    preds["transformer"] = pred_torch(trans, tsc, wave, x)
    residual_mlp, rsc = fit_torch(ResidualMLP(x.shape[1]), wave, x, ylog - np.log(np.maximum(birks, 1e-6)), train, 6, 21)
    preds["physics_residual_mlp"] = birks * pred_torch(residual_mlp, rsc, wave, x)
...
def write_report(out: Path, cfg: dict, result: dict, metrics: pd.DataFrame, byrun: pd.DataFrame, real_metrics: pd.DataFrame, response: pd.DataFrame, raw_counts: pd.DataFrame):
    ...
    lines = [
        ...
        "",
        md_table(response, ["method", "sim_res68_frac", "real_s27b_res68_frac", "delta_sim_minus_real", "interpretation"]),
        "",
        ...
        "",
        "```bash",
        f"/home/billy/anaconda3/bin/python scripts/s27c_1783806624_5871_585133e5_digitized_gain_transition_response_closure.py --config {cfg.get('_config_arg', 'configs/s27c_1783806624_5871_585133e5_digitized_gain_transition_response_closure.yaml')}",
        "```",
    ]
...
def main():
    ...
    ap.add_argument("--config", default="configs/s27c_1783806624_5871_585133e5_digitized_gain_transition_response_closure.yaml")
    args = ap.parse_args()
    t0 = time.time()
    ...
    for _, row in metrics.iterrows():
        ...
        if len(r):
            ...
            response_rows.append({"method": row["method"], "sim_res68_frac": float(row["res68_frac"]), "real_s27b_res68_frac": real, "delta_sim_minus_real": float(row["res68_frac"]) - real, "interpretation": "matched method"})
    ...
    result = {
        ...
        "real_s27b_reference_metrics": json.loads(real_metrics.head(8).to_json(orient="records")),
        ...
        "next_tickets": [
            {
                "title": "S27d: tune digitized gain-transition response against real S27b strata",
                "body": "Scan optical yield, shaping time, pedestal drift, common-mode noise, and ADC clipping in the S27c digitizer against S27b real held-out gain-transition strata to identify which response parameter family explains the remaining Birks-versus-ML residual mismatch.",
            }
        ],
    ...
    manifest = {
        ...
        "command": f"/home/billy/anaconda3/bin/python scripts/s27c_1783806624_5871_585133e5_digitized_gain_transition_response_closure.py --config {args.config}",
        ...
        "outputs": {p.name: sha256_file(p) for p in out.iterdir() if p.is_file() and p.name != "manifest.json"},
        "manifest_self_hash": "excluded because manifest.json is written after output hashes are computed",
    }
