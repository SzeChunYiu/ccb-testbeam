...
Ticket `1784064870.931.2c5305bf` asks whether pretrigger pedestal memory explains cross-run PID
...
Raw B-stack ROOT files are read from `/home/billy/.tb-workers/testbeam-laptop-1/data/root/root`.  The `h101/HRDv`
...
single proposed next ticket is `S36d: external PID-label join for pedestal-memory calibration`.

## Verdict

`result.json` names **pedestal_memory_fusion_new** as the S36c winner.  The pedestal-memory
...
Runtime was `83.3` s on `Linux-5.15.0-139-generic-x86_64-with-debian-bullseye-sid`.
