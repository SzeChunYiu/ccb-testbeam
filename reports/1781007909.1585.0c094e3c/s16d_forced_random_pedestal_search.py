#!/usr/bin/env python3
"""S16d forced/random-trigger HRD pedestal search and closure.

The current mirror has no populated ROOT entries tagged as forced/random triggers.
This script reproduces the raw-ROOT B-stack pulse count first, searches DAQ
metadata and extracted file mirrors for forced/random pedestal events, then runs
a no-pulse proxy benchmark on quiet B-stack events with held-out runs.
"""
...
def mirror_file_audit(config: dict) -> pd.DataFrame:
    tokens = ("force", "forced", "random", "rand", "ped", "pedestal", "pulser", "noise")
    root = Path(config["extracted_data_dir"])
    rows = []
        ...
        rows.append(
            {
                ...
                "forced_random_filename_tokens": ";".join(matches),
                "is_root_or_archive": bool(path.suffix.lower() in [".root", ".zip", ".gz", ".tar"]),
            }
        )
    return pd.DataFrame(rows)


def reproduce_selected_b_stave_pulses(config: dict) -> pd.DataFrame:
    ...
    for run in configured_runs(config):
        ...
        for batch in iter_bstack(bstack_path(config, run)):
            ...
            amp = (waveform - seed[:, :, None]).max(axis=2)
            run_events += int(waveform.shape[0])
            selected_pulses += int((amp > amp_cut).sum())
        rows.append({"run": int(run), "events_total": run_events, "selected_b_stave_pulses": selected_pulses})
    return pd.DataFrame(rows)
...
def trigger_audit(config: dict) -> pd.DataFrame:
    ...
    for path in raw_roots(config):
        ...
        metadata_branches = [
            ...
            if any(token in name.lower() for token in ["trig", "event", "evt", "spill", "time", "clock", "run", "type", "daq"])
        ]
        if tree.num_entries:
...
def train_ml(features: pd.DataFrame, config: dict) -> Tuple[pd.DataFrame, pd.DataFrame, dict, HistGradientBoostingRegressor, LinearRegression]:
    ...
    max_iter = int(config["ml"].get("max_iter", 80))
...
def write_report(outdir: Path, config: dict, numbers: dict) -> None:
    ...
    report = f"""# Study report: S16d - forced/random-trigger HRD pedestal event search and closure
...
DAQ metadata available in these raw ROOT mirrors is limited to `TRIGGER`, `EVENTNO`, and `EVT` plus the HRD waveform branches. The audit found `{numbers["root_files"]}` raw ROOT files, `{numbers["populated_root_files"]}` populated files, `{numbers["file_token_matches"]}` extracted mirror filenames with forced/random/pedestal-like tokens, and `{numbers["forced_random_entries"]}` forced/random-tagged ROOT entries (`TRIGGER != 1` or a token-matched ROOT filename).
...
def main() -> None:
    ...
    raw_reproduction_counts = reproduce_selected_b_stave_pulses(config)
    raw_reproduction_counts.to_csv(outdir / "raw_reproduction_counts_by_run.csv", index=False)
    selected_pulses = int(raw_reproduction_counts["selected_b_stave_pulses"].sum())

    audit = trigger_audit(config)
    audit.to_csv(outdir / "trigger_audit.csv", index=False)
    mirror_audit = mirror_file_audit(config)
    mirror_audit.to_csv(outdir / "mirror_file_audit.csv", index=False)
    forced_random_entries = int(audit["non_beam_trigger_entries"].sum() + audit.loc[audit["filename_forced_random_match"], "entries"].sum())
    file_token_matches = int((mirror_audit["forced_random_filename_tokens"].astype(str) != "").sum())

    meta, waveforms, counts = load_quiet_proxy(config)
    counts.to_csv(outdir / "quiet_counts_by_run.csv", index=False)
    proxy_selected_pulses = int(counts["selected_b_stave_pulses"].sum())
    ...
    reproduction = pd.DataFrame(
        [
            ...
            {
                "quantity": "quiet-proxy loader selected B-stave pulses cross-check",
                "expected": selected_pulses,
                "reproduced": proxy_selected_pulses,
                "delta": proxy_selected_pulses - selected_pulses,
                "pass": proxy_selected_pulses == selected_pulses,
            },
            {
    ...
    numbers = {
        ...
        "root_files": int(len(audit)),
        "populated_root_files": int((audit["entries"] > 0).sum()),
    ...
    result = {
        ...
        "raw_reproduction": {
            ...
            "proxy_loader_selected_b_stave_pulses": proxy_selected_pulses,
            "forced_random_tagged_entries": forced_random_entries,
            "forced_random_filename_token_matches": file_token_matches,
    ...
    manifest = {
        "command": f"/home/billy/anaconda3/bin/python {outdir / 's16d_forced_random_pedestal_search.py'} --config {outdir / 's16d_config.json'}",
