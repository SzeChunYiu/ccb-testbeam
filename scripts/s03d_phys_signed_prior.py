#!/usr/bin/env python3
"""S03d physically signed inverse-amplitude prior versus S03a/S03b/HGB."""
...
def fit_signed_prior_model(
    ...
    for stave in staves:
        mask = train_mask & (stave_arr == stave) & np.isfinite(x) & np.isfinite(targets)
        if int(mask.sum()) < 2:
            models[stave] = {
                "power": float(power),
                "x_mean": 0.0,
        ...
        unconstrained = 0.0 if denom <= 0.0 else float(np.dot(xc, y_train - y_mean) / denom)
        slope = max(unconstrained, 0.0)
        models[stave] = {
            "power": float(power),
            "x_mean": x_mean,
            "intercept_ns": y_mean,
            "signed_slope_ns": slope,
            "unconstrained_slope_ns": unconstrained,
            "n_train_pulses": int(mask.sum()),
        }
    return models


def predict_signed_prior(pulses: pd.DataFrame, config: dict, models: Dict[str, dict]) -> np.ndarray:
    stave_arr = pulses["stave"].to_numpy()
    ...
    for stave, model in models.items():
        ...
        if len(idx) == 0:
            continue
        x = inverse_amp_feature(pulses.iloc[idx], config, float(model["power"]))
        pred[idx] = float(model["intercept_ns"]) + float(model["signed_slope_ns"]) * (
            x - float(model["x_mean"])
        )
    return pred


def signed_prior_table(models: Dict[str, dict]) -> pd.DataFrame:
    return pd.DataFrame(
        [
            {
                "stave": stave,
                ...
                "signed_slope_ns": model["signed_slope_ns"],
                "unconstrained_slope_ns": model["unconstrained_slope_ns"],
                "slope_clipped_to_physical_sign": bool(model["unconstrained_slope_ns"] < 0.0),
                "n_train_pulses": model["n_train_pulses"],
            }
            for stave, model in models.items()
        ]
    )
...
def scan_signed_prior(
    ...
    n_splits = min(int(config["phys_prior"]["cv_folds"]), len(np.unique(groups)))
    gkf = GroupKFold(n_splits=n_splits)
    cv_rows = []
    best = {"score": math.inf, "power": None}
    for power in config["phys_prior"]["powers"]:
        fold_scores = []
        for fold, (tr, va) in enumerate(gkf.split(idx_train, targets[train_mask], groups=groups)):
            ...
            models = fit_signed_prior_model(pulses, targets, fold_train_mask, config, float(power))
            pred = predict_signed_prior(pulses, config, models)
            corrected = pulses[f"t_{base_method}_ns"].to_numpy(dtype=float) - pred
            va_idx = idx_train[va]
            va_runs = sorted(np.unique(runs[va_idx]).astype(int).tolist())
            vals = evaluate_corrected(pulses.iloc[va_idx].copy(), "signed_prior_cv", corrected[va_idx], config, va_runs)
        ...
        cv_rows.append(
            {"power": float(power), "fold": -1, "sigma68_ns": mean_score, "n_pair_residuals": 0}
        )
        if mean_score < best["score"]:
            best = {"score": mean_score, "power": float(power)}

    models = fit_signed_prior_model(pulses, targets, train_mask, config, float(best["power"]))
    pred = predict_signed_prior(pulses, config, models)
    out = pulses.copy()
...
def run_signed_prior_shuffled_control(pulses: pd.DataFrame, config: dict, base_method: str, best: dict) -> float:
    rng = np.random.default_rng(int(config["phys_prior"]["random_seed"]) + 503)
    targets = s02.event_residual_targets(pulses, base_method, 2.0, config)
    runs = pulses["run"].to_numpy(dtype=int)
    ...
    models = fit_signed_prior_model(pulses, shuffled, train_mask, config, float(best["power"]))
    pred = predict_signed_prior(pulses, config, models)
    corrected = pulses[f"t_{base_method}_ns"].to_numpy(dtype=float) - pred
    vals = evaluate_corrected(pulses, "signed_prior_shuffled", corrected, config, list(config["timing"]["heldout_runs"]))
...
def bootstrap_rows(
    ...
    for method, label in methods:
        ...
        ci = s02.bootstrap_ci(vals, rng, int(config["phys_prior"]["bootstrap_samples"]))
        rows.append(
            {
...
def run_one_fold(
    ...
    signed_pulses, signed_cv, signed_models, signed_best = scan_signed_prior(pulses, config, base_method)
    ...
    benchmark, residuals = bootstrap_rows(
        ...
        [
            ...
            ("signed_prior", "phys_signed_inverse_amp"),
    ...
    benchmark["signed_prior_power"] = signed_best["power"]
    benchmark["signed_prior_cv_sigma68_ns"] = signed_best["score"]
    ...
    leakage = pd.DataFrame(
        [
            ...
            {
                "heldout_run": heldout_run,
                "check": "signed_prior_shuffled_target_sigma68",
                "value": run_signed_prior_shuffled_control(pulses, config, base_method, signed_best),
                "unit": "ns",
            },
            {
                "heldout_run": heldout_run,
    ...
    signed_table = signed_prior_table(signed_models)
    signed_table["heldout_run"] = heldout_run
    hgb_cv["heldout_run"] = heldout_run
    binned_table = s03b.binned_model_table(binned_models)
    binned_table["heldout_run"] = heldout_run
    return benchmark, residuals, leakage, s03a_cv, binned_cv, signed_cv, hgb_cv, signed_table
...
def plot_outputs(out_dir: Path, per_run: pd.DataFrame, pooled: pd.DataFrame) -> None:
    ...
    ax.set_title("S03d signed-prior leave-one-run-out timing width")
    ax.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(out_dir / "fig_s03d_signed_prior_per_run_sigma68.png", dpi=130)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "fig_s03d_signed_prior_pooled_run_bootstrap.png", dpi=130)
    plt.close(fig)
...
def write_report(
    ...
    model_preview = signed_table.sort_values(["heldout_run", "stave"])
    lines = [
        ...
        model_preview[[
            ...
            "intercept_ns",
            "signed_slope_ns",
            "unconstrained_slope_ns",
            "slope_clipped_to_physical_sign",
            "n_train_pulses",
        ...
        f"Signed prior pooled sigma68 is `{result['traditional']['phys_signed_inverse_amp']['value']:.3f} ns`; S03a amp-only is `{result['traditional']['s03a_amp_only']['value']:.3f} ns`; S03b monotone-binned is `{result['traditional']['s03b_monotone_binned']['value']:.3f} ns`; HGB is `{result['ml']['value']:.3f} ns`.",
        "",
        "## 6. Reproducibility",
        "",
        "Generated by:",
        "",
        "```bash",
        f"{sys.executable} scripts/s03d_phys_signed_prior.py --config {config_path}",
        "```",
        "",
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/s03d_phys_signed_prior_1781011005.yaml")
    ...
    rng = np.random.default_rng(int(config["phys_prior"]["random_seed"]))
    ...
    for heldout_run in all_runs:
        bench, residuals, leakage, s03a_cv, s03b_cv, signed_cv, hgb_cv, signed_table = run_one_fold(
        ...
        signed_table_parts.append(signed_table)
    ...
    signed_table = pd.concat(signed_table_parts, ignore_index=True)
    pooled = run_level_bootstrap(residuals, rng, int(config["phys_prior"]["bootstrap_samples"]))
    ...
    signed_table.to_csv(out_dir / "signed_prior_model_table.csv", index=False)
    ...
    signed_row = pooled_idx.loc["phys_signed_inverse_amp"]
    ...
    min_shuffle = float(
        leakage[
            leakage["check"].isin(
                [
                    "signed_prior_shuffled_target_sigma68",
    ...
    signed_beats_s03b = bool(signed_row["value"] < s03b_row["value"])
    verdict = (
        "signed_prior_beats_isotonic_no_leakage"
        if signed_beats_s03b and event_overlap == 0 and not leakage_flag
        else "signed_prior_does_not_beat_isotonic_or_leakage_concern"
    )
    ...
    result = {
        ...
        "traditional": {
            ...
            "phys_signed_inverse_amp": {
                ...
                "delta_vs_s03a_amp_only_ns": float(s03a_row["value"] - signed_row["value"]),
                "delta_vs_s03b_monotone_binned_ns": float(s03b_row["value"] - signed_row["value"]),
            },
        },
        "ml": {
            ...
            "gain_vs_phys_signed_inverse_amp_ns": hgb_gain_vs_signed,
        },
        "leakage": {
            "split_by_run": True,
        ...
        "critic": "pending",
        "next_tickets": [
            "S03e: blind Sample-I to Sample-II transfer for the physically signed inverse-amplitude prior",
            "S03f: hierarchical shrinkage for signed per-stave inverse-amplitude scales",
        ],
    }
    (out_dir / "result.json").write_text(json.dumps(result, indent=2), encoding="utf-8")
    write_report(out_dir, config, config_path, repro_counts, run65_repro, per_run, pooled, signed_table, leakage, result)
    ...
    manifest = {
        ...
        "random_seed": int(config["phys_prior"]["random_seed"]),
        ...
        json.dumps(
            {
                ...
                "phys_signed_inverse_amp": float(signed_row["value"]),
