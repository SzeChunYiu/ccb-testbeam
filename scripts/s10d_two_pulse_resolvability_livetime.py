#!/usr/bin/env python3
"""S10d two-pulse template resolvability live-time benchmark.

The script reads raw HRD ROOT files, reproduces the relevant S10/S10b numbers
first, then creates a shared injected two-pulse benchmark from raw-pulse-derived
...
def bias_metrics(frame: pd.DataFrame, prefix: str) -> dict:
    positives = frame[(frame["is_overlap"] == 1) & (~frame[f"{prefix}_failed"].astype(bool))]
    if len(positives) == 0:
        return {
            "n_positive": 0,
            "timing_bias_ns": float("nan"),
            "abs_timing_bias_ns": float("nan"),
            "area_bias_fraction": float("nan"),
            "abs_area_bias_fraction": float("nan"),
            "time_sigma68_ns": float("nan"),
            "area_res68_fraction": float("nan"),
        }
    terr, qerr = recovery_errors(positives, prefix)
    timing_bias = float(np.nanmedian(terr)) if len(terr) else float("nan")
    area_bias = float(np.nanmedian(qerr)) if len(qerr) else float("nan")
    return {
        "n_positive": int(len(positives)),
        "timing_bias_ns": timing_bias,
        "abs_timing_bias_ns": abs(timing_bias),
        "area_bias_fraction": area_bias,
        "abs_area_bias_fraction": abs(area_bias),
        "time_sigma68_ns": sigma68(terr),
        "area_res68_fraction": sigma68(qerr),
    }
...
def delay_summary(delay_rows: pd.DataFrame, frame: pd.DataFrame, config: dict, rng: np.random.Generator) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    rows = []
    boot_rows = []
    run_rows = []
    ...
    for label in ["constrained_template_fit", "compact_mlp_classifier_regressor"]:
        prefix = "trad" if label == "constrained_template_fit" else "ml"
        method_rows = delay_rows[delay_rows["method"] == label]
        delay_ns = first_stable_delay(method_rows)
        rows.append({"method": label, "resolvable_delay_ns": delay_ns, "n_heldout_runs": len(held_runs)})
    ...
    return pd.DataFrame(rows), pd.DataFrame(boot_rows), pd.DataFrame(run_rows)
...
def reproduce_s10b_live_time(config: dict) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Rerun the tracked S10b raw-ROOT live-time gate before S10d.

    The imported module only writes when its main() runs. Calling the functions
    here keeps all new S10d outputs in this ticket's report directory.
    """
    module_path = Path("reports/1781000867.546870.5c124aaf/s10b_tau_eff_template_fit.py")
    spec = importlib.util.spec_from_file_location("s10b_tau_eff_template_fit", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot import {module_path}")
    s10b = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(s10b)

    pulses = s10b.read_selected_pulses()
    _topology, reproduction, rmax = s10b.reproduce_s10(pulses)
    _fits, heldout = s10b.traditional_template_fits(pulses)
    trad_live10 = heldout["traditional_template_live10_ns"].to_numpy(dtype=float)
    measured_live10 = float(np.nanmean(trad_live10))
    combined = rmax[rmax["requirement"] == "combined_dt1ns_area20pct"].iloc[0]
    reproduced_rmax = float(combined["reproduced_Rmax_MHz"])
    rescaled_rmax = float(0.380 / (measured_live10 * 1e-9) / 1e6)
    expected = config["s10b_reproduction"]
    rows = [
        {
            "quantity": "S10 assumed tau_eff combined Rmax MHz",
            "report_value": float(expected["expected_assumed_tau_rmax_mhz"]),
            "reproduced": reproduced_rmax,
            "delta": reproduced_rmax - float(expected["expected_assumed_tau_rmax_mhz"]),
            "tolerance": float(expected["tolerance_rmax_mhz"]),
            "pass": abs(reproduced_rmax - float(expected["expected_assumed_tau_rmax_mhz"])) <= float(expected["tolerance_rmax_mhz"]),
        },
        {
            "quantity": "S10b measured traditional live10 ns",
            "report_value": float(expected["expected_measured_live10_ns"]),
            "reproduced": measured_live10,
            "delta": measured_live10 - float(expected["expected_measured_live10_ns"]),
            "tolerance": float(expected["tolerance_live10_ns"]),
            "pass": abs(measured_live10 - float(expected["expected_measured_live10_ns"])) <= float(expected["tolerance_live10_ns"]),
        },
        {
            "quantity": "S10b measured-tau rescaled Rmax MHz",
            "report_value": float(expected["expected_rescaled_rmax_mhz"]),
            "reproduced": rescaled_rmax,
            "delta": rescaled_rmax - float(expected["expected_rescaled_rmax_mhz"]),
            "tolerance": float(expected["tolerance_rmax_mhz"]),
            "pass": abs(rescaled_rmax - float(expected["expected_rescaled_rmax_mhz"])) <= float(expected["tolerance_rmax_mhz"]),
        },
    ]
    summary = pd.DataFrame(rows)
    heldout = heldout.copy()
    heldout["source"] = "S10b raw ROOT rerun"
    rmax = rmax.copy()
    rmax["source"] = "S10b raw ROOT rerun"
    return summary, heldout, rmax
...
def write_report(
    ...
    trad_delay = delay_ci[delay_ci["method"] == "constrained_template_fit"].iloc[0]
    ml_delay = delay_ci[delay_ci["method"] == "compact_mlp_classifier_regressor"].iloc[0]
    verdict = (
        "The compact ML method reaches the bias criteria at a shorter held-out delay than the constrained template fit in this closure. "
        "That win is treated cautiously because both methods are trained/evaluated on template-like injections and the leakage probes are required for interpretation."
        if float(ml_delay["value"]) < float(trad_delay["value"])
        else
        "The constrained template fit reaches the bias criteria at the same or shorter held-out delay than ML. "
        "ML remains useful as an overlap detector, but its regressor does not improve the resolvability live-time in this closure."
    )
    text = f"""# Study report: S10d - two-pulse template resolvability live-time
...
The S10b live-time gate was then rerun from raw ROOT before S10d. It reproduced `{float(s10b[s10b['quantity'] == 'S10b measured traditional live10 ns'].iloc[0]['reproduced']):.2f} ns` for the measured live10 window and `{float(s10b[s10b['quantity'] == 'S10b measured-tau rescaled Rmax MHz'].iloc[0]['reproduced']):.2f} MHz` for the measured-tau rescaled combined Rmax. See `s10b_reproduction.csv`.
...
|---|---:|---:|---:|---:|---:|---:|
| constrained template fit | {trad_delay['value']:.1f} | [{trad_delay['ci_low']:.1f}, {trad_delay['ci_high']:.1f}] | {trad['detection_ap']:.3f} | {trad['time_rms_ns']:.2f} | {trad['charge_fractional_bias']:.3f} | {trad['failure_rate']:.3f} |
| compact ML | {ml_delay['value']:.1f} | [{ml_delay['ci_low']:.1f}, {ml_delay['ci_high']:.1f}] | {ml['detection_ap']:.3f} | {ml['time_rms_ns']:.2f} | {ml['charge_fractional_bias']:.3f} | {ml['failure_rate']:.3f} |

{verdict} Bootstrap CIs are run-held-out resamples from the held-out source runs and are in `resolvability_bootstrap_ci.csv`.
...
Runtime in this run was `{runtime:.2f}` s. Outputs include `REPORT.md`, `result.json`, `manifest.json`, `input_sha256.csv`, reproduction tables, delay tables, metrics tables, leakage checks, and figures.
"""
...
def main() -> int:
    ...
    s10b, s10b_heldout, s10b_rmax = reproduce_s10b_live_time(config)
    ...
    delay_rows = resolvability_by_delay(combined, config)
    delay_rows.to_csv(out_dir / "resolvability_by_delay.csv", index=False)
    delay_overall, delay_ci, run_delay = delay_summary(delay_rows, combined, config, rng)
    ...
    write_report(out_dir, config, match, s10, s10b, template_summary, overall, delay_rows, delay_ci, run_delay, leak, runtime)
    ...
    result = {
        ...
        "ml_beats_baseline": bool(ml_delay["value"] < trad_delay["value"]),
        "falsification": {
            "split": "by source run",
            "train_runs": train_runs,
            "heldout_runs": heldout_runs,
            "leakage_checks_pass": bool(leak["pass"].all()),
            "run_heldout_resolvability": run_delay.to_dict(orient="records"),
