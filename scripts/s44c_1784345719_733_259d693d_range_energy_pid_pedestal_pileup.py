...
All model inputs use only even readout waveforms and event topology. The real
HRD ROOT files have no particle-truth branch, so PID is evaluated as a
charge-depth weak-label robustness benchmark, not a particle-truth adoption.
"""
...
def extract_tables(config: dict) -> Tuple[pd.DataFrame, pd.DataFrame, np.ndarray, np.ndarray, pd.DataFrame]:
    ...
    for run in configured_runs(config):
        ...
        count = {
            ...
            "selected_pulses": 0,
            "baseline_sum_adc": 0.0,
            "baseline_sq_sum_adc": 0.0,
            "baseline_n": 0,
        }
        ...
        for batch in iter_batches(path):
            ...
            count["baseline_sum_adc"] += float(baseline[:, even_ch].sum())
            count["baseline_sq_sum_adc"] += float((baseline[:, even_ch] ** 2).sum())
            count["baseline_n"] += int(baseline[:, even_ch].size)
    ...
    counts_df["baseline_mean_adc"] = counts_df["baseline_sum_adc"] / counts_df["baseline_n"].clip(lower=1)
    counts_df["baseline_rms_adc"] = np.sqrt(
        np.maximum(counts_df["baseline_sq_sum_adc"] / counts_df["baseline_n"].clip(lower=1) - counts_df["baseline_mean_adc"] ** 2, 0.0)
    )
...
def pid_proxy_labels(events: pd.DataFrame, y_energy: np.ndarray, train_mask: np.ndarray, config: dict) -> tuple[np.ndarray, np.ndarray, dict]:
    """Build a train-frozen charge-depth weak PID target from duplicate-readout closure."""
    depth = events["depth_idx"].to_numpy(dtype=float)
    mult = events["multiplicity"].to_numpy(dtype=float)
    odd_charge = events["odd_total_charge"].to_numpy(dtype=float)
    dedx_proxy = np.log1p(np.maximum(odd_charge, 0.0)) - 0.42 * depth - 0.08 * mult
    lo_q = float(config.get("pid_quantile_low", 0.35))
    hi_q = float(config.get("pid_quantile_high", 0.65))
    lo, hi = np.quantile(dedx_proxy[train_mask], [lo_q, hi_q])
    label = np.full(len(events), -1, dtype=int)
    label[dedx_proxy <= lo] = 0
    label[dedx_proxy >= hi] = 1
    info = {
        "source": "duplicate odd-readout charge-depth weak label",
        "positive_name": "deuteron_like_high_ionization_shallow_range",
        "negative_name": "proton_like_low_ionization_penetrating",
        "low_quantile": lo_q,
        "high_quantile": hi_q,
        "low_threshold": float(lo),
        "high_threshold": float(hi),
        "caveat": "No particle-truth branch exists in real HRD ROOT; PID metrics are weak-label robustness diagnostics.",
    }
    return label, dedx_proxy, info


def traditional_pid_score(events: pd.DataFrame, train_mask: np.ndarray, eval_mask: np.ndarray, labels: np.ndarray) -> np.ndarray:
    depth = events["depth_idx"].to_numpy(dtype=float)
    mult = events["multiplicity"].to_numpy(dtype=float)
    even_charge = events["even_total_charge"].to_numpy(dtype=float)
    coord = np.log1p(np.maximum(even_charge, 0.0)) - 0.42 * depth - 0.08 * mult
    valid_train = train_mask & (labels >= 0)
    x = coord[valid_train]
    y = labels[valid_train]
    mu0 = float(np.mean(x[y == 0]))
    mu1 = float(np.mean(x[y == 1]))
    sd0 = float(np.std(x[y == 0]) + 1e-6)
    sd1 = float(np.std(x[y == 1]) + 1e-6)
    prior1 = float(np.clip(y.mean(), 1e-3, 1 - 1e-3))
    z = coord
    ll0 = -0.5 * ((z - mu0) / sd0) ** 2 - np.log(sd0) + np.log(1.0 - prior1)
    ll1 = -0.5 * ((z - mu1) / sd1) ** 2 - np.log(sd1) + np.log(prior1)
    return 1.0 / (1.0 + np.exp(np.clip(ll0 - ll1, -40.0, 40.0)))


def fit_pid_mlp(x: np.ndarray, labels: np.ndarray, train_mask: np.ndarray, config: dict) -> object:
    from sklearn.neural_network import MLPClassifier

    idx = np.flatnonzero(train_mask & (labels >= 0))
    rng = np.random.default_rng(int(config["random_seed"]) + 510)
    if len(idx) > int(config["ml_max_train_events"]):
        idx = rng.choice(idx, size=int(config["ml_max_train_events"]), replace=False)
    model = make_pipeline(
        StandardScaler(),
        MLPClassifier(hidden_layer_sizes=(40, 20), alpha=1e-3, max_iter=int(config["mlp_max_iter"]) * 12, random_state=int(config["random_seed"]) + 511),
    )
    model.fit(x[idx], labels[idx])
    return model
...
def fit_pid_cnn(event_wave: np.ndarray, x: np.ndarray, labels: np.ndarray, train_mask: np.ndarray, config: dict) -> tuple[object, StandardScaler]:
    if torch is None:
        raise RuntimeError("torch unavailable")
    idx = np.flatnonzero(train_mask & (labels >= 0))
    rng = np.random.default_rng(int(config["random_seed"]) + 520)
    if len(idx) > int(config["cnn_max_train_events"]):
        idx = rng.choice(idx, size=int(config["cnn_max_train_events"]), replace=False)
    scaler = StandardScaler().fit(x[idx])
    xs = scaler.transform(x[idx]).astype(np.float32)
    w = event_wave[idx].astype(np.float32)
    scale = np.maximum(np.percentile(np.abs(w).reshape(len(w), -1), 95, axis=1), 1.0)
    ...
    loader = DataLoader(ds, batch_size=512, shuffle=True)
    torch.manual_seed(int(config["random_seed"]) + 521)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = SmallCNNClassifier(x.shape[1]).to(device)
    opt = torch.optim.AdamW(model.parameters(), lr=9e-4, weight_decay=1e-4)
...
def predict_pid_cnn(model: object, scaler: StandardScaler, event_wave: np.ndarray, x: np.ndarray) -> np.ndarray:
    device = next(model.parameters()).device
    xs = scaler.transform(x).astype(np.float32)
    pred = []
    for start in range(0, len(x), 4096):
        ...
        w = (w / scale[:, None, None]).astype(np.float32)
        with torch.no_grad():
            logits = model(torch.from_numpy(w).to(device), torch.from_numpy(xs[start:stop]).to(device)).cpu().numpy()
        pred.append(1.0 / (1.0 + np.exp(-np.clip(logits, -40.0, 40.0))))
    return np.concatenate(pred)


class MultiTaskWaveformTransformer(nn.Module):
    def __init__(self, n_tab: int, n_samples: int):
        super().__init__()
        width = 32
        ...
        self.tab = nn.Sequential(nn.Linear(n_tab, 32), nn.GELU(), nn.LayerNorm(32))
        self.head = nn.Sequential(nn.Linear(width + 32, 48), nn.GELU())
        self.energy = nn.Linear(48, 1)
        self.pid = nn.Linear(48, 1)

    def forward(self, wave, tab):
        seq = wave.transpose(1, 2)
        z = self.encoder(self.sample_proj(seq) + self.pos[:, : seq.shape[1], :]).mean(dim=1)
        h = self.head(torch.cat([z, self.tab(tab)], dim=1))
        return self.energy(h).squeeze(1), self.pid(h).squeeze(1)


def fit_multitask_transformer(
    ...
    if torch is None:
        raise RuntimeError("torch unavailable")
    idx = sample_train_indices(
        train_mask,
        int(config.get("transformer_max_train_events", config["cnn_max_train_events"])),
        int(config["random_seed"]) + 700,
    )
    scaler = StandardScaler().fit(x[idx])
    xs = scaler.transform(x[idx]).astype(np.float32)
    w = event_wave[idx].astype(np.float32)
    scale = np.maximum(np.percentile(np.abs(w).reshape(len(w), -1), 95, axis=1), 1.0)
    w = (w / scale[:, None, None]).astype(np.float32)
    ye = np.log(np.maximum(y_energy[idx], 1e-6)).astype(np.float32)
    yp = labels[idx].astype(np.float32)
    has_pid = (labels[idx] >= 0).astype(np.float32)
    ds = TensorDataset(
        torch.from_numpy(w),
        torch.from_numpy(xs),
        torch.from_numpy(ye),
        torch.from_numpy(yp),
        torch.from_numpy(has_pid),
    )
    loader = DataLoader(ds, batch_size=512, shuffle=True)
    torch.manual_seed(int(config["random_seed"]) + 701)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = MultiTaskWaveformTransformer(x.shape[1], event_wave.shape[2]).to(device)
    opt = torch.optim.AdamW(model.parameters(), lr=8e-4, weight_decay=2e-4)
    energy_loss = nn.SmoothL1Loss()
    pid_loss = nn.BCEWithLogitsLoss(reduction="none")
    model.train()
    for _ in range(int(config.get("transformer_epochs", 4))):
        for wb, xb, yb, pb, mb in loader:
            wb = wb.to(device)
            xb = xb.to(device)
            yb = yb.to(device)
            pb = pb.to(device)
            mb = mb.to(device)
            opt.zero_grad()
            ehat, phat = model(wb, xb)
            loss = energy_loss(ehat, yb)
            if float(mb.sum().item()) > 0.0:
                loss = loss + 0.35 * (pid_loss(phat, torch.clamp(pb, 0, 1)) * mb).sum() / mb.sum()
            loss.backward()
            opt.step()
    model.eval()
    return model, scaler
...
def predict_multitask_transformer(
    model: object,
    ...
    for start in range(0, len(x), 4096):
        ...
        w = (w / scale[:, None, None]).astype(np.float32)
        with torch.no_grad():
            ehat, phat = model(torch.from_numpy(w).to(device), torch.from_numpy(xs[start:stop]).to(device))
        energies.append(ehat.cpu().numpy())
        scores.append(1.0 / (1.0 + np.exp(-np.clip(phat.cpu().numpy(), -40.0, 40.0))))
    return exp_clip(np.concatenate(energies)), np.concatenate(scores)


def pid_metrics(events: pd.DataFrame, labels: np.ndarray, scores: Dict[str, np.ndarray], held_mask: np.ndarray, config: dict) -> tuple[pd.DataFrame, pd.DataFrame]:
    eval_mask = held_mask & (labels >= 0)
    y = labels[eval_mask]
    runs = events.loc[eval_mask, "run"].to_numpy(dtype=int)
    rows = []
    run_rows = []
    rng = np.random.default_rng(int(config["random_seed"]) + 540)
    blocks = [np.flatnonzero(runs == run) for run in np.unique(runs)]
    for method, score_all in scores.items():
        s = score_all[eval_mask]
        auc = float(roc_auc_score(y, s)) if len(np.unique(y)) == 2 else float("nan")
        ap = float(average_precision_score(y, s)) if len(np.unique(y)) == 2 else float("nan")
        ...
        for _ in range(int(config["bootstrap_reps"])):
            ...
            if len(np.unique(y[idx])) < 2:
                continue
            auc_boot.append(float(roc_auc_score(y[idx], s[idx])))
            pp = (s[idx] >= 0.5).astype(int)
            btn, bfp, bfn, btp = confusion_matrix(y[idx], pp, labels=[0, 1]).ravel()
            bacc_boot.append(0.5 * (btp / max(btp + bfn, 1) + btn / max(btn + bfp, 1)))
        rows.append(
            {
                "method": method,
                "n": int(len(y)),
                "roc_auc": auc,
                "roc_auc_ci95": [float(np.percentile(auc_boot, 2.5)), float(np.percentile(auc_boot, 97.5))] if auc_boot else [None, None],
                "average_precision": ap,
                "balanced_accuracy": float(bacc),
                "balanced_accuracy_ci95": [float(np.percentile(bacc_boot, 2.5)), float(np.percentile(bacc_boot, 97.5))] if bacc_boot else [None, None],
                "tn": int(tn),
                "fp": int(fp),
                "fn": int(fn),
                "tp": int(tp),
        ...
        for run in np.unique(runs):
            ...
            if len(np.unique(y[m])) < 2:
                continue
            pp = (s[m] >= 0.5).astype(int)
            rtn, rfp, rfn, rtp = confusion_matrix(y[m], pp, labels=[0, 1]).ravel()
            run_rows.append(
                {
                    "run": int(run),
                    "method": method,
                    "n": int(m.sum()),
                    "roc_auc": float(roc_auc_score(y[m], s[m])),
                    "balanced_accuracy": float(0.5 * (rtp / max(rtp + rfn, 1) + rtn / max(rtn + rfp, 1))),
                    "tn": int(rtn),
                    "fp": int(rfp),
                    "fn": int(rfn),
                    "tp": int(rtp),
    ...
    return pd.DataFrame(rows).sort_values("roc_auc", ascending=False), pd.DataFrame(run_rows)


def add_stress_columns(events: pd.DataFrame, counts: pd.DataFrame) -> pd.DataFrame:
    out = events.copy()
    lookup = counts.set_index("run")
    rate = out["run"].map(lookup["selected_pulses"]).astype(float)
    base_rms = out["run"].map(lookup["baseline_rms_adc"]).astype(float)
    out["occupancy_proxy"] = rate
    out["pedestal_rms_adc"] = base_rms
    out["occupancy_stratum"] = pd.qcut(rate, 3, labels=["low", "mid", "high"], duplicates="drop").astype(str)
    out["pedestal_stratum"] = pd.qcut(base_rms, 3, labels=["quiet", "nominal", "drift"], duplicates="drop").astype(str)
    out["saturation_stratum"] = np.where(out["saturated_count"].to_numpy(dtype=int) > 0, "saturated", "unsaturated")
    charge = np.clip(out["even_total_charge"].to_numpy(dtype=float), 1.0, None)
    amp = np.clip(out["even_max_amp"].to_numpy(dtype=float), 1.0, None)
    out["late_tail_proxy"] = np.log1p(charge / amp)
    out["late_tail_stratum"] = pd.qcut(out["late_tail_proxy"], 3, labels=["short_tail", "mid_tail", "late_tail"], duplicates="drop").astype(str)
    out["pileup_stratum"] = np.where(out["multiplicity"].to_numpy(dtype=int) > 1, "mild_pileup", "single")
    return out
...
def stress_rows(
    ...
    rows = []
    for stratum in ["pedestal_stratum", "occupancy_stratum", "saturation_stratum", "pileup_stratum", "late_tail_stratum"]:
        for value, sub in events.loc[held_mask].groupby(stratum):
            idx = sub.index.to_numpy(dtype=int)
            for method, pred in predictions.items():
                row = {
                    "stratum": stratum,
                    ...
                    "res68_frac": res68(y[idx], pred[idx]),
                    "mae_mev": float(mean_absolute_error(y[idx], pred[idx])),
                    "timing_residual_corr": float(np.corrcoef(frac_residual(y[idx], pred[idx]), sub["late_tail_proxy"].to_numpy(dtype=float))[0, 1]) if len(idx) > 3 else float("nan"),
                }
                if method in pid_scores:
                    valid = idx[labels[idx] >= 0]
                    if len(valid) > 3 and len(np.unique(labels[valid])) == 2:
                        score = pid_scores[method][valid]
                        pred_label = (score >= 0.5).astype(int)
                        tn, fp, fn, tp = confusion_matrix(labels[valid], pred_label, labels=[0, 1]).ravel()
                        row.update(
                            {
                                "pid_auc": float(roc_auc_score(labels[valid], score)),
                                "pid_ece": ece_score(labels[valid], score),
                                "pid_confusion": f"tn={tn};fp={fp};fn={fn};tp={tp}",
                            }
                        )
                    else:
                        row.update({"pid_auc": float("nan"), "pid_ece": float("nan"), "pid_confusion": ""})
                rows.append(row)
    return pd.DataFrame(rows)
...
def make_report(
    ...
    pid_ranked = pid_summary.sort_values("roc_auc", ascending=False).copy()
    comp_ranked = composite.sort_values("composite_loss").copy()
    pedestal_cols = ["run", "group", "events_total", "selected_pulses", "baseline_mean_adc", "baseline_rms_adc"]
    lines = [
        "# S44c: Range-Energy PID Calibration Transfer Under Pedestal Pile-Up Stress",
        "",
        "## Abstract",
        "",
        (
            "This ticket studies how much PID and calibrated energy information survives pedestal "
            "drift, late tails, and mild pile-up after traditional range-energy calibration. The analysis reads the raw "
            "ROOT waveform branch, subtracts per-event pretrigger pedestals, benchmarks a strong "
            "traditional dE/dx/Birks method against ridge, gradient-boosted trees, MLP, 1D-CNN, and a "
            "multitask waveform transformer plus a new range-gated residual MLP, and estimates 95% confidence intervals by held-out run "
            "bootstrap. The raw ROOT reproduction gate passes "
            f"exactly at {result['raw_reproduction']['reproduced_selected_pulses']:,} selected B-stave pulses. "
            f"The composite winner named in `result.json` is **{winner}**, with energy res68="
            f"{result['winner']['res68_frac']:.5f} (95% CI [{ci[0]:.5f}, {ci[1]:.5f}]) and "
            f"weak-label PID ROC AUC={result['winner']['weak_label_pid_roc_auc']:.5f}."
        ),
        "",
        ...
        "",
        md_table(counts[pedestal_cols], pedestal_cols),
        "",
        "## GEANT4/dE/dx Anchor",
        "",
        ...
        "",
        md_table(pid_ranked, ["method", "n", "roc_auc", "roc_auc_ci95", "average_precision", "balanced_accuracy", "balanced_accuracy_ci95", "tn", "fp", "fn", "tp"]),
        "",
        ...
        "",
        md_table(stress[stress["method"].isin([winner, "traditional_dedx_birks_likelihood", "geant4_birks_lookup", "multitask_waveform_transformer"])].head(180), ["stratum", "value", "method", "n", "bias_frac", "res68_frac", "mae_mev", "pid_auc", "pid_ece", "timing_residual_corr", "pid_confusion"]),
        "",
        "## Composite Ranking",
        "",
        ...
        "",
        md_table(comp_ranked, ["method", "family", "res68_frac", "roc_auc", "composite_loss"]),
        "",
        ...
        "## Per-Run PID Scores",
        "",
        md_table(pid_byrun[pid_byrun["method"].isin([winner, "traditional_dedx_likelihood"])], ["run", "method", "n", "roc_auc", "balanced_accuracy", "tn", "fp", "fn", "tp"]),
        "",
...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/s44c_1784345719_733_259d693d_range_energy_pid_pedestal_pileup.yaml")
    ...
    claimed_ticket = """1784345719.733.259d693d
# S44c range-energy PID calibration transfer under pedestal pile-up stress

Question: How much PID and calibrated energy information survives pedestal drift, late tails, and mild pile-up after traditional range-energy calibration?
Traditional baseline: pedestal sideband subtraction plus Bethe-Bloch/dE-E likelihood calibration with run-wise gain constants.
Compare ridge regression, gradient-boosted trees, MLP, 1D-CNN, and a multitask waveform transformer for joint energy regression and PID.
Report bootstrap CIs for energy scale/resolution, PID AUC and confusion, pedestal and occupancy strata, saturation flags, and timing-residual correlations.
"""
    (out_dir / "claimed_ticket.txt").write_text(claimed_ticket, encoding="utf-8")
    ...
    events = add_stress_columns(events, counts)
    ...
    predictions["range_gated_residual_mlp_new"] = predict_residual_mlp(residual_model, residual_scaler, x, birks_pred)
    predictions = {name: clip_to_train_target_range(pred, y, train) for name, pred in predictions.items()}

    print("6b/7 weak-label PID panel", flush=True)
    pid_label, pid_coord, pid_info = pid_proxy_labels(events, y, train, config)
    pid_scores: Dict[str, np.ndarray] = {
        "traditional_dedx_likelihood": traditional_pid_score(events, train, held, pid_label),
    }
    pid_train = train & (pid_label >= 0)
    pid_lr = make_pipeline(StandardScaler(), LogisticRegression(C=1.0, max_iter=500, random_state=int(config["random_seed"]) + 501))
    pid_lr.fit(x[pid_train], pid_label[pid_train])
    pid_scores["ridge"] = pid_lr.predict_proba(x)[:, 1]
    pid_gbt = GradientBoostingClassifier(n_estimators=70, max_depth=3, learning_rate=0.05, subsample=0.8, random_state=int(config["random_seed"]) + 502)
    pid_gbt.fit(x[pid_train], pid_label[pid_train])
    pid_scores["gradient_boosted_trees"] = pid_gbt.predict_proba(x)[:, 1]
    pid_mlp = fit_pid_mlp(x, pid_label, train, config)
    pid_scores["mlp"] = pid_mlp.predict_proba(x)[:, 1]
    try:
        pid_cnn, pid_cnn_scaler = fit_pid_cnn(event_wave, x, pid_label, train, config)
        pid_scores["1d_cnn"] = predict_pid_cnn(pid_cnn, pid_cnn_scaler, event_wave, x)
        pid_cnn_status = "trained"
    except Exception as exc:
        pid_scores["1d_cnn"] = np.full(len(y), np.nan)
        pid_cnn_status = f"failed: {exc}"
    try:
        mt_model, mt_scaler = fit_multitask_transformer(event_wave, x, y, pid_label, train, config)
        mt_energy, mt_pid = predict_multitask_transformer(mt_model, mt_scaler, event_wave, x)
        predictions["multitask_waveform_transformer"] = clip_to_train_target_range(mt_energy, y, train)
        pid_scores["multitask_waveform_transformer"] = mt_pid
        transformer_status = "trained"
    except Exception as exc:
        predictions["multitask_waveform_transformer"] = np.full(len(y), np.nan)
        pid_scores["multitask_waveform_transformer"] = np.full(len(y), np.nan)
        transformer_status = f"failed: {exc}"
    residual_features = np.column_stack(
        [
            x,
            np.log(np.maximum(birks_pred, 1e-6)),
            np.log(np.maximum(predictions["range_gated_residual_mlp_new"], 1e-6)),
            frac_residual(birks_pred, predictions["range_gated_residual_mlp_new"]),
        ]
    )
    pid_resid = make_pipeline(StandardScaler(), LogisticRegression(C=0.8, max_iter=500, random_state=int(config["random_seed"]) + 503))
    pid_resid.fit(residual_features[pid_train], pid_label[pid_train])
    pid_scores["range_gated_residual_mlp_new"] = pid_resid.predict_proba(residual_features)[:, 1]
    ...
    families = {
        ...
        "multitask_waveform_transformer": "neural_multitask_transformer",
    ...
    pid_summary, pid_byrun = pid_metrics(events, pid_label, {k: v for k, v in pid_scores.items() if np.isfinite(v).all()}, held, config)
    stress = stress_rows(
        events,
        y,
        {k: v for k, v in predictions.items() if np.isfinite(v).all()},
        pid_label,
        {k: v for k, v in pid_scores.items() if np.isfinite(v).all()},
        held,
    )
    comparable = metrics[metrics["method"].isin(pid_summary["method"])].merge(pid_summary[["method", "roc_auc"]], on="method", how="inner")
    trad_energy = metrics.loc[metrics["method"] == "geant4_birks_lookup"].iloc[0].copy()
    trad_pid = pid_summary.loc[pid_summary["method"] == "traditional_dedx_likelihood"].iloc[0]
    trad_energy["method"] = "traditional_dedx_birks_likelihood"
    trad_energy["family"] = "traditional_geant4_birks_plus_dedx_likelihood"
    trad_energy["roc_auc"] = float(trad_pid["roc_auc"])
    comparable = pd.concat([comparable, pd.DataFrame([trad_energy])], ignore_index=True)
    comparable["composite_loss"] = comparable["res68_frac"] + (1.0 - comparable["roc_auc"])
    winner_row = comparable.sort_values("composite_loss").iloc[0].to_dict()
    ...
    leakage = pd.DataFrame(
        [
            ...
            {
                "check": "pid_cnn_status",
                "value": pid_cnn_status,
                "pass": pid_cnn_status == "trained",
            },
            {
                "check": "multitask_transformer_status",
                "value": transformer_status,
                "pass": transformer_status == "trained",
            },
            {
            ...
            {
                "check": "pid_truth_branch_absent",
                "value": "h101 branches are TRIGGER,EVENTNO,EVT,NO,HRD,HRDI,HRDv",
                "pass": True,
            },
        ]
    )
    ...
    pid_summary.to_csv(out_dir / "pid_method_metrics.csv", index=False)
    pid_byrun.to_csv(out_dir / "pid_run_metrics.csv", index=False)
    comparable.to_csv(out_dir / "composite_method_ranking.csv", index=False)
    stress.to_csv(out_dir / "stress_strata_metrics.csv", index=False)
    ...
    result = {
        ...
        "winner": {
            ...
            "composite_loss": float(winner_row["composite_loss"]),
            "res68_frac": float(winner_row["res68_frac"]),
            "res68_ci95": winner_row["res68_ci95"],
            "weak_label_pid_roc_auc": float(winner_row["roc_auc"]),
        ...
        "stress_strata_metrics": json.loads(stress.to_json(orient="records")),
        "required_method_coverage": {
            "strong_traditional": "traditional_dedx_birks_likelihood",
            "ridge": "ridge",
            "gradient_boosted_trees": "gradient_boosted_trees",
            "mlp": "mlp",
            "one_dimensional_cnn": "1d_cnn",
            "multitask_waveform_transformer": "multitask_waveform_transformer",
            "new_architecture": "range_gated_residual_mlp_new",
        },
        "composite_ranking": json.loads(comparable.sort_values("composite_loss").to_json(orient="records")),
        ...
        "finding": (
            ...
            f"Across the methods with both energy and weak-label PID endpoints, the held-out composite winner is {winner_row['method']} "
            f"with energy res68={float(winner_row['res68_frac']):.5f} and PID ROC AUC={float(winner_row['roc_auc']):.5f}. "
            "The MeV scale is GEANT4/dE/dx anchored but remains conditional on the assumed B-stave thickness, geometry centers, and duplicate-readout closure target; PID is explicitly proxy-limited because the raw ROOT has no species branch."
        ),
        "runtime_sec": round(time.time() - t0, 1),
    }
    (out_dir / "result.json").write_text(json.dumps(result, indent=2), encoding="utf-8")
    make_report(out_dir, config, result, metrics, prior, byrun, leakage, counts, pid_summary, pid_byrun, comparable, stress)
    ...
    outputs = [
        ...
        "pid_method_metrics.csv",
        "pid_run_metrics.csv",
        "composite_method_ranking.csv",
        "leakage_checks.csv",
        "stress_strata_metrics.csv",
        "claimed_ticket.txt",
    ]
    manifest = {
        "study": config["study_id"],
        ...
        "command": "/home/billy/anaconda3/bin/python scripts/s44c_1784345719_733_259d693d_range_energy_pid_pedestal_pileup.py --config configs/s44c_1784345719_733_259d693d_range_energy_pid_pedestal_pileup.yaml",
