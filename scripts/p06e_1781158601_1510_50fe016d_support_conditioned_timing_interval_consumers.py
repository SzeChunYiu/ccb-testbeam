#!/usr/bin/env python3
"""P06e: propagate S06b support-conditioned timing intervals to consumers."""
...
os.environ.setdefault("MPLCONFIGDIR", "/tmp/matplotlib-p06e-1781158601")
...
def interval_abstention_table(rows: pd.DataFrame, config: dict) -> pd.DataFrame:
    rng = np.random.default_rng(int(config["random_seed"]))
    records = []
    for method, group in rows.groupby("method", sort=False):
        for budget in config["abstention_budgets"]:
            budget = float(budget)
            threshold = float("inf") if budget <= 0 else float(group["sigma_hat_ns"].quantile(1.0 - budget))
...
def per_run_interval_table(rows: pd.DataFrame, config: dict, primary_budget: float) -> pd.DataFrame:
    ...
    for method, group in rows.groupby("method"):
        thresholds[method] = float("inf") if primary_budget <= 0 else float(group["sigma_hat_ns"].quantile(1.0 - primary_budget))
    out = []
...
def write_report(out_dir: Path, config: dict, result: dict, intervals: pd.DataFrame, scored: pd.DataFrame, consumers: pd.DataFrame, per_run: pd.DataFrame, coverage_delta: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        f"**Primary split:** complete held-out runs {result['split']['heldout_runs']} with run-block bootstrap CIs  ",
        f"**Primary abstention budget:** {primary_budget:.0%}",
        "",
        "## Abstract",
        "",
        "P06e asks whether the S06b support-conditioned timing intervals can be propagated into downstream PID and energy consumers without retuning the consumers. The raw ROOT selected-pulse count is reproduced before any consumer scoring. The fixed interval-abstention budgets are applied to the frozen S06b pair-residual panel, then the same method families are joined to the current PID and energy/action-band consumer scoreboards. The winner named in `result.json` is "
        f"**{winner['method']}** with composite consumer loss **{winner['consumer_loss']:.4f}** at the 10% interval-abstention budget.",
        "",
        ...
        "",
        md_table(primary_intervals, ["method", "n", "abstain_fraction", "coverage68", "coverage68_ci_low", "coverage68_ci_high", "coverage95", "coverage95_ci_low", "coverage95_ci_high", "calibration_loss", "calibration_loss_ci_low", "calibration_loss_ci_high", "sigma68_ns"], digits=5),
        "",
        ...
        "",
        f"**Winner:** `{winner['method']}`. It is selected at the preregistered {primary_budget:.0%} interval-abstention budget by minimum composite consumer loss, not by timing calibration alone.",
        "",
...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/p06e_1781158601_1510_50fe016d_support_conditioned_timing_interval_consumers.json")
    ...
    intervals = interval_abstention_table(rows, config)
    ...
    intervals.to_csv(out_dir / "interval_abstention_summary.csv", index=False)
    ...
    result = {
        ...
        "split": {
            "unit": "run",
            "heldout_runs": sorted(int(x) for x in rows["run"].unique()),
            "bootstrap": "run-block bootstrap over held-out S06b timing runs",
        ...
        "ml": {
            ...
            "metric": "composite consumer loss at fixed interval-abstention budget",
        },
        "winner": {
            "method": str(winner["method"]),
            "method_label": str(winner["method_label"]),
            "metric": "composite_consumer_loss",
        ...
        "method_summary_file": "interval_abstention_summary.csv",
        ...
        "next_tickets": [],
        "finding": (
            f"At the fixed {float(config['primary_abstention_budget']):.0%} interval-abstention budget, "
            ...
            "S06b support-conditioned gated timing intervals as a consumer dry-run gate, while keeping PID/energy "
    ...
    write_report(out_dir, config, result, intervals, scored, consumers, per_run, deltas)
    manifest = {
        "config": str(config_path),
        "command": f"/home/billy/anaconda3/bin/python scripts/p06e_1781158601_1510_50fe016d_support_conditioned_timing_interval_consumers.py --config {config_path}",
        "inputs": {
