...
os.environ.setdefault("MPLCONFIGDIR", "/tmp/matplotlib-p06c-1781131868")
...
def plot_outputs(out_dir: Path, summary: pd.DataFrame) -> None:
    ...
    ax.set_title("P06c atom-conditioned pull-calibration benchmark")
    fig.tight_layout()
...
def write_report(
    ...
    lines = [
        ...
        "## Abstract",
        "",
        f"P06c asks whether the P06b atom-level timing-risk ledger can improve downstream PID, energy, pile-up, and covariance consumers through calibrated per-pulse uncertainty inflation or abstention across the 18-sample waveform phase, with special attention to peak samples 3-6 where earlier CFD/smoothing studies found artifacts. The raw ROOT reproduction gate matches exactly, and the winner by pre-registered pooled calibration loss is **{result['winner']['method']}** with loss **{result['winner']['calibration_loss']:.4f}** and bootstrap 95% CI **[{result['winner']['ci_low']:.4f}, {result['winner']['ci_high']:.4f}]**.",
        "",
        "## Reproduction Gate",
        "",
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/p06c_1781131868_2067_7f7516d4_atom_conditioned_timing_uncertainty.json")
