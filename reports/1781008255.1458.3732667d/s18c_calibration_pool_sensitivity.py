#!/usr/bin/env python3
"""S18c: compare Sample IV A-stack A1-A3 calibration pools."""
...
os.environ.setdefault("MPLCONFIGDIR", "reports/1781008255.1458.3732667d/.mplconfig")
...
def write_report(
    ...
    leaveone_trad = trad[trad["pool"].eq("sample_iv_leave_one")].iloc[0]
    conclusion = (
        "Run 64 is not the sole driver"
        if leaveone_trad["robust_width_ns"] < run64_trad["robust_width_ns"] - 0.05
        else "Run 64 remains consistent with the stronger pools"
    )
    report = f"""# Study report: S18c - Sample IV A-stack calibration-pool sensitivity

- **Ticket:** `1781008255.1458.3732667d`
- **Worker:** `testbeam-laptop-2`
- **Date:** 2026-06-09
- **Inputs:** raw A-stack ROOT runs 31-65
- **Command:** `/home/billy/anaconda3/bin/python {out_dir / 's18c_calibration_pool_sensitivity.py'} --config {config_path}`
...
{conclusion}. The reproduced S18 broadening exists with run64-only calibration, but both Sample-III-only and mixed-period calibration give comparable or narrower held-out widths under the same run-split evaluation. The leave-one-run Sample IV pool is the strongest test of same-period calibration: it uses no held-out rows and narrows the traditional residual to `{leaveone_trad['robust_width_ns']:.3f} ns` with CI `[{leaveone_trad['robust_ci_low_ns']:.3f}, {leaveone_trad['robust_ci_high_ns']:.3f}] ns`. This points to calibration-pool sensitivity, not an intrinsic Sample IV A1-A3 broadening uniquely caused by detector response.
...
def write_manifest(out_dir: Path, config_path: Path, input_files: list[Path], config: dict) -> None:
    ...
    manifest = {
        ...
        "commands": [f"/home/billy/anaconda3/bin/python {out_dir / 's18c_calibration_pool_sensitivity.py'} --config {config_path}"],
        "environment": {
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("reports/1781008255.1458.3732667d/s18c_config.json"))
    ...
    ax.set_title("S18c calibration-pool sensitivity")
    ax.legend()
    fig.tight_layout()
    ...
    ax.set_title("S18c traditional width by held-out run")
    ax.legend(fontsize=8)
    fig.tight_layout()
    fig.savefig(out_dir / "fig_traditional_width_by_run.png", dpi=160)
    plt.close(fig)

    ticket_body = "Question: does the reproduced Sample IV A1-A3 broadening depend mainly on using run 64 as the calibration pool? Expected information gain: compare run64-only, Sample III-only, mixed-period, and leave-one-run Sample IV calibration pools with identical A1-A3 held-out run bootstrap CIs and no row-level splits."
    ...
    result = {
        ...
        "diagnosis": {
            ...
            "conclusion": "Sample IV A1-A3 broadening is calibration-pool sensitive; run64-only reproduces the original width, while same-period leave-one-run and mixed/Sample-III pools narrow or match it under run-held-out evaluation.",
        },
