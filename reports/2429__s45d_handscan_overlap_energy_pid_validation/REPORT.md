...
Ticket `#2429` asks whether the S45c winner keeps its fixed-FPR recall, recovered-energy stability, and PID-proxy boundary advantage on a hand-scan-defined high-current candidate surface.  The worker was `testbeam-laptop-2` and the project was
...
`template_residual_boosted_stack_new` as the winner.  This S45d run therefore treats `template_residual_boosted_stack_new` as a frozen
...
For `template_residual_boosted_stack_new`, `D = 0.6152`, `R_0.05 =
0.8289` with 95% CI
`[0.7947,
0.8659]`, accepted energy sigma68 is
`0.0779` with 95% CI
`[0.07319,
0.08162]`, and the PID-proxy
bias span is `0.02099`.
...
high-current overlap validation for the S45d winner**.  It should ask whether the
S45d winner keeps its fixed-FPR recall and false-merge advantage on real
...
Runtime was `101.7` s on `Linux-5.15.0-139-generic-x86_64-with-debian-bullseye-sid`.
