...
``testbeam-laptop-1`` and no body, so the pre-registered target is the generic
...
TICKET = "1784068210.870.38ff282e"
SLUG = "s37a_sparse_ticket_pileup_saturation_frontier"
...
def load_config() -> dict:
    ...
    cfg.update(
        {
            "ticket_id": TICKET,
            "title": "Sparse-ticket pile-up/saturation recovery frontier",
            "worker": WORKER,
            "output_dir": str(OUT),
...
The claimed local-queue item was `{TICKET}` with title `testbeam-laptop-1` and an
...
def main() -> None:
    ...
    result = {
        ...
        "worker": WORKER,
        "title": cfg["title"],
        "claimed_ticket_title": "testbeam-laptop-1",
        "claimed_ticket_body": "",
        "status": "complete",
        "claimed_once": True,
