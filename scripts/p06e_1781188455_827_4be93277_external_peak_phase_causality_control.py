...
count from raw ROOT, breaks same-event timing physics with a deterministic
same-run forced-random reference, then benchmarks a support-matched traditional
atlas against classical ML and small neural models on a run-held-out split.
"""
...
CONFIG_DEFAULT = "configs/p06e_1781188455_827_4be93277_external_peak_phase_causality_control.json"
...
def add_targets(meta: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    timing["row_index"] = timing.index.to_numpy()
    event_ref = (
        timing.groupby(["run", "eventno"])["cfd20_sample"]
        .median()
        .reset_index(name="event_median_sample")
    )
    rng = np.random.default_rng(int(config["random_seed"]) + 1701)
    ref_parts = []
    for run, sub in event_ref.groupby("run", sort=True):
        shuffled = sub["event_median_sample"].to_numpy(dtype=float).copy()
        if len(shuffled) > 1:
            for _ in range(8):
                rng.shuffle(shuffled)
                if not np.any(shuffled == sub["event_median_sample"].to_numpy(dtype=float)):
                    break
            if np.any(shuffled == sub["event_median_sample"].to_numpy(dtype=float)):
                shuffled = np.roll(shuffled, 1)
        tmp = sub[["run", "eventno"]].copy()
        tmp["forced_random_event_median_sample"] = shuffled
        ref_parts.append(tmp)
    timing = timing.merge(pd.concat(ref_parts, ignore_index=True), on=["run", "eventno"], how="left")
    resid = (timing["cfd20_sample"] - timing["forced_random_event_median_sample"]) * float(config["period_ns"])
    meta.loc[timing["row_index"].to_numpy(dtype=int), "timing_residual_ns"] = resid.to_numpy(dtype=float)
...
def write_plots(outdir: Path, summary: pd.DataFrame, preds: pd.DataFrame, atlas: pd.DataFrame) -> None:
    ...
    ax.set_title("P06e forced-random peak-phase benchmark")
    fig.tight_layout()
...
def write_report(
    ...
    report = f"""# P06e: external peak-phase causality control
...
```bash
.venv/bin/python scripts/p06e_1781188455_827_4be93277_external_peak_phase_causality_control.py --config {config_path}
```
"""
