...
with known truth injected into raw-ROOT residuals.  The worker is `testbeam-laptop-4`.  The
...
The traditional comparator is **analytic_clipped_template_sideband_traditional**, standing in for charge-integration PID with pedestal subtraction and likelihood-ratio-style one-versus-two-pulse cuts.
...
The required helper command `tn-ticket claim testbeam-laptop-4 --project testbeam` was run once and returned the known null pseudo-ticket pattern (`null`, `# null`, `null`) despite a non-empty queue.  Direct GitHub inspection showed no issue claimed for `worker:testbeam-laptop-4`, so issue #2468 was manually label-swapped to `factory:claimed` and `worker:testbeam-laptop-4` without rerunning the helper.  This recovery matches the existing documented pattern for factory-ticket #2440.

## Recommendation

Use `saturation_residual_fusion_new` as the preferred S53c controlled-overlay waveform/PID-proxy method
...
Runtime was `40.8` s on `Linux-5.15.0-139-generic-x86_64-with-glibc2.35`.
