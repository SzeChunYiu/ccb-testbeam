...
and adds a pile-up-mask transformer row for the requested deconvolution
...
TICKET = "1784064858.859.4e603bae"
WORKER = "testbeam-laptop-4"
SLUG = "s36b_overlapping_pulse_timing_deconvolution_architecture_bakeoff"
    ...
    cfg.update(
        {
            "study_id": "S36b",
            "ticket_id": TICKET,
            "title": "S36b overlapping-pulse timing deconvolution architecture bakeoff",
            "worker": WORKER,
            "raw_root_dir": str(RAW_ROOT_DIR),
            "output_dir": str(OUT),
            "random_seed": 2026071414,
...
def pileup_mask_transformer_prediction(events: pd.DataFrame, waveforms: np.ndarray, cfg: dict) -> pd.DataFrame:
    if torch is None:
        raise RuntimeError("torch is required for the S36b pile-up mask transformer benchmark")
...
def tcn_prediction(events: pd.DataFrame, waveforms: np.ndarray, cfg: dict) -> pd.DataFrame:
    if torch is None:
        raise RuntimeError("torch is required for the S36b temporal convolution benchmark")
...
def write_report(
    ...
    text = f"""# S36b: overlapping-pulse timing deconvolution architecture bakeoff
...
def main() -> None:
    ...
    write_report(cfg, match, template_summary, overall, endpoints, ranked, by_run, strata, source_ci, calibration, winner, runtime)
    ...
    result = {
        ...
        "evaluation_design": {
            ...
            "winner_score": "registered S36b composite endpoint score",
        },
        ...
        "winner": {
            "name": winner,
            "criterion": "minimum registered S36b composite endpoint score with run-block bootstrap CIs",
        ...
        "next_tickets": [
            {
                ...
                "ticket_id": "1784065616.1187.1fd56f45",
                "body": (
                    "Question: Can run-held-out conformal calibration fix the S36b TCN uncertainty "
                    "under-coverage while preserving overlap AP and timing sigma68? Compare split-conformal "
                    "and run-block conformalized intervals on the S36b synthetic-injection and real high-rate "
                    "strata, report 68/95 coverage, interval width, AP, timing RMSE, energy bias, and PID proxy "
                    "confusion with bootstrap CIs. Expected information gain: separates architecture error from "
                    "uncertainty-calibration error for deployable overlapping-pulse deconvolution."
                ),
            }
        ],
        "novel_tickets_appended": [
            "1784065616.1187.1fd56f45: S36c: conformal timing intervals for overlapping-pulse deconvolution"
        ],
        "caveats": [
