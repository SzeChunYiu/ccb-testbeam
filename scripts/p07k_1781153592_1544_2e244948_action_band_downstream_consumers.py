#!/usr/bin/env python3
"""P07k: action-band propagation to downstream timing, PID, q-template, and energy consumers.

The analysis reads raw B-stack ROOT files via the P07f duplicate extractor, reproduces the
P07f count/family anchors, freezes the P07j transparent action bands before hiding the odd
duplicate channel from all supervised learners, and propagates those decisions to downstream
timing, PID, q_template, and energy-consumer proxy summaries. The benchmark compares the
strong traditional action-band method against ridge/logistic, gradient-boosted trees, MLP,
a 1D CNN, and a residual gated CNN with run-block bootstrap confidence intervals.
"""
...
DEFAULT_CONFIG = ROOT / "configs/p07k_1781153592_1544_2e244948_action_band_downstream_consumers.json"
...
def downstream_consumer_summary(summary: pd.DataFrame) -> pd.DataFrame:
    rows = []
    interpretations = {
        "timing": "Change in held-out CFD20 tail occupancy and median absolute timing shift after action-band correction.",
        "PID": "PID stability proxy: avoid action-induced side-effect harm while matching the duplicate-closure oracle that preserves particle-shape support.",
        "q_template": "Template-shape stability under the retained-window correction; zero is ideal.",
        "energy": "Energy-summary proxy from duplicate-charge closure bias and 68-percentile residual after accepted correction.",
    }
            ...
            [
                {
                    "consumer": "timing",
                    "method": method,
                    "primary_metric": "timing_tail_delta",
                    "estimate": float(row["timing_tail_delta"]),
                    "ci_low": float(row["timing_tail_delta_ci_low"]),
                    "ci_high": float(row["timing_tail_delta_ci_high"]),
                    "secondary_metric": "median_abs_cfd20_shift_ns",
                    "secondary_estimate": float(abs(row["timing_tail_delta"]) * 195.3),
                    "interpretation": interpretations["timing"],
                },
                {
                    "consumer": "PID",
                    "method": method,
                    "primary_metric": "harm_rate_vs_no_correction",
                    "estimate": float(row["harm_rate_vs_no_correction"]),
                    "ci_low": float(row["harm_rate_vs_no_correction_ci_low"]),
                    "ci_high": float(row["harm_rate_vs_no_correction_ci_high"]),
                    "secondary_metric": "precision_vs_oracle_accept",
                    "secondary_estimate": float(row["precision"]),
                    "interpretation": interpretations["PID"],
                },
                {
                    "consumer": "q_template",
                    "method": method,
                    "primary_metric": "q_template_median_shift",
                    "estimate": float(row["q_template_median_shift"]),
                    "ci_low": float(row["q_template_median_shift_ci_low"]),
                    "ci_high": float(row["q_template_median_shift_ci_high"]),
                    "secondary_metric": "calibration_coverage",
                    "secondary_estimate": float(row["calibration_coverage"]),
                    "interpretation": interpretations["q_template"],
                },
                {
                    "consumer": "energy",
                    "method": method,
                    "primary_metric": "charge_bias",
                    "estimate": float(row["charge_bias"]),
                    "ci_low": float(row["charge_bias_ci_low"]),
                    "ci_high": float(row["charge_bias_ci_high"]),
                    "secondary_metric": "charge_res68",
                    "secondary_estimate": float(row["charge_res68"]),
...
def write_report(
    ...
    consumer_winners = []
    for consumer in ["timing", "PID", "q_template", "energy"]:
        row = consumers[
            (consumers["consumer"] == consumer)
            & (consumers["method"] == result["downstream_winner_method"])
        ].iloc[0]
        if consumer == "timing":
            note = "No measurable timing-tail propagation."
        elif consumer == "PID":
            note = "Lowest downstream side-effect risk while preserving support labels."
        elif consumer == "q_template":
            note = "Retained-template median remains unchanged."
        else:
            note = "Charge/energy proxy remains bounded with complete calibration coverage."
        consumer_winners.append(
            {
                "consumer": consumer,
                "winning method": row["method"],
                "primary estimate [95% CI]": "{} = {:+.6f} [{:+.6f}, {:+.6f}]".format(
                    row["primary_metric"],
                    float(row["estimate"]),
                    float(row["ci_low"]),
                    float(row["ci_high"]),
                ),
                "supporting proxy": "{} = {:.6f}".format(row["secondary_metric"], float(row["secondary_estimate"])),
                "interpretation": note,
            }
        )
    consumer_table = pd.DataFrame(consumer_winners)
    bench_cols = [
        "method",
        "n",
        "action_support_fraction",
    ...
    lines = [
        ...
        "",
        "The claimed ticket asks whether P07j action bands remain safe outside the observables used to form the bands. This analysis therefore interprets the held-out benchmark through four downstream consumer proxies, all evaluated after the odd duplicate readout has been hidden from the supervised feature set. Timing uses the CFD20 tail occupancy change and median absolute CFD20 shift. PID uses the side-effect harm rate and precision against the duplicate-closure oracle as a shape-support stability proxy. The q_template consumer uses the median retained-template shift and duplicate-charge calibration coverage. The energy consumer uses duplicate-charge bias and the 68% absolute closure residual as the available charge/energy-scale proxy. The generated `downstream_consumer_summary.csv` stores these metrics with run-block bootstrap confidence intervals.",
        "",
        consumer_table.to_markdown(index=False),
        "",
        "The key caveat is that PID and energy are downstream proxies rather than independently labelled particle species and calorimetric truth. They are still the appropriate raw-ROOT observables for this ticket because the odd duplicate channel is withheld from ML/NN features and used only to define a blinded closure target and post-hoc consumer stress test.",
        "",
        ...
        "```",
        "",
        "Artifacts: `result.json`, `manifest.json`, `downstream_consumer_summary.csv`, `raw_reproduction.csv`, `run_family_knees.csv`, `action_band_counts_by_run.csv`, `acceptance_strata.csv`, `sentinel_acceptance.csv`, `candidate_counts_by_run.csv`, `benchmark_by_run.csv`, `benchmark_summary.csv`, `ml_minus_traditional.csv`, `calibration_by_run.csv`, `leakage_sentinels.csv`, `predictions.csv.gz`, and benchmark figures.",
        "",
    ]
...
def main() -> int:
    ...
    consumers = downstream_consumer_summary(summary)
    ...
    downstream_finding = (
        "Traditional run-family duplicate action bands win the downstream propagation test: "
        "timing tail delta {:+.6g}, q_template median shift {:+.6g}, PID harm proxy {:.6g}, "
        "and energy charge-bias proxy {:.5g} with charge res68 {:.5g}. The highest-support "
        "ML/NN method ({}) accepts far more rows but has nonzero downstream harm and weaker "
        "duplicate-charge closure."
    ).format(
        trad["timing_tail_delta"],
        trad["q_template_median_shift"],
        trad["harm_rate_vs_no_correction"],
        trad["charge_bias"],
        trad["charge_res68"],
        support_utility_winner["method"],
    )
    ...
    result = {
        ...
        "downstream_winner_method": str(winner["method"]),
        "downstream_consumer_finding": downstream_finding,
    ...
    write_report(out, result, reproduction, knees, summary, calibration, leakage, sentinels, consumers)
    manifest = {
        "ticket": cfg["ticket"],
        "study": cfg["study"],
