#!/usr/bin/env python3
"""S02e current/rate-constrained drift nuisance test for template/timewalk closure.

The study extends S02b without changing its raw ROOT gate or amplitude-binned
template construction.  The only new ingredient is a low-dimensional per-run
current/rate basis derived before timing labels, learned on train runs only and
extrapolated to the held-out run.
"""
...
def current_nA_for_run(config: dict, run: int) -> float:
    mapping = {int(k): float(v) for k, v in config.get("run_covariates", {}).get("current_nA_by_run", {}).items()}
    return float(mapping.get(int(run), config.get("run_covariates", {}).get("default_current_nA", 20.0)))


def raw_run_covariates(config: dict) -> pd.DataFrame:
    """Build run-level covariates from raw counters and amplitude gates only.

    These are deliberately computed without pairwise timing residuals, target
    labels, or held-out timing outcomes.  The Sample-II current is constant in
    the docs, so the trigger/amplitude-rate proxies carry the run variation.
    """

    baseline_idx = [int(i) for i in config["baseline_samples"]]
    all_staves = {name: int(ch) for name, ch in config["staves"].items()}
    stave_names = list(all_staves.keys())
    channels = np.asarray([all_staves[name] for name in stave_names])
    downstream_idx = [stave_names.index(name) for name in config["timing"]["downstream_staves"]]
    ...
    for run in sorted(set(config["timing"]["train_runs"] + config["timing"]["heldout_runs"])):
        ...
        selected_pulses = 0
        selected_events = 0
        downstream_allhit_events = 0
        for batch in s02.iter_raw(path, ["TRIGGER", "EVENTNO", "HRDv"]):
            eventno = np.asarray(batch["EVENTNO"]).astype(int)
            trigger = np.asarray(batch["TRIGGER"]).astype(float)
            ...
            if first_eventno is None and len(eventno):
                first_eventno = int(eventno[0])
            if len(eventno):
                last_eventno = int(eventno[-1])
            entries += int(len(eventno))
            trigger_sum += float(trigger.sum())
            selected_pulses += int(selected.sum())
            selected_events += int(selected.any(axis=1).sum())
            downstream_allhit_events += int(selected[:, downstream_idx].all(axis=1).sum())
        eventno_span = max(int(last_eventno - first_eventno + 1), 1) if first_eventno is not None else 1
        rows.append(
            {
                "run": int(run),
                "current_nA": current_nA_for_run(config, int(run)),
                "raw_entries": int(entries),
                "eventno_span": int(eventno_span),
                "trigger_sum": float(trigger_sum),
                "trigger_entry_density": float(trigger_sum / eventno_span),
                "entries_per_eventno": float(entries / eventno_span),
                "selected_multiplicity_per_event": float(selected_pulses / max(entries, 1)),
                "selected_event_fraction": float(selected_events / max(entries, 1)),
                "downstream_allhit_fraction": float(downstream_allhit_events / max(entries, 1)),
            }
        )
    return pd.DataFrame(rows)


def ensure_run_covariates(config: dict) -> pd.DataFrame:
    cached = config.get("_s02e_run_covariates")
    if cached is not None:
        return cached
    table = raw_run_covariates(config)
    train = table[table["run"].isin(config["timing"]["train_runs"])]
    for col in config["timewalk"].get("run_covariates", []):
    ...
    config["_s02e_run_covariates"] = table
    return table
...
def drift_features(pulses: pd.DataFrame, config: dict, order: int) -> Tuple[np.ndarray, List[str]]:
    ...
    if int(order) <= 0:
        return base, columns
    cov_table = ensure_run_covariates(config)
    cov_cols = [f"{name}_z" for name in config["timewalk"].get("run_covariates", [])]
    cov_by_run = cov_table.set_index("run")[cov_cols]
    cov = pulses["run"].map({int(run): row for run, row in cov_by_run.iterrows()})
    cov_values = np.vstack([np.asarray(row, dtype=float) for row in cov.to_numpy()])
    ...
    for power in range(1, int(order) + 1):
        ...
        for stave in config["timing"]["downstream_staves"]:
            ...
            names.extend([f"{stave}_{name}_p{power}" for name in cov_cols])
...
def leakage_checks(
    ...
    rows = [
        ...
        {"check": "covariate_basis_contains_run_one_hot", "value": 0, "pass": True},
        {"check": "covariate_basis_contains_chronological_run_z", "value": 0, "pass": True},
        {"check": "covariates_derived_before_timing_labels", "value": 1, "pass": True},
        {"check": "covariate_basis_uses_heldout_targets", "value": 0, "pass": True},
...
def write_plots(out_dir: Path, bench: pd.DataFrame, pulses: pd.DataFrame, methods: List[Tuple[str, str]], config: dict, cv: pd.DataFrame) -> None:
    ...
    ax.set_title("S02e current/rate run-held-out benchmark")
    fig.tight_layout()
    ...
    if len(cv):
        ...
        ax.set_title("Current/rate covariate-power selection diagnostic")
        fig.tight_layout()
...
def write_report(
    ...
    binned0 = bench[bench["method"] == "S02b binned timewalk no covariate"].iloc[0]
    binned_sel = bench[bench["method"] == selected_binned_label].iloc[0]
    global0 = bench[bench["method"] == "S02b global timewalk no covariate"].iloc[0]
    ...
    md = f"""# S02e: current/rate-constrained drift in amplitude-binned template/timewalk closure
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/s02e_current_rate_drift_timewalk.json")
    ...
    run_covariates = ensure_run_covariates(config)
    run_covariates.to_csv(out_dir / "run_covariates_prelabel.csv", index=False)

    work, binned_cv, binned_cal, binned_coef = add_timewalk_candidates(work, config, "s02b_template", "s02e_binned_timewalk")
    work, global_cv, global_cal, global_coef = add_timewalk_candidates(work, config, "template_phase", "s02e_global_timewalk")
    ...
    for method, label, ref_key in [
        ("s02e_binned_timewalk_drift0", "S02b binned-template timewalk", "binned_template_timewalk_sigma68_ns"),
        ("s02e_global_timewalk_drift0", "S02b global-template timewalk", "global_template_timewalk_sigma68_ns"),
    ]:
    ...
    if selected_binned != "s02e_binned_timewalk_drift0":
        methods.insert(2, (selected_binned, f"S02e binned selected power {selected_binned.rsplit('drift', 1)[1]}"))
    if selected_global != "s02e_global_timewalk_drift0":
        methods.insert(-1, (selected_global, f"S02e global selected power {selected_global.rsplit('drift', 1)[1]}"))
    ...
    plot_methods = [
        ("s02e_binned_timewalk_drift0", "binned no covariate"),
        (selected_binned, "binned selected drift"),
        ("s02e_global_timewalk_drift0", "global no covariate"),
    ...
    binned0 = bench[bench["internal_method"] == "s02e_binned_timewalk_drift0"].iloc[0]
    binned_sel = bench[bench["internal_method"] == selected_binned].iloc[0]
    global0 = bench[bench["internal_method"] == "s02e_global_timewalk_drift0"].iloc[0]
    ...
    result = {
        ...
        "amplitude_binned_template_timewalk": {
            ...
            "covariate_drift_improves": bool(float(binned_sel["value"]) < float(binned0["value"])),
        },
        "ml": {
        ...
        "next_tickets": [
            "S02f: leave-one-run-out current/rate-constrained drift scan over all Sample II runs",
            "S02g: external spill-clock/scaler rate covariate audit if scaler data are available",
        ],
