...
are selected. The traditional score is pre-registered as |C_t| only; the ML
score is a run-held-out random forest over amplitude-normalized waveform shapes.
"""
...
def write_report(out_dir: Path, config: dict, reproduction: pd.DataFrame, run_counts: pd.DataFrame, scoreboard: pd.DataFrame, leakage: pd.DataFrame, fixed_eff: pd.DataFrame, result: dict) -> None:
    ml = scoreboard[scoreboard["method"] == "shape-only RF"].iloc[0]
    trad = scoreboard[scoreboard["method"] == "curvature-only traditional"].iloc[0]
    text = f"""# S07e: all-three-downstream curvature-only timing-control RF audit
...
The guarded `D_t>51 ns` count reproduces the documented App.I **72 gross events** exactly before the S07e all-three restriction is applied. The all-three subset keeps {result['details']['n_all_three_control']} control events and {result['details']['n_all_three_gross_guarded']} guarded gross events.
...
The RF score is still very high after the all-three topology restriction (`ROC AUC={ml['roc_auc']:.3f}`), but curvature-only is perfect (`ROC AUC={trad['roc_auc']:.3f}`). This is not evidence of software leakage: in the raw-derived all-three extreme set, max clean `|C_t|` is {result['details']['max_clean_abs_c_t_ns']:.3f} ns and min gross `|C_t|` is {result['details']['min_gross_abs_c_t_ns']:.3f} ns. The shuffled-label control is near chance, so there is no obvious row/run leakage through the feature matrix. The leaky `D_t` control is perfect by construction and confirms that the target remains self-referential to downstream timing. The amplitude-only probe is non-trivial, so the RF should be treated as a morphology/amplitude-correlated timing-tail diagnostic, not an independent timing truth.

## Verdict
After removing missing-stave topology, most of the S07b-style RF ranking remains: shape-only RF AUC is {ml['roc_auc']:.3f} [{ml['roc_auc_ci_low']:.3f}, {ml['roc_auc_ci_high']:.3f}], versus curvature-only AUC {trad['roc_auc']:.3f} [{trad['roc_auc_ci_low']:.3f}, {trad['roc_auc_ci_high']:.3f}]. The result argues that missing-stave topology was not the dominant source of the high App.I RF score, but the label is still `D_t`-defined and the positive class is only {result['details']['n_gross']} events in the all-three extreme benchmark.

## Reproducibility
```bash
uv run --with uproot --with numpy --with pandas --with scikit-learn python {out_dir / 's07e_all_three_downstream_curvature_rf.py'} --config {out_dir / 's07e_config.json'}
```
...
def main() -> int:
    ...
    parser.add_argument("--config", default="reports/1781006037.1500.1d8044e2__s07e_all_three_downstream_curvature_rf/s07e_config.json")
    ...
    reproduction = pd.DataFrame(
        [
            ...
            {"quantity": "S07e all-three control events", "report_value": None, "reproduced": int(all_three.sum()), "delta": None, "tolerance": None, "pass": True},
            {"quantity": "S07e all-three guarded gross events", "report_value": None, "reproduced": int((all_three & gross_guard_parent).sum()), "delta": None, "tolerance": None, "pass": True},
        ]
    )
    ...
    result = {
        ...
        "next_tickets": [
            "S07f: validate all-three App.I RF against an independent non-D_t timing-tail target",
            "S07g: stratify all-three App.I RF by amplitude/current to separate morphology from rate-dependent pulse quality",
        ],
    }

    write_report(out_dir, config, reproduction, run_counts, scoreboard, leakage, fixed_eff, result)
