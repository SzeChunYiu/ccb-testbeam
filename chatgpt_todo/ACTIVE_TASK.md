...
- **Publication child:** the first two-file replace design was rejected because product and manifest could become mixed across a crash. The current branch stages both artifacts privately, fsyncs them, binds the product digest in the manifest, then renames one immutable generation directory under `flock`; duplicate generation IDs fail closed and there is no mutable latest alias.
- **Tests/falsifiers:** step splitting, multi-record aggregation, neutral-vs-charged deposit, A-arm exclusion, malformed EDep/layers, invalid weights, duplicate IDs, broken trigger nesting, source mutation, mocked Uproot file-like integration, immutable-generation duplicate rejection and injected pre-publication failure. Private isolated aggregation harness: 16 passed with builder integration excluded; this is not repository validation.
- **Expert votes:** detector/Geant4 `ACCEPT H3 / BLOCK detector closure`; adversarial `ACCEPT bounded aggregation+publication / REVISE production-scale execution`; statistics `ACCEPT statistical unit / BLOCK p-value`; claims/provenance `ACCEPT nonauthorising provenance / BLOCK promotion`.
...
- **Current gate:** require exact-head/current-base MC Validation CI for the final PR #1169 head before merge. Earlier green/failed heads are non-authorising after code/docs changes. #1052/#1164 remain open even after H3 because H4/H5 and real-product validation remain.
