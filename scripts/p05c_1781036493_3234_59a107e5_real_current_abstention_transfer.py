#!/usr/bin/env python3
"""P05c real-current abstention transfer benchmark.

The ticket asks whether P05b-style failure-aware abstention transfers from
injected two-pulse closures to real high-current S10e/S11b candidate windows.
This script keeps the reviewed S11b raw-ROOT event construction and adds a
run-held-out bakeoff: a bounded template fit versus ridge, gradient-boosted
trees, MLP, a small 1D-CNN, and a consensus abstention ensemble.
"""
...
DEFAULT_CONFIG = ROOT / "configs" / "p05c_1781036493_3234_59a107e5_real_current_abstention_transfer.json"
THIS_SCRIPT = "scripts/p05c_1781036493_3234_59a107e5_real_current_abstention_transfer.py"
...
def score_fold(
    ...
    return pd.concat(score_frames, ignore_index=True), pd.concat(run_frames, ignore_index=True), template_summary, pd.DataFrame(cal_rows)
...
def write_report(
    ...
    lines = [
        ...
        "",
        (
            "This study tests whether abstention ideas from P05b injection closures transfer to real high-current "
            "candidate windows. The primary observable is not truth-labelled pile-up recovery, because real data lack "
            "constituent truth. Instead, methods are compared on raw-ROOT candidate windows with run-held-out training, "
            "synthetic low-current calibration, matched high-minus-low secondary-fraction contrasts, and explicit "
            "retention of the high-amplitude/large-lowering/broad-late support region that a useful gate must not hide."
        ),
        "",
        "## Reproduction Gate",
        "",
        ...
        "",
        (
            "The dominant systematic is label mismatch: ML thresholds are calibrated on synthetic overlays, while the "
            "real-current sample has no constituent truth. The bootstrap captures run-to-run variation but not all "
            "model-form uncertainty. The support-retention column is therefore critical: an apparently clean gate that "
            "rejects the high-amplitude, large-lowering, broad-late support would not be operationally acceptable."
        ),
        "",
        "## Conclusion",
        "",
        (
            ...
            "The result supports cautious transfer of P05b-style abstention only as a real-data operating rule; it is "
            "not a truth-level decomposition of high-current pile-up."
        ),
        "",
        "## Reproducibility",
        "",
...
def main() -> int:
    ...
    for heldout_run in sorted(sample["run"].unique()):
        scores, per_run, templates, cal = score_fold(s11b, config, events, waves, sample, int(heldout_run), rng)
    ...
    write_report(out_dir, config, topology, reproduction, stratum_table, summary, bootstrap, metric_ci, cal, leakage, winner, runtime)
    ...
    result = {
        ...
        "next_tickets": [
            "P05d truth-stress abstention: build an external blinded hand-scan of accepted/rejected high-current broad-late candidates to validate whether P05c support-retention proxies correspond to real recoverability."
        ],
    }
