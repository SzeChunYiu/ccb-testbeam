...
Open testbeam PR #1226 proposes a gitlink to `ccb-sipm-core@5c6bb0278be4246f3da9007b71a5d13024b968ca`, but that core commit is based on `2027b06e...` rather than current validated core main. It therefore omits dark-history merge `f009b0d...` and kernel-history merge `68eadf3f...`. A blocking coordination comment was posted. An attempted connector action to convert #1226 back to draft was blocked by the safety classifier, so its visible draft state was not changed. Do not resolve its conflict by accepting the existing gitlink; rebase/cherry-pick measured-impulse work onto current core main and rerun exact-head Core CI.

A further child concern is that core commit `5c6bb027...` labels two provenance fields as SHA-256 while its own commit description says they are `LEN-<size>` placeholders. Treat those fields as unvalidated provenance until the rebased implementation is inspected and tested.
