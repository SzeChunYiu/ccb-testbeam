#!/usr/bin/env python3
"""S07m: charge-preserved shape-cue localization benchmark."""
...
os.environ.setdefault("MPLCONFIGDIR", "/tmp/ccb-testbeam-s07m-matplotlib-cache")
...
def write_report(
    ...
    text = f"""# S07m: charge-preserved shape-cue localization
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/s07m_1781063920_486_09951fba_charge_preserved_shape_cue_localization.json")
    ...
    s07f = load_module(S07F_PATH, "s07f_reference_s07m")
    s07g = load_module(S07G_PATH, "s07g_reference_s07m")
    ...
    data = s07g.make_preserved_dataset(config, utils, clean_payloads, "charge_preserved")
    y = data["label_injected"].to_numpy(dtype=int)
    runs = data["run"].to_numpy(dtype=int)
    shape_cols = strict_shape_columns(data, utils)
        ...
        [
            summarize_method(
                ...
                n_boot,
                {
                    ...
                    "ridge logistic": "L2 logistic regression on strict normalized shape features.",
                    "gradient-boosted trees": "HistGradientBoostingClassifier on strict normalized shape features.",
                    "MLP": "Two-hidden-layer feed-forward network on strict normalized shape features.",
                    "1D-CNN": "Compact convolutional network on normalized four-stave waveform tensors.",
                    "WaveAtomNet": "New feature-fused convolutional waveform plus strict-shape atom network.",
                }[name],
            )
    ...
    leakage = pd.DataFrame(
        [
            ...
            {"probe": "absolute-amplitude-only GBT", "roc_auc": auc(y, amp_score), "average_precision": ap(y, amp_score), "notes": "Excluded nuisance channel; charge preservation should reduce but not force chance."},
            {"probe": "pair split violations", "roc_auc": float(pair_split_violations), "average_precision": float("nan"), "notes": "Must be 0."},
            {"probe": "forbidden strict-shape columns", "roc_auc": 0.0, "average_precision": float("nan"), "notes": "strict_shape_columns raised on violations before model fitting."},
        ]
    )
    ...
    write_report(out_dir, config, reproduction, counts, scoreboard, fold_scores, traditional_choices, localization, permutation, leakage, result)
    ...
    manifest = {
        ...
        "inputs": input_rows,
        "outputs": {},
        "command": f"/home/billy/anaconda3/bin/python scripts/s07m_1781063920_486_09951fba_charge_preserved_shape_cue_localization.py --config {config_path}",
    }
