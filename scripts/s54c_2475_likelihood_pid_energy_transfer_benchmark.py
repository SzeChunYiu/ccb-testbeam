#!/usr/bin/env python3
"""S54c: rate-baseline energy and PID robustness benchmark.
...
a train-run Birks calibration anchored by geant4/configs/dedx_p_in_CD2.txt.
...
def make_report(
    ...
    lines = [
        ...
        "",
        (
            "This ticket studies likelihood PID and calibrated energy transfer by linking waveform "
            "shape, pedestal state, timing/peak summaries, pile-up multiplicity, and saturation "
            "censoring to duplicate-readout dE-E observables in raw B-stack data. The analysis reads "
            "the raw ROOT waveform branch, subtracts per-event pretrigger pedestals, benchmarks a "
            "strong traditional dE/dx/Birks likelihood method against ridge, gradient-boosted trees, "
            "MLP, 1D-CNN, and a new range-gated residual MLP, and estimates 95% confidence intervals "
            "by held-out run bootstrap. The raw ROOT reproduction gate passes "
...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/s54c_2475_likelihood_pid_energy_transfer_benchmark.yaml")
    ...
    manifest = {
        ...
        "command": "/home/billy/anaconda3/bin/python scripts/s54c_2475_likelihood_pid_energy_transfer_benchmark.py --config configs/s54c_2475_likelihood_pid_energy_transfer_benchmark.yaml",
