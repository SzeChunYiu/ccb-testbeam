#!/usr/bin/env python3
"""S02f Sample-II LORO current/rate-constrained run-drift study.
...
leave-one-run-out split requested by ticket 1781022115.1020.625b5a98.
"""
...
def run_fold(all_pulses: pd.DataFrame, config: dict, heldout_run: int, raw_covariates: pd.DataFrame, rng: np.random.Generator) -> dict:
    ...
    methods_for_bench = [
        ...
        (selected_binned, f"S02f binned current/rate selected power {selected_binned.rsplit('drift', 1)[1]}"),
        ("s02e_global_timewalk_drift0", "S02b global timewalk no covariate"),
        (selected_global, f"S02f global current/rate selected power {selected_global.rsplit('drift', 1)[1]}"),
...
def write_plots(out_dir: Path, bench: pd.DataFrame, run_boot: pd.DataFrame) -> None:
    ...
    ax.set_title("S02f current/rate LORO held-out performance")
...
def write_report(
    ...
    binned_sel = run_boot[run_boot["method"].str.startswith("S02f binned current/rate selected")].sort_values("mean_sigma68_ns").iloc[0]
    global_sel = run_boot[run_boot["method"].str.startswith("S02f global current/rate selected")].sort_values("mean_sigma68_ns").iloc[0]
    ...
    md = f"""# S02f: leave-one-run-out current/rate drift scan
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/s02f_1781022115_1020_625b5a98_current_rate_loro.json")
    ...
    result = {
        ...
        "amplitude_binned_current_rate_selected_best": run_boot[run_boot["method"].str.startswith("S02f binned current/rate selected")].sort_values("mean_sigma68_ns").iloc[0].to_dict(),
        "global_current_rate_selected_best": run_boot[run_boot["method"].str.startswith("S02f global current/rate selected")].sort_values("mean_sigma68_ns").iloc[0].to_dict(),
