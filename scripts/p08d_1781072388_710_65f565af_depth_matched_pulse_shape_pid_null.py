#!/usr/bin/env python3
"""P08d: depth-matched pulse-shape PID null.

This study is deliberately weak-label only. It rebuilds the P08b calibrated
range-energy residual labels from raw B-stack ROOT, uses run-held-out matching
and control probes, and asks whether waveform shape carries PID-like separation
after accounting for penetration depth, charge, run family, saturation, anomaly,
and action-band support.
"""
...
DEFAULT_CONFIG = ROOT / "configs/p08d_1781072388_710_65f565af_depth_matched_pulse_shape_pid_null.json"
...
def write_report(out_dir: Path, config: dict, result: dict, reproduction: pd.DataFrame, composition: pd.DataFrame, scoreboard: pd.DataFrame, deltas: pd.DataFrame, audit: pd.DataFrame) -> None:
    ...
    report = """# P08d: Depth-matched pulse-shape PID null
...
This study tests the ticket's null hypothesis: after matching penetration depth,
charge, current/run family, saturation and anomaly support, does B2 waveform shape
carry any residual PID-like separation?  The result is not a truth-PID measurement.
I rebuild the calibrated P08b weak labels directly from raw B-stack ROOT, merge
the existing out-of-fold S14g veto-ladder and P07j saturation action-band decisions,
and benchmark a frozen traditional calibrated charge/depth score against ridge,
gradient-boosted trees, MLP, 1D-CNN, and a new action-gated residual waveform
ensemble.  Controls include charge-only, depth/topology-only, action-only,
run-family-only, and shuffled-label waveform probes.
...
```bash
/home/billy/anaconda3/bin/python scripts/p08d_1781072388_710_65f565af_depth_matched_pulse_shape_pid_null.py --config configs/p08d_1781072388_710_65f565af_depth_matched_pulse_shape_pid_null.json
```
...
def main() -> int:
    ...
    result = {
        ...
        "command": "/home/billy/anaconda3/bin/python scripts/p08d_1781072388_710_65f565af_depth_matched_pulse_shape_pid_null.py --config {}".format(args.config),
    }
    ...
    manifest = {
        "ticket_id": config["ticket_id"],
        "script": "scripts/p08d_1781072388_710_65f565af_depth_matched_pulse_shape_pid_null.py",
