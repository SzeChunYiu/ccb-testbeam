...
whether quiet adjacent-current/external control runs can expand the low-current
support atoms used by S13e without importing current-label leakage.
"""
...
os.environ.setdefault("MPLCONFIGDIR", str(ROOT / "reports" / ".mplconfig_s13f_1781173865"))
...
def run_family_lookup(config: dict) -> Dict[int, str]:
    ...
    for family, groups in config["run_families"].items():
        for run in groups["low"] + groups["high"]:
            out[int(run)] = family
    for run in config.get("external_control_runs", []):
        out[int(run)] = "external_control"
    return out
...
def topology_by_run(config: dict, data_by_run: Dict[int, dict]) -> pd.DataFrame:
    ...
    for run, data in sorted(data_by_run.items()):
        ...
        if run in low_runs:
            current_group = "low_2nA"
            high_current = 0
        elif run in high_runs:
            current_group = "high_20nA"
            high_current = 1
        else:
            current_group = "external_control"
            high_current = -1
        rows.append(
            {
                "run": int(run),
...
def reproduction_table(config: dict, topology: pd.DataFrame) -> pd.DataFrame:
    ...
    panel = topology[topology["run"].isin(low_runs + high_runs)].copy()
    ...
    rows = [
        ...
        },
        {
            ...
            "reproduced": float(panel["events_with_selected"].sum()),
            "delta": float(panel["events_with_selected"].sum() - int(config["expected_s13b_events_with_selected"])),
            "tolerance": 0.0,
            "pass": int(panel["events_with_selected"].sum()) == int(config["expected_s13b_events_with_selected"]),
        },
        {
            "quantity": "S13b selected B-stack pulses",
            "report_value": float(config["expected_s13b_selected_pulses"]),
            "reproduced": float(panel["selected_pulses"].sum()),
            "delta": float(panel["selected_pulses"].sum() - int(config["expected_s13b_selected_pulses"])),
            "tolerance": 0.0,
            "pass": int(panel["selected_pulses"].sum()) == int(config["expected_s13b_selected_pulses"]),
        },
    ]
    return pd.DataFrame(rows)
...
def build_pulse_dataset(config: dict, data_by_run: Dict[int, dict]) -> Tuple[pd.DataFrame, pd.DataFrame, np.ndarray]:
    low_runs = set(int(x) for x in config["low_current_runs"])
    high_runs = set(int(x) for x in config["high_current_runs"])
    external_runs = set(int(x) for x in config.get("external_control_runs", []))
    ...
    for run, data in sorted(data_by_run.items()):
        ...
        for stave_idx, stave_name in enumerate(stave_names):
            ...
            for local_idx in event_idx:
                run_i = int(run)
                if run_i in external_runs:
                    current_group = "external_control"
                    high_current = -1
                elif run_i in low_runs:
                    current_group = "low_2nA"
                    high_current = 0
                elif run_i in high_runs:
                    current_group = "high_20nA"
                    high_current = 1
                else:
                    raise ValueError(f"run {run_i} is not assigned to low, high, or external control")
                rows.append(
                    {
                        "run": run_i,
                        ...
                        "run_family": family_for_run[run_i],
                        "current_group": current_group,
                        "high_current": high_current,
...
def traditional_matched_score(
    train_atoms: pd.DataFrame,
    train_y: np.ndarray,
    test_atoms: pd.DataFrame,
    min_per_current: int,
    external_atoms: pd.DataFrame | None = None,
    ...
    if external_atoms is not None and len(external_atoms) > 0 and external_weight > 0:
        ext = external_atoms.copy()
        ext["external_count"] = 1
        ext_counts = ext.groupby(keys)["external_count"].sum().reset_index()
        grouped = grouped.merge(ext_counts, on=keys, how="left")
    else:
        grouped["external_count"] = 0
    grouped["external_count"] = grouped["external_count"].fillna(0).astype(float)
    grouped["expanded_low_support_count"] = grouped["low_count"] + float(external_weight) * grouped["external_count"]
    grouped["effective_pairs"] = np.minimum(grouped["sum"], grouped["expanded_low_support_count"])
    grouped["labeled_supported"] = (grouped["sum"] >= min_per_current) & (grouped["low_count"] >= min_per_current)
    grouped["supported"] = (grouped["sum"] >= min_per_current) & (grouped["expanded_low_support_count"] >= min_per_current)
    grouped["support_expanded_by_external"] = grouped["supported"] & ~grouped["labeled_supported"]
    grouped["score"] = (grouped["sum"] + 12.0 * global_rate) / (grouped["count"] + 12.0)
    merged = test_atoms.merge(
        grouped[keys + ["score", "supported", "labeled_supported", "support_expanded_by_external", "sum", "low_count", "external_count", "expanded_low_support_count", "effective_pairs"]],
        on=keys,
        how="left",
    )
...
def evaluate(preds: pd.DataFrame, support: pd.DataFrame, seed: int, n_boot: int) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    trad = metrics[metrics["method"] == "traditional_external_support_table"].iloc[0]
    ...
    for method in methods:
        if method == "traditional_external_support_table":
            continue
        mboot = boot_all[boot_all["method"] == method].reset_index(drop=True)
        tboot = boot_all[boot_all["method"] == "traditional_external_support_table"].reset_index(drop=True)
...
def run_study(config: dict, data_by_run: Dict[int, dict]) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for fold_i, fold in enumerate(config["folds"]):
        ...
        external_runs = [int(x) for x in config.get("external_control_runs", [])]
        ...
        external_idx = capped_indices(meta, external_runs, int(config["max_external_events_per_run"]), seed + 30 + fold_i) if external_runs else np.array([], dtype=int)
        train_meta = meta.loc[train_idx].reset_index(drop=True)
        test_meta = meta.loc[test_idx].reset_index(drop=True)
        external_meta = meta.loc[external_idx].reset_index(drop=True) if len(external_idx) else pd.DataFrame(columns=meta.columns)
        train_features = features.loc[train_idx].reset_index(drop=True)
        test_features = features.loc[test_idx].reset_index(drop=True)
        external_features = features.loc[external_idx].reset_index(drop=True) if len(external_idx) else pd.DataFrame(columns=features.columns)
        train_y = train_meta["high_current"].to_numpy(dtype=int)
        test_y = test_meta["high_current"].to_numpy(dtype=int)
        if (train_y < 0).any() or (test_y < 0).any():
            raise RuntimeError("external-control rows entered labeled train/test current splits")
        support_features = pd.concat([train_features, external_features], ignore_index=True)
        support_meta = pd.concat([train_meta, external_meta], ignore_index=True)
        support_atoms, test_atoms, _edges = atomize(support_features, support_meta, test_features, test_meta)
        train_atoms = support_atoms.iloc[: len(train_meta)].reset_index(drop=True)
        external_atoms = support_atoms.iloc[len(train_meta) :].reset_index(drop=True)
        trad_score, supported, atom_table = traditional_matched_score(
            train_atoms,
            train_y,
            test_atoms,
            int(config["support_min_per_current"]),
            external_atoms=external_atoms,
            external_weight=float(config.get("external_support_weight", 0.0)),
        )
        atom_table["fold"] = fold["name"]
        atom_parts.append(atom_table)

        support_x_res, test_x_res = residualize_by_atom(support_features, support_atoms, test_features, test_atoms, scalar_cols)
        train_x_res = support_x_res[: len(train_meta)]
        support_w_res, test_w_res = residualize_waves(np.concatenate([waves[train_idx], waves[external_idx]], axis=0), support_atoms, waves[test_idx], test_atoms)
        train_w_res = support_w_res[: len(train_meta)]
        ...
        method_raws: Dict[str, Tuple[np.ndarray, np.ndarray]] = {
            "traditional_external_support_table": (
                traditional_matched_score(
                    train_atoms,
                    train_y,
                    train_atoms,
                    int(config["support_min_per_current"]),
        ...
        leakage_rows.extend(
            [
                ...
                {"fold": fold["name"], "check": "external_rows_labeled", "value": int((train_y < 0).sum() + (test_y < 0).sum()), "flag": False, "note": "External control runs are support-only and never enter labeled train/test current arrays."},
                {"fold": fold["name"], "check": "external_train_test_run_overlap", "value": len(set(external_runs).intersection(set(train_runs).union(test_runs))), "flag": bool(set(external_runs).intersection(set(train_runs).union(test_runs))), "note": "External support runs must be disjoint from all labeled high/low runs."},
                {"fold": fold["name"], "check": "external_support_rows", "value": int(len(external_meta)), "flag": False, "note": "External control rows are atomized only to expand low-current-like support counts."},
...
def external_support_audit(atom_table: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for fold, sub in atom_table.groupby("fold"):
        all_cells = int(len(sub))
        expanded = int(sub["support_expanded_by_external"].sum())
        rows.append(
            {
                "fold": fold,
                "all_cells": all_cells,
                "supported_cells": int(sub["supported"].sum()),
                "labeled_supported_cells": int(sub["labeled_supported"].sum()),
                "cells_expanded_by_external": expanded,
                "support_expansion_fraction": float(expanded / all_cells) if all_cells else 0.0,
                "external_count_total": float(sub["external_count"].sum()),
                "expanded_low_support_total": float(sub["expanded_low_support_count"].sum()),
                "effective_pairs_total": float(sub["effective_pairs"].sum()),
            }
        )
    return pd.DataFrame(rows)
...
def write_report(config: dict, out_dir: Path, repro: pd.DataFrame, topology: pd.DataFrame, support: pd.DataFrame, atom_table: pd.DataFrame, audit: pd.DataFrame, metrics: pd.DataFrame, deltas: pd.DataFrame, leakage: pd.DataFrame, runtime: float) -> None:
    traditional_method = "traditional_external_support_table"
    ...
    trad = metrics[metrics["method"] == traditional_method].iloc[0]
    ml_candidates = candidates[candidates["method"] != traditional_method].copy()
    best_ml = ml_candidates.sort_values(["auc", "brier"], ascending=[False, True]).iloc[0]
    ...
    lines = [
        ...
        f"- **Config:** `configs/s13f_1781173865_934_2a1e2781_external_low_current_support_validation.json`",
        "",
        "## 0. Question",
        "",
        ...
        "## 5. Results",
        "",
        f"The overall held-out benchmark winner is **{winner['method']}** with AUC **{winner['auc']:.4f}** [{winner['auc_ci_low']:.4f}, {winner['auc_ci_high']:.4f}], AP **{winner['average_precision']:.4f}**, ECE **{winner['ece_10bin']:.4f}**, and score excess **{winner['score_excess_high_minus_low']:.4f}**. The best ML/NN candidate is **{best_ml['method']}** with AUC **{best_ml['auc']:.4f}** [{best_ml['auc_ci_low']:.4f}, {best_ml['auc_ci_high']:.4f}] and AUC-minus-traditional **{best_ml['auc_minus_traditional']:.4f}**.",
        "",
        ...
        "",
        "```bash",
        "/home/billy/anaconda3/bin/python scripts/s13f_1781173865_934_2a1e2781_external_low_current_support_validation.py --config configs/s13f_1781173865_934_2a1e2781_external_low_current_support_validation.json",
        "```",
        "",
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/s13f_1781173865_934_2a1e2781_external_low_current_support_validation.json"))
    ...
    all_runs = sorted(set(int(x) for x in config["low_current_runs"] + config["high_current_runs"] + config.get("external_control_runs", [])))
    ...
    audit = external_support_audit(atom_table)
    ...
    audit.to_csv(out_dir / "external_support_audit.csv", index=False)
    ...
    result = {
        ...
        "traditional": {
            ...
            "description": "frozen matched-atom high-current probability table over charge, topology, anomaly taxon, baseline lowering, and stave, with external controls used only to expand low-current-like support counts",
            "auc": float(trad["auc"]),
        ...
        "winner": {
            ...
            "metric": "highest held-out high-current AUC among traditional and ML/NN benchmark methods after support matching",
        ...
        "best_ml_candidate": {
            ...
            "auc": float(best_ml["auc"]),
            "auc_ci": [float(best_ml["auc_ci_low"]), float(best_ml["auc_ci_high"])],
            "auc_minus_traditional": float(best_ml["auc_minus_traditional"]),
        },
        "support": {
            ...
            "external_support_expanded_cells": int(audit["cells_expanded_by_external"].sum()),
            "external_support_weight": float(config.get("external_support_weight", 0.0)),
            "external_control_runs": [int(x) for x in config.get("external_control_runs", [])],
        },
        "interpretation": "External controls expanded low-current-like support only; they were excluded from high/low labels and model training. Residual CWoLa information is accepted only where paired deltas exceed the matched nuisance table under held-out run-family support.",
    ...
    write_report(config, out_dir, repro, topology, support, atom_table, audit, metrics, deltas, leakage, runtime)
    ...
    manifest = {
        ...
        "commands": [f"{sys.executable} scripts/s13f_1781173865_934_2a1e2781_external_low_current_support_validation.py --config {args.config}"],
