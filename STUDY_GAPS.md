...
| **NEW-04** | **Data-side trigger residual**: model accidental/pile-up coincidences and audit data Sample-I purity to explain the 6.7% deep-stave coincidence population the *ideal* trigger vetoes (0.06% of deep-proton events fire the A-paddle in MC). Links R_max/MC03 live-time (pile-up), B-M6 (run-set/beam), B-M8 (early-peak). | MV3 quantitative closure (real SD trigger done; residual is data-side, not a new production) | Medium (data-side + digitizer) |
