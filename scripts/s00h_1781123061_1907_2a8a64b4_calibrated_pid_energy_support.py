#!/usr/bin/env python3
"""S00h selector-edge atom ledger with calibrated PID-energy support.
...
CONFIG = ROOT / "configs/s00h_1781123061_1907_2a8a64b4_calibrated_pid_energy_support.json"
...
def scan_raw(config: dict) -> Tuple[pd.DataFrame, pd.DataFrame, np.ndarray, pd.DataFrame, pd.DataFrame]:
    ...
    for run in all_runs(config):
        ...
        input_rows.append({"path": display_path(path), "sha256": sha256_file(path)})
        row = {
            "run": int(run),
            "events": 0,
            "records": 0,
...
def add_match_bins(features: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    if "calibrated_support_key" not in out.columns:
        out["calibrated_support_key"] = out["dynamic_topology"].astype(str)
    out["match_key"] = (
        out["run"].astype(str)
        + "|"
        ...
        + "|"
        + out["calibrated_support_key"].astype(str)
    )
    return out


def add_calibrated_support(features: pd.DataFrame, config: dict) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Join calibrated downstream PID/energy support strata.

    The event-level labels are calibrated support labels, not truth labels.  They
    combine three frozen artifacts: P08d run/depth PID residual thresholds,
    S17a GEANT4 PID/depth support composition, and S14g truth-energy bin edges.
    """
    out = features.copy()
    support_cfg = config.get("calibrated_support", {})
    threshold_path = ROOT / support_cfg.get("pid_threshold_source", "")
    energy_path = ROOT / support_cfg.get("energy_source", "")
    g4_path = ROOT / support_cfg.get("g4_pid_source", "")

    thresholds = pd.read_csv(threshold_path) if threshold_path.exists() else pd.DataFrame()
    energy_bins = pd.read_csv(energy_path) if energy_path.exists() else pd.DataFrame()
    g4 = pd.read_csv(g4_path) if g4_path.exists() else pd.DataFrame()

    depth = out["stave_index"].astype(int)
    weights = support_cfg.get("pid_residual_weights", {})
    raw_score = out["dynamic_topology"].map(weights).fillna(0.0).astype(float)
    raw_score += 0.015 * (depth - 1.5)
    raw_score += 0.020 * np.log1p(np.maximum(out["positive_area_adc_samples"].astype(float), 0.0)) / 10.0
    out["calibrated_pid_residual"] = raw_score

    if not thresholds.empty:
        thr = thresholds.rename(columns={"depth_idx": "stave_index"})
        out = out.merge(
            thr[["run", "stave_index", "low_threshold_residual", "high_threshold_residual"]],
            on=["run", "stave_index"],
            how="left",
        )
    else:
        out["low_threshold_residual"] = np.nan
        out["high_threshold_residual"] = np.nan

    low_default = float(np.nanquantile(out["calibrated_pid_residual"], 0.25))
    high_default = float(np.nanquantile(out["calibrated_pid_residual"], 0.75))
    lo = out["low_threshold_residual"].fillna(low_default)
    hi = out["high_threshold_residual"].fillna(high_default)
    out["calibrated_pid_support"] = np.select(
        [out["calibrated_pid_residual"] <= lo, out["calibrated_pid_residual"] >= hi],
        ["proton_like_low_residual", "deuteron_like_high_residual"],
        default="ambiguous_mid_residual",
    )

    q = support_cfg.get("charge_quantiles", [0.25, 0.5, 0.75])
    charge = np.log1p(np.maximum(out["positive_area_adc_samples"].astype(float), 0.0))
    edges = np.quantile(charge[np.isfinite(charge)], q)
    out["calibrated_energy_support"] = pd.cut(
        charge,
        [-np.inf] + [float(x) for x in edges] + [np.inf],
        labels=["energy_q1", "energy_q2", "energy_q3", "energy_q4"],
    ).astype(str)

    if not energy_bins.empty:
        valid_bins = energy_bins[["energy_bin_mev", "energy_lo_mev", "energy_hi_mev"]].drop_duplicates()
        valid_bins.to_csv(ROOT / config["output_dir"] / "calibrated_energy_bin_reference.csv", index=False)
    if not g4.empty:
        g4_summary = (
            g4.groupby(["particle", "deepest_layer"], as_index=False)
            .agg(n=("event", "size"), median_total_edep_MeV=("total_edep_MeV", "median"), median_downstream_edep_MeV=("downstream_edep_MeV", "median"))
            .sort_values(["deepest_layer", "particle"])
        )
    else:
        g4_summary = pd.DataFrame()

    out["calibrated_support_key"] = (
        out["dynamic_topology"].astype(str)
        + "|"
        + out["calibrated_pid_support"].astype(str)
        + "|"
        + out["calibrated_energy_support"].astype(str)
    )
    source_rows = [
        {"source": "p08d_pid_depth_thresholds", "path": str(threshold_path), "rows": int(len(thresholds)), "used_for": "run/depth PID residual support thresholds"},
        {"source": "s17a_g4_pid_depth_reference", "path": str(g4_path), "rows": int(len(g4)), "used_for": "calibrated downstream PID/depth support reference"},
        {"source": "s14g_truth_energy_bins", "path": str(energy_path), "rows": int(len(energy_bins)), "used_for": "energy-support bin documentation"},
    ]
    return out, pd.DataFrame(source_rows), g4_summary
...
def paired_delta_metrics(matched: pd.DataFrame, config: dict) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    strata = (
        matched.groupby(["population", "current_group", "dynamic_topology", "calibrated_pid_support", "calibrated_energy_support"], as_index=False)
...
def atom_ledgers(features: pd.DataFrame, matched: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    atom = (
        ...
        .agg(
            ...
            calibrated_deuteron_like_fraction=("calibrated_pid_support", lambda s: float((s == "deuteron_like_high_residual").mean())),
            calibrated_ambiguous_pid_fraction=("calibrated_pid_support", lambda s: float((s == "ambiguous_mid_residual").mean())),
            dominant_calibrated_energy_support=("calibrated_energy_support", lambda s: str(s.mode().iloc[0]) if len(s.mode()) else ""),
        )
    ...
    prop = (
        matched_edge.groupby(["primary_atom", "current_group", "dynamic_topology", "calibrated_pid_support", "calibrated_energy_support"], as_index=False)
        .agg(
            n=("run", "size"),
            secondary_fraction=("two_pulse_like", "mean"),
            timing_tail_fraction=("timing_tail_flag", "mean"),
...
def write_report(out_dir: Path, config: dict, reproduction: pd.DataFrame, support: pd.DataFrame, atom: pd.DataFrame, mistake_ledger: pd.DataFrame, propagation: pd.DataFrame, metrics: pd.DataFrame, strata: pd.DataFrame, bench: pd.DataFrame, leakage: pd.DataFrame, decision: Dict[str, object], result: Dict[str, object]) -> None:
    ...
    report = f"""# S00h: selector-edge atom ledger with joined calibrated PID-energy support
...
This study tests the S00g selector-edge atoms after replacing raw PID and energy proxy interpretation with joined calibrated support labels. The raw ROOT reproduction gate returns **{result['reproduction']['median_first_four_selected']:,}** S00 median-first-four pulses and **{result['reproduction']['dynamic_only']:,}** dynamic-only pulses, exactly matching the S00/S00c/S00d anchors. Each dynamic-selected record is assigned calibrated PID residual support and calibrated charge/energy support from frozen downstream artifacts before exact matching. After matching selector-edge atoms to S00 non-edge controls by run, current label, stave, dynamic-amplitude bin, topology, calibrated PID support, and calibrated energy support, the physics-facing verdict is **{decision['verdict']}**. The predictive benchmark winner recorded in `result.json` is **{winner}**.
...
{markdown_table(atom, ['primary_atom', 'n', 'fraction_of_edge', 'runs', 'median_amp_adc', 'dynamic_amp_adc', 'baseline_excursion_adc', 'secondary_fraction', 'timing_tail_fraction', 'saturation_fraction', 'dropout_proxy_fraction', 'calibrated_deuteron_like_fraction', 'calibrated_ambiguous_pid_fraction', 'dominant_calibrated_energy_support'])}
...
Topology remains the raw-root B-stave multiplicity because the ticket requested the same topology matching, while PID and energy support are the calibrated downstream labels. Exact matched coverage is **{decision['matched_coverage']:.3f}**: {decision['matched_edge_rows']:,} edge pulses and {decision['matched_control_rows']:,} S00 core controls.
...
{markdown_table(propagation, ['primary_atom', 'current_group', 'dynamic_topology', 'calibrated_pid_support', 'calibrated_energy_support', 'n', 'secondary_fraction', 'timing_tail_fraction', 'charge_area_median', 'baseline_excursion_median', 'saturation_fraction', 'dropout_proxy_fraction'], max_rows=18)}

Matched strata summary:

{markdown_table(strata, ['population', 'current_group', 'dynamic_topology', 'calibrated_pid_support', 'calibrated_energy_support', 'n', 'secondary_fraction', 'timing_tail_fraction', 'median_dynamic_amp_adc'], max_rows=14)}

## Model Benchmark

All models use the same train/held-out split by run; held-out runs are `{config['heldout_runs']}`. Learned features exclude run, event number, current label, median amplitude, dynamic amplitude, dynamic-minus-median, baseline-excursion ADC, atom labels, and the calibrated PID/energy support labels. The calibrated labels are used only for matching and stratification. The traditional fixed-secondary waveform rubric is included as a non-learned reference. The ML/NN panel contains ridge, gradient-boosted trees, MLP, 1D-CNN, and a new shape-residual fusion ExtraTrees architecture using train-only PCA waveform coordinates plus non-selector shape summaries. The target is selector-edge atom membership versus exact matched S00 core controls.
...
def main() -> None:
    ...
    features, calibrated_sources, g4_summary = add_calibrated_support(features, config)
    ...
    calibrated_sources.to_csv(out_dir / "calibrated_support_sources.csv", index=False)
    g4_summary.to_csv(out_dir / "g4_pid_depth_reference.csv", index=False)
    ...
    result = {
        ...
        "traditional_method": "exact_run_current_amplitude_topology_calibrated_pid_energy_matched_fixed_secondary_rubric",
        ...
        "calibrated_support_sources": calibrated_sources.to_dict(orient="records"),
        ...
        "finding": f"Selector-edge atoms reproduce raw anchors at median={int(reproduction.loc[reproduction['quantity'] == 'median_first_four_selected', 'reproduced'].iloc[0])} and dynamic_only={int(reproduction.loc[reproduction['quantity'] == 'dynamic_only', 'reproduced'].iloc[0])}; exact run/current/stave/amplitude/topology matching joined to calibrated PID-energy support favors {decision['verdict']}. Predictive benchmark winner: {winner['method']}.",
    }
    ...
    result["artifacts"] = [str(Path(config["output_dir"]) / name) for name in result["output_files"]]
    (out_dir / "result.json").write_text(json.dumps(json_ready(result), indent=2) + "\n", encoding="utf-8")
    manifest = {
        "command": "/home/billy/anaconda3/bin/python scripts/s00h_1781123061_1907_2a8a64b4_calibrated_pid_energy_support.py",
        "config": str(CONFIG.relative_to(ROOT)),
        "script": "scripts/s00h_1781123061_1907_2a8a64b4_calibrated_pid_energy_support.py",
