#!/bin/bash -l
#SBATCH --account=lu2026-2-51 --partition=gpua40 --gres=gpu:1 --time=00:20:00 --job-name=ccb-sd-verify
set +e
SPIKE=/projects/hep/fs10/shared/nnbar/billy/opticks-spike
LOG=$SPIKE/logs/sdetect_verify.log
...
echo "=== SDETECT-VERIFY START $(date) host=$(hostname) ===" > $LOG
nvidia-smi -L >> $LOG 2>&1

echo "##### [1] REBUILD ccb_hit_debug (corrected flag mapping) #####" >> $LOG
...
echo "##### [2] ccb_hit_debug (corrected) #####" >> $LOG
...
echo "=== SDETECT-VERIFY END $(date) ===" >> $LOG
