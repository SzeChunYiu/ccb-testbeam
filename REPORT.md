...
frontier.  The worker is `testbeam-laptop-4`.  The held-out winner written to `result.json`
...
`result.json` names **saturation_residual_fusion_new** as the S58b winner.  The traditional clipped
...
Runtime was `78.8` s on `Linux-5.15.0-139-generic-x86_64-with-glibc2.29`.
