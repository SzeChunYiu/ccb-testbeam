#!/usr/bin/env python3
"""P08a: penetration-depth weak-label PID null test from raw B-stack ROOT."""
...
def scan_raw(config: dict, raw_dir: Path) -> Tuple[np.ndarray, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for run in configured_runs(config):
        ...
        for batch in iter_raw(path):
            ...
            positive = (
                b2
                & (downstream_selected >= int(label_cfg["positive_min_downstream_selected"]))
                & (downstream_fraction >= float(label_cfg["positive_min_downstream_charge_fraction"]))
            )
            negative = (
                b2
                & (downstream_selected <= int(label_cfg["negative_max_downstream_selected"]))
                & (downstream_fraction <= float(label_cfg["negative_max_downstream_charge_fraction"]))
            )
            weak = positive | negative
            if weak.any():
                label = positive[weak].astype(np.int8)
                    ...
                    pd.DataFrame(
                        {
                            ...
                            "weak_label": label,
                            "weak_label_name": np.where(label == 1, label_cfg["positive_name"], label_cfg["negative_name"]),
                ...
                run_counts["weak_terminal_b2_like"] += int((label == 0).sum())
                run_counts["weak_penetrating_like"] += int((label == 1).sum())
            event_offset += int(len(raw))

        run_rows.append({**run_counts, **stave_counts})
        print(
            "run {:04d}: selected_pulses={} weak_terminal={} weak_penetrating={}".format(
                run,
                run_counts["selected_pulses"],
                run_counts["weak_terminal_b2_like"],
                run_counts["weak_penetrating_like"],
            ),
            flush=True,
        )
...
def build_benchmark(
    ...
    charge_depth_score = np.full(len(meta), np.nan)
    ...
    log_charge_depth = meta[charge_depth_cols].copy()
    for col in ["b2_amp", "b4_amp", "b6_amp", "b8_amp", "total_charge_adc_samples"]:
        log_charge_depth[col] = np.log1p(log_charge_depth[col].to_numpy(dtype=float))
    ...
    for fold_number, (train, test, heldout_run) in enumerate(folds, start=1):
        ...
        train_y = y[train]
        charge_depth_score[test] = fit_logistic_score(log_charge_depth.loc[train].to_numpy(dtype=float), train_y, log_charge_depth.loc[test].to_numpy(dtype=float))
    ...
    scores = {
        ...
        "ML raw B2 waveform + train-only PCA HGB": ml_score[valid],
        "leakage sentinel: charge/depth logistic": charge_depth_score[valid],
    ...
    diff = paired_auc_diff(
        ...
        scores["ML raw B2 waveform + train-only PCA HGB"],
        runs_eval,
        seed + 777,
        int(config["benchmark"]["bootstrap_replicates"]),
    )
    leakage = pd.DataFrame(
        [
            {
                "probe": "charge/depth logistic",
                "roc_auc": scoreboard.loc[scoreboard["method"] == "leakage sentinel: charge/depth logistic", "roc_auc"].iloc[0],
                "average_precision": scoreboard.loc[scoreboard["method"] == "leakage sentinel: charge/depth logistic", "average_precision"].iloc[0],
                "interpretation": "Forbidden ceiling: includes depth and presence fields used directly by the weak-label definition.",
            },
            {
...
def write_report(out_dir: Path, config: dict, result: dict, reproduction: pd.DataFrame, scoreboard: pd.DataFrame, leakage: pd.DataFrame, fold_counts: pd.DataFrame) -> None:
    ...
    ml = scoreboard[scoreboard["method"] == "ML raw B2 waveform + train-only PCA HGB"].iloc[0]
    charge = scoreboard[scoreboard["method"] == "leakage sentinel: charge/depth logistic"].iloc[0]
    shuffled = scoreboard[scoreboard["method"] == "leakage sentinel: shuffled-label HGB"].iloc[0]
    diff = result["ml_vs_traditional"]
    report = """# P08a: penetration-depth weak-label PID null test
...
- `{neg}`: B2 selected, zero downstream selected staves, downstream charge
  fraction <= {neg_frac:.2f}.
- `{pos}`: B2 selected, at least {pos_down} downstream selected staves, downstream
  charge fraction >= {pos_frac:.2f}.

The held-out benchmark uses {n_eval:,} balanced events across {n_runs} runs. Runs
...
| ML raw/PCA waveform HGB | {ml_auc:.3f} [{ml_lo:.3f}, {ml_hi:.3f}] | {ml_ap:.3f} | {ml_purity:.3f} |
...
""".format(
        ...
        neg_frac=float(config["weak_label"]["negative_max_downstream_charge_fraction"]),
        pos_down=int(config["weak_label"]["positive_min_downstream_selected"]),
        pos_frac=float(config["weak_label"]["positive_min_downstream_charge_fraction"]),
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/p08a_1781012712_914_09cf1a30.json"))
    ...
    manifest = {
        "ticket_id": config["ticket_id"],
        "script": "scripts/p08a_1781012712_914_09cf1a30_penetration_weak_pid.py",
