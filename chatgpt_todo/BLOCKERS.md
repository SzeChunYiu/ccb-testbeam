...
- **State:** OPEN
- **Task:** `AUD-G4-020`.
- **Confirmed defect:** current source blob `4e45e55b48c1d51320b9e6d0959b0b8423d0b2fc` groups ratios from distinct configured energies by species and prints `statistics.mean(ratios)` as `mean point-estimate ratio`, although every point records `uncertainty_method=NOT_EVALUATED` and no covariance, weighting model, or combined measurand exists.
- **Validated diagnostic:** `audit_stopping_power_cross_energy_summary.py` v1.0.0 detects the unsafe source pattern and fails closed. Four focused tests passed; Markdown/JSON/SVG evidence is committed. The visual uses point position, text, and a cross-out rather than color alone.
- **Why acceptance is blocked:** an equal-weight arithmetic mean can conceal energy dependence and changes when the configured energy grid changes. NIST Technical Note 1297, DOI `10.6028/NIST.tn.1297`, is the recorded method basis for requiring identified uncertainty components and a documented combination method.
- **Resolution:** remove the cross-energy arithmetic mean from `compare_stopping_power.py`; report individual energy points and, if useful, descriptive min/max bounds only; record `NO_CROSS_ENERGY_COMBINATION_WITHOUT_UNCERTAINTY_MODEL`; add canonical regression and CI coverage; rerun the existing stopping-power suites before marking `AUD-G4-020` COMPLETE.
- **Do not claim until resolved:** a global, average, combined, or energy-integrated Geant4/PSTAR ratio from the current arithmetic mean.
