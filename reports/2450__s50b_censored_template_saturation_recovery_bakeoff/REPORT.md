# Ticket #2450 / S50b: Censored-Template Saturation Recovery Bakeoff

## Abstract

Ticket `2450` asks for an academic-grade comparison of a strong traditional
...
reconstruction under pile-up and ADC saturation.  The worker is `testbeam-laptop-4`; the `tn-ticket claim` helper was run exactly once, returned the null pseudo-ticket transcript recorded in `claimed_ticket.txt`, and issue `#2450` was manually recovered without a second claim attempt.  The
...
Use `saturation_residual_fusion_new` as the preferred S50b controlled-overlay energy-closure method
...
Runtime was `43.8` s on `Linux-5.15.0-139-generic-x86_64-with-glibc2.35`.

## Saturation-Onset Classification and Coverage

The classification endpoint treats injected overlapping/clipped waveforms as positives and matched clipped single-pulse controls as negatives.  Detection average precision and ROC-AUC therefore measure the practical saturation/pile-up onset gate that decides whether a censored two-pulse recovery is invoked.

| method                                         |   detection_ap |   detection_ap_ci_low |   detection_ap_ci_high |   detection_auc |   detection_auc_ci_low |   detection_auc_ci_high |   pileup_miss_rate |   false_split_rate |
|:-----------------------------------------------|---------------:|----------------------:|-----------------------:|----------------:|-----------------------:|------------------------:|-------------------:|-------------------:|
| saturation_residual_fusion_new                 |         0.8629 |                0.8456 |                 0.8815 |          0.8398 |                 0.8189 |                  0.8611 |             0.341  |            0.1359  |
| gradient_boosted_trees                         |         0.8584 |                0.845  |                 0.8719 |          0.8393 |                 0.8219 |                  0.8575 |             0.3538 |            0.1462  |
| ridge                                          |         0.8495 |                0.8251 |                 0.8729 |          0.8493 |                 0.8228 |                  0.8741 |             0.3051 |            0.1333  |
| 1d_cnn                                         |         0.8228 |                0.773  |                 0.8633 |          0.8162 |                 0.7619 |                  0.8554 |             0.2667 |            0.2179  |
| analytic_clipped_template_sideband_traditional |         0.668  |                0.6401 |                 0.6983 |          0.6226 |                 0.6001 |                  0.6475 |             0.5846 |            0.1974  |
| tiny_sequence_transformer                      |         0.7944 |                0.7517 |                 0.8327 |          0.7969 |                 0.743  |                  0.8397 |             0.5949 |            0.06923 |
| mlp                                            |         0.8456 |                0.7925 |                 0.8857 |          0.8493 |                 0.8081 |                  0.8835 |             0.3846 |            0.09231 |

Uncertainty coverage is reported as run-block bootstrap transfer coverage for aggregate metrics, not as calibrated per-event Bayesian predictive intervals.  The latter would require a separate conformal or likelihood calibration layer and is listed as a caveat rather than inferred from point-estimator residuals.

## Queue Provenance

The required claim helper was run once as `tn-ticket claim testbeam-laptop-4 --project testbeam` and returned `null / # null / null`.  Because the testbeam queue was non-empty and the helper did not attach this worker label to any issue, issue `#2450` was recovered without rerunning the claim command using `gh issue edit 2450 --repo SzeChunYiu/factory-tickets --add-label factory:claimed --add-label worker:testbeam-laptop-4 --remove-label factory:open`.  Completion is recorded with `tn-ticket done 2450`.  No novel follow-up ticket was appended.
