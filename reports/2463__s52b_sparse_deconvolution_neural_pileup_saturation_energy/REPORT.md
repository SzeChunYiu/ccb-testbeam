...
Ticket `#2463` asks for an academic-grade comparison of a strong traditional sparse deconvolution plus censored-template method against ridge, gradient-boosted trees, MLP, 1D-CNN,
...
Use `saturation_residual_fusion_new` as the preferred S52b controlled-overlay energy-closure method
when the analysis goal is sparse deconvolution of saturated doublets with run-held-out
...
Runtime was `45.2` s on `Linux-5.15.0-139-generic-x86_64-with-glibc2.35`.

## Ticket Workflow Provenance

`tn-ticket claim testbeam-laptop-1 --project testbeam` was run exactly once. It returned `null`, matching the known claim-helper edge case tracked by factory issue #2440, while the testbeam queue was non-empty. Issue #2463 was therefore manually label-swapped to `factory:claimed` and `worker:testbeam-laptop-1`; the claim command was not rerun.
