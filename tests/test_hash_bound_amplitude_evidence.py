...
def record(convention: str, basis: str) -> dict[str, str]:
    return {
...
def test_hash_bound_evidence_accepts_exact_table(tmp_path: Path) -> None:
    ...
    evidence = {digest: record("NET", "PRODUCER_CODE_PROVENANCE")}
...
def test_evidence_does_not_transfer_after_file_change(tmp_path: Path) -> None:
    ...
    evidence = {digest: record("ABSOLUTE", "INDEPENDENTLY_REVIEWED_PEDESTAL_EVIDENCE")}
