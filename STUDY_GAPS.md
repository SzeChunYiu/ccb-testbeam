...
- **Update 2026-07-05 (B-M1 / NEW-01 DONE, real trigger):** a full 1M `Trig_bar`-scored GEANT4 production ([MV3 v5](reports/mv3_v5_realtrigger_1783242005/REPORT.md)) replaced the A-HRD proxy with a real sensitive-detector two-arm coincidence. It **establishes the trigger as the mechanism** (untriggered B2 45.9% → triggered 99.7%; deep-proton events vetoed because the conjugate 37 MeV deuteron never reaches the A-paddle — 0.06% fire it) but **over-purifies**: MC B2 99.7% vs data Sample I 93.3%, so the profile is NOT reproduced (χ²/ndf vs Sample I ~1.3×10⁵). The residual is now attributed to data-side effects the ideal trigger omits (accidental/pile-up coincidences, Sample-I purity), NOT to geometry or the SD production — **budgeted in NEW-04 (2026-07-05):** ~2.0 pts accidental (upper-bounded, S10-anchored) + ~1.5 pts paddle-fidelity (loose) + ~2.9 pts still-unexplained; no forced closure. The proxy's χ²/ndf ≈ 625 is retired as over-optimistic (coincidence-MC vs mixed all-data).
- **Action (superseded):** ~~score the `Trig_bar` volumes as sensitive detectors~~ ✅ DONE (NEW-01). Do **not** merge geobuilder PR #8 as-is (its inter-stave Al default injects ~10× the realistic wrapping budget). Next: NEW-04 (data-side trigger residual).
...
|---|---|---|---|
| ~~**NEW-01**~~ | ✅ **DONE 2026-07-05** (B-M1, [MV3 v5](reports/mv3_v5_realtrigger_1783242005/REPORT.md), jobs 3348610/3348673). Scored `Trig_bar` as a real GEANT4 SD via `Detectors TARGET,ProtoTPC,Sci_bar,Trig_bar` and ran a full 1M `Trig_bar`-scored production on LUNARC (`output_krakow_1M_trig.root`, md5 `f73f35e1…`). **Result:** the *real* two-arm coincidence **establishes the trigger as the mechanism** (untriggered B2 45.9% → triggered 99.7%, material hypothesis independently falsified) but **over-purifies** — B2 99.7% vs data Sample I 93.3%, so the profile is NOT reproduced (χ²/ndf vs Sample I ~1.3×10⁵). The v4 proxy's χ²/ndf ≈ 625 is retired as an artifact of comparing a coincidence MC to *mixed* all-data. Residual (6.7% Sample-I deep-stave data) now points to data-side pile-up/accidental coincidences + Sample-I purity (→ NEW-04). | residual MV3 tension; S23 DR deficit; trigger-poisoned MC occupancy weights | Medium — **closed** |
| ~~**NEW-04**~~ | ✅ **DONE 2026-07-05** (`reports/new04_trigger_residual_1783275727/`, first-principles budget; no new compute). Budgeted the **6.4-pt** Sample-I non-B2 residual (MC ideal 0.3% vs data 6.7%). **Accidentals**: `f_acc/(1-f_acc) ≈ R_B·Δt` (clean closed form: R_true≈R_A_from_pd since A⊂B) → f_acc **4.4–8.4%** (Δt=15–30 ns; **UPPER bound** — R_max ≤ 3.05 MHz corrected, the 4.22 MHz@90ns is retracted), carrying the Sample-II B-singles profile (30.5% non-B2) → **1.3–2.6 pts** (central ~2.0). Independently **data-anchored** by the S10 current-driven downstream excess (+1.03 pt [0.64,1.42]). **Paddle fidelity** (deep-proton A-firing above the 0.06% truth; real threshold/δ-rays/straggling) → **0.5–3.7 pts** if p_deep 0.5–1.5%, but **poorly constrained**. **Budget: 6.4 ≈ 2.0 accidental (UB, solid) + 1.5 paddle-fidelity (loose) + ~2.9 unexplained ± large.** Accidentals are real but MODEST (~1/3, cannot close alone); the larger, softer term is paddle/selection fidelity. **No forced closure.** Next: digitize `Trig_bar` SD hits to pin p_deep; B-M6 for run-set/beam. | MV3 quantitative closure (real SD trigger done; residual is data-side, not a new production) | Medium (data-side + digitizer) — **done** |
...
| **S21** (Sample I/II trigger truth) | — | ✅ DONE | D-enrichment confirmed at truth level: B2 f_d ratio 1.519 [1.510, 1.528] (excl. 1.912); 91.2% d\|p pd-elastic pairs. **B-M9 (2026-07-05):** LayerID semantics/ordering geometry-signed-off (B2/total/ratio mapping-invariant); deep-stave B4/B6/B8 fractions carried mapping-conditional | `reports/s21_sample12_trigger_truth_1783077969/`, `reports/bm9_layerid_stave_mapping_20260705_204303/` |
...
| **S23** (data-side I/II + data–MC) | — | ✅ DONE | D-enrichment **directionally** confirmed in data (B2 f(A>5000) ratio 3.45 [3.41, 3.50]); trigger mimic moves MC toward data (KS 0.192→0.131, occupancy χ² 624k→20k); DR 0.738 (z=−99). **B-M6 (2026-07-05):** run-set/beam-drift confound bounded — run-clustered ratio CI [2.52, 4.64] excludes 1, gap = 3.5× within-sample run SD, drift ≤~29% (central ~3%) | `reports/s23_sample12_data_mc_1783108675/`, `reports/bm6_runset_confound_20260705_202638/` |
