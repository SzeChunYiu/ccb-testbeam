...
Ticket `1784063447.914.74ba7793` asks for a raw-ROOT reproduction followed by an academic-grade
comparison of energy reconstruction under clipped saturation and unresolved
pile-up.  The worker is `testbeam-laptop-1`.  The held-out winner written to `result.json`
...
Raw B-stack files are read from `/home/billy/.tb-workers/testbeam-laptop-1/data/root/root`.  For each ROOT file, the
...
`result.json` names **saturation_residual_fusion_new** as the S35b winner.  The traditional clipped
...
Runtime was `65.9` s on `Linux-5.15.0-139-generic-x86_64-with-glibc2.35`.
