#!/usr/bin/env python3
"""S04i support-preserving B2 abstention rule.

This ticket turns the S04h all-hit timing harm map into an operational
accept/abstain benchmark based only on B2 amplitude imbalance, peak-spread, and
baseline-excursion atoms. It rebuilds the population from raw HRDv ROOT, trains
the same traditional and ML/NN timing corrections, calibrates a train-run
support threshold, and evaluates retained all-hit timing closure on held-out
runs.
"""
...
def load_config(path: Path) -> dict:
    ...
    if "abstention" in config and "veto" not in config:
        config["veto"] = dict(config["abstention"])
    return config
...
def hand_pathology_score(events: pd.DataFrame, method: str) -> np.ndarray:
    ...
    if len(finite):
        ...
        fill = 0.0
        high = 0.0
    return np.nan_to_num(score, nan=fill, posinf=high, neginf=0.0)
...
def evaluate_vetoes(
    ...
    for method in [m for m in PRODUCTION_METHODS if m in predictions]:
        ...
        policy_rows.append(
            {
                "method": method,
                "policy": "train95_b2_atom_abstention",
...
def summarize_bootstrap(per_run: pd.DataFrame, support: pd.DataFrame, config: dict) -> pd.DataFrame:
    rng = np.random.default_rng(int(config["random_seed"]) + 8500)
...
def write_report(
    ...
    report = f"""# S04i: support-preserving B2 abstention rule for all-hit timing closure
...
where `r_e` is the B2 amplitude ratio from the all-hit event record, `s_e` is peak spread in samples, and `b_e` is baseline span in ADC counts.  The threshold `tau_0.95` is the train-run `{config['veto']['target_acceptance']:.0%}` quantile of `a_e`; held-out event `e` is retained if and only if `a_e <= tau_0.95`.  Because the abstention score is independent of method predictions, differences among methods reflect timing correction behavior on an identical retained event support.
...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/s04i_1781188500_1408_35504e34_support_preserving_b2_abstention_rule.json")
    ...
    policy.to_csv(out_dir / "abstention_policy_by_method.csv", index=False)
    per_run.to_csv(out_dir / "heldout_run_abstention_metrics.csv", index=False)
    residuals.to_csv(out_dir / "heldout_abstention_pair_residuals.csv.gz", index=False)
    ...
    next_tickets = []
    conclusion = (
        f"The S04i B2-atom abstention winner is {winner}, with retained all-six mean held-out-run sigma68 "
        ...
        f"The traditional explicit-timewalk abstention result gives {trad_row['mean_run_sigma68_ns']:.3f} ns "
        f"[{trad_row['sigma68_ci_low_ns']:.3f}, {trad_row['sigma68_ci_high_ns']:.3f}]. "
        f"The result supports B2 support-based abstention as a timing-quality boundary; it does not claim unconditional B2-inclusive timing adoption."
    )
    result = {
