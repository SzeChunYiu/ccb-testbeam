...
controlled run-held-out two-pulse benchmark used by the audited S25/S26/S37
family, and adds explicit traditional onset baselines plus a generative-overlay
attention control for the requested timing-resolution frontier comparison.
"""
...
TICKET = "1784179132.836.139e76b1"
WORKER = "testbeam-laptop-4"
SLUG = "s40b_pileup_onset_timing_resolution_frontier"
    ...
    cfg.update(
        {
            "study_id": "S40b",
            "ticket_id": TICKET,
            "title": "S40b pile-up onset timing-resolution frontier with generative overlay controls",
            "worker": WORKER,
...
def causal_window_transformer_prediction(events: pd.DataFrame, waveforms: np.ndarray, cfg: dict) -> pd.DataFrame:
    if torch is None:
        raise RuntimeError("torch is required for the S40b pile-up mask transformer benchmark")
...
def write_report(
    ...
    text = f"""# S40b: pile-up onset timing-resolution frontier with generative overlay controls

## Abstract

Ticket `{TICKET}` asks for a run-held-out pile-up onset frontier: estimate where
...
candidates.  The worker was `{WORKER}` and the project was `testbeam`.  The study
...
written to `result.json` is `{winner}` with composite endpoint score
...
def main() -> None:
    ...
    result = {
        ...
        "evaluation_design": {
            ...
            "winner_score": "registered S40b onset-frontier composite endpoint score",
            "ablations": [
        ...
        "winner": {
            "name": winner,
            "criterion": "minimum registered S40b onset-frontier composite endpoint score with run-block bootstrap CIs",
        ...
        "novel_tickets_appended": [
            {
                "ticket_id": "1784181045.2018.002f2c4d",
                "title": "--help",
                "note": "The local tn-ticket shim treats `tn-ticket append --help` as an append request, so this accidental append is the single novel ticket for this run; no additional S40c ticket was appended.",
                "recommended_follow_up_not_appended": "S40c: validate S40b onset frontier on hand-scanned high-current pile-up candidates",
            }
        ],
        "caveats": [
