#!/usr/bin/env python3
"""S11i: bounded-fit score calibration audit for the all-three injected target."""
...
def ece(y: np.ndarray, prob: np.ndarray, n_bins: int = 10) -> float:
    ...
    for lo, hi in zip(edges[:-1], edges[1:]):
        if hi >= 1.0:
            ...
            take = (pp >= lo) & (pp < hi)
        if take.any():
            acc += float(take.sum()) / total * abs(float(yy[take].mean()) - float(pp[take].mean()))
    return acc
...
def summarize_method(
    ...
    ece_ci = run_bootstrap_ci(y, prob, runs, ece, seed + 3, n_boot)
    reject_ci = run_bootstrap_ci(y, score, runs, fixed_95_clean_rejection, seed + 4, n_boot)
    recovery = accepted_recovery_bias(y, score, data, fit_oof)
    return {
        "method": name,
        "roc_auc": auc(y, score),
        ...
        "fixed_95_clean_rejection_ci_low": reject_ci[0],
        "fixed_95_clean_rejection_ci_high": reject_ci[1],
        "accepted_injected_count": recovery["accepted_injected_count"],
        "accepted_injected_coverage": recovery["accepted_injected_coverage"],
        "accepted_delay_bias_samples": recovery["accepted_delay_bias_samples"],
        "accepted_delay_rmse_samples": recovery["accepted_delay_rmse_samples"],
        "accepted_secondary_scale_bias": recovery["accepted_secondary_scale_bias"],
        "accepted_secondary_scale_rmse": recovery["accepted_secondary_scale_rmse"],
        "notes": notes,
    }
...
def fit_output_matrix(fit_oof: pd.DataFrame, fit_score: np.ndarray, fit_prob: np.ndarray, rf_score: np.ndarray | None = None) -> tuple[list[str], np.ndarray]:
    cols = [
        "valid",
        "delay_samples",
        "primary_amp_norm",
        ...
        "sse_two",
        "delta_sse",
        "frac_sse_improvement",
        "chi2_ndf",
    ]
    frame = pd.DataFrame(index=fit_oof.index)
    for col in cols:
        if col == "valid":
            frame[col] = fit_oof[col].astype(float).to_numpy()
        else:
            frame[col] = fit_oof[col].to_numpy(dtype=float)
    frame["neg_chi2_ndf"] = -frame["chi2_ndf"]
    frame["fit_selected_score"] = fit_score
    frame["fit_isotonic_prob"] = fit_prob
    if rf_score is not None:
        frame["shape_rf_score"] = rf_score
        frame["shape_minus_fit_prob"] = rf_score - fit_prob
        frame["shape_times_fit_prob"] = rf_score * fit_prob
    return list(frame.columns), frame.to_numpy(dtype=np.float32)
    ...
    for fold, held_run in enumerate(sorted(np.unique(runs))):
        ...
        if model_name == "logistic":
            clf = make_pipeline(
                SimpleImputer(strategy="median"),
                StandardScaler(),
                LogisticRegression(max_iter=1000, C=0.7, class_weight="balanced", random_state=seed + fold),
            )
...
def accepted_recovery_bias(y: np.ndarray, score: np.ndarray, data: pd.DataFrame, fit_oof: pd.DataFrame) -> dict:
    clean = score[y == 0]
    clean = clean[np.isfinite(clean)]
    if len(clean) == 0:
        return {
            "accepted_injected_count": 0,
            "accepted_injected_coverage": float("nan"),
            "accepted_delay_bias_samples": float("nan"),
            "accepted_delay_rmse_samples": float("nan"),
            "accepted_secondary_scale_bias": float("nan"),
            "accepted_secondary_scale_rmse": float("nan"),
        }
    threshold = float(np.percentile(clean, 95.0))
    accepted = (y == 1) & np.isfinite(score) & (score > threshold)
    all_injected = int(np.sum(y == 1))
    if not accepted.any():
        return {
            "accepted_injected_count": 0,
            "accepted_injected_coverage": 0.0 if all_injected else float("nan"),
            "accepted_delay_bias_samples": float("nan"),
            "accepted_delay_rmse_samples": float("nan"),
            "accepted_secondary_scale_bias": float("nan"),
            "accepted_secondary_scale_rmse": float("nan"),
    ...
    delay_err = fit_oof.loc[idx, "delay_samples"].to_numpy(dtype=float) - data.loc[idx, "injected_delay_samples"].to_numpy(dtype=float)
    scale_err = fit_oof.loc[idx, "secondary_fraction"].to_numpy(dtype=float) - data.loc[idx, "injected_scale"].to_numpy(dtype=float)
    return {
        "accepted_injected_count": int(len(idx)),
        "accepted_injected_coverage": float(len(idx) / max(all_injected, 1)),
        "accepted_delay_bias_samples": float(np.nanmean(delay_err)),
        "accepted_delay_rmse_samples": float(np.sqrt(np.nanmean(delay_err**2))),
        "accepted_secondary_scale_bias": float(np.nanmean(scale_err)),
        "accepted_secondary_scale_rmse": float(np.sqrt(np.nanmean(scale_err**2))),
    }
...
def write_report(
    ...
    head = scoreboard[
        [
            ...
            "ece_ci_low",
            "ece_ci_high",
            "fixed_95_clean_rejection",
            "fixed_95_clean_rejection_ci_low",
            "fixed_95_clean_rejection_ci_high",
        ]
    ]
    recovery = scoreboard[
        [
            "method",
            "accepted_injected_count",
            "accepted_injected_coverage",
            "accepted_delay_bias_samples",
            "accepted_delay_rmse_samples",
            "accepted_secondary_scale_bias",
            "accepted_secondary_scale_rmse",
        ]
    ]
    text = f"""# S11i: bounded-fit score calibration audit
...
`z_s = a t_s + c t_shifted(s,d) + b + eps_s`, with `a>0`, `c>=0`, `0 <= c/(a+c) <= {config['max_secondary_fraction']}`, and `|b| <= {config['max_baseline_abs_norm']}`.
...
The winner by global ROC AUC is `{winner}` with AUC {result['winner_roc_auc']:.3f}. The bounded-fit isotonic traditional baseline has AUC {result['traditional_auc']:.3f}; the winner-minus-traditional AUC difference is {result['winner_minus_traditional_auc']:.3f}. Brier and ECE are calibration losses, so lower is better. Fixed-95%-clean rejection reports the fraction of injected pulses rejected above the 95th percentile of clean scores.
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/s11i_1781065299_555_20535839_bounded_fit_score_calibration_audit.json")
    ...
    s07f = load_module("s07f_source_for_s11i", ROOT / config["s07f_script"])
    s11e = load_module("s11e_source_for_s11i", ROOT / config["s11e_script"])
    fitmod = load_module("s11b_fit_source_for_s11i", ROOT / config["s11b_fit_script"])
    ...
    print("3/8 constructing S11i dataset and traditional fit ...", flush=True)
    ...
    fit_cols, fit_X = fit_output_matrix(fit_oof, fit_score, fit_prob)
    fit_plus_cols, fit_plus_X = fit_output_matrix(fit_oof, fit_score, fit_prob, rf_score)
    logit_fit_score, logit_fit_fold = calibration_oof(data, y, fit_X, "logistic", seed + 1)
    et_fit_score, et_fit_fold = calibration_oof(data, y, fit_X, "extra_trees", seed + 2)
    et_plus_score, et_plus_fold = calibration_oof(data, y, fit_plus_X, "extra_trees", seed + 3)
    ...
    method_scores = {
        "bounded two-pulse fit isotonic": fit_score,
        "fit-output logistic calibration": logit_fit_score,
        "fit-output ExtraTrees calibration": et_fit_score,
        "fit-plus-shape-residual ExtraTrees": et_plus_score,
    ...
    method_probs = {
        "bounded two-pulse fit isotonic": fit_prob,
        "fit-output logistic calibration": logit_fit_score,
        "fit-output ExtraTrees calibration": et_fit_score,
        "fit-plus-shape-residual ExtraTrees": et_plus_score,
    ...
    notes = {
        "bounded two-pulse fit isotonic": "Strong traditional constrained template fit; scalar score chosen from fit outputs on training runs only and calibrated by fold-local isotonic regression.",
        "fit-output logistic calibration": "Logistic calibration layer over bounded-fit variables only.",
        "fit-output ExtraTrees calibration": "Nonlinear ExtraTrees calibration layer over bounded-fit variables only.",
        "fit-plus-shape-residual ExtraTrees": "New calibration architecture: ExtraTrees over fit variables plus out-of-fold shape RF score/residual interaction.",
    ...
    scoreboard = pd.DataFrame(
        [
            summarize_method(method, y, method_scores[method], method_probs[method], runs, data, fit_oof, seed + 100 * i, n_boot, notes[method])
    ...
    leakage = pd.DataFrame(
        [
            ...
            {"probe": "fit-output-only logistic sentinel", "roc_auc": auc(y, logit_fit_score), "average_precision": ap(y, logit_fit_score), "notes": "Interpretable bounded-fit variables only."},
            {"probe": "fit-output-only ExtraTrees sentinel", "roc_auc": auc(y, et_fit_score), "average_precision": ap(y, et_fit_score), "notes": "Nonlinear fit-output-only calibration."},
            {"probe": "shape-only RF sentinel", "roc_auc": auc(y, rf_score), "average_precision": ap(y, rf_score), "notes": "Waveform-shape baseline without fit outputs."},
    ...
    pd.DataFrame({"feature": fit_cols}).to_csv(out_dir / "fit_calibration_feature_columns.csv", index=False)
    pd.DataFrame({"feature": fit_plus_cols}).to_csv(out_dir / "fit_plus_shape_feature_columns.csv", index=False)
    ...
    traditional_auc = float(scoreboard.loc[scoreboard["method"] == "bounded two-pulse fit isotonic", "roc_auc"].iloc[0])
    ...
    result = {
        ...
        "winner_brier": float(winner_row["brier"]),
        "winner_ece": float(winner_row["ece"]),
        "winner_fixed_95_clean_rejection": float(winner_row["fixed_95_clean_rejection"]),
        "traditional_auc": traditional_auc,
        "winner_minus_traditional_auc": float(winner_row["roc_auc"] - traditional_auc),
        "fit_output_logistic_auc": float(scoreboard.loc[scoreboard["method"] == "fit-output logistic calibration", "roc_auc"].iloc[0]),
        "fit_output_extratrees_auc": float(scoreboard.loc[scoreboard["method"] == "fit-output ExtraTrees calibration", "roc_auc"].iloc[0]),
        "fit_plus_shape_residual_auc": float(scoreboard.loc[scoreboard["method"] == "fit-plus-shape-residual ExtraTrees", "roc_auc"].iloc[0]),
    ...
    manifest = {
        ...
        "python": sys.version,
        "command": f"/home/billy/anaconda3/bin/python scripts/s11i_1781065299_555_20535839_bounded_fit_score_calibration_audit.py --config {config_path.relative_to(ROOT)}",
        "inputs": input_rows,
        "outputs": {},
    }
