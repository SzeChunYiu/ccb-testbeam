...
CONFIG = ROOT / "configs/s54a_2473_matched_filter_neural_pedestal_drift.json"
...
METHOD_ORDER = [
    ...
    "matched_filter_residual_transformer_new",
]
...
def fit_matched_filter_residual_transformer(df: pd.DataFrame, config: dict, seed: int) -> np.ndarray:
    try:
        import torch
        from torch import nn
        from torch.utils.data import DataLoader, TensorDataset
    except Exception as exc:  # pragma: no cover
        raise RuntimeError("torch is required for matched_filter_residual_transformer_new") from exc

    class MatchedFilterResidualTransformer(nn.Module):
    ...
    model = MatchedFilterResidualTransformer()
...
def write_report(
    ...
    text = f"""# S54a Matched-Filter Timing Versus Neural Pulse-Shape Encoders

## Abstract

Ticket `{config['ticket_id']}` asks how matched-filter timing and neural
...
def main() -> None:
    ...
    preds["matched_filter_residual_transformer_new"] = fit_matched_filter_residual_transformer(data, config, seed=int(config["random_seed"]) + 3)
