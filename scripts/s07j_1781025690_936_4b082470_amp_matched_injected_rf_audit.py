...
os.environ.setdefault("MPLCONFIGDIR", "/tmp/ccb-testbeam-s07j-matplotlib-cache")
...
def write_report(
    ...
    peak_amp_probe = leakages["peak_preserved"][leakages["peak_preserved"]["probe"] == "absolute-amplitude-only RF"].iloc[0]
    charge_amp_probe = leakages["charge_preserved"][leakages["charge_preserved"]["probe"] == "absolute-amplitude-only RF"].iloc[0]

    text = f"""# S07j: all-three injected RF amplitude-matching nuisance audit
...
The raw all-three App.I gate reproduces the parent S07e population before any S07j result is used. The S07f unnormalized injection number is also reproduced from the same raw ROOT: RF AUC {s07f_rf['roc_auc']:.6f} versus the prior {config['expected_s07f_shape_rf_auc']:.6f}.
...
After forcing the peak-amplitude nuisance to chance, the all-three shape-only RF still reaches ROC AUC {peak_rf['roc_auc']:.3f} [{peak_rf['roc_auc_ci_low']:.3f}, {peak_rf['roc_auc_ci_high']:.3f}], compared with the traditional timing/template score {peak_trad['roc_auc']:.3f} [{peak_trad['roc_auc_ci_low']:.3f}, {peak_trad['roc_auc_ci_high']:.3f}]. The peak-renormalized amplitude-only RF is {peak_amp_probe['roc_auc']:.3f}, so the main RF advantage is not a peak-height classifier. Under positive-charge preservation, the RF reaches {charge_rf['roc_auc']:.3f} [{charge_rf['roc_auc_ci_low']:.3f}, {charge_rf['roc_auc_ci_high']:.3f}] versus traditional {charge_trad['roc_auc']:.3f} [{charge_trad['roc_auc_ci_low']:.3f}, {charge_trad['roc_auc_ci_high']:.3f}], while the amplitude-only RF is {charge_amp_probe['roc_auc']:.3f}.
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/s07j_1781025690_936_4b082470.json")
    ...
    peak_amp_auc = float(
        leakages["peak_preserved"].loc[
            leakages["peak_preserved"]["probe"] == "absolute-amplitude-only RF", "roc_auc"
        ].iloc[0]
    )
    if abs(peak_amp_auc - 0.5) > 0.05:
        raise RuntimeError(f"peak-renormalized amplitude-only RF is not near chance: {peak_amp_auc:.6f}")
    ...
    result = {
        ...
        "peak_preserved_amplitude_only_auc": peak_amp_auc,
        "peak_preserved_amplitude_only_near_chance": bool(abs(peak_amp_auc - 0.5) <= 0.05),
    ...
    for path in [config_path, Path(__file__).resolve(), S07F_PATH, ROOT / config["utility_script"]]:
        input_rows.append({"path": str(path), "sha256": sha256_file(path), "bytes": path.stat().st_size})
    ...
    manifest = {
        ...
        "inputs": input_rows,
        "outputs": {},
        "command": f"/home/billy/anaconda3/bin/python scripts/s07j_1781025690_936_4b082470_amp_matched_injected_rf_audit.py --config {config_path}",
    }
