...
overlap.  The worker is `testbeam-laptop-1`.  The
...
The traditional comparator is **analytic_clipped_template_sideband_traditional**, the ticket's censored two-pulse least-squares template deconvolution baseline.
...
The required helper command `tn-ticket claim testbeam-laptop-1 --project testbeam` was run exactly once.  It returned the known null pseudo-ticket pattern (`null`, `# null`, `null`) tracked by factory-ticket #2440 while open testbeam issues remained visible.  Without rerunning the helper, issue #2467 was manually label-swapped to `factory:claimed` and `worker:testbeam-laptop-1` with `gh issue edit 2467 --repo SzeChunYiu/factory-tickets --add-label factory:claimed --add-label worker:testbeam-laptop-1 --remove-label factory:open`.  No novel follow-up ticket was appended for this study.

## Recommendation

Use `saturation_residual_fusion_new` as the preferred S53b/#2467 controlled-overlay energy-closure method
...
Runtime was `35.7` s on `Linux-5.15.0-139-generic-x86_64-with-glibc2.35`.
