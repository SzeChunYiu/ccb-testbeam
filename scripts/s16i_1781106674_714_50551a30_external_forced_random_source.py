#!/usr/bin/env python3
"""S16i external forced/random HRD pedestal acquisition-source audit.

This ticket is not another quiet-proxy pedestal benchmark. It asks whether an
unmounted DAQ archive, run log, trigger-mode spreadsheet, acquisition script,
or operator note identifies true B-stack runs 58-65 forced/random/no-pulse HRD
pedestal events. The script
therefore:
...
def write_report(
    ...
    report = f"""# S16i: external forced/random HRD pedestal acquisition-source audit
...
- **Config:** `configs/s16i_1781106674_714_50551a30_external_forced_random_source.json`

## 0. Question

Can an unmounted DAQ archive, run log, or other bounded external source visible from this worker identify true forced/random/no-pulse HRD pedestal events for B-stack runs 58-65? The decision distinguishes "truth genuinely absent from the mirrored ROOT bundle" from "recorded elsewhere but absent from the mounted laptop mirror".
...
3. Search bounded external documentation/report locations for DAQ logbooks, trigger-mode spreadsheets, acquisition scripts, operator notes, and archive member names.
...
This ticket's direct forced/random truth label is absent, so a new supervised ML benchmark on direct pedestal truth would be post-hoc and ill-posed. The relevant benchmark was already completed in `{config["prior_proxy_benchmark_dir"]}` and is carried forward here as context. It used Sample-II leave-one-run-out splitting and compared the traditional quantile baseline against ridge, gradient-boosted trees, MLP, 1D-CNN, and a pair-symmetric `siamese_cnn_meta` architecture on the same held-out proxy timing-tail task.
...
Pre-registration from the claimed ticket: an external DAQ archive, run log, trigger-mode spreadsheet, acquisition script, or operator note identifying forced/random/pedestal/no-pulse B-stack runs 58-65 would falsify the current-mirror absence interpretation.

Falsification test: a candidate must be independent of derived project reports and must contain a forced/random/pedestal/no-pulse token in the source name or text. If such a candidate is found, the result changes from `blocked_missing_external_record` to `external_candidate_found` and direct S16 truth closure becomes the next task.
...
Systematics and caveats: absence from the visible laptop and Desktop paths is not proof that CCB never recorded forced/random pedestal runs. The LUNARC canonical path is unmounted in this run. The analysis PDF may summarize acquisition conditions, but it is not a raw DAQ logbook and does not identify forced/random B-stack runs 58-65.
...
No independent external HRD DAQ acquisition record is visible from the configured bounded sources. This supports the narrower conclusion that the S16 forced/random truth gate is blocked in the mounted mirrors and local documentation, not the stronger conclusion that forced/random runs were never recorded.
...
```bash
/home/billy/anaconda3/bin/python scripts/s16i_1781106674_714_50551a30_external_forced_random_source.py --config configs/s16i_1781106674_714_50551a30_external_forced_random_source.json
```
...
def write_manifest(out_dir: Path, config_path: Path, command: str, input_paths: list[Path], start: float) -> None:
    ...
    manifest = {
        "script": "scripts/s16i_1781106674_714_50551a30_external_forced_random_source.py",
        "config": str(config_path),
        "output_dir": str(out_dir),
        "ticket": "1781106674.714.50551a30",
...
def main() -> None:
    ...
    result = {
        ...
        "scientific_summary": (
            ...
            "acquisition script, or operator note identifying true B-stack forced/random/pedestal runs 58-65. "
            "The S16 forced/random direct truth gate therefore remains blocked in this mounted data state."
        ),
    ...
    command = f"/home/billy/anaconda3/bin/python scripts/s16i_1781106674_714_50551a30_external_forced_random_source.py --config {args.config}"
