...
def cfd_time(wave: np.ndarray, fraction: float = 0.5) -> tuple[np.ndarray, np.ndarray]:
    """Linear-interpolated rising-edge constant-fraction time in sample units."""
    amp = np.maximum(wave.max(axis=1), 1.0)
    threshold = fraction * amp
    above = wave >= threshold[:, None]
    first = above.argmax(axis=1)
    first = np.clip(first, 1, wave.shape[1] - 1)
    row = np.arange(len(wave))
    y0 = wave[row, first - 1]
    y1 = wave[row, first]
    denom = np.where(np.abs(y1 - y0) < 1e-6, 1.0, y1 - y0)
    t = first - 1 + (threshold - y0) / denom
    slope = denom
    bad = ~above.any(axis=1)
    t[bad] = np.nan
    slope[bad] = np.nan
    return t.astype(np.float32), slope.astype(np.float32)


def parabolic_peak_time(wave: np.ndarray) -> np.ndarray:
    peak = np.argmax(wave, axis=1)
    peak = np.clip(peak, 1, wave.shape[1] - 2)
    row = np.arange(len(wave))
    ym = wave[row, peak - 1]
    y0 = wave[row, peak]
    yp = wave[row, peak + 1]
    denom = np.where(np.abs(ym - 2 * y0 + yp) < 1e-6, np.nan, ym - 2 * y0 + yp)
    delta = 0.5 * (ym - yp) / denom
    return (peak + np.clip(delta, -0.5, 0.5)).astype(np.float32)


def nearest_template_phase(wave: np.ndarray, template: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    """Small-lag normalized cross-correlation phase against a fold-independent proxy template."""
    z = wave - np.median(wave[:, :4], axis=1)[:, None]
    t = template - np.median(template[:4])
    t = t / max(float(np.linalg.norm(t)), 1.0)
    lags = np.asarray([-2, -1, 0, 1, 2], dtype=int)
    scores = []
    for lag in lags:
        if lag < 0:
            a = z[:, :lag]
            ...
            a = z
            b = t
        denom = np.maximum(np.linalg.norm(a, axis=1) * np.linalg.norm(b), 1.0)
        scores.append((a @ b) / denom)
    s = np.vstack(scores).T
    best = np.argmax(s, axis=1)
    lag = lags[best].astype(np.float32)
    # Parabolic refinement around the best lag where possible.
    mid = (best > 0) & (best < len(lags) - 1)
    row = np.arange(len(wave))
    y_m = s[row, np.clip(best - 1, 0, len(lags) - 1)]
    y_0 = s[row, best]
    y_p = s[row, np.clip(best + 1, 0, len(lags) - 1)]
    denom = np.where(np.abs(y_m - 2 * y_0 + y_p) < 1e-6, np.nan, y_m - 2 * y_0 + y_p)
    lag[mid] = lag[mid] + np.clip(0.5 * (y_m[mid] - y_p[mid]) / denom[mid], -0.5, 0.5)
    return lag.astype(np.float32), s.max(axis=1).astype(np.float32)
...
def extract_dataset(config: dict) -> tuple[pd.DataFrame, np.ndarray, pd.DataFrame]:
    ...
    for run in runs(config):
        ...
        for batch in iter_batches(path):
            ...
            odd_ev = odd[idx] * sel[:, :, None]
            even_q = even_charge[idx] * sel
            odd_q = odd_charge[idx] * sel
            amp = even_amp[idx] * sel
            odd_a = odd_amp[idx] * sel
            primary = np.argmax(amp, axis=1)
            rows = np.arange(len(primary))
            primary_even = ev[rows, primary]
            primary_odd = odd_ev[rows, primary]
            even_cfd, even_slope = cfd_time(primary_even)
            odd_cfd, odd_slope = cfd_time(primary_odd)
            peak_time = parabolic_peak_time(primary_even)
            template = np.nanmedian(primary_even, axis=0)
            xcorr_lag, xcorr_score = nearest_template_phase(primary_even, template)
            ...
            # Duplicate channels occasionally have effectively zero positive charge; bound those closures so
            # the study measures saturation hysteresis rather than denominator pathologies.
            charge_loss = np.clip(charge_loss_raw, -4.0, 4.0)
            timing_proxy = (peak_sel.sum(axis=1) / np.maximum(sel.sum(axis=1), 1)) - 5.0
            cfd_transfer = np.clip(odd_cfd - even_cfd, -3.0, 3.0)
            shape_residual = np.sqrt(np.mean((primary_even / np.maximum(amp.max(axis=1), 1.0)[:, None] - np.nanmedian(primary_even / np.maximum(amp.max(axis=1), 1.0)[:, None], axis=0)) ** 2, axis=1))
            # Target is duplicate-readout phase transfer in sample units, with small
            # deterministic penalties for low-quality phase observables. This keeps the
            # learning target centered on sub-sample timing while retaining the ticket's
            # requested pulse-shape, pedestal, pile-up, saturation, energy, and PID stress axes.
            target = cfd_transfer + 0.04 * xcorr_lag + 0.02 * (peak_time - even_cfd)
                ...
                pd.DataFrame(
                    {
                        ...
                        "target_phase_transfer_sample": target,
                        "even_cfd_sample": even_cfd,
                        "odd_cfd_sample": odd_cfd,
                        "cfd_transfer_sample": cfd_transfer,
                        "cfd_slope_adc_per_sample": even_slope,
                        "duplicate_cfd_slope_adc_per_sample": odd_slope,
                        "parabolic_peak_sample": peak_time,
                        "xcorr_lag_sample": xcorr_lag,
                        "xcorr_peak_score": xcorr_score,
                        "pulse_shape_residual": shape_residual,
...
def feature_matrix(events: pd.DataFrame, waves: np.ndarray) -> tuple[np.ndarray, list[str]]:
    cols = [
        ...
        "even_cfd_sample",
        "parabolic_peak_sample",
        "xcorr_lag_sample",
        "xcorr_peak_score",
        "cfd_slope_adc_per_sample",
        "pulse_shape_residual",
    ]
    parts = []
    names = []
    for col in cols:
...
def write_report(out: Path, config: dict, result: dict, counts: pd.DataFrame, summary: pd.DataFrame, strata: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        "",
        f"The winner's phase residual score is accompanied by a PID stability diagnostic: held-out AUC={result['winner']['pid_auc']:.4f}, AP={result['winner']['pid_average_precision']:.4f}. The label is a duplicate-readout high-amplitude or multi-hit proxy and is used only as a caveat-level side diagnostic, not as the primary optimization target.",
        "",
...
def main() -> None:
    ...
    y = events[config.get("target_column", "target_phase_transfer_sample")].to_numpy(dtype=float)
    ...
    preds["birks_huber_saturation"] = bounded_predict(huber, huber_x, y_cal)
    ...
    stratum_defs = {
        ...
        "high_pulse_shape_residual": events["pulse_shape_residual"].to_numpy() >= np.percentile(events.loc[train, "pulse_shape_residual"], 75),
        "high_energy_transfer_residual": np.abs(events["charge_loss"].to_numpy()) >= np.percentile(np.abs(events.loc[train, "charge_loss"]), 75),
        "pid_proxy_positive": events["pid_label"].to_numpy(dtype=bool),
        "near_knee": events["knee_count"].to_numpy() > 0,
        ...
        "high_pedestal_drift": events["pedestal_iqr_adc"].to_numpy() >= np.percentile(events.loc[train, "pedestal_iqr_adc"], 75),
    ...
    for name, smask in stratum_defs.items():
        method_order = [winner_method, "traditional_fractional_delay_template", "ridge", "gradient_boosted_trees", "mlp", "1d_cnn", "waveform_transformer", "gated_residual_cnn"]
        for method in list(dict.fromkeys(method_order)):
