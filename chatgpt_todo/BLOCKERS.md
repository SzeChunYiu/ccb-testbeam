...
- **State:** OPEN
- **Resolved engineering defects:** the PR #890 default reference path and masking inline fallback were corrected under `AUD-G4-004`; silent endpoint clamping was corrected under `AUD-G4-006`; and silent malformed-row skipping, row reordering, duplicate energies, and nonfinite/nonphysical reference values were corrected under `AUD-G4-007`. Current code selects the committed reference, validates every row in declared order, rejects unsupported proton-equivalent lookup energies, and records the lookup plus table bounds.
...
- **Validated boundary:** current output is explicitly `DIAGNOSTIC_ONLY`; synthetic tests establish arithmetic, reference wiring, reference integrity, and domain handling only.
