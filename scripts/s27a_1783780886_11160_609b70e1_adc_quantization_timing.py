#!/usr/bin/env python3
"""S27a ADC quantization pulse-shape timing bias map.

The study reproduces the raw ROOT selected-pulse count, then tests whether
same-pulse ADC quantization residual features help run-held-out downstream
timing correction beyond a strong template/constant-fraction baseline and
standard ML/NN models.
"""
...
os.environ.setdefault("MPLCONFIGDIR", "/tmp/matplotlib-s27a-adc-quantization")
...
def save_plots(out_dir: Path, benchmark: pd.DataFrame, strata: pd.DataFrame) -> None:
    ...
    ax.set_title("S27a run-held-out timing benchmark")
    fig.tight_layout()
...
def write_report(
    ...
    lines = [
        ...
        "",
        f"The strong traditional candidate is selected on training runs only from leading edge, CFD fractions, template phase, and optimal-filter windows. The selected baseline is `{info['best_traditional']}`. Held-out traditional diagnostics are:",
        "",
        ...
        "",
        "```bash",
        f"{sys.executable} scripts/s27a_1783780886_11160_609b70e1_adc_quantization_timing.py --config configs/s27a_1783780886_11160_609b70e1_adc_quantization_timing.yaml",
        "```",
        "",
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/s27a_1783780886_11160_609b70e1_adc_quantization_timing.yaml")
    ...
    result = {
        ...
        "scientific_summary": (
            ...
            "ADC quantization residual features are mapped as a systematic across pulse-shape, timing, pile-up, saturation, pedestal, energy, and PID-proxy strata; the result should be read as run-held-out timing-bias evidence, not external time truth."
        ),
        "next_tickets": [
            {
                "title": "S27d: external run-block replication of ADC quantization timing correction",
                "body": "Freeze the S27a winning correction and repeat the quantization-stratified timing benchmark on an independent run block or acquisition period, retaining the same traditional baseline and paired bootstrap CI procedure. Expected information gain: distinguishes a stable ADC quantization correction from a run-local waveform support artifact.",
            }
        ],
    }
