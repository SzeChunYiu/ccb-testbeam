...
Status: `PARTIAL / IMPLEMENTED_ON_PR / REAL_HIBEAM_RUNTIME_NOT_AVAILABLE`
...
1. **Build/runtime physics lead** — Linux/ELF and Geant4 deployment provenance; responsible for the exact executable/runtime-object contract and for separating configured package labels from actually mapped code.
2. **Adversarial systems reviewer** — process identity, pathname/inode races, deleted/replaced mappings, anonymous executable memory, loader environment, and stale/symlinked same-soname mechanisms.
...
Mappings with the same `(dev_major,dev_minor,inode)` are one backing-file identity even if represented by multiple VM segments or hardlink-equivalent path spellings. The receipt therefore collapses those observationally equivalent parameterizations before assigning a SHA-256.
...
and the file must remain stable while hashed. The recorded `SHA256_j` is therefore an exact identity of the stable backing file observed at this boundary.
...
Linux `proc_pid_maps(5)` defines `/proc/<pid>/maps` as the process's current mapped memory regions with address, permissions, offset, device, inode, and pathname; it also documents the ambiguous `" (deleted)"` pathname suffix. Linux kernel/man-pages documentation also exposes `/proc/<pid>/map_files`, but opening those mapped-file links requires elevated capabilities on modern Linux. Therefore the repository implementation does **not** pretend that `map_files` is universally available; it instead validates the maps-reported device/inode against a stable opened pathname and fails closed when the mapped file has been deleted/replaced or cannot be hashed.
...
Rejected. A same-name path can be atomically replaced after a process maps an older inode. The executed synthetic falsifier preserves the pathname while replacing the file and is rejected because maps device/inode no longer match the current pathname target.
...
### H6 — every `/proc/<pid>/maps` row is an independent dependency

Collapsed. Multiple executable VM segments backed by the same `(device,inode)` are one backing-file universe; retain segment metadata but hash the backing object once.

### H7 — kernel synthetic mappings and arbitrary anonymous executable mappings are equivalent
...
### H8 — stable actual file-backed executable mapping set + exact hashes + executable identity is a valid bounded runtime provenance primitive
...
5. executable mapped object carrying the procfs ` (deleted)` suffix → reject rather than guess;
6. unattributed anonymous executable mapping → reject;
7. changed `LD_LIBRARY_PATH` with unchanged mappings → receipt identity changes because loader provenance changed;
8. real Linux `/proc` round trip against a sleeping Python child process → PASS, binding the child executable and live executable mappings.

This focused result is a software/provenance test only. The repository's required exact-head CI has not yet been observed for the branch at archive creation time.

## Micro → meso → event → study → claim propagation

- **micro:** exact process executable object, executable mapping rows, device/inode/path, object bytes, loader-control environment;
