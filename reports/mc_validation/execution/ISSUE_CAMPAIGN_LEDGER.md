...
## #962 — config run-role conformance + raw-product binding (Wave B)

| Issue | Disposition | Evidence |
|-------|-------------|----------|
| #962 | Config-conformance enforcement + raw-product SHA binding landed (paper-facing scope: source-bound per-run ledger incl. raw SHA + trigger record separation via EXTERNAL_COLLABORATION_SOURCE BOM rows; cross-config CI detection now exists). `assert_configs_consistent_with_ledger` sweeps all 403 config run-role blocks vs the canonical ledger (calib==[64], group membership incl. 38/43 exclusion, per-block calib∩heldout=∅). Ledger `raw_products:` binds 34 runs x 2 stacks (hrda/hrdb path+size+SHA-256) from the committed s23 hash sweep; run 38 absent by ledger exclusion. `paper/hardware_bom.csv` re-synced byte-identical to the publication table + drift test; 5 Rataj 2026 rows added as `EXTERNAL_COLLABORATION_SOURCE` (campaign identity with B-stack runs not established, #1296). Remaining #962 scope for the trigger record itself is tracked under #1045. | `src/ccb_mc_validation/daq/run_ledger.py`, `tests/test_962_run_role_conformance.py`, `configs/daq/run_ledger.yaml` (raw_products), `tests/test_hardware_bom_format.py`, `docs/contracts/RUN_LEDGER.md` |
|#1088|WLS fluorescence multiplicity contract (three-mode CLI, Bernoulli thinning, n_wls_absorbed counter, optical_v2 digest, known-answer ctest, sourced q=0.70 Pla-Dalmau NIM A361 1995)|docs/contracts/WLS_FLUORESCENCE_MULTIPLICITY.md|OPEN|

## #1400 — injected correlated-noise MC discriminant (B2 broad residual, S29b)

Follow-up to #968/S29a (PR #1398). Branch fix/1400-s29b-injected-noise-mc.

| Issue | Disposition | Evidence |
|-------|-------------|----------|
| #1400 | DONE (harness G1 bootstrap PASS {KS<=0.02 all staves}; G1b baseline-split borderline FAIL documented — V0 +27.2 vs pooled downstream +29.8 ns, cause measured by the secondary-rate observable: linear saturation approximation under-populates saturated secondaries, V0 sat rate 0.222 vs downstream measured 0.30-0.38; reweighting family conclusive on G2: V5 calibrated stratum reweighting KS(B2,meas) 0.111, split +21.1 ns vs measured +8.9 +/- 2, KS(B2syn,DNsyn) 0.315 vs measured 0.279 -> FAIL; manifold test WITHIN — island/downstream NN ratio 1.35 < 2, island R frac below lib min 0.000; secondary RATE falls with load on every stave while the template deficit grows into saturation -> template excess and unsaturated island carried by different populations; V1-V4 literature-parametric variants SKIPPED-literature-absent -> discriminant_status PARTIAL, rerunnable on the same seeds) | `reports/1786870106.1400000.68374937__s29b_b2_injected_correlated_noise_mc/` (result.json, REPORT.md, fig1-fig6, manifest.json, figures.json), `scripts/s29b_1786870106_1400_68374937_b2_injected_correlated_noise_mc.py`, `scripts/s29b_figs_1786870106.py`, `configs/1786870106.1400000.68374937_b2_injected_correlated_noise_mc.json` |
| #1401 | CLOSED STRUCTURALLY_UNAVAILABLE (electronics impulse-response discriminant requires bench/SPE data; none exists in the 33 calibration runs or the extracted tree — searched run content lists, config registry, reports/, scope-justified NOT_FOUND distinct from checked-and-absent; extracted pulse shapes conflate electronics response with light-production timing, so only the conflated effective response is recoverable — the reason S29b models the effective response empirically) | `result.json: discriminants_unavailable.electronics_impulse_response` |
