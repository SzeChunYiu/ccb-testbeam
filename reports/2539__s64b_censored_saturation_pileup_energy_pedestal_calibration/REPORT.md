...
nuisance shifts.  The worker is `testbeam-laptop-3`.  The
...
Use `saturation_residual_fusion_new` as the preferred S64b/#2539 controlled-overlay energy-closure method
...
Runtime was `93.4` s on `Linux-5.15.0-139-generic-x86_64-with-glibc2.35`.
...
applying the same label transition directly: `gh issue edit 2539 --repo SzeChunYiu/factory-tickets --add-label factory:claimed --add-label worker:testbeam-laptop-3 --remove-label factory:open`.  Completion is
recorded with `tn-ticket done 2539`.  No novel follow-up ticket was appended.
