...
Local/GitHub checkpoint, generated 2026-06-27 11:12 UTC / 13:12 CEST:

- The public wiki now has an explicit **Study coverage and remaining gaps** page: `https://github.com/SzeChunYiu/ccb-testbeam/wiki/Study-Coverage-and-Remaining-Gaps`. It is generated from `STUDY_IMPLEMENTATION_GAP_AUDIT.json` so the wiki cannot hide unstudied or under-studied topics behind narrative prose.
- The page records the current fail-closed truth: `all_study_implementations_ready=false`, `blocked_count=5`, with MV4 timing, MV5 pile-up, MV6 representation comparison, MV7 pedestal/noise, and MV8 saturation/dynamic-range still blocked until their required production artifacts exist.
- Added a repository-side GitHub Wiki HTTP verifier (`scripts/mc_validation/verify_wiki_publication.py`) that checks every page listed by `wiki/WIKI_MANIFEST.json` reaches HTTP 200 on the live GitHub Wiki and contains page-specific provenance snippets. Fresh verification against the live wiki returned `status=PASS`, `published_count=11`, `page_count=11`, `blocked_count=0` for run `20260625T064500Z_full_input_artifacted`.
