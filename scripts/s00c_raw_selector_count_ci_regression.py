...
CONFIG = Path("configs/s00c_1781013145_1122_4ccc6db4.json")
...
def bootstrap_accuracy_ci(y_true: np.ndarray, y_pred: np.ndarray, rng: np.random.Generator, n_boot: int) -> tuple[float, float]:
    return bootstrap_ci((y_true == y_pred).astype(float), rng, n_boot)
...
def benchmark_methods(sample: pd.DataFrame, config: dict) -> tuple[pd.DataFrame, pd.DataFrame]:
    ml_cfg = config["ml"]
    ...
    for method, pred, score, notes in methods:
        acc_ci = bootstrap_accuracy_ci(y_test, pred, rng, n_boot)
        row = {
            "method": method,
    ...
    return pd.DataFrame(cv_rows), pd.DataFrame(bench_rows)
...
## 3. ML Method
The ML method is a logistic classifier trained to predict the median-first-four selector from raw waveform summaries. The split is by run, with runs {', '.join(str(x) for x in config['ml']['heldout_runs'])} held out. Cross-validation on training runs is grouped by run.
...
def main() -> None:
    ...
    cv, bench = benchmark_methods(sample, config)
    ...
    result = {
        ...
        "next_tickets": [
            "S00d: wire S00c into repository CI so selector semantic drift blocks merges",
            "S01d: rebuild selected-pulse table from the S00c checked selector and compare row hashes to S01b",
        ],
    }
    ...
    manifest = {
        ...
        "ticket": config["ticket_id"],
        "worker": config["worker"],
        "commands": ["/home/billy/anaconda3/bin/python scripts/s00c_raw_selector_count_ci_regression.py"],
        "config": str(CONFIG),
