#!/usr/bin/env python3
"""P10j independent acquisition-window live-time falsification.
...
CFD-aligned, amplitude-normalized waveform, but winner selection is falsified
against delayed/acquisition-window handles: live10, live20, and late-tail
secondary fraction. These controls are deliberately separated from the primary
q-template reconstruction score and compared to shuffled-live10 sentinels.
"""
...
def waveform_metrics(y: np.ndarray, rel_grid: np.ndarray, sample_period_ns: float) -> pd.DataFrame:
    ...
    for wave in y:
        ...
        above20 = np.flatnonzero((idx_grid >= peak_i) & (yy >= 0.20))
        live10 = float(rel_grid[above[-1]] * sample_period_ns) if len(above) else np.nan
        live20 = float(rel_grid[above20[-1]] * sample_period_ns) if len(above20) else np.nan
        ...
        rows.append({"live10_ns": live10, "live20_ns": live20, "tau_eff_ns": tau_eff, "tail_sum": tail_sum, "secondary_fraction": secondary, "cfd_sample": cfd})
        ...
        rows.append(
            {
                ...
                "live20_bias_ns": float(pred_metrics.loc[i, "live20_ns"] - obs.loc[i, "live20_ns"]),
                "live20_abs_error_ns": float(abs(pred_metrics.loc[i, "live20_ns"] - obs.loc[i, "live20_ns"])),
...
def run_summary(rows: pd.DataFrame, config: dict) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    metrics = ["q_template_mse", "tail_mse", "live10_abs_error_ns", "live20_abs_error_ns", "tau_eff_abs_error_ns", "timing_abs_error_ns", "secondary_abs_error"]
    per_run = []
...
def action_atlas(rows: pd.DataFrame, summary: pd.DataFrame, secondary: pd.DataFrame, config: dict) -> Tuple[pd.DataFrame, pd.DataFrame]:
    merged = rows.merge(rows[rows["method"] == "traditional_empirical_template"][["row_id", "q_template_mse", "tail_mse", "live10_abs_error_ns", "live20_abs_error_ns", "timing_abs_error_ns", "secondary_abs_error"]].rename(columns={
        "q_template_mse": "trad_q_template_mse",
        "tail_mse": "trad_tail_mse",
        "live10_abs_error_ns": "trad_live10_abs_error_ns",
        "live20_abs_error_ns": "trad_live20_abs_error_ns",
    ...
    for keys, group in merged.groupby(["method", "current_stratum", "saturation_bin", "amp_bin_label", "peak_phase_bin", "q_proxy_bin"], observed=True):
        ...
        rec["live20_delta_ns"] = float(np.nanmean(group["live20_abs_error_ns"] - group["trad_live20_abs_error_ns"]))
...
def write_report(out_dir: Path, config: dict, repro: pd.DataFrame, summary: pd.DataFrame, deltas: pd.DataFrame, secondary: pd.DataFrame, atlas_support: pd.DataFrame, sentinels: pd.DataFrame, result: dict) -> None:
    ...
    head = nonsentinel[["method", "n_runs", "n_rows", "q_template_mse", "tail_mse", "timing_sigma68_ns", "live10_abs_error_ns", "live20_abs_error_ns", "tau_eff_abs_error_ns", "secondary_abs_error"]].copy()
    delta_tab = deltas[(deltas["metric"].isin(["q_template_mse", "tail_mse", "timing_sigma68_ns", "live10_abs_error_ns", "live20_abs_error_ns", "secondary_abs_error"])) & ~deltas["method"].str.startswith("sentinel")].copy()
    ...
    lines = [
        ...
        "",
        "The winner named in `result.json` is **{}** by the preregistered primary ordering: minimum tail MSE, then live10 error, live20 error, and secondary-fraction error. Its tail MSE is {:.6g} with CI [{:.6g}, {:.6g}].".format(result["winner"], float(winner["tail_mse"]), float(winner["tail_mse_ci_low"]), float(winner["tail_mse_ci_high"])),
        "",
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/p10j_1781182678_1624_461c5bb5_acquisition_window_livetime_falsification.json")
    ...
    result = {
        ...
        "winner": winner,
        "winner_rule": "lowest tail_mse among non-sentinel methods, then live10_abs_error_ns, live20_abs_error_ns, and secondary_abs_error as acquisition-window falsification controls",
    ...
    manifest = {
        "ticket_id": config["ticket_id"],
        "study": config["study_id"],
        "command": "/home/billy/anaconda3/bin/python scripts/p10j_1781182678_1624_461c5bb5_acquisition_window_livetime_falsification.py --config {}".format(config_path),
        "git_commit": result["git_commit"],
        "platform": platform.platform(),
        "python": platform.python_version(),
        "config": str(config_path),
        "config_sha256": sha256_file(config_path),
        "script": "scripts/p10j_1781182678_1624_461c5bb5_acquisition_window_livetime_falsification.py",
        "script_sha256": sha256_file(ROOT / "scripts/p10j_1781182678_1624_461c5bb5_acquisition_window_livetime_falsification.py"),
        "support_scripts": [
