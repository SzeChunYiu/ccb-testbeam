...
# Issue #984: paired multi-seed design — same replicate seed at every knob value.
N_REPLICATES = int(os.environ.get("CCB_CAMPASSIGN_N_REPLICATES", "3"))
...
def emit(knob: Knob, outdir: Path) -> Path:
    ...
    Issue #984: for each replicate r in [0, N_REPLICATES), emit every knob value
    with seed = SEED_BASE + r (common-random-number across the sweep). Labels
    encode the replicate as ``<knob>=<value>__r<r>`` so root outputs remain unique.
    """
    outdir.mkdir(parents=True, exist_ok=True)
    path = outdir / f"points_{knob.name}.csv"
    n_rep = max(1, int(os.environ.get("CCB_CAMPASSIGN_N_REPLICATES", str(N_REPLICATES))))
    seed_base = int(os.environ.get("CCB_CAMPASSIGN_SEED_BASE", str(SEED_BASE)))
    lines = [
        f"# knob: {knob.name}",
        ...
        f"# n_replicates: {n_rep}",
        f"# seed_policy: paired_multi_seed issue_984 seed=SEED_BASE+replicate",
        "# columns: label,seed,nevents,cli_args,env_vars,replicate",
        "label,seed,nevents,cli_args,env_vars,replicate",
    ]
    for r in range(n_rep):
        seed = seed_base + r  # same seed for every value within this replicate
        for v in knob.values:
            sv = _to_str(v)
            label = f"{knob.name}={sv}__r{r}"
            cli = ""
            envv = ""
            if knob.channel == "cli":
            ...
            lines.append(f"{label},{seed},{DEFAULT_NEVENTS},{cli},{envv},{r}")
    path.write_text("\n".join(lines) + "\n")
    return path
...
def main() -> int:
    ...
    n_rep = max(1, int(os.environ.get("CCB_CAMPASSIGN_N_REPLICATES", str(N_REPLICATES))))
    manifest_rows = [
        "knob,channel,target,unit,n_values,n_replicates,npoints,seed_policy,rationale"
    ]
    emitted = []
    for k in knobs:
        p = emit(k, outdir)
        emitted.append(p)
        npoints = len(k.values) * n_rep
        manifest_rows.append(
            f"{k.name},{k.channel},{k.target},{k.unit},{len(k.values)},{n_rep},"
            f'{npoints},paired_multi_seed_SEED_BASE_plus_replicate,"{k.rationale}"'
        )
