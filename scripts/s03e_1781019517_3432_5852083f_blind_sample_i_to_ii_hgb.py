...
def fit_transfer_models(
    ...
    return combined, analytic_cv, coef, binned_cv, s03b.binned_model_table(binned_models), hgb_cv, hgb_cal, best
...
def plot_outputs(out_dir: Path, per_run: pd.DataFrame, pooled: pd.DataFrame) -> None:
    order = ["template_phase_base", "analytic_timewalk", "s03b_binned_timewalk", "hgb_timewalk"]
...
def write_report(
    ...
    lines = [
        ...
        "",
        f"All models used the fixed base pickoff `{config['timing']['base_method']}` with templates built only from Sample I train runs. The S03a analytic traditional model selected `{best['analytic_candidate']}` with Ridge alpha `{best['analytic_alpha']:g}` by GroupKFold over Sample-I runs. The S03b constrained binned traditional model selected mode `{best['binned_mode']}`, direction `{best['binned_direction']}`, bins `{best['binned_n_bins']}`. The ML comparator is an HGB residual corrector selected only by Sample-I grouped CV, trained on a deterministic cap of `{config['hgb'].get('max_train_rows', 'all')}` Sample-I rows.",
        "",
        ...
        "",
        f"The HGB result trips the too-good screen (`too_good_flag={result['leakage']['too_good_flag']}`) because it is `{result['ml']['delta_vs_sample_ii_loro_hgb_ns']:.3f} ns` below the Sample-II LORO HGB reference. I therefore treated the shuffled-target controls and train/held-out overlap checks as gating leakage probes; the overall shuffled-target minimum remains `{result['leakage']['shuffled_target_min_sigma68_ns']:.3f} ns`, well above the HGB result.",
        "",
        ...
        f"The S03b binned traditional correction gives `{binned['value']:.3f} ns`; the HGB ML comparator gives `{ml['value']:.3f} ns`.",
        ...
        "",
        "```bash",
        f"{sys.executable} scripts/s03e_1781019517_3432_5852083f_blind_sample_i_to_ii_hgb.py --config {config_path}",
        "```",
        "",
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/s03e_1781019517_3432_5852083f_blind_sample_i_to_ii_hgb.yaml")
    ...
    combined, analytic_cv, coef, binned_cv, binned_table, hgb_cv, hgb_cal, best = fit_transfer_models(
    ...
    verdict = (
        ...
        if float(analytic["value"]) > float(config["reference_numbers"]["sample_ii_s03a_amp_only_sigma68_ns"]) + 0.25
