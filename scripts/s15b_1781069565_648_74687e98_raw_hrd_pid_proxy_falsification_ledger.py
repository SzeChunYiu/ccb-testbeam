...
This study is a weak-label falsification test, not a truth PID measurement. It
starts from raw B-stack ROOT, rebuilds a transparent range/depth charge proxy,
and asks whether waveform pulse atoms add PID-like information beyond charge,
penetration depth, topology, saturation, and run-family support. The benchmark
panel is the preregistered traditional method plus ridge, gradient-boosted
trees, MLP, 1D-CNN, and a support-residual hybrid MLP.
"""
...
def write_report(
    ...
    report = """# S15b: raw-HRD PID proxy falsification ledger
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/s15b_1781069565_648_74687e98_raw_hrd_pid_proxy_falsification_ledger.json"))
    ...
    result = {
        ...
        "primary_interpretation": (
            "This is a raw-HRD PID proxy falsification ledger. The named winner is a weak-label "
            "benchmark winner only; adoption is blocked without particle truth labels and by the "
            "support/nuisance sentinel pattern documented in REPORT.md."
        ),
        "benchmark": details,
        "next_tickets": [
            {
    ...
    write_report(out_dir, config, result, reproduction, metrics, pairwise, leakage, support_fraction)

    manifest = {
        "ticket_id": config["ticket_id"],
        "script": "scripts/s15b_1781069565_648_74687e98_raw_hrd_pid_proxy_falsification_ledger.py",
