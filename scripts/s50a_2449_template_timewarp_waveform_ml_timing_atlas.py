#!/usr/bin/env python3
"""S50a: trapezoid/template timing versus ML and neural pulse-shape models."""
...
def fit_predict_models(norm_waves: np.ndarray, x_all: np.ndarray, targets: np.ndarray, train_mask: np.ndarray, heldout_mask: np.ndarray, config: dict, rng: np.random.Generator) -> Tuple[Dict[str, np.ndarray], pd.DataFrame]:
    ...
    predictions["compact_residual_fusion"] = supported(0.45 * gb_pred + 0.35 * extra_pred + 0.20 * cnn_pred)
    rows.append({"method": "compact_residual_fusion", "hyperparameter": "0.45 HGB + 0.35 ExtraTrees + 0.20 CNN", "train_residual_sigma68_ns": sigma68(y_train - extra.predict(X_train))})
...
def write_report(out_dir: Path, result: dict, summary: pd.DataFrame, strata: pd.DataFrame, training: pd.DataFrame, leakage: pd.DataFrame) -> None:
    ...
    report = f"""# S50a: template time-warp versus waveform ML pulse-shape timing atlas
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/s50a_2449_template_timewarp_waveform_ml_timing_atlas.json"))
    ...
    result = {
        ...
        "new_architecture": "compact_residual_fusion",
