#!/usr/bin/env python3
"""P09i: broad-width reviewer-disagreement propagation benchmark.
...
def add_propagation_features(meta: pd.DataFrame, train_mask: np.ndarray) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    out["baseline_lowering_score"] = -robust_z(
        out["baseline_slope"].to_numpy(float), train["baseline_slope"].to_numpy(float)
    )
    thresholds = pd.DataFrame(
        [
            {"name": "baseline_score_q995_train", "value": float(out.loc[train_mask, "baseline_score"].quantile(0.995))},
            {"name": "baseline_lowering_score_q995_train", "value": float(out.loc[train_mask, "baseline_lowering_score"].quantile(0.995))},
    ...
    out["prop_baseline_lowering"] = out["baseline_lowering_score"] > lookup["baseline_lowering_score_q995_train"]
    ...
    out["label_broad_downstream_risk"] = out["broad_width_source"] & (
        out["prop_timing_tail"]
        | out["prop_charge_outlier"]
        | out["prop_pileup_like"]
        | out["prop_baseline_lowering"]
...
def add_reviewer_nuisance(frame: pd.DataFrame, waves: np.ndarray, p09b, p09c) -> pd.DataFrame:
    base = frame.copy().reset_index(drop=True)
    if "source_index" not in base.columns:
        raise RuntimeError("source_index is required before reviewer annotation")
    base["normalized_waveform"] = [
        [round(float(x), 5) for x in waves[int(idx)]]
        for idx in base["source_index"].to_numpy(dtype=int)
    ]
    b = p09b.add_reviewer_labels(base)
    c = p09c.add_external_review(base)
    out = base.copy()
    out["p09b_reviewer_a_label"] = b["reviewer_a_label"].to_numpy(dtype=object)
    out["p09b_reviewer_b_label"] = b["reviewer_b_label"].to_numpy(dtype=object)
    out["p09b_consensus_label"] = b["consensus_label"].to_numpy(dtype=object)
    out["p09b_reviewers_agree"] = b["reviewers_agree"].to_numpy(dtype=bool)
    out["p09b_consensus_curated_any"] = b["consensus_curated_any"].to_numpy(dtype=bool)
    out["p09c_external_alpha_label"] = c["external_reviewer_alpha_label"].to_numpy(dtype=object)
    out["p09c_external_beta_label"] = c["external_reviewer_beta_label"].to_numpy(dtype=object)
    out["p09c_fixed_morphology_label"] = c["traditional_fixed_morphology_label"].to_numpy(dtype=object)
    out["p09c_external_reviewers_agree"] = c["external_reviewers_agree"].to_numpy(dtype=bool)
    out["reviewer_disagreement"] = (
        ~out["p09b_reviewers_agree"].to_numpy(dtype=bool)
        | ~out["p09c_external_reviewers_agree"].to_numpy(dtype=bool)
        | (out["p09b_consensus_label"].to_numpy(dtype=object) != out["p09c_fixed_morphology_label"].to_numpy(dtype=object))
    )
    out["curated_broad_by_review"] = (
        (out["p09b_consensus_label"].to_numpy(dtype=object) == TARGET)
        | (out["p09c_fixed_morphology_label"].to_numpy(dtype=object) == TARGET)
    )
    return out


def select_fixed_coverage(
    meta: pd.DataFrame,
    eval_idx: np.ndarray,
    score: np.ndarray,
    method: str,
    ...
    frame = meta.loc[eval_idx].copy()
    frame["source_index"] = eval_idx
    frame["score"] = score[eval_idx]
    rows = []
    for _, subset in frame.groupby(["run", "stave"], sort=True):
        rows.append(subset.sort_values("score", ascending=False).head(int(k_per_run_stave)))
...
def propagation_metric_row(method: str, selected: pd.DataFrame, reference: pd.DataFrame, method_disagreement_rate: float) -> dict:
    base_pileup = float(reference["prop_pileup_like"].mean()) if len(reference) else float("nan")
    base_lower = float(reference["prop_baseline_lowering"].mean()) if len(reference) else float("nan")
    sel_lower = float(selected["prop_baseline_lowering"].mean()) if len(selected) else float("nan")
    return {
        "method": method,
        "n_selected": int(len(selected)),
        "curated_broad_precision": float(selected["curated_broad_by_review"].mean()) if len(selected) else float("nan"),
        "reviewer_disagreement_rate": float(selected["reviewer_disagreement"].mean()) if len(selected) else float("nan"),
        "method_disagreement_rate_vs_traditional": float(method_disagreement_rate),
        "downstream_risk_rate": float(selected["label_broad_downstream_risk"].mean()) if len(selected) else float("nan"),
        "timing_sigma68_shift": sigma68(selected["timing_score"]) - sigma68(reference["timing_score"]),
        "charge_res68_shift": sigma68(selected["charge_score"]) - sigma68(reference["charge_score"]),
        "pileup_excess_shift": float(selected["prop_pileup_like"].mean() - base_pileup) if len(selected) else float("nan"),
        "baseline_lowering_enrichment": float(sel_lower / max(base_lower, 1.0e-12)) if len(selected) else float("nan"),
    }


def fixed_coverage_tables(
    method_scores: Dict[str, np.ndarray],
    meta: pd.DataFrame,
    eval_idx: np.ndarray,
    waves: np.ndarray,
    ...
    selected_by_method: Dict[str, pd.DataFrame] = {}
    for method, scores in method_scores.items():
        selected_by_method[method] = select_fixed_coverage(meta, eval_idx, scores, method, k_per_run_stave)
    trad_keys = set(
        selected_by_method["traditional_robust_broad_veto"][["run", "event_index", "stave"]]
        .astype(str)
        .agg(":".join, axis=1)
    )
    annotated = []
    metric_rows = []
    reference = meta.loc[eval_idx].copy()
    reference["source_index"] = eval_idx
    for method, selected in selected_by_method.items():
        selected = add_reviewer_nuisance(selected, waves, p09b, p09c)
        keys = set(selected[["run", "event_index", "stave"]].astype(str).agg(":".join, axis=1))
        method_disagreement_rate = 0.0 if method == "traditional_robust_broad_veto" else 1.0 - len(keys.intersection(trad_keys)) / max(1, len(keys))
        annotated.append(selected)
        metric_rows.append(propagation_metric_row(method, selected, reference, method_disagreement_rate))
    selected_all = pd.concat(annotated, ignore_index=True)
    metrics = pd.DataFrame(metric_rows)

    runs = np.asarray(sorted(reference["run"].unique()), dtype=int)
    boot_rows = []
    for method, selected in selected_all.groupby("method", sort=True):
        keys = set(selected[["run", "event_index", "stave"]].astype(str).agg(":".join, axis=1))
        method_disagreement_rate = 0.0 if method == "traditional_robust_broad_veto" else 1.0 - len(keys.intersection(trad_keys)) / max(1, len(keys))
        values = {col: [] for col in metrics.columns if col not in {"method", "n_selected"}}
        for _ in range(int(n_boot)):
            sampled = rng.choice(runs, size=len(runs), replace=True)
            sel = pd.concat([selected[selected["run"] == run] for run in sampled], ignore_index=True)
            ref = pd.concat([reference[reference["run"] == run] for run in sampled], ignore_index=True)
            row = propagation_metric_row(method, sel, ref, method_disagreement_rate)
            for col in values:
    ...
    return metrics, pd.DataFrame(boot_rows), selected_all


def fixed_ci_string(ci: pd.DataFrame, method: str, metric: str) -> str:
    row = ci[(ci["method"] == method) & (ci["metric"] == metric)]
    if row.empty:
        return ""
    return "[{:.3g}, {:.3g}]".format(float(row.iloc[0]["ci_low"]), float(row.iloc[0]["ci_high"]))
...
def write_report(
    ...
    fixed_view = fixed_metrics.copy()
    for metric in [
        "curated_broad_precision",
        "reviewer_disagreement_rate",
        "method_disagreement_rate_vs_traditional",
        "timing_sigma68_shift",
        "charge_res68_shift",
        "pileup_excess_shift",
        "baseline_lowering_enrichment",
    ]:
        fixed_view[metric + "_ci"] = [fixed_ci_string(fixed_ci, method, metric) for method in fixed_view["method"]]
    lines = [
        ...
        "",
        "For held-out predictions \(s_i\), the ranking target is downstream risk. Average precision is the model-selection metric, with ROC-AUC, balanced accuracy at the median score, and top-1% recall/precision as diagnostics. Separately, fixed-coverage selections take the top `{}` pulses per held-out run/stave and report the propagation metrics requested in the ticket. CIs are bootstrap intervals over held-out runs, not row bootstraps.".format(config["coverage_per_run_stave"]),
        "",
        "## Held-out ranking benchmark",
        view.to_markdown(index=False),
        "",
        "## Fixed-coverage propagation",
        fixed_view.to_markdown(index=False),
        "",
        "## Per-run diagnostics",
        ...
        "",
        "## Result",
        "The winner by primary downstream-risk average precision is `{}` with AP {:.4f} (95% run-bootstrap CI {}). At fixed coverage, the same method has curated broad precision {:.4f} and reviewer-disagreement rate {:.4f}. The fixed-coverage table gives timing sigma68 shift, charge res68 shift, pile-up excess shift, and baseline-lowering enrichment with run-bootstrap CIs.".format(
            winner["method"], float(winner["average_precision"]), ci_string(ci, winner["method"], "average_precision")
            , float(fixed_metrics.set_index("method").loc[winner["method"], "curated_broad_precision"])
            , float(fixed_metrics.set_index("method").loc[winner["method"], "reviewer_disagreement_rate"])
        ),
        "",
        "## Caveats",
...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/p09i_1781058292_535_650c13f1_broad_width_reviewer_disagreement_propagation.json")
    ...
    p09c_review = load_module(
        "p09c_independent_gallery_review",
        Path("scripts/p09c_1781021714_1194_414e1474_independent_gallery_review.py"),
    )
        ...
        [
            {
                ...
                "broad_downstream_risk": int((p09c_heldout_mask & p09c_meta["label_broad_downstream_risk"].to_numpy(bool)).sum()),
            }
        ]
    )
        ...
        label_counts.append(
            {
                ...
                "broad_downstream_risk": int(subset["label_broad_downstream_risk"].sum()),
                "baseline_lowering": int(subset["prop_baseline_lowering"].sum()),
    ...
    fixed_metrics, fixed_ci, fixed_selected = fixed_coverage_tables(
        method_scores,
        meta,
        eval_idx,
        waves,
        p09b,
        p09c_review,
        int(config["coverage_per_run_stave"]),
        int(config["bootstrap_replicates"]),
        rng,
    )
    fixed_metrics.to_csv(out_dir / "fixed_coverage_propagation_metrics.csv", index=False)
    fixed_ci.to_csv(out_dir / "fixed_coverage_run_bootstrap_ci.csv", index=False)
    fixed_selected.to_csv(out_dir / "fixed_coverage_selected_rows.csv", index=False)
    ...
    top_rows = meta.loc[eval_idx, [
        ...
        "label_broad_downstream_risk",
        "prop_baseline_lowering",
    ]].copy()
    ...
    leakage = pd.DataFrame(
        [
            ...
            },
            {
                ...
                "value": 0,
                "pass": True,
                "note": "run, event, eventno, evt, stave, source index, and reviewer labels are excluded from model matrices",
            },
            {
            ...
            {
                "check": "expanded_downstream_risk_runs",
                "value": int((expanded_counts["broad_downstream_risk"] > 0).sum()),
                "pass": True,
                "note": "number of expanded held-out runs with at least one broad downstream-risk positive",
            },
            {
                "check": "reviewer_disagreement_used_as_target",
                "value": 0,
                "pass": True,
                "note": "reviewer disagreement is computed only after scoring for nuisance reporting",
            },
        ]
    )
    ...
    for path in [p09a_config_path, config_path, Path(config["p09c_reference_config"]), Path(config["p09b_config"]), Path(config["p09c_review_config"])]:
        input_hashes.append({"path": str(path), "sha256": sha256_file(path), "bytes": int(path.stat().st_size)})
    ...
    result = {
        ...
        "fixed_coverage_propagation_metrics": fixed_metrics.to_dict(orient="records"),
        "fixed_coverage_bootstrap_ci": fixed_ci.to_dict(orient="records"),
        "per_run_metrics": per_run.to_dict(orient="records"),
        "leakage_checks": leakage.to_dict(orient="records"),
        "model_info": model_info,
        "follow_up_tickets": [
            {
                "title": "P09j blinded review of fixed-coverage broad-width propagation positives",
                "rationale": "P09i uses deterministic propagation labels and fixed rubrics; a small blinded review of selected downstream-risk positives would separate genuine broad scintillator atoms from detector-quality vetoes.",
            }
        ],
    ...
    manifest = {
        ...
        "command": "{} scripts/p09i_1781058292_535_650c13f1_broad_width_reviewer_disagreement_propagation.py --config {}".format(
        ...
        "code_sha256": {
            "scripts/p09i_1781058292_535_650c13f1_broad_width_reviewer_disagreement_propagation.py": sha256_file(Path(__file__)),
            ...
            "scripts/p09c_1781021714_1194_414e1474_independent_gallery_review.py": sha256_file(Path("scripts/p09c_1781021714_1194_414e1474_independent_gallery_review.py")),
        },
