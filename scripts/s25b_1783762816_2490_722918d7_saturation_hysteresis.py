...
def fit_torch_wave(cls, waves: np.ndarray, x: np.ndarray, y: np.ndarray, train: np.ndarray, config: dict, seed_offset: int):
    ...
    scaler = StandardScaler().fit(x[idx])
    model = cls(x.shape[1])
    return fit_torch_model(model, scaler, x[idx], waves[idx], y[idx], int(config["cnn_epochs"]), int(config["random_seed"]) + seed_offset + 1)
...
def write_report(out: Path, config: dict, result: dict, counts: pd.DataFrame, summary: pd.DataFrame, strata: pd.DataFrame) -> None:
    ...
    lines = [
        f"# {config['study_id']}: saturation-onset hysteresis waveform recovery bakeoff",
        "",
        "## Abstract",
        "",
        f"This study reproduces the S00 B-stave selected-pulse number directly from raw ROOT and then benchmarks a traditional saturation-aware waveform correction against ridge, gradient-boosted trees, MLP, 1D-CNN, and a new gated residual CNN. The winner is **{winner}** under held-out-run res68 with run-block bootstrap confidence intervals.",
        "",
...
def main() -> None:
    ...
    for name, smask in stratum_defs.items():
        for method in [winner_method, "traditional_clipped_template", "gradient_boosted_trees", "gated_residual_cnn"]:
    ...
    result = {
        ...
        "claimed_ticket_text": "S25b saturation-onset hysteresis waveform recovery bakeoff",
    }
