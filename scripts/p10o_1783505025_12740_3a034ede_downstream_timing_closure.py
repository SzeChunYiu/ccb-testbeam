...
def run_metrics(config: dict, eval_: pd.DataFrame, pred: np.ndarray, method: str, fold: str) -> pd.DataFrame:
    ...
    for run, sub in work.groupby("run", observed=True):
        ...
        raw_tres, tres, qres = [], [], []
        for a, b in pairs:
            if a in raw_wide_t and b in raw_wide_t:
                raw_tres.append((raw_wide_t[a] - raw_wide_t[b]).to_numpy())
...
def write_report(out_dir: Path, config: dict, repro: pd.DataFrame, summary: pd.DataFrame, diagnostics: pd.DataFrame, result: dict) -> None:
    ...
    report = rf"""# P10o: Downstream Timing Closure for P10n Phase-Stable No-Tail Gains
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/p10o_1783505025_12740_3a034ede_downstream_timing_closure.yaml")
    ...
    manifest = {
        ...
        "script": "scripts/p10o_1783505025_12740_3a034ede_downstream_timing_closure.py",
