#!/usr/bin/env python3
"""S29c pile-up timing deconvolution architecture bakeoff.
...
TICKET = "1783827793.6970.25ea5036"
WORKER = "testbeam-laptop-3"
SLUG = "s29c_pileup_timing_deconvolution_architecture_bakeoff"
    ...
    cfg.update(
        {
            "study_id": "S29c",
            "ticket_id": TICKET,
            "title": "S29c pile-up timing deconvolution architecture bakeoff",
            "worker": WORKER,
...
def pileup_mask_transformer_prediction(events: pd.DataFrame, waveforms: np.ndarray, cfg: dict) -> pd.DataFrame:
    if torch is None:
        raise RuntimeError("torch is required for the S29c pile-up mask transformer benchmark")
...
def write_report(
    ...
    text = f"""# S29c: pile-up timing deconvolution architecture bakeoff

## Abstract

Ticket `{TICKET}` asks for a run-held-out architecture bakeoff for pulse timing
under controlled pile-up.  The worker was `{WORKER}` and the project was
...
def main() -> None:
    ...
    write_report(cfg, match, template_summary, overall, endpoints, ranked, by_run, strata, source_ci, winner, runtime)
    ...
    result = {
        ...
        "evaluation_design": {
            ...
            "winner_score": "registered S29c composite endpoint score",
        },
        ...
        "winner": {
            "name": winner,
            "criterion": "minimum registered S29c composite endpoint score with run-block bootstrap CIs",
