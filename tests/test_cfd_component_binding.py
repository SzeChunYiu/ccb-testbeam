...
def test_component_bound_censoring_is_relative_to_selected_component():
    """Earlier above-threshold activity may recover before the selected peak.

    At CFD20 the selected 100-ADC peak has threshold 20. Sample 0 is already
    above 20, but the waveform then returns below threshold at sample 1 before
    rising into the selected component. The selected-component crossing is
    therefore observed (1->2), not left-censored by unrelated early activity.
    """
    wave = np.asarray(
...
def test_component_bound_is_left_censored_only_if_selected_rise_is_unseen():
    """If no pre-peak sample is below threshold, the selected rise is censored."""
