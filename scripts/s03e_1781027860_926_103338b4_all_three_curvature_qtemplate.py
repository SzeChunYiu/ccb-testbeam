#!/usr/bin/env python3
"""S03e all-three q_template clean-cut validation against curvature tails.

The script reads raw B-stack ROOT files, reproduces the S00 selected-pulse
counts and the S07 all-three downstream count gate first, then evaluates
whether q_template-only event scores validate clean timing on held-out runs
when the primary tail label is curvature C_t, not App.A labels.
"""
...
def write_report(
    ...
    text = f"""# S03e: all-three q_template clean-cut validation with curvature C_t
...
The q-template-only RF is not treated as independent timing truth: the forbidden `|C_t|` ceiling is label-defining, and downstream q features share waveform provenance with the curvature label. The shuffled-label control is AUC {shuffle['roc_auc']:.3f}; downstream-only q is stronger than B2-only q ({ds['roc_auc']:.3f} vs {b2['roc_auc']:.3f}), so the validation is best interpreted as a downstream waveform-quality validation of a curvature-tail gate. No App.A label, timing span, curvature, run/event id, selected flag, waveform sample, or amplitude column enters the main ML matrix.

## Verdict
The raw reproduction gate passed exactly. On the all-three curvature-tail label, the traditional q score reaches AUC {trad['roc_auc']:.3f} [{trad['roc_auc_ci_low']:.3f}, {trad['roc_auc_ci_high']:.3f}], and the q-template-only RF reaches AUC {ml['roc_auc']:.3f} [{ml['roc_auc_ci_low']:.3f}, {ml['roc_auc_ci_high']:.3f}]. This supports using q_template as a conservative all-three clean-cut diagnostic, not as a replacement truth label for pile-up or App.A.

## Reproducibility
```bash
/home/billy/anaconda3/bin/python scripts/s03e_1781027860_926_103338b4_all_three_curvature_qtemplate.py --config {config.get('config_path_for_report', 'configs/s03e_1781027860_926_103338b4_all_three_curvature_qtemplate.json')}
```
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/s03e_1781027860_926_103338b4_all_three_curvature_qtemplate.json")
    ...
    abs_ct = np.abs(data["c_t_ns"].to_numpy(dtype=float))
    data["abs_c_t_ns"] = abs_ct
    clean = abs_ct < float(config["clean_ct_max_ns"])
    tail = abs_ct > float(config["tail_ct_min_ns"])
    bench = data[clean | tail].copy().reset_index(drop=True)
    bench["label_tail"] = (np.abs(bench["c_t_ns"].to_numpy(dtype=float)) > float(config["tail_ct_min_ns"])).astype(int)
    ...
    dataset_counts = pd.DataFrame(
        [
            ...
            {"quantity": "clean events |C_t|<3 ns", "value": int(clean.sum())},
            {"quantity": "tail events |C_t|>51 ns", "value": int(tail.sum())},
    ...
    leaky_score = np.abs(bench["c_t_ns"].to_numpy(dtype=float))
    dt_crosscheck_score = bench["d_t_ns"].to_numpy(dtype=float)
    ...
    leakage = pd.DataFrame(
        [
            ...
            {"probe": "leaky |C_t| score", "roc_auc": auc(y, leaky_score), "average_precision": ap(y, leaky_score), "notes": "Forbidden label-defining ceiling, reported only as a reference."},
            {"probe": "D_t cross-check score", "roc_auc": auc(y, dt_crosscheck_score), "average_precision": ap(y, dt_crosscheck_score), "notes": "Not used as the primary target; reports how close span is to the curvature label."},
    ...
    result = {
        ...
        "traditional": {
            ...
            "metric": "leave-one-run-out ROC AUC for |C_t|>51 ns all-three curvature tails",
