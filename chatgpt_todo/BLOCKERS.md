...
- **State:** OPEN
- **Task:** `AUD-G4-021`.
- **Confirmed defect:** current source blob `360f3e46db664f4eead48021536f210e2f7a85c9` opens the requested final path directly with `out_path.open("w")` after validating the simulation and PSTAR inputs. It has no explicit output/input alias rejection and no atomic replacement path.
- **Risk:** `--out` equal to `--sim` or `--reference` can destroy exact scientific input bytes. A serialization, process, or filesystem failure after opening the final path can leave a truncated report with the canonical requested filename.
- **Validated evidence:** AST audit v1.0.0 detects direct final-path writes and missing alias/atomic helpers; five focused synthetic tests passed in 1.63s; Markdown/JSON/SVG evidence is on `main`.
- **Resolution:** reject output paths resolving to either input; serialize in a same-directory temporary file; close successfully; atomically publish with `os.replace`; remove temporary files on failure; retain any pre-existing final report; add direct CLI regressions; record final report bytes and SHA-256.
- **Do not claim until resolved:** that report generation preserves raw/reference inputs, that a final-path CSV is complete after a failed invocation, or that output artifact provenance is immutable.
