...
TICKET = "1783888239.810.6d016e26"
SLUG = "s33b_pileup_saturation_energy_recovery_architecture_bakeoff"
    ...
    cfg.update(
        {
            "study_id": "S33b",
            "ticket_id": TICKET,
            "title": "Pile-up saturation energy recovery architecture bakeoff",
            "worker": WORKER,
            "output_dir": str(OUT),
            "random_seed": 2026071213,
...
def transformer_prediction(events: pd.DataFrame, waveforms: np.ndarray, cfg: dict) -> pd.DataFrame:
    if torch is None:
        raise RuntimeError("torch is required for the S33b transformer benchmark")
...
def write_residual_plots(joined: pd.DataFrame, winner: str, out_dir: Path) -> pd.DataFrame:
    import matplotlib
    ...
    held["energy_residual"] = (held["pred_energy_adc"] - held["true_energy_adc"]) / np.maximum(
        held["true_energy_adc"], 1.0
    )
    held["time_abs_residual_ns"] = 10.0 * np.nanmean(
        np.abs(
            held[["t1_sample", "t2_sample"]].to_numpy(float)
            - held[["true_t1_sample", "true_t2_sample"]].to_numpy(float)
        ),
        axis=1,
    )
    held["saturated_proxy"] = held["true_energy_adc"] > 11000.0
    plot = held[held["method"] == winner].copy()
    plot["detected"] = ~plot["failed"].astype(bool)

    bins = pd.cut(plot["spacing_ns"], bins=[0, 10, 25, 45, 70], include_lowest=True)
    failure = (
        plot.assign(spacing_bin=bins)
        .groupby(["spacing_bin", "saturated_proxy"], observed=False)
        .agg(
            n=("event_id", "size"),
            miss_rate=("failed", "mean"),
            energy_sigma68=("energy_residual", lambda z: (np.percentile(z, 84) - np.percentile(z, 16)) / 2.0),
            median_abs_time_ns=("time_abs_residual_ns", "median"),
        )
        .reset_index()
    )
    failure.to_csv(out_dir / "winner_failure_morphology.csv", index=False)

    fig, axes = plt.subplots(2, 2, figsize=(10, 8), constrained_layout=True)
    colors = np.where(plot["detected"], "#1f77b4", "#d62728")
    axes[0, 0].scatter(plot["spacing_ns"], plot["energy_residual"], c=colors, s=16, alpha=0.55, linewidths=0)
    axes[0, 0].axhline(0.0, color="black", lw=0.8)
    axes[0, 0].set_xlabel("Injected pulse spacing (ns)")
    axes[0, 0].set_ylabel("Fractional energy residual")
    axes[0, 0].set_title(f"{winner}: energy residual vs spacing")

    axes[0, 1].scatter(plot["true_ratio"], plot["energy_residual"], c=colors, s=16, alpha=0.55, linewidths=0)
    axes[0, 1].axhline(0.0, color="black", lw=0.8)
    axes[0, 1].set_xlabel("Injected amplitude ratio")
    axes[0, 1].set_ylabel("Fractional energy residual")
    axes[0, 1].set_title("Residual vs amplitude morphology")

    axes[1, 0].scatter(plot["true_energy_adc"], plot["time_abs_residual_ns"], c=colors, s=16, alpha=0.55, linewidths=0)
    axes[1, 0].axvline(11000.0, color="black", lw=0.8, ls="--")
    axes[1, 0].set_xlabel("Injected summed amplitude (ADC)")
    axes[1, 0].set_ylabel("Mean absolute constituent timing residual (ns)")
    axes[1, 0].set_title("Timing failures near saturation proxy")

    for saturated, grp in failure.groupby("saturated_proxy"):
        labels = [str(x) for x in grp["spacing_bin"]]
        axes[1, 1].plot(labels, grp["miss_rate"], marker="o", label=f"saturated={saturated}")
    axes[1, 1].set_xlabel("Spacing bin (ns)")
    axes[1, 1].set_ylabel("Miss rate")
    axes[1, 1].set_title("Failure rate by spacing and saturation")
    axes[1, 1].tick_params(axis="x", rotation=25)
    axes[1, 1].legend(frameon=False)

    fig.savefig(out_dir / "winner_residual_morphology.png", dpi=180)
    plt.close(fig)
    return failure
...
def write_report(
    ...
    text = f"""# S33b: saturation energy recovery architecture bakeoff
...
def main() -> None:
    started = time.time()
    os.environ.setdefault("MPLCONFIGDIR", "/tmp/matplotlib-s33b")
    ...
    failure_morphology = write_residual_plots(joined, winner, OUT)
    ...
    result = {
        ...
        "artifacts": {
            ...
            "winner_residual_morphology": "winner_residual_morphology.png",
            "winner_failure_morphology": "winner_failure_morphology.csv",
        },
        "winner_failure_morphology_rows": int(len(failure_morphology)),
    ...
    manifest = {
        ...
        "command": f"{sys.executable} scripts/s33b_1783888239_810_6d016e26_pileup_saturation_energy_recovery_architecture_bakeoff.py",
