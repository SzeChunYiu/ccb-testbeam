...
pedestal robustness, reconstructed energy, and PID-boundary proxy movement.  The worker is `testbeam-laptop-4`.  The
...
The winner reaches held-out timing sigma68 `8.024` ns with 95% CI [`7.365`, `8.669`] ns, pile-up detection AUC `0.8377`, AP `0.8593`, miss rate `0.3256`, and clean-control false-split rate `0.1385`.  Relative to the transparent traditional deconvolution, it changes timing sigma68 by `-1.430` ns, detection AUC by `0.2152`, miss rate by `-0.2590`, false-split rate by `-0.0590`, and energy sigma68 by `-0.03224`.  Across held-out runs, the winner's timing sigma68 spans `7.283`--`8.790` ns.  The machine-readable `timing_frontier_summary.csv` table lists the winner by overlap/spacing bin, saturation-onset bin, and PID-boundary proxy class.
...
The required helper command `tn-ticket claim testbeam-laptop-4 --project testbeam` was run exactly once and returned the known null pseudo-ticket pattern (`null`, `# null`, `null`).  Direct GitHub inspection showed issue #2498 remained `factory:open`.  Following the established project recovery pattern for this helper failure, #2498 was manually label-swapped to `factory:claimed` and `worker:testbeam-laptop-4` without rerunning the claim helper.  No novel follow-up ticket was appended.

## Recommendation

Use `saturation_residual_fusion_new` as the preferred S55b/#2498 timing-frontier energy-closure method
...
Runtime was `45.3` s on `Linux-5.15.0-139-generic-x86_64-with-glibc2.35`.
