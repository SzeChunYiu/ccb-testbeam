#!/usr/bin/env python3
"""S16e external forced/random pedestal ingest audit and no-proxy benchmark.
...
def write_report(outdir: Path, config: dict, result: dict, summary: pd.DataFrame, leakage: pd.DataFrame) -> None:
    ...
    report = """# S16e: external forced/random pedestal ingest audit
...
```bash
/home/billy/anaconda3/bin/python {script_path} --config {config_path}
```
...
def main() -> None:
    ...
    result = {
        ...
        "conclusion": "No true forced/random pedestal data or external run-log candidate is present locally; no-proxy held-out pre-trigger benchmark completed without amplitude-proxy selection, but direct S16d forced/random truth remains not estimable.",
        "next_tickets": [],
    }
