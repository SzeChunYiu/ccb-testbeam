#!/usr/bin/env python3
"""S18h current ticket: fixed-efficiency A-stack gate decomposition."""
...
def write_report(out_dir: Path, cfg: dict, result: dict, tables: dict) -> None:
    ...
    report = f"""# S18h: Fixed-Efficiency A-Stack Gate Decomposition

- **Ticket:** `{cfg['ticket']}`
- **Worker:** `{cfg['worker']}`
- **Date:** 2026-07-09
- **Input:** raw A-stack ROOT `HRDv` from `{cfg['raw_root_dir']}`
- **Command:** `/home/billy/anaconda3/bin/python {cfg['script_path']} --config configs/s18h_1781111715_1694_3de93cb7_fixed_efficiency_gate_decomposition.json`
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/s18h_1781111715_1694_3de93cb7_fixed_efficiency_gate_decomposition.json"))
    ...
    result = {
        ...
        "next_tickets": [
            "S18k: scan CFD fraction under fixed-efficiency A-stack gates to isolate interpolation-noise optima from selection-support changes."
        ],
