...
Ticket question: can the original DAQ logbook, trigger-mode spreadsheet, or
acquisition scripts be located for runs 0000-0065 and matched to HRD ROOT run
numbers? This script treats the current data mirror as evidence: it inventories
raw archives and extracted files, reproduces the raw ROOT run coverage and
trigger-tag absence first, then runs traditional and ML cross-checks for hidden
...
def write_report(outdir: Path, config: dict, result: dict, tables: dict) -> None:
    ...
    report = """# S16e: external DAQ run-log acquisition audit
...
Archive member inventory and extracted filesystem inventory found `{external_hits}` external-log candidates. The only non-ROOT document under the data mirror is `{doc_note}`; no DAQ logbook, trigger-mode spreadsheet, or acquisition script was found in the local mirror or raw zip member names.
...
```bash
/home/billy/anaconda3/bin/python reports/{ticket}/s16e_external_daq_run_log.py --config reports/{ticket}/s16e_external_daq_run_log_config.json
```
...
def main() -> None:
    ...
    result = {
        ...
        "conclusion": "No external DAQ logbook/spreadsheet/acquisition script was found in the current mirror; raw ROOT has no non-beam trigger tags.",
        "git_commit": commit,
    }
    (outdir / "result.json").write_text(json.dumps(result, indent=2, allow_nan=False), encoding="utf-8")
    write_report(outdir, config, result, {"traditional": traditional, "ml_heldout": ml_heldout, "leakage": leakage})

    manifest = {
        "command": "/home/billy/anaconda3/bin/python {} --config {}".format(outdir / "s16e_external_daq_run_log.py", outdir / "s16e_external_daq_run_log_config.json"),
        "config": str(outdir / "s16e_external_daq_run_log_config.json"),
        "git_commit": commit,
