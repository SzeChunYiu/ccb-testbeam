#!/usr/bin/env python3
"""P02e: all-three early-peak pulse topology versus timing tails.

This report-local script reads raw B-stack ROOT files, reproduces the P02
early-peak rate and S07 guarded gross timing-tail count first, then restricts
the timing-tail benchmark to B2 plus all three downstream staves.  Transparent
P02-like morphology scores are compared with a run-held-out RF morphology score.
D_t, C_t, run id, event id, absolute amplitudes, and selected-stave flags are
excluded from the main ML feature matrix.
"""
...
os.environ.setdefault("MPLCONFIGDIR", "/tmp/ccb-testbeam-p02e-mpl")
...
def build_tables(config: dict) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for run in all_runs:
        ...
        run_all_three_control = 0
        run_all_three_clean = 0
        run_all_three_gross_guard = 0
        ...
        for batch in iter_raw(path, ["EVENTNO", "EVT", "HRDv"]):
            ...
            for idx in np.where(control_mask)[0]:
                ...
                if bool(row["all_three_downstream"]):
                    run_all_three_control += 1
                    run_all_three_clean += int(d_t < float(config["clean_dt_max_ns"]))
                    run_all_three_gross_guard += int(d_t > float(config["gross_dt_min_ns"]))
            event_uid_offset += len(eventno)
        ...
        run_rows.append(
            {
                ...
                "all_three_control_events": int(run_all_three_control),
                "all_three_clean_dt_lt3": int(run_all_three_clean),
                "all_three_gross_dt_gt51": int(run_all_three_gross_guard),
            }
        )
        print(
            f"run {run:04d}: selected_pulses={run_selected_pulses} "
            f"parent_control={run_parent_control} gross_gt51={run_parent_gross_guard} "
            f"all_three={run_all_three_control} all_three_gross_gt51={run_all_three_gross_guard}"
        )
...
def write_report(out_dir: Path, config: dict, reproduction: pd.DataFrame, run_counts: pd.DataFrame, scoreboard: pd.DataFrame, leakage: pd.DataFrame, association: pd.DataFrame, result: dict) -> None:
    ...
    text = f"""# P02e: all-three early-peak timing-tail validation
...
def main() -> int:
    ...
    parser.add_argument("--config", default="reports/1781015838.1380.00770dd4/p02e_config.json")
    ...
    all_three_parent = events["all_three_downstream"].astype(bool)
    all_three_gross_guard = all_three_parent & gross_guard_parent
    s07_rep = {
    ...
    all_three_control_rep = {
        "quantity": "all-three downstream control events",
        "report_value": int(config["expected_all_three_control_events"]),
        "reproduced": int(all_three_parent.sum()),
        "delta": int(all_three_parent.sum()) - int(config["expected_all_three_control_events"]),
        "tolerance": 0,
        "pass": bool(int(all_three_parent.sum()) == int(config["expected_all_three_control_events"])),
        "sample_size": int(len(events)),
    }
    all_three_gross_rep = {
        "quantity": "all-three downstream guarded gross events, D_t>51 ns",
        "report_value": int(config["expected_all_three_guarded_gross_events"]),
        "reproduced": int(all_three_gross_guard.sum()),
        "delta": int(all_three_gross_guard.sum()) - int(config["expected_all_three_guarded_gross_events"]),
        "tolerance": 0,
        "pass": bool(int(all_three_gross_guard.sum()) == int(config["expected_all_three_guarded_gross_events"])),
        "sample_size": int(all_three_parent.sum()),
    }
    reproduction = pd.DataFrame([p02_rep, s07_rep, all_three_control_rep, all_three_gross_rep])
    reproduction.to_csv(out_dir / "reproduction_match_table.csv", index=False)
    if not bool(reproduction["pass"].all()):
        raise RuntimeError("Raw reproduction gate failed")

    benchmark = events[all_three_parent & (clean_parent | gross_guard_parent)].copy().reset_index(drop=True)
    ...
    leakage = pd.DataFrame(
        [
            {"probe": "constant all-three topology RF", "roc_auc": auc(y, topo_score), "average_precision": ap(y, topo_score), "notes": f"Selected-stave flags and downstream count only; all benchmark rows have downstream_count unique={sorted(benchmark['downstream_count'].unique().tolist())}."},
    ...
    result = {
        ...
        "traditional": {
            ...
            "metric": "leave-one-run-out ROC AUC on all-three S07 D_t extreme timing-tail labels",
        ...
        "details": {
            ...
            "n_all_three_control": int(all_three_parent.sum()),
            "n_all_three_gross_dt_gt51": int(all_three_gross_guard.sum()),
            "all_three_downstream_unique": sorted(benchmark["downstream_count"].unique().astype(int).tolist()),
        ...
        "next_tickets": [],
        "follow_up_ticket_status": "skipped: prior reports already proposed the non-D_t P02/S07 timing-tail validation follow-up",
    }
    ...
    print(
        json.dumps(
            {
                "out_dir": str(out_dir),
                "p02_early_rate": p02_rep["reproduced"],
                "s07_parent_gross_gt51": s07_rep["reproduced"],
                "all_three_control": all_three_control_rep["reproduced"],
                "all_three_gross_gt51": all_three_gross_rep["reproduced"],
                "traditional_auc": result["traditional"]["value"],
                "ml_auc": result["ml"]["value"],
            },
            indent=2,
        )
    )
