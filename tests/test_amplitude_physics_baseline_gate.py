...
def test_hash_bound_absolute_requires_unique_pedestal_column(tmp_path: Path) -> None:
    ...
    result = MODULE.audit(path, None, 3500.0, 5000.0, evidence_for(path, "ABSOLUTE"))
...
def test_hash_bound_net_does_not_require_optional_pedestal_data(tmp_path: Path) -> None:
    ...
    evidence_path.write_text(json.dumps(evidence_for(path, "NET")), encoding="utf-8")
    result = MODULE.audit(path, None, 3500.0, 5000.0, evidence_for(path, "NET"))
