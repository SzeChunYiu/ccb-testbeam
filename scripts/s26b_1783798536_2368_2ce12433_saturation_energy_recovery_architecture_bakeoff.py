...
TICKET = "1783798536.2368.2ce12433"
SLUG = "s26b_saturation_energy_recovery_architecture_bakeoff"
    ...
    cfg.update(
        {
            "study_id": "S26b",
            "ticket_id": TICKET,
            "title": "Saturation energy recovery architecture bakeoff",
            "worker": WORKER,
            "output_dir": str(OUT),
            "random_seed": 2026071204,
...
def transformer_prediction(events: pd.DataFrame, waveforms: np.ndarray, cfg: dict) -> pd.DataFrame:
    if torch is None:
        raise RuntimeError("torch is required for the S26b transformer benchmark")
...
def write_report(
    ...
    text = f"""# S26b: saturation energy recovery architecture bakeoff
...
def main() -> None:
    ...
    manifest = {
        ...
        "command": f"{sys.executable} scripts/s26b_1783798536_2368_2ce12433_saturation_energy_recovery_architecture_bakeoff.py",
