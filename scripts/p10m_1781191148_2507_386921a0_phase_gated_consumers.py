#!/usr/bin/env python3
"""P10m frozen phase-gated template downstream consumer benchmark."""
...
def run_metrics(config: dict, eval_: pd.DataFrame, pred: np.ndarray, method: str, fold: str) -> pd.DataFrame:
    ...
    for run, sub in work.groupby("run", observed=True):
        ...
        tres, qres = [], []
        for a, b in pairs:
            if a in wide_t and b in wide_t:
                tres.append((wide_t[a] - wide_t[b]).to_numpy())
        ...
        rows.append({"fold": fold, "method": method, "run": int(run), "n_eval_pulses": int(len(sub)), "n_events": int(sub["event_id"].nunique()), "timing_sigma68_ns": sigma68(tv), "timing_rms_ns": float(np.nanstd(tv)), "charge_sigma68_log": sigma68(qv), "charge_rms_log": float(np.nanstd(qv))})
    return pd.DataFrame(rows)


def bootstrap(run_df: pd.DataFrame, rng: np.random.Generator, reps: int) -> pd.DataFrame:
    rows = []
    metrics = ["timing_sigma68_ns", "charge_sigma68_log", "timing_rms_ns", "charge_rms_log"]
    for (fold, method), sub in run_df.groupby(["fold", "method"], observed=True):
        ...
        row["primary_loss"] = float(row["timing_sigma68_ns"] + 10.0 * row["charge_sigma68_log"])
        rows.append(row)
    return pd.DataFrame(rows)
...
def write_report(out_dir: Path, config: dict, repro: pd.DataFrame, summary: pd.DataFrame, diagnostics: pd.DataFrame, result: dict) -> None:
    ...
    report = rf"""# P10m: Frozen Phase-Gated Tail Templates for Downstream Consumers
...
The winner named in `result.json` is **{winner}**, selected among non-sentinel methods by the mean of the fold-level primary loss `timing_sigma68_ns + 10 * charge_sigma68_log`. Timing and charge are both reported separately so the scalar score cannot hide a detector-performance tradeoff.
...
def plot_summary(out_dir: Path, summary: pd.DataFrame) -> None:
    ...
    ax2 = ax.twinx()
    ax2.plot(x, held["charge_sigma68_log"], "o-", color="tab:red", label="charge sigma68 log")
    ax.set_xticks(x)
    ax.set_xticklabels(labels, rotation=75, ha="right", fontsize=7)
    ax.set_ylabel("timing sigma68 (ns)")
    ax2.set_ylabel("charge sigma68 (log)")
    fig.tight_layout()
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/p10m_1781191148_2507_386921a0_phase_gated_consumers.yaml")
    ...
    overall = summary.groupby("method", observed=True)[["timing_sigma68_ns", "charge_sigma68_log", "primary_loss"]].mean().reset_index().sort_values("primary_loss")
    ...
    result = {
        ...
        "worker": config["worker"],
        "winner": winner,
        "winner_selection": "lowest non-sentinel mean fold primary_loss = timing_sigma68_ns + 10 * charge_sigma68_log",
