#!/usr/bin/env python3
"""Ticket 2383 S13 current-scaling and CWoLa weak-supervision bakeoff.
...
os.environ.setdefault("MPLCONFIGDIR", str(ROOT / "reports" / ".mplconfig_ticket_2383_s13"))
...
def build_pulse_current_dataset(config: dict, data_by_run: Dict[int, dict]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    rows = []
    feats = []
    ...
    return pd.DataFrame(rows), pd.concat(feats, ignore_index=True)


def app_h_pulse_auc(config: dict, data_by_run: Dict[int, dict]) -> float:
    """Reproduce App. H as a pulse-level CWoLa current classifier."""
    meta, features = build_pulse_current_dataset(config, data_by_run)
    feature_cols = [f"norm_s{i:02d}" for i in range(int(config["samples_per_channel"]))] + [
        "log_amp",
        "peak_sample",
        "area_over_peak",
        "tail_fraction",
    ...
    scored_parts = []
    seed = int(config.get("app_h_random_seed", config["random_seed"]))
    for fold_i, fold in enumerate(config["folds"]):
        ...
        train_idx = []
        rng = np.random.default_rng(seed + 7100 + fold_i)
        for (_run, _stave, _label), subset in meta[meta["run"].isin(train_runs)].groupby(["run", "stave", "high_current"]):
            idx = subset.index.to_numpy()
            cap = int(config.get("app_h_max_train_pulses_per_run_stave", 900))
            if len(idx) > cap:
                idx = rng.choice(idx, cap, replace=False)
            train_idx.append(idx)
        train_idx = np.concatenate(train_idx)
        low_idx = train_idx[meta.loc[train_idx, "high_current"].to_numpy(dtype=int) == 0]
        high_idx = train_idx[meta.loc[train_idx, "high_current"].to_numpy(dtype=int) == 1]
        n = min(len(low_idx), len(high_idx))
        train_idx = rng.permutation(np.r_[rng.choice(low_idx, n, replace=False), rng.choice(high_idx, n, replace=False)])
        test_idx = []
        for (_run, _stave), subset in meta[meta["run"].isin(test_runs)].groupby(["run", "stave"]):
            idx = subset.index.to_numpy()
            cap = int(config.get("app_h_max_eval_pulses_per_run_stave", 2500))
            if len(idx) > cap:
                idx = rng.choice(idx, cap, replace=False)
            test_idx.append(idx)
        ...
        clf = RandomForestClassifier(
            ...
            random_state=seed + 7200 + fold_i,
            n_jobs=1,
        )
        clf.fit(features.loc[train_idx, feature_cols], meta.loc[train_idx, "high_current"].to_numpy(dtype=int))
        part = meta.loc[test_idx, ["run", "high_current"]].copy()
        part["cwola_score"] = clf.predict_proba(features.loc[test_idx, feature_cols])[:, 1]
        scored_parts.append(part)
    scored = pd.concat(scored_parts, ignore_index=True)
    return float(roc_auc_score(scored["high_current"].to_numpy(dtype=int), scored["cwola_score"].to_numpy(dtype=float)))
...
def current_scaling_fit(topology: pd.DataFrame) -> pd.DataFrame:
    grouped = topology.groupby("high_current").agg(
        events=("events_with_selected", "sum"),
        multi_stave_events=("multi_stave_events", "sum"),
    ).reset_index()
    grouped["current_nA"] = np.where(grouped["high_current"].to_numpy(dtype=int) == 1, 20.0, 2.0)
    grouped["fraction"] = grouped["multi_stave_events"] / grouped["events"]
    grouped["sigma"] = np.sqrt(np.maximum(grouped["fraction"] * (1.0 - grouped["fraction"]) / grouped["events"], 1e-12))
    x = grouped["current_nA"].to_numpy(dtype=float)
    y = grouped["fraction"].to_numpy(dtype=float)
    w = 1.0 / np.square(grouped["sigma"].to_numpy(dtype=float))
    design = np.c_[np.ones_like(x), x]
    beta = np.linalg.solve(design.T @ (w[:, None] * design), design.T @ (w * y))
    pred = design @ beta
    chi2 = float(np.sum(np.square((y - pred) / grouped["sigma"].to_numpy(dtype=float))))
    grouped["fit_fraction"] = pred
    grouped["residual_fraction"] = y - pred
    grouped["f0"] = float(beta[0])
    grouped["k_per_nA"] = float(beta[1])
    grouped["chi2"] = chi2
    grouped["ndf"] = max(int(len(grouped) - 2), 0)
    return grouped.sort_values("current_nA")
...
def reproduction_table(config: dict, topology: pd.DataFrame) -> pd.DataFrame:
    ...
    low = topology[topology["run"].isin(low_runs)]
    high = topology[topology["run"].isin(high_runs)]
    low_multi_pct = 100.0 * float(low["multi_stave_events"].sum() / low["events_with_selected"].sum())
    high_multi_pct = 100.0 * float(high["multi_stave_events"].sum() / high["events_with_selected"].sum())
    rows = [
        {
            "quantity": "App. H low-current raw multi-stave fraction (%)",
            "report_value": float(config["expected_low_multi_stave_fraction_percent"]),
            "reproduced": low_multi_pct,
            "delta": low_multi_pct - float(config["expected_low_multi_stave_fraction_percent"]),
            "tolerance": float(config["expected_current_fraction_tolerance_percent"]),
            "pass": abs(low_multi_pct - float(config["expected_low_multi_stave_fraction_percent"])) <= float(config["expected_current_fraction_tolerance_percent"]),
        },
        {
            "quantity": "App. H high-current raw multi-stave fraction (%)",
            "report_value": float(config["expected_high_multi_stave_fraction_percent"]),
            "reproduced": high_multi_pct,
            "delta": high_multi_pct - float(config["expected_high_multi_stave_fraction_percent"]),
            "tolerance": float(config["expected_current_fraction_tolerance_percent"]),
            "pass": abs(high_multi_pct - float(config["expected_high_multi_stave_fraction_percent"])) <= float(config["expected_current_fraction_tolerance_percent"]),
        },
        {
...
def fit_ridge(train_x: pd.DataFrame, train_y: np.ndarray, test_x: pd.DataFrame, c_grid: List[float], seed: int) -> np.ndarray:
    ...
    best = None
    for c in c_grid:
        clf = LogisticRegression(C=float(c), class_weight="balanced", max_iter=1000, random_state=seed)
    ...
    for lr in config["gbt_learning_rates"]:
        for leaves in config["gbt_max_leaf_nodes"]:
            clf = HistGradientBoostingClassifier(
                ...
                max_iter=int(config.get("gbt_max_iter", 120)),
                random_state=seed,
            )
...
def fit_mlp(train_x: pd.DataFrame, train_y: np.ndarray, test_x: pd.DataFrame, config: dict, seed: int) -> np.ndarray:
    ...
    for hidden in config["mlp_hidden_layers"]:
        clf = MLPClassifier(hidden_layer_sizes=tuple(int(x) for x in hidden), alpha=1e-3, max_iter=int(config.get("mlp_max_iter", 250)), random_state=seed, early_stopping=True)
        clf.fit(scaler.transform(train_x), train_y)
        p = clf.predict_proba(scaler.transform(train_x))[:, 1]
        score = brier_score_loss(train_y, p)
        if best is None or score < best[0]:
...
def run_study(config: dict, data_by_run: Dict[int, dict], out_dir: Path) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    app_h_auc = app_h_pulse_auc(config, data_by_run)
    repro = pd.concat(
        [
            repro,
            pd.DataFrame(
                [
                    {
                        "quantity": "App. H weak current classifier run-transfer AUC",
                        "report_value": float(config["expected_app_h_auc"]),
                        "reproduced": app_h_auc,
                        "delta": app_h_auc - float(config["expected_app_h_auc"]),
                        "tolerance": float(config["expected_app_h_auc_tolerance"]),
                        "pass": abs(app_h_auc - float(config["expected_app_h_auc"])) <= float(config["expected_app_h_auc_tolerance"]),
                    }
                ]
            ),
        ],
...
def write_report(config: dict, out_dir: Path, repro: pd.DataFrame, topology: pd.DataFrame, metrics: pd.DataFrame, leakage: pd.DataFrame, runtime: float) -> None:
    ...
    current_fit = current_scaling_fit(topology)
    lines = [
        ...
        f"- **Config:** `configs/ticket_2383_s13_current_scaling_cwola_bakeoff.json`",
        "",
        "## 0. Question",
        "",
        ...
        "",
        current_fit[["current_nA", "events", "multi_stave_events", "fraction", "sigma", "fit_fraction", "f0", "k_per_nA", "chi2", "ndf"]].to_markdown(index=False),
        "",
        ...
        "",
        f"Queued follow-up appended after completion: `#{config.get('appended_ticket_number', 'pending')}` `S13e: low-current support expansion for topology calibration`. Expected information gain: separates real topology-bridge performance from the dominant two-low-run support systematic.",
        "",
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/ticket_2383_s13_current_scaling_cwola_bakeoff.json"))
    ...
    current_fit = current_scaling_fit(topology)
    current_fit.to_csv(out_dir / "current_scaling_fit.csv", index=False)
    write_plots(out_dir, metrics, preds)

    input_sha = pd.DataFrame(
        [{"file": str(raw_file(config, run)), "sha256": sha256_file(raw_file(config, run)), "bytes": raw_file(config, run).stat().st_size} for run in all_runs]
    )
    ...
    result = {
        ...
        "reproduction": {
            "gate": "all raw-ROOT reproduction rows pass",
        ...
        "required_method_coverage": {
            "traditional": "traditional_stratified plus explicit f(I)=f0+kI current_scaling_fit",
            "ridge": "ridge",
            "gradient_boosted_trees": "gradient_boosted_trees",
            "mlp": "mlp",
            "one_dimensional_cnn": "cnn1d",
            "new_architecture": "hybrid_cnn_score_gate",
        },
        "traditional": {
        ...
        "traditional_current_scaling_fit": current_fit.to_dict(orient="records"),
        ...
        "next_tickets": [
            {
                "number": int(config["appended_ticket_number"]) if "appended_ticket_number" in config else None,
                "title": "S13e: low-current support expansion for topology calibration",
                "body": "Question: does the S13 current-scaling/CWoLa calibration ranking survive when additional low-current-like support is formed from adjacent quiet runs or stricter matched strata? Expected information gain: separates real topology bridge performance from the dominant two-low-run support systematic."
            }
        ],
        "novel_tickets_appended": [int(config["appended_ticket_number"])] if "appended_ticket_number" in config else [],
        "runtime_sec": round(runtime, 2),
    }
    (out_dir / "result.json").write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
    write_report(config, out_dir, repro, topology, metrics, leakage, runtime)
    ...
    manifest = {
        ...
        "commands": [f"{sys.executable} scripts/ticket_2383_s13_current_scaling_cwola_bakeoff.py --config {args.config}"],
