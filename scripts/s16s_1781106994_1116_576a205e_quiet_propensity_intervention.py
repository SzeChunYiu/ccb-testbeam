...
CONFIG = ROOT / "configs" / "s16s_1781106994_1116_576a205e_quiet_propensity_intervention.json"
OUT = ROOT / "reports" / "1781106994.1116.576a205e__s16s_quiet_propensity_intervention"
TICKET = "1781106994.1116.576a205e"
WORKER = "testbeam-laptop-3"
METHODS = [
    "traditional_matched_strata",
    "ridge",
...
def fit_predict(method: str, train: pd.DataFrame, test: pd.DataFrame, train_w, test_w, seed: int) -> np.ndarray:
    if method == "traditional_matched_strata":
...
def main() -> None:
    ...
    result = {
        ...
        "artifacts": ["REPORT.md", "result.json", "metrics_summary.csv", "per_run_metrics.csv", "intervention_curve.csv", "raw_root_inventory.csv", "raw_root_selection_counts.csv", "benchmark_panel.csv"],
    ...
    report = f"""# S16s: Support-Matched Quiet-Propensity Intervention Curves
...
`{winner}` is the named winner for this S16s run-held-out benchmark.  The intervention curve supports the qualitative ticket claim that quiet-proxy support contains information beyond gross amplitude/topology, but the caveats require a ROOT-enabled rerun before an operational veto is proposed.
"""
