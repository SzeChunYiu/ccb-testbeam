#!/usr/bin/env python3
"""P07i: duplicate-readout run-family acceptance gate for natural B2 saturation.

The analysis reads raw B-stack ROOT files via the P07f duplicate extractor, reproduces the
P07f count/family anchors, and benchmarks a transparent per-run duplicate-knee gate against
ridge, gradient-boosted trees, MLP, a 1D CNN, and a residual gated CNN.  All supervised
methods are leave-one-run-out and exclude run/event identifiers plus all odd-channel variables.
"""
...
DEFAULT_CONFIG = ROOT / "configs/p07i_1781051216_479_669827f2_run_family_acceptance_gate.json"
...
def candidate_frame(frame: pd.DataFrame, wave: np.ndarray, fits: Dict[int, object], knees: pd.DataFrame, cfg: dict) -> Tuple[pd.DataFrame, np.ndarray]:
    ...
    out["traditional_accept"] = in_band[keep].astype(int)
...
def write_report(
    ...
    lines = [
        ...
        "",
        f"Winner by side-effect-gated utility is **{winner['method']}**. The transparent traditional baseline has accepted fraction {trad['accepted_fraction']:.4f} and harm rate {trad['harm_rate_vs_no_correction']:.4f}; the winner has accepted fraction {winner['accepted_fraction']:.4f} and harm rate {winner['harm_rate_vs_no_correction']:.4f}.",
        "",
        "## 5. Falsification",
        "",
...
def main() -> int:
    ...
    finding = (
        "The configured side-effect-gated utility ranks {} first, with accepted fraction "
        "{:.4f} [{:.4f}, {:.4f}], charge res68 {:.4f} [{:.4f}, {:.4f}], "
        "timing-tail delta {:+.4f}, q_template median shift {:+.4f}, and harm rate {:.4f}. "
        "That win is driven by near-total coverage, not label purity: precision is only {:.4f}. "
        "The transparent duplicate-run-family gate is the conservative deployment recommendation "
        "because it has much lower harm ({:.4f}) and directly enforces odd-readout knee support, "
        "although its accepted fraction is smaller ({:.4f}). Waveform-only ML therefore does not "
    ...
    result = {
        ...
        "next_tickets": [
            {
                "title": "P07j duplicate-gated natural B2 correction on independent q/timing consumers",
                "body": "Apply the P07i traditional run-family duplicate gate to downstream timing, PID, and q_template consumers with the odd duplicate channel hidden after gate formation; this tests whether the safe acceptance support remains safe outside the gate-construction observables and has high information gain because it can falsify production adoption directly."
            }
        ],
        "runtime_sec": None,
    }
