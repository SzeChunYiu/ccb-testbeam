...
PATH = Path(__file__).resolve().parents[1] / "scripts/clusterE/clusterE_canonical_frontdoor.py"
...
def repo(tmp_path: Path) -> tuple[Path, str]:
    ...
    subprocess.run(["git", "-C", str(root), "config", "user.email", "x@invalid"], check=True)
    ...
    with ledger.open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow(row("CL-013", current_value="92", unit="ADC/MeV", syst_unc="28", truth_type="data_mc_calibration_proxy", status="GATED", source_data=mod.SOURCES["CL-013"], blocked_by="BLK-MV0-001"))
        writer.writerow(row("CL-021", current_value="68269.40598948313", unit="dimensionless", truth_type="legacy_data_mc_profile_diagnostic", status="FLAWED", source_data=mod.SOURCES["CL-021"], blocked_by="BLK-MV3-LEGACY-001"))
        writer.writerow(row("CL-022", current_value="0.003232254011764034", unit="fraction", numerator="283", denominator="87555", truth_type="mc_truth_only", status="TRUTH_LEVEL_MC_ONLY", source_data=mod.SOURCES["CL-022"], blocked_by="AUD-ANOM-001"))
    write_json(root / mod.SOURCES["CL-013"], {"calibration": {"gain_adc_per_mev": 92.0, "gain_systematic_unc_pct": 30}})
    write_json(root / mod.SOURCES["CL-021"], {"chi2_per_ndf": 68269.40598948313})
    write_json(root / mod.SOURCES["CL-022"], {"n_tracks": 87555, "morphology_counts": {"early_peak": 283}, "anomaly_frac_total": 0.003232254011764034})
...
def test_exact_claims_and_distinct_diagnostics(tmp_path: Path) -> None:
    ...
    result = mod.build(root, commit, "2026-07-26T140553Z")
    assert result["status"] == "VALIDATED"
    summary = (root / "reports/studies/clusterE/SUMMARY.md").read_text()
    for token in ("92 ADC/MeV", "28 ADC/MeV", "68269.40598948313", "283/87555"):
        assert token in summary
    assert "does **not supersede CL-021**" in summary
    assert "25/38 toy early-peak C12" not in summary


def test_full_content_identities(tmp_path: Path) -> None:
    root, commit = repo(tmp_path)
    mod.build(root, commit, "2026-07-26T140553Z")
    value = json.loads((root / "reports/studies/clusterE/provenance.json").read_text())
    assert value["base_commit"] == commit
    assert set(value["input_identities"]) == {mod.LEDGER, *mod.SOURCES.values(), mod.DIAGNOSTIC, mod.PRODUCER}
...
def test_source_mismatch_fails_closed(tmp_path: Path) -> None:
    ...
    subprocess.run(["git", "-C", str(root), "add", "."], check=True)
    subprocess.run(["git", "-C", str(root), "commit", "-qm", "bad"], check=True)
    commit = subprocess.check_output(["git", "-C", str(root), "rev-parse", "HEAD"], text=True).strip()
...
def test_duplicate_claim_fails_closed(tmp_path: Path) -> None:
    ...
    ledger.write_text("\n".join(lines + [lines[1]]) + "\n")
    subprocess.run(["git", "-C", str(root), "add", "."], check=True)
    subprocess.run(["git", "-C", str(root), "commit", "-qm", "dup"], check=True)
    commit = subprocess.check_output(["git", "-C", str(root), "rev-parse", "HEAD"], text=True).strip()
...
def test_atomic_failure_preserves_target(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
