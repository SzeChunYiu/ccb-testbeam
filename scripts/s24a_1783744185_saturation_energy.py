#!/usr/bin/env python3
"""S24a: saturation-aware energy reconstruction from pulse morphology.
...
class WaveTransformer(nn.Module):
    def __init__(self, n_tab: int, d_model: int = 32, nhead: int = 4):
        super().__init__()
        self.sample_proj = nn.Linear(4, d_model)
        ...
        self.head = nn.Sequential(nn.Linear(d_model + n_tab, 64), nn.GELU(), nn.Linear(64, 1))

    def forward(self, wave, tab):
        # event_wave is (batch, four B staves, eighteen samples); attention runs over time.
        z = wave.permute(0, 2, 1)
        z = self.sample_proj(z) + self.pos[:, : z.shape[1], :]
        z = self.encoder(z).mean(dim=1)
        return self.head(torch.cat([z, tab], dim=1)).squeeze(1)


def fit_transformer(event_wave: np.ndarray, x: np.ndarray, y: np.ndarray, train_mask: np.ndarray, config: dict) -> Tuple[object, StandardScaler]:
    if torch is None:
        raise RuntimeError("torch unavailable")
    max_rows = int(config.get("transformer_max_train_events", config["cnn_max_train_events"]))
    idx = sample_train_indices(train_mask, max_rows, int(config["random_seed"]) + 140)
    scaler = StandardScaler().fit(x[idx])
    x_train = scaler.transform(x[idx]).astype(np.float32)
    w = event_wave[idx].astype(np.float32)
    scale = np.maximum(np.percentile(np.abs(w).reshape(len(w), -1), 95, axis=1), 1.0)
    ...
    ds = TensorDataset(torch.from_numpy(w), torch.from_numpy(x_train), torch.from_numpy(t))
    loader = DataLoader(ds, batch_size=512, shuffle=True)
    torch.manual_seed(int(config["random_seed"]) + 141)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = WaveTransformer(x.shape[1]).to(device)
    opt = torch.optim.AdamW(model.parameters(), lr=8e-4, weight_decay=1e-4)
    loss_fn = nn.SmoothL1Loss()
    model.train()
    for _ in range(int(config.get("transformer_epochs", config["cnn_epochs"]))):
        for wb, xb, yb in loader:
            wb = wb.to(device)
            xb = xb.to(device)
            yb = yb.to(device)
...
def predict_transformer(model: object, scaler: StandardScaler, event_wave: np.ndarray, x: np.ndarray) -> np.ndarray:
    device = next(model.parameters()).device
    xs = scaler.transform(x).astype(np.float32)
    pred = []
    for start in range(0, len(x), 4096):
...
def stratified_rows(events: pd.DataFrame, y: np.ndarray, predictions: Dict[str, np.ndarray], held_mask: np.ndarray, config: dict) -> pd.DataFrame:
    rows = []
    held_events = events.loc[held_mask]
    pedestal_proxy = held_events["even_total_charge"].to_numpy(dtype=float) / np.maximum(held_events["even_max_amp"].to_numpy(dtype=float), 1.0)
    definitions = [
        ("adc_saturation_onset", events["any_saturated"].to_numpy(dtype=bool), "any_saturated"),
        ("pileup_or_multihit", events["multiplicity"].to_numpy(dtype=int) >= 2, "multiplicity>=2"),
        (
            "pedestal_drift_proxy_high",
            events["even_total_charge"].to_numpy(dtype=float) / np.maximum(events["even_max_amp"].to_numpy(dtype=float), 1.0)
            >= float(np.median(pedestal_proxy)),
            "charge/peak above heldout median",
        ),
        ("late_pulse_shape", events["depth_idx"].to_numpy(dtype=int) >= 2, "deepest selected B stave index>=2"),
    ]
    for stratum, mask, definition in definitions:
        for label, subset_mask in [("in_stratum", held_mask & mask), ("complement", held_mask & ~mask)]:
            if int(subset_mask.sum()) < 20:
                continue
            for method, pred in predictions.items():
                row = {
                    "stratum": stratum,
                    ...
                    "bias_frac": bias(y[subset_mask], pred[subset_mask]),
                    "res68_frac": res68(y[subset_mask], pred[subset_mask]),
                    "mae_mev": float(mean_absolute_error(y[subset_mask], pred[subset_mask])),
                }
                row.update(
                    run_block_bootstrap(
                        events,
                        ...
                        int(config["random_seed"]) + len(method) + len(stratum) + len(label),
                    )
                )
                rows.append(row)
    return pd.DataFrame(rows)
...
def make_report(
    ...
    report_heading = str(
        config.get("report_heading", "S24a: saturation-aware pulse-morphology energy reconstruction")
    )
    ...
    lines = [
        ...
        (
            ...
            "a 1D-CNN, a compact waveform transformer, and a Birks-residual MLP. The raw ROOT reproduction gate passes "
        ...
        "",
        md_table(
            strata[strata["method"].isin([winner, "geant4_birks_lookup", "transformer", "1d_cnn"])],
...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/s24a_1783744185_saturation_energy.yaml")
    ...
    print("6/8 waveform transformer", flush=True)
    try:
        transformer, transformer_scaler = fit_transformer(event_wave, x, y, train, config)
        predictions["transformer"] = predict_transformer(transformer, transformer_scaler, event_wave, x)
        transformer_status = "trained"
    except Exception as exc:
        predictions["transformer"] = np.full(len(y), np.nan)
        transformer_status = f"failed: {exc}"

    print("7/8 physics-residual MLP", flush=True)
    ...
    families = {
        ...
        "transformer": "neural_waveform_attention",
    ...
    strata = stratified_rows(events, y, finite_predictions, held, config)
    ...
    strata.to_csv(out_dir / "saturation_shape_strata_metrics.csv", index=False)
    ...
    result = {
        ...
        "saturation_shape_strata_metrics": json.loads(strata.to_json(orient="records")),
        "truth_vs_s14g_delta": json.loads(comparison.to_json(orient="records")),
        "leakage_checks": json.loads(leakage.to_json(orient="records")),
        "new_architecture": "transformer: a compact attention encoder over the 18 waveform time samples, benchmarked alongside a physics_residual_mlp that predicts a multiplicative correction to the GEANT4-truth Birks baseline.",
        "finding": (
            ...
            f"Across the ML/NN panel including ridge, gradient-boosted trees, MLP, 1D-CNN, transformer, and residual MLP, the held-out winner is {winner_row['method']} with res68={float(winner_row['res68_frac']):.5f}. "
        ...
        "next_tickets": [
            {
                ...
                "body": (
                    ...
                    "Benchmark the S24a truth-Birks lookup and residual MLP on simulated ADC waveforms with known true deposited energy, then compare the residual structure to real run-held-out data. "
    ...
    make_report(out_dir, config, result, metrics, prior, truth_event_summary, calibration, byrun, energy_bins, strata, comparison, leakage)
    ...
    outputs = [
        ...
        "saturation_shape_strata_metrics.csv",
    ...
    manifest = {
        ...
        "command": str(
            config.get(
                "reproduction_command",
                f"/home/billy/anaconda3/bin/python scripts/s24a_1783744185_saturation_energy.py --config {config_path.relative_to(ROOT)}",
            )
        ),
