...
def run_one_fold(
    ...
    return benchmark, residuals, leakage, hier_cv, hier_coef, hgb_cv, oracle
...
def plot_outputs(out_dir: Path, per_run: pd.DataFrame, pooled: pd.DataFrame, oracle: pd.DataFrame) -> None:
    order = ["template_phase_base", "s03a_amp_only_global", "hierarchical_shrinkage", "hgb_timewalk"]
...
def write_report(
    ...
    ordered = ["template_phase_base", "s03a_amp_only_global", "hierarchical_shrinkage", "hgb_timewalk"]
    ...
    lines = [
        ...
        f"Hierarchical shrinkage pooled sigma68 is `{result['traditional']['hierarchical_shrinkage']['value']:.3f} ns`; global S03a is `{result['traditional']['s03a_amp_only_global']['value']:.3f} ns`; HGB is `{result['ml']['value']:.3f} ns`.",
        ...
        "",
        "```bash",
        f"{sys.executable} scripts/s03d_1781011277_910_1e815d8f_hierarchical_timewalk.py --config {config_path}",
        "```",
        "",
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/s03d_1781011277_910_1e815d8f_hierarchical_timewalk.yaml")
    ...
    run65_bench, _, _, _, _, _, _ = run_one_fold(pulses_all, config, 65, all_runs, rng)
    ...
    for heldout_run in all_runs:
        bench, residuals, leakage, hier_cv, hier_coef, hgb_cv, oracle = run_one_fold(
    ...
    min_shuffle = float(
        leakage[leakage["check"].isin(["hier_shuffled_target_sigma68", "hgb_shuffled_target_sigma68"])]["value"].min()
    )
    looks_too_good = bool(float(hier_row["value"]) < 0.8 or float(hier_row["value"] - hgb_row["value"]) < -0.3)
    leakage_flag = bool(event_overlap != 0 or min_shuffle < min(float(hier_row["value"]), float(hgb_row["value"])) + 0.2)
    ...
    result = {
        ...
        "raw_root_reproduction": {
            ...
            "run65_s03a_pass": bool(run65_repro["pass"].all()),
        },
        "split": {
            "unit": "run",
        ...
        "critic": "pending",
        "next_tickets": [
            "S03e: blind Sample-I to Sample-II transfer for S03a/S03d population coefficients",
            "S03f: add robust heavy-tail analytic timewalk loss for run-61-like folds",
        ],
    }
