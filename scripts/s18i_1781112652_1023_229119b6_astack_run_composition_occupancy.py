...
os.environ.setdefault("MPLCONFIGDIR", "reports/1781112652.1023.229119b6__s18i_astack_run_composition_occupancy/.mplconfig")
...
ATOM_MODES = [
    "full_features",
    "drop_leading_edge_samples",
    "drop_tail_samples",
    "amplitude_only",
    "waveform_shuffled_control",
]
...
def atom_feature_matrix(df: pd.DataFrame, mode: str, rng: np.random.Generator) -> np.ndarray:
    """Return an S18i atom-ablation feature matrix using the S18g feature order."""
    x = engineered_features(df).copy()
    if mode == "full_features":
        return x
    if mode == "amplitude_only":
        keep = [0, 1, 2, 3, 4, 5, 6, 9, 10, 11]
        return x[:, keep]
    if mode == "drop_leading_edge_samples":
        # moments 0:22, waveform samples 22:58, left/right 18 each, diffs 58:76.
        for sample in range(0, 6):
            x[:, 22 + sample] = 0.0
            x[:, 22 + 18 + sample] = 0.0
            x[:, 58 + sample] = 0.0
        return x
    if mode == "drop_tail_samples":
        for sample in range(10, 18):
            x[:, 22 + sample] = 0.0
            x[:, 22 + 18 + sample] = 0.0
            x[:, 58 + sample] = 0.0
        return x
    if mode == "waveform_shuffled_control":
        wave = x[:, 22:].copy()
        for col in range(wave.shape[1]):
            wave[:, col] = wave[rng.permutation(len(wave)), col]
        x[:, 22:] = wave
        return x
    raise ValueError(f"unknown atom mode: {mode}")


def atom_audit(train: pd.DataFrame, test: pd.DataFrame, config: dict, rng: np.random.Generator) -> tuple[pd.DataFrame, pd.DataFrame]:
    rows = []
    preds = test[["run", "event", "raw_residual_ns"]].copy()
    y_train = train["raw_residual_ns"].to_numpy(dtype=float)
    y_test = test["raw_residual_ns"].to_numpy(dtype=float)
    for mode in ATOM_MODES:
        x_train = atom_feature_matrix(train, mode, rng)
        x_test = atom_feature_matrix(test, mode, rng)
        estimators = {
            ...
            "gradient_boosted_trees": HistGradientBoostingRegressor(
                ...
                random_state=int(config["random_seed"]) + len(rows),
            ),
        }
        for family, estimator in estimators.items():
            estimator.fit(x_train, y_train)
            residual = y_test - estimator.predict(x_test)
            col = f"atom_{mode}_{family}"
            preds[col] = residual
            tmp = test[["run"]].copy()
            tmp[col] = residual
            row = metric_row(mode, family, tmp.assign(pool=mode), col, config, rng)
            row["atom_mode"] = mode
...
def occupancy_audit(all_pred: pd.DataFrame, config: dict, primary_label: str) -> pd.DataFrame:
    """Summarize predeclared residual-bin occupancy deviations by run and method."""
    edges = np.asarray(config["support_matching"]["occupancy_bins_ns"], dtype=float)
    labels = []
    for lo, hi in zip(edges[:-1], edges[1:]):
        left = "-inf" if lo < -1e8 else f"{lo:.1f}"
        right = "inf" if hi > 1e8 else f"{hi:.1f}"
        labels.append(f"[{left},{right})")
    sub = all_pred[all_pred["pool"].eq(primary_label)].copy()
    rows = []
    for method in METHOD_ORDER:
        if method not in sub.columns:
            continue
        centered = sub[method].to_numpy(dtype=float) - np.nanmedian(sub[method].to_numpy(dtype=float))
        overall_counts, _ = np.histogram(centered, bins=edges)
        expected = overall_counts / max(int(overall_counts.sum()), 1)
        for run, group in sub.groupby("run", sort=True):
            run_centered = group[method].to_numpy(dtype=float) - np.nanmedian(sub[method].to_numpy(dtype=float))
            counts, _ = np.histogram(run_centered, bins=edges)
            frac = counts / max(int(counts.sum()), 1)
            for label, count, observed, target in zip(labels, counts, frac, expected):
                rows.append(
                    {
                        "method": method,
                        "run": int(run),
                        ...
                        "pooled_target_fraction": float(target),
                        "abs_deviation": float(abs(observed - target)),
                    }
                )
    return pd.DataFrame(rows)
...
def write_report(
    ...
    occ_summary = (
        occupancy.groupby("method")
        .agg(max_abs_bin_deviation=("abs_deviation", "max"), mean_abs_bin_deviation=("abs_deviation", "mean"))
        .reset_index()
        .sort_values("max_abs_bin_deviation")
    )
    ...
    report = f"""# S18i: A-stack run-composition stress test with residual-bin occupancy targets
...
This study asks whether the run-63 stabilizing-composition diagnosis can be falsified by predeclared residual-bin occupancy targets across Sample IV A1-A3 runs. The primary gate is CFD `{config['stress_cfd_fractions']}` with amplitude cut `{config['stress_amplitude_cuts_adc']}` ADC. Raw A1-A3 residuals are reconstructed directly from ROOT, then corrected with a strong constrained traditional timewalk model and five learned alternatives: ridge, gradient-boosted trees, MLP, 1D-CNN, and a new gated residual CNN. The ticket-specific audits measure residual-bin occupancy drift, central-bin stability, amplitude balance, and waveform ablations without conditioning model training on held-out residual labels.
...
The occupancy audit predeclares bins `{config['support_matching']['occupancy_bins_ns']}` ns after centering each method's held-out residuals by its pooled median. For method `m`, run `r`, and residual bin `b`, the observed occupancy is `p_{{m,r,b}} = n_{{m,r,b}} / n_{{m,r}}`; the target is the pooled held-out occupancy `p_{{m,.,b}}`. The stress statistic is `max_b |p_{{m,r,b}} - p_{{m,.,b}}|`, summarized over runs.
...
{occ_summary.to_markdown(index=False)}
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/s18i_1781112652_1023_229119b6_astack_run_composition_occupancy.json"))
    ...
    atom_metrics, atom_predictions = atom_audit(leakage_train, sample_iv_test, config, rng)
    occupancy = occupancy_audit(all_pred, config, primary_label)
    ...
    occupancy.to_csv(out_dir / "residual_bin_occupancy.csv", index=False)
    atom_metrics.to_csv(out_dir / "atom_ablation_metrics.csv", index=False)
    atom_predictions.to_csv(out_dir / "atom_ablation_predictions.csv.gz", index=False, compression="gzip")
    ...
    result = {
        ...
        "atom_audit": {
            "best_atom_model": atom_metrics.sort_values("robust_width_ns").iloc[0].to_dict(),
            "modes": ATOM_MODES,
            "interpretation": "Support-matched monotone baseline plus full, leading-edge-dropped, tail-dropped, amplitude-only, and waveform-shuffled controls on the same Sample III to Sample IV split.",
        },
        "occupancy_audit": {
            "bins_ns": [float(x) for x in config["support_matching"]["occupancy_bins_ns"]],
            "best_occupancy_method": occupancy_summary.iloc[0].to_dict(),
            "summary": occupancy_summary.to_dict("records"),
        },
        "input_sha256": str(out_dir / "input_sha256.csv"),
        "git_commit": git_head(),
        "next_tickets": [
            "S18k: validate the S18i residual-bin occupancy stress test under fixed-efficiency per-run amplitude quantile gates to separate pulse-selection support loss from CFD interpolation noise."
        ],
    }
    ...
    write_report(out_dir, config, args.config, counts, repro, raw_metrics, metrics, deltas, per_run, occupancy, ridge_cv, leakage, result)
    with (out_dir / "REPORT.md").open("a", encoding="utf-8") as handle:
        handle.write(
            "\n## S18i Support and Waveform Nuisance Audit\n\n"
            "This ticket-specific audit asks whether support-matched monotone transfer is enough to explain the late Sample-IV A1-A3 width, or whether waveform-local nuisance capacity is still required. "
            "The table below retrains ridge and gradient-boosted-tree residual models on the primary CFD20/cut1000 split after targeted ablations. "
            "`drop_leading_edge_samples` zeros normalized samples 0-5 in both channels and their pairwise differences; "
            "`drop_tail_samples` zeros samples 10-17; `amplitude_only` retains only log-amplitude and positive-area summaries; "
            "`waveform_shuffled_control` preserves marginal waveform-sample distributions but breaks event-local waveform shape. "
            "All intervals are 95% run-bootstrap intervals over held-out Sample IV runs.\n\n"
            + atom_metrics[
                [
                    "atom_mode",
                    "model_family",
                    ...
                    "robust_ci_high_ns",
                    "full_rms_ns",
                    "tail_fraction_abs_gt_5ns",
                ]
            ].sort_values("robust_width_ns").to_markdown(index=False)
            + "\n\n"
            "Ablation interpretation: if dropping leading-edge samples or shuffling waveform samples materially widens the residuals relative to `full_features`, "
            "the narrow transfer is waveform-shape dependent rather than only an amplitude-channel artifact. If `amplitude_only` is comparable to the full model, "
            "the dominant atom is support/amplitude matching rather than learned local pulse shape. The shuffled control is the direct leakage guard because it keeps one-dimensional sample marginals but destroys event-local waveform timing structure.\n"
        )
