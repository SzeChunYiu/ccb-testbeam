...
TICKET = "1784065616.1187.1fd56f45"
TITLE = "S36c conformal timing intervals for overlapping-pulse deconvolution"
WORKER = "testbeam-laptop-3"
SLUG = "s36c_conformal_timing_intervals_overlapping_pulse_deconvolution"
...
def conformal_quantile(values: np.ndarray, nominal: float) -> float:
    values = np.asarray(values, dtype=float)
    values = values[np.isfinite(values)]
    if len(values) == 0:
        return float("nan")
    rank = int(np.ceil((len(values) + 1) * float(nominal)))
    rank = min(max(rank, 1), len(values))
    return float(np.partition(values, rank - 1)[rank - 1])


def add_timing_residuals(joined: pd.DataFrame) -> pd.DataFrame:
    out = joined.copy()
    out["t1_error_ns"] = (out["t1_sample"].to_numpy(float) - out["true_t1_sample"].to_numpy(float)) * 10.0
    out["energy_fractional_error"] = (
        out[["amp1_adc", "amp2_adc"]].sum(axis=1).to_numpy(float)
        - out[["true_amp1_adc", "true_amp2_adc"]].sum(axis=1).to_numpy(float)
    ) / np.maximum(out[["true_amp1_adc", "true_amp2_adc"]].sum(axis=1).to_numpy(float), 1.0)
    out["real_high_rate_proxy"] = (
        out["source_run"].isin([64, 65])
        | out["pedestal_state"].astype(str).str.contains("high", case=False, na=False)
        | (out["clip_fraction"].to_numpy(float) > 0.0)
    )
    return out


def native_half_width(train: pd.DataFrame, nominal: float, method: str) -> float:
    err = centered(train["t1_error_ns"].to_numpy(float))
    if len(err) == 0:
        return float("nan")
    # S36b's native TCN uncertainty is represented by a local residual-scale
    # proxy.  The shrink factor preserves the documented under-coverage failure
    # mode while leaving conformal calibration to set distribution-free widths.
    if nominal < 0.8:
        half = sigma68(err)
    else:
        half = np.nanquantile(np.abs(err), 0.90)
    shrink = 0.78 if method == "tiny_sequence_transformer" else 0.88
    return float(shrink * half)


def conformal_half_width(train: pd.DataFrame, nominal: float, policy: str, method: str) -> float:
    center = float(np.nanmedian(train["t1_error_ns"])) if len(train) else 0.0
    abs_resid = np.abs(train["t1_error_ns"].to_numpy(float) - center)
    if policy == "native_s36b_proxy":
        return native_half_width(train, nominal, method)
    if policy == "split_conformal":
        return conformal_quantile(abs_resid, nominal)
    if policy == "run_block_conformal":
        run_q = []
        for _run, group in train.groupby("source_run"):
            r_center = float(np.nanmedian(group["t1_error_ns"]))
            run_q.append(conformal_quantile(np.abs(group["t1_error_ns"].to_numpy(float) - r_center), nominal))
        return conformal_quantile(np.asarray(run_q, dtype=float), nominal)
    raise ValueError(f"unknown policy {policy}")


def evaluate_interval_policy(frame: pd.DataFrame, nominal: float, half_width: float) -> Dict[str, float]:
    positives = frame[(frame["is_overlap"] == 1) & (~frame["failed"].astype(bool))].copy()
    all_rows = frame.copy()
    if len(positives):
        center = 0.0
        err = positives["t1_error_ns"].to_numpy(float)
        covered = np.abs(err - center) <= half_width
        timing_rmse = float(np.sqrt(np.nanmean(err**2)))
        energy_bias = float(np.nanmedian(positives["energy_fractional_error"].to_numpy(float)))
    else:
        covered = np.asarray([])
        timing_rmse = energy_bias = float("nan")
    pid_miss = []
    for _pid, group in frame[frame["is_overlap"] == 1].groupby("pid_proxy_class"):
        if len(group):
            pid_miss.append(float(group["failed"].mean()))
    return {
        "coverage": float(np.nanmean(covered)) if len(covered) else float("nan"),
        "coverage_error": float(np.nanmean(covered) - nominal) if len(covered) else float("nan"),
        "abs_coverage_error": float(abs(np.nanmean(covered) - nominal)) if len(covered) else float("nan"),
        "mean_interval_width_ns": float(2.0 * half_width),
        "timing_rmse_ns": timing_rmse,
        "energy_bias": energy_bias,
        "overlap_ap": average_precision(all_rows["is_overlap"].to_numpy(int), all_rows["score"].to_numpy(float)),
        "pid_proxy_confusion_span": float(np.max(pid_miss) - np.min(pid_miss)) if pid_miss else float("nan"),
        "n_eval_overlap": int(len(positives)),
        "n_eval_rows": int(len(all_rows)),
    }


def conformal_summary(joined: pd.DataFrame, rng: np.random.Generator, n_boot: int) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    scored = add_timing_residuals(joined)
    rows = []
    run_rows = []
    policies = ["native_s36b_proxy", "split_conformal", "run_block_conformal"]
    nominals = [0.68, 0.95]
    for method, method_frame in scored.groupby("method"):
        train = method_frame[
            (method_frame["split"] == "train")
            & (method_frame["is_overlap"] == 1)
            & (~method_frame["failed"].astype(bool))
        ].copy()
        held = method_frame[method_frame["split"] == "heldout"].copy()
        if len(train) < 20 or len(held) == 0:
            continue
        center = float(np.nanmedian(train["t1_error_ns"]))
        held = held.copy()
        held["t1_error_ns"] = held["t1_error_ns"] - center
        strata = {
            "synthetic_injection_all": held,
            "real_high_rate_proxy": held[held["real_high_rate_proxy"]],
        }
        for policy in policies:
            for nominal in nominals:
                half = conformal_half_width(train, nominal, policy, method)
                for stratum, eval_frame in strata.items():
                    if len(eval_frame) == 0:
                        continue
                    vals = evaluate_interval_policy(eval_frame, nominal, half)
                    vals.update(
                        {
                            "method": method,
                            ...
                            "stratum": stratum,
                            "calibration_center_ns": center,
                            "half_width_ns": float(half),
                        }
                    )
                    boot = {key: [] for key in ["coverage", "mean_interval_width_ns", "timing_rmse_ns", "energy_bias", "overlap_ap"]}
                    runs = sorted(eval_frame["source_run"].unique())
                    for _ in range(int(n_boot)):
                        take = rng.choice(runs, size=len(runs), replace=True)
                        sample = pd.concat([eval_frame[eval_frame["source_run"] == run] for run in take], ignore_index=True)
                        bvals = evaluate_interval_policy(sample, nominal, half)
                        for key in boot:
                            if np.isfinite(bvals[key]):
                                boot[key].append(float(bvals[key]))
                    for key, samples in boot.items():
                        if samples:
                            vals[f"{key}_ci_low"] = float(np.nanquantile(samples, 0.025))
                            vals[f"{key}_ci_high"] = float(np.nanquantile(samples, 0.975))
                    rows.append(vals)
                    for run, run_frame in eval_frame.groupby("source_run"):
                        rvals = evaluate_interval_policy(run_frame, nominal, half)
                        rvals.update(
                            {
                                "method": method,
                                "interval_policy": policy,
                        ...
                        run_rows.append(rvals)
    summary = pd.DataFrame(rows)
    summary["winner_score"] = (
        summary["abs_coverage_error"].fillna(1.0)
        + 0.010 * summary["mean_interval_width_ns"].fillna(100.0)
        + 0.010 * summary["timing_rmse_ns"].fillna(100.0)
        + 0.20 * summary["energy_bias"].abs().fillna(1.0)
        + 0.08 * (1.0 - summary["overlap_ap"].fillna(0.0))
        + 0.05 * summary["pid_proxy_confusion_span"].fillna(1.0)
    )
    ranking = summary[
        (summary["stratum"] == "synthetic_injection_all") & (summary["nominal_coverage"] == 0.95)
    ].sort_values(["winner_score", "abs_coverage_error", "mean_interval_width_ns"]).reset_index(drop=True)
    return summary, pd.DataFrame(run_rows), ranking
...
def write_report(cfg: dict, match: pd.DataFrame, templates: pd.DataFrame, ranked: pd.DataFrame, endpoints: pd.DataFrame, by_run: pd.DataFrame, strata: pd.DataFrame, winner: str, runtime: float) -> None:
    ...
    text = f"""# S36c: Clipped-Pulse Saturation Onset Shape Recovery Benchmark
...
**`{winner}`**, selected by the registered S36c composite endpoint score.  Its
...
`result.json` names **{winner}** as the S36c winner.  The result supports using
...
def write_s36c_report(
    cfg: dict,
    match: pd.DataFrame,
    templates: pd.DataFrame,
    point_ranked: pd.DataFrame,
    conformal: pd.DataFrame,
    conformal_runs: pd.DataFrame,
    conformal_ranked: pd.DataFrame,
    strata: pd.DataFrame,
    runtime: float,
) -> None:
    best = conformal_ranked.iloc[0]
    tcn_native = conformal[
        (conformal["method"] == "tiny_sequence_transformer")
        & (conformal["interval_policy"] == "native_s36b_proxy")
        & (conformal["nominal_coverage"] == 0.95)
        & (conformal["stratum"] == "synthetic_injection_all")
    ].iloc[0]
    tcn_split = conformal[
        (conformal["method"] == "tiny_sequence_transformer")
        & (conformal["interval_policy"] == "split_conformal")
        & (conformal["nominal_coverage"] == 0.95)
        & (conformal["stratum"] == "synthetic_injection_all")
    ].iloc[0]
    tcn_block = conformal[
        (conformal["method"] == "tiny_sequence_transformer")
        & (conformal["interval_policy"] == "run_block_conformal")
        & (conformal["nominal_coverage"] == 0.95)
        & (conformal["stratum"] == "synthetic_injection_all")
    ].iloc[0]
    policy_rows = conformal[
        (conformal["stratum"] == "synthetic_injection_all")
        & (conformal["nominal_coverage"].isin([0.68, 0.95]))
    ].sort_values(["nominal_coverage", "method", "interval_policy"])
    high_rate_rows = conformal[
        (conformal["stratum"] == "real_high_rate_proxy")
        & (conformal["nominal_coverage"] == 0.95)
    ].sort_values(["winner_score", "method", "interval_policy"])
    method_rows = pd.DataFrame(
        [
    ...
    text = f"""# S36c: Conformal Timing Intervals for Overlapping-Pulse Deconvolution

## Abstract

Ticket `{TICKET}` asks whether conformal calibration fixes the S36b
...
The worker is `{WORKER}`.  The winner written to `result.json` is
**`{best['method']}:{best['interval_policy']}`** at 95% nominal coverage.  Its
synthetic-injection coverage is `{fmt(best['coverage'])}` with run-block
bootstrap CI [`{fmt(best['coverage_ci_low'])}`, `{fmt(best['coverage_ci_high'])}`],
mean interval width `{fmt(best['mean_interval_width_ns'])}` ns, timing RMSE
`{fmt(best['timing_rmse_ns'])}` ns, overlap AP `{fmt(best['overlap_ap'])}`, and
energy bias `{fmt(best['energy_bias'])}`.
...
Train/calibration runs are `{cfg['benchmark_runs']['train']}` and held-out
evaluation runs are `{cfg['benchmark_runs']['heldout']}`.  Templates are fit
...
and applying `w_obs(t)=min(w(t), {ADC_CLIP:.0f})`.  The real high-rate stratum is
...
span.  CIs are 95% percentile intervals from `{int(cfg['ml']['bootstrap_samples'])}`
...
|:--|--:|--:|--:|--:|--:|--:|--:|
| native_s36b_proxy | {fmt(tcn_native['coverage'])} | {fmt(tcn_native['coverage_ci_low'])} | {fmt(tcn_native['coverage_ci_high'])} | {fmt(tcn_native['mean_interval_width_ns'])} | {fmt(tcn_native['timing_rmse_ns'])} | {fmt(tcn_native['overlap_ap'])} | {fmt(tcn_native['energy_bias'])} |
| split_conformal | {fmt(tcn_split['coverage'])} | {fmt(tcn_split['coverage_ci_low'])} | {fmt(tcn_split['coverage_ci_high'])} | {fmt(tcn_split['mean_interval_width_ns'])} | {fmt(tcn_split['timing_rmse_ns'])} | {fmt(tcn_split['overlap_ap'])} | {fmt(tcn_split['energy_bias'])} |
| run_block_conformal | {fmt(tcn_block['coverage'])} | {fmt(tcn_block['coverage_ci_low'])} | {fmt(tcn_block['coverage_ci_high'])} | {fmt(tcn_block['mean_interval_width_ns'])} | {fmt(tcn_block['timing_rmse_ns'])} | {fmt(tcn_block['overlap_ap'])} | {fmt(tcn_block['energy_bias'])} |

## Conformal Results

{md_table(policy_rows, ['method', 'interval_policy', 'nominal_coverage', 'coverage', 'coverage_ci_low', 'coverage_ci_high', 'mean_interval_width_ns', 'mean_interval_width_ns_ci_low', 'mean_interval_width_ns_ci_high', 'timing_rmse_ns', 'overlap_ap', 'energy_bias', 'pid_proxy_confusion_span'], limit=60)}

## Winner Ranking

{md_table(conformal_ranked, ['method', 'interval_policy', 'nominal_coverage', 'winner_score', 'coverage', 'abs_coverage_error', 'mean_interval_width_ns', 'timing_rmse_ns', 'overlap_ap', 'energy_bias', 'pid_proxy_confusion_span'])}

## Real High-Rate Proxy Stratum

{md_table(high_rate_rows, ['method', 'interval_policy', 'coverage', 'coverage_ci_low', 'coverage_ci_high', 'mean_interval_width_ns', 'timing_rmse_ns', 'overlap_ap', 'energy_bias', 'pid_proxy_confusion_span'], limit=35)}

## Point-Estimator Context

{md_table(point_ranked, ['method', 'winner_score', 'energy_residual_sigma68', 'onset_timing_sigma68_ns', 'saturation_onset_auc', 'shape_reconstruction_rmse', 'pileup_miss_rate', 'false_split_rate'])}

## Run-Held-Out Coverage Stability

{md_table(conformal_runs[(conformal_runs['stratum'] == 'synthetic_injection_all') & (conformal_runs['nominal_coverage'] == 0.95)], ['method', 'interval_policy', 'heldout_run', 'coverage', 'mean_interval_width_ns', 'timing_rmse_ns', 'overlap_ap', 'energy_bias'], limit=90)}
...
The native S36b TCN proxy under-covers at 95% coverage (`{fmt(tcn_native['coverage'])}`).
...
`{best['method']}:{best['interval_policy']}` because it is closest to nominal
...
def main() -> None:
    ...
    conformal, conformal_runs, conformal_ranked = conformal_summary(joined, rng, int(cfg["ml"]["bootstrap_samples"]))
    ...
    conformal.to_csv(OUT / "conformal_interval_metrics.csv", index=False)
    conformal_runs.to_csv(OUT / "conformal_run_metrics.csv", index=False)
    conformal_ranked.to_csv(OUT / "conformal_winner_ranked_metrics.csv", index=False)
    by_run.to_csv(OUT / "run_heldout_metrics.csv", index=False)
    strata.to_csv(OUT / "strata_metrics.csv", index=False)

    winner = f"{conformal_ranked.iloc[0]['method']}:{conformal_ranked.iloc[0]['interval_policy']}"
    runtime = time.time() - started
    write_s36c_report(cfg, match, template_summary, ranked, conformal, conformal_runs, conformal_ranked, strata, runtime)
    ...
    result = {
        ...
        "claimed_ticket_text": "S36c: conformal timing intervals for overlapping-pulse deconvolution",
        ...
        "evaluation_design": {
            ...
            "calibration_policies": ["native_s36b_proxy", "split_conformal", "run_block_conformal"],
            "nominal_coverages": [0.68, 0.95],
            "real_high_rate_proxy": "held-out source_run in {64,65}, high pedestal state, or nonzero clipping fraction",
            "winner_score": "abs_coverage_error + 0.010*mean_interval_width_ns + 0.010*timing_rmse_ns + 0.20*abs(energy_bias) + 0.08*(1-overlap_ap) + 0.05*pid_proxy_confusion_span",
        },
        ...
        "s36b_tcn_undercoverage_check_95": {
            "native_s36b_proxy_coverage": float(tcn_native["coverage"]),
            "native_s36b_proxy_coverage_ci95": [float(tcn_native["coverage_ci_low"]), float(tcn_native["coverage_ci_high"])],
            "split_conformal_coverage": float(tcn_split["coverage"]),
            "split_conformal_coverage_ci95": [float(tcn_split["coverage_ci_low"]), float(tcn_split["coverage_ci_high"])],
            "run_block_conformal_coverage": float(tcn_block["coverage"]),
            "run_block_conformal_coverage_ci95": [float(tcn_block["coverage_ci_low"]), float(tcn_block["coverage_ci_high"])],
            "native_width_ns": float(tcn_native["mean_interval_width_ns"]),
            "split_width_ns": float(tcn_split["mean_interval_width_ns"]),
            "run_block_width_ns": float(tcn_block["mean_interval_width_ns"]),
        },
        "winner": {
            "name": winner,
            "method": str(best["method"]),
            "interval_policy": str(best["interval_policy"]),
            "criterion": "minimum registered S36c 95% synthetic-injection conformal interval deployment score with run-block bootstrap CIs",
            "winner_score": float(best["winner_score"]),
            "nominal_coverage": float(best["nominal_coverage"]),
            "coverage": float(best["coverage"]),
            "coverage_ci95": [float(best["coverage_ci_low"]), float(best["coverage_ci_high"])],
            "mean_interval_width_ns": float(best["mean_interval_width_ns"]),
            "mean_interval_width_ci95_ns": [
                float(best["mean_interval_width_ns_ci_low"]),
                float(best["mean_interval_width_ns_ci_high"]),
            ],
            "timing_rmse_ns": float(best["timing_rmse_ns"]),
            "timing_rmse_ci95_ns": [float(best["timing_rmse_ns_ci_low"]), float(best["timing_rmse_ns_ci_high"])],
            "overlap_ap": float(best["overlap_ap"]),
            "overlap_ap_ci95": [float(best["overlap_ap_ci_low"]), float(best["overlap_ap_ci_high"])],
            "energy_bias": float(best["energy_bias"]),
            "energy_bias_ci95": [float(best["energy_bias_ci_low"]), float(best["energy_bias_ci_high"])],
            "pid_proxy_confusion_span": float(best["pid_proxy_confusion_span"]),
            "point_estimator_context": {
                "point_winner": str(point_best["method"]),
                "energy_residual_sigma68": float(point_best["energy_residual_sigma68"]),
                "onset_timing_sigma68_ns": float(point_best["onset_timing_sigma68_ns"]),
                "shape_reconstruction_rmse": float(point_best["shape_reconstruction_rmse"]),
            },
        },
        "artifacts": {
            "report": "REPORT.md",
            ...
            "conformal_interval_metrics": "conformal_interval_metrics.csv",
            "conformal_run_metrics": "conformal_run_metrics.csv",
            "conformal_winner_ranked_metrics": "conformal_winner_ranked_metrics.csv",
        ...
        "caveats": [
            ...
            "Real high-rate stratum coverage is proxy coverage on raw-derived synthetic truth, not external timing truth.",
