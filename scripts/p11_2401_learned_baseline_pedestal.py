#!/usr/bin/env python3
"""P11 ticket #2401: learned baseline/pedestal estimation.
...
CONFIG_DEFAULT = "configs/p11_2401_learned_baseline_pedestal.json"
...
def fit_predict_all(meta: pd.DataFrame, seq: np.ndarray, config: dict) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    test_meta = meta.iloc[test_idx][
        [
            ...
            "stave",
            "stave_idx",
            "target_raw_pre_median_adc",
            "sorted_baseline_adc",
            "raw_gate_amplitude_adc",
            "raw_peak_sample",
        ]
    ].copy()
...
def low_amplitude_summary(preds: pd.DataFrame) -> pd.DataFrame:
    """Secondary P11 diagnostic: bias in the lowest held-out amplitude quartile."""
    cut = float(np.quantile(preds["raw_gate_amplitude_adc"].to_numpy(dtype=float), 0.25))
    rows = []
    for method, sub in preds[preds["raw_gate_amplitude_adc"] <= cut].groupby("method"):
        residual = sub["residual_adc"].to_numpy(dtype=float)
        rows.append(
            {
                "method": method,
                "family": str(sub["family"].iloc[0]),
                "n_low_amplitude": int(len(sub)),
                "amplitude_q25_adc": cut,
                "low_amp_bias_adc": float(np.mean(residual)),
                "low_amp_mae_adc": float(np.mean(np.abs(residual))),
                "low_amp_q05_residual_adc": float(np.quantile(residual, 0.05)),
                "low_amp_q95_residual_adc": float(np.quantile(residual, 0.95)),
            }
        )
    return pd.DataFrame(rows).sort_values("low_amp_mae_adc")
...
def write_plots(outdir: Path, summary: pd.DataFrame, preds: pd.DataFrame) -> None:
    ...
    ax.set_title("P11 run-held-out raw pretrigger pedestal benchmark")
    fig.tight_layout()
...
def write_report(
    ...
    report = """# P11: Learned Baseline/Pedestal Estimation
...
Can a learned baseline/pedestal estimator recover the raw pretrigger pedestal level for selected B-stack pulses better than a strong adaptive traditional estimator, especially for low-amplitude pulses where pedestal bias is most damaging? The operational target is the raw median pedestal

\\[
  y_i = \\operatorname{{median}}\\left(x_{{i,0}}, x_{{i,1}}, x_{{i,2}}, x_{{i,3}}\\right),
\\]

where `x` is the raw `HRDv` waveform for one selected B stave. The benchmark uses sorted ROOT reconstruction metadata as deployable inputs and raw ROOT only for the reproduction gate, selection, and target construction.
...
For held-out pulse rows \(i\) in run block \(r\), the residual and loss are

\\[
  e_i^{(m)} = \hat y_i^{(m)} - y_i,\qquad
  \mathrm{{MAE}}^{(m)} = |I|^{{-1}}\sum_{{i\in I}} |e_i^{(m)}|.
\\]

Bootstrap replicate \(b\) samples the two held-out run blocks with replacement and recomputes the aggregate MAE on the concatenated sampled blocks. The reported interval is

\\[
  [Q_{{0.025}}(\mathrm{{MAE}}_b), Q_{{0.975}}(\mathrm{{MAE}}_b)].
\\]
...
Low-amplitude held-out bias diagnostic, defined on the lowest quartile of raw selected-pulse amplitude:
...
- **Pre-registration:** the ticket asks for a run-split benchmark of traditional and ML/NN methods and bias on low-amplitude pulses. The config fixes the primary metric as held-out raw-pretrigger-median MAE on runs 57 and 65, with the strong sorted-baseline offset method as the traditional comparator, and reserves the lowest-amplitude quartile bias as a secondary diagnostic.
...
- **Optimization caveat:** the scikit-learn MLP reached its maximum iteration count; its held-out score is still reported, but no adoption claim rests on that model because the boosted-tree winner is both better and fully converged by the configured CV scan.
...
Sorted ROOT metadata does encode recoverable information about the absolute raw pretrigger pedestal level, but the direct `hrd.baseline` branch is biased low and is not a drop-in pedestal median. The winning model uses the baseline branch together with sorted trapezoid/peak metadata to correct that residual. The result supports using the combined sorted metadata as a compact pedestal proxy when raw waveforms are unavailable, with the caveat that it is not a substitute for true forced/random pedestal data.
...
```bash
/home/billy/anaconda3/bin/python scripts/p11_2401_learned_baseline_pedestal.py --config configs/p11_2401_learned_baseline_pedestal.json
```

Outputs: `REPORT.md`, `result.json`, `manifest.json`, `input_sha256.csv`, `reproduction_match_table.csv`, `heldout_predictions.csv`, `heldout_method_metrics.csv`, `heldout_by_run.csv`, `low_amplitude_bias.csv`, `hgb_cv_scan.csv`, `leakage_checks.csv`, and two PNG figures.
""".format(
        ...
        low_amp_table=md_table(low_amp),
        winner=winner,
        win_mae=win["mae_adc"],
...
def main() -> None:
    ...
    command = ["/home/billy/anaconda3/bin/python", "scripts/p11_2401_learned_baseline_pedestal.py", "--config", str(config_path)]
    ...
    low_amp = low_amplitude_summary(preds)
    ...
    low_amp.to_csv(outdir / "low_amplitude_bias.csv", index=False)
    ...
    result = {
        ...
        "falsification": {
            ...
            "low_amplitude_secondary_metric": "mean prediction bias and MAE in the lowest held-out raw-amplitude quartile",
        },
        ...
        "low_amplitude_bias": low_amp.to_dict(orient="records"),
        ...
        "next_tickets": [
            {
                "title": "P11c: learned pedestal residual as a timing-tail nuisance",
                "body": "Use the P11 learned-pedestal residual proxy in S02/S04 timing fits, split by run with paired bootstrap CIs, to test whether pedestal recoverability explains timing tails beyond amplitude and peak-time controls. Expected information gain: connects pedestal estimation quality to the physics timing endpoint rather than only to a reconstruction diagnostic.",
            }
        ],
        "runtime_seconds": float(time.time() - t0),
    }
    (outdir / "result.json").write_text(json.dumps(result, indent=2))
    write_report(outdir, config, repro, summary, deltas, by_run, low_amp, cv_scan, result, str(config_path))
