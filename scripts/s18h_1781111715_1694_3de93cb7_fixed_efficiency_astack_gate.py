#!/usr/bin/env python3
"""S18h: decompose A-stack percentile68 drift with fixed-efficiency gates."""
...
os.environ.setdefault("MPLCONFIGDIR", "reports/1781111715.1694.3de93cb7__s18h_fixed_efficiency_astack_gate/.mplconfig")
...
def raw_batches(path: Path, step_size: int = 500000) -> Iterable[dict]:
...
def load_pair_table(
    ...
    for run in runs:
        ...
        for batch in raw_batches(path):
            ...
            score = np.minimum(amplitude[:, 0], amplitude[:, 1])
            if gate_mode == "fixed_efficiency":
                if target_efficiency is None:
                    raise ValueError("fixed_efficiency gate requires target_efficiency")
                n_select = max(1, int(round(float(target_efficiency) * len(score))))
                n_select = min(n_select, len(score))
                threshold = float(np.partition(score, len(score) - n_select)[len(score) - n_select])
            ...
            frame["gate_mode"] = gate_mode
            frame["gate_threshold_adc"] = threshold
            frame["target_efficiency"] = np.nan if target_efficiency is None else float(target_efficiency)
            rows.append(frame)
    return pd.concat(rows, ignore_index=True) if rows else pd.DataFrame()


def fixed_efficiency_target(config: dict, cfd_fraction: float) -> dict:
    runs = config["fixed_efficiency_reference"]["runs"]
    cut = float(config["fixed_efficiency_reference"]["amplitude_cut_adc"])
    ...
    for run in runs:
        for batch in raw_batches(root_path(config, int(run))):
            waveforms = np.stack(batch["HRDv"]).astype(float).reshape(-1, 8, int(config["samples_per_channel"]))
            _, amplitude, _, _, _, _, _ = cfd_times(waveforms[:, channels, :], baseline_samples, float(cfd_fraction))
            total += int(len(amplitude))
            selected += int(((amplitude[:, 0] > cut) & (amplitude[:, 1] > cut)).sum())
    eff = selected / total if total else float("nan")
    return {"reference_runs": [int(r) for r in runs], "reference_cut_adc": cut, "selected_pairs": selected, "events_total": total, "target_efficiency": eff}
...
def gate_label(cfd_fraction: float, amplitude_cut_adc: float, gate_mode: str = "fixed_adc") -> str:
    suffix = "adc" if gate_mode == "fixed_adc" else "eff"
    return f"{suffix}_cfd{float(cfd_fraction):.2f}_cut{float(amplitude_cut_adc):.0f}"
...
def add_gate_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "pool" not in out.columns or out.empty:
        return out
    parts = out["pool"].str.extract(r"(?P<gate_mode>adc|eff)_cfd(?P<cfd_fraction>[0-9.]+)_cut(?P<amplitude_cut_adc>[0-9.]+)")
    out["gate_mode"] = parts["gate_mode"].map({"adc": "fixed_adc", "eff": "fixed_efficiency"})
...
def write_figures(out_dir: Path, metrics: pd.DataFrame, raw_metrics: pd.DataFrame, primary_label: str, all_pred: pd.DataFrame) -> None:
    ...
    ax.set_title("A-stack raw percentile68 fixed-efficiency decomposition")
...
def write_report(
    ...
    fixed_eff = result["fixed_efficiency_target"]
    gate_decomposition = (
        raw_metrics.pivot_table(
            index=["cfd_fraction", "amplitude_cut_adc"],
            columns="gate_mode",
            values=["n_pairs", "robust_width_ns", "robust_ci_low_ns", "robust_ci_high_ns"],
            aggfunc="first",
        )
        .reset_index()
    )
    gate_decomposition.columns = [
        "_".join(str(part) for part in col if str(part))
        if isinstance(col, tuple)
        else str(col)
        for col in gate_decomposition.columns
    ]
    if {"robust_width_ns_fixed_adc", "robust_width_ns_fixed_efficiency"}.issubset(gate_decomposition.columns):
        gate_decomposition["support_normalized_minus_adc_width_ns"] = (
            gate_decomposition["robust_width_ns_fixed_efficiency"] - gate_decomposition["robust_width_ns_fixed_adc"]
        )
    report = f"""# S18h: A-stack fixed-efficiency gate decomposition
...
This study asks whether A-stack `percentile68_ns` drift follows low-amplitude support loss or CFD interpolation noise when every run is compared at fixed A1/A3 selection efficiency instead of fixed ADC thresholds. For every gate in the Cartesian grid CFD `{config['stress_cfd_fractions']}` by amplitude reference `{config['stress_amplitude_cuts_adc']}` ADC, raw A1-A3 residuals are reconstructed directly from ROOT under both fixed-ADC and run-wise fixed-efficiency selection. The selected pairs are then corrected with a strong constrained traditional timewalk model and five learned alternatives: ridge, gradient-boosted trees, MLP, 1D-CNN, and a new gated residual CNN.

The fixed-efficiency target is **{fixed_eff['target_efficiency']:.6f}**, derived from **{fixed_eff['selected_pairs']}** selected A1/A3 pairs in **{fixed_eff['events_total']}** reference-run events at the conventional 1000 ADC threshold. At the preregistered fixed-efficiency CFD20/cut1000-equivalent gate, the winner is **{winner['method']}**, with held-out width **{winner['robust_width_ns']:.3f} ns** and run-bootstrap CI **[{winner['robust_ci_low_ns']:.3f}, {winner['robust_ci_high_ns']:.3f}] ns**. The uncorrected fixed-efficiency A-stack width is **{raw_standard['robust_width_ns']:.3f} ns** with CI **[{raw_standard['robust_ci_low_ns']:.3f}, {raw_standard['robust_ci_high_ns']:.3f}] ns**.
...
{raw_metrics[['gate_mode', 'cfd_fraction', 'amplitude_cut_adc', 'n_pairs', 'robust_width_ns', 'robust_ci_low_ns', 'robust_ci_high_ns', 'full_rms_ns']].sort_values(['gate_mode', 'amplitude_cut_adc', 'cfd_fraction']).to_markdown(index=False)}

Fixed-efficiency minus fixed-ADC raw width decomposition:

{gate_decomposition.to_markdown(index=False)}

Best method at each gate:

{best_by_gate[['gate_mode', 'cfd_fraction', 'amplitude_cut_adc', 'method', 'n_pairs', 'robust_width_ns', 'robust_ci_low_ns', 'robust_ci_high_ns']].sort_values(['gate_mode', 'amplitude_cut_adc', 'cfd_fraction']).to_markdown(index=False)}
...
{deltas[['gate_mode', 'cfd_fraction', 'amplitude_cut_adc', 'comparison', 'ci_low_ns', 'ci_high_ns', 'p_value']].sort_values(['gate_mode', 'amplitude_cut_adc', 'cfd_fraction', 'comparison']).to_markdown(index=False)}
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/s18h_1781111715_1694_3de93cb7_fixed_efficiency_astack_gate.json"))
    ...
    primary_mode = config["primary_gate"].get("gate_mode", "fixed_efficiency")
    primary_label = gate_label(primary_cfd, primary_cut, primary_mode)
    fixed_eff = fixed_efficiency_target(config, primary_cfd)
    (out_dir / "fixed_efficiency_target.json").write_text(json.dumps(json_safe(fixed_eff), indent=2) + "\n", encoding="utf-8")
    run64_train = load_pair_table(config, config["sample_iv_calib_runs"], "sample_iv_calib", primary_cfd, primary_cut, "fixed_adc")
    sample_iv_test = load_pair_table(config, config["sample_iv_analysis_runs"], "sample_iv_analysis", primary_cfd, primary_cut, "fixed_adc")

    repro = reproduction_table(config, run64_train, sample_iv_test)
    repro.to_csv(out_dir / "reproduction_match_table.csv", index=False)
    if not bool(repro["pass"].all()):
        raise RuntimeError("S18h reproduction gate failed")
    ...
    for gate_mode in config["gate_modes"]:
        for cfd_fraction in config["stress_cfd_fractions"]:
            for amplitude_cut_adc in config["stress_amplitude_cuts_adc"]:
                label = gate_label(float(cfd_fraction), float(amplitude_cut_adc), gate_mode)
                target = fixed_eff["target_efficiency"] if gate_mode == "fixed_efficiency" else None
                train = load_pair_table(config, config["train_runs"], "sample_iii_train", float(cfd_fraction), float(amplitude_cut_adc), gate_mode, target)
    ...
    leakage_train = load_pair_table(config, config["train_runs"], "sample_iii_train", primary_cfd, primary_cut, primary_mode, fixed_eff["target_efficiency"])
    ...
    result = {
        ...
        "primary_gate": {"cfd_fraction": primary_cfd, "amplitude_cut_adc": primary_cut, "gate_mode": primary_mode, "target_efficiency": fixed_eff["target_efficiency"]},
        "fixed_efficiency_target": fixed_eff,
        ...
        "primary_metric": "Fixed-efficiency CFD20 Sample IV A1-A3 percentile68 residual width, bootstrap over held-out runs",
        ...
        "gate_grid": {
            ...
            "gate_modes": list(config["gate_modes"]),
        },
