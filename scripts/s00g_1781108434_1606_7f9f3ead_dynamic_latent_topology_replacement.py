#!/usr/bin/env python3
"""S00g dynamic-only joined-latent topology replacement study.

The script rescans raw B-stack ROOT first, reproduces the S00/S00a selector
counts exactly, joins the S00e/P02-style latent release to every selected
row, then compares baseline-excursion dynamic-only pulses to exact matched
S00 controls using latent topology in place of the S00f raw multiplicity
topology proxy. The ML panel is deliberately diagnostic: run labels, event
identifiers, current labels, selector amplitudes, and the baseline excursion
cut variable are excluded from learned feature matrices.
"""
...
CONFIG = ROOT / "configs/s00g_1781108434_1606_7f9f3ead_dynamic_latent_topology_replacement.json"
...
def scan_raw(config: dict) -> Tuple[pd.DataFrame, pd.DataFrame, np.ndarray, pd.DataFrame]:
    ...
    return pd.DataFrame(count_rows), features, waves, pd.DataFrame(input_rows)
...
def add_match_bins(features: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    topology_col = str(config.get("match_topology_column", "dynamic_topology"))
    if topology_col not in out.columns:
        raise KeyError(f"configured match topology column is missing: {topology_col}")
    out["match_topology"] = out[topology_col].astype(str)
    out["match_key"] = (
        out["run"].astype(str)
        + "|"
        ...
        + "|"
        + out["match_topology"].astype(str)
    )
    return out


def join_latent_topology(features: pd.DataFrame, config: dict) -> Tuple[pd.DataFrame, pd.DataFrame]:
    artifact = ROOT / config["latent_artifact"]
    ...
    with np.load(artifact) as data:
        ...
        latent = pd.DataFrame(
            {
                "run": np.asarray(data["run"], dtype=np.int64),
                "event_index": np.asarray(data["event_index"], dtype=np.int64),
                "stave_index": np.asarray(data["stave_index"], dtype=np.int64),
                "dynamic_only": np.asarray(data["dynamic_only"], dtype=np.int8),
            }
        )
    if len(latent) != len(z):
        raise RuntimeError("latent row count does not match z embedding count")

    k = int(config["latent_cluster_count"])
    km = MiniBatchKMeans(n_clusters=k, random_state=int(config["random_seed"]), batch_size=16384, n_init=10)
    latent["latent_cluster"] = km.fit_predict(z).astype(int)
    latent["latent_topology"] = latent["latent_cluster"].map(lambda value: f"p02_latent_c{int(value):02d}")
    for dim in range(z.shape[1]):
        latent[f"latent_z{dim}"] = z[:, dim]

    key = ["run", "event_index", "stave_index", "dynamic_only"]
    if latent.duplicated(key).any():
        raise RuntimeError("latent artifact has duplicate join keys")
    out = features.merge(latent, on=key, how="left", validate="one_to_one")
    missing = int(out["latent_topology"].isna().sum())
    if missing:
        raise RuntimeError(f"latent topology join missed {missing} selected rows")
    summary = (
        out.groupby(["latent_topology", "latent_cluster", "dynamic_only"], as_index=False)
        .agg(rows=("run", "size"), runs=("run", "nunique"), median_dynamic_amp_adc=("dynamic_amp_adc", "median"), raw_dynamic_topologies=("dynamic_topology", "nunique"))
        .sort_values(["latent_topology", "dynamic_only"])
    )
    return out, summary
...
def make_matched_support(features: pd.DataFrame, config: dict) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    dyn = frame[(frame["dynamic_only"] == 1) & (frame["baseline_excursion_flag"] == 1)].copy()
    ctrl = frame[(frame["s00_selected"] == 1)].copy()
    ctrl_groups = {key: group.index.to_numpy(dtype=int) for key, group in ctrl.groupby("match_key", sort=False)}
    dyn_parts = []
    ctrl_parts = []
    support_rows = []
    for key, group in dyn.groupby("match_key", sort=False):
        cands = ctrl_groups.get(key, np.asarray([], dtype=int))
        n = min(len(group), len(cands))
        support_rows.append({"match_key": key, "dynamic_n": int(len(group)), "control_n": int(len(cands)), "matched_n": int(n)})
        if n <= 0:
            continue
        dyn_ids = group.index.to_numpy()
        ctrl_ids = cands
        if len(dyn_ids) > n:
            dyn_ids = rng.choice(dyn_ids, size=n, replace=False)
        if len(ctrl_ids) > n:
            ctrl_ids = rng.choice(ctrl_ids, size=n, replace=False)
        dyn_parts.append(frame.loc[dyn_ids].assign(population="dynamic_baseline_excursion"))
        ctrl_parts.append(frame.loc[ctrl_ids].assign(population="matched_s00_control"))
    matched = pd.concat(dyn_parts + ctrl_parts, ignore_index=True) if dyn_parts else pd.DataFrame()
    return matched, pd.DataFrame(support_rows).sort_values(["matched_n", "dynamic_n"], ascending=False)
...
def paired_delta_metrics(matched: pd.DataFrame, config: dict) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    populations = ["dynamic_baseline_excursion", "matched_s00_control"]
    metrics = [
    ...
    for name, kind, col in metrics:
        dyn_v = val_from_arrays(runs, "dynamic_baseline_excursion", kind, col)
        ctrl_v = val_from_arrays(runs, "matched_s00_control", kind, col)
        obs_delta = dyn_v - ctrl_v
        boot = []
        for _ in range(int(config["bootstrap_samples"])):
            sample_runs = rng.choice(runs, size=len(runs), replace=True)
            boot.append(
                val_from_arrays(sample_runs, "dynamic_baseline_excursion", kind, col)
                - val_from_arrays(sample_runs, "matched_s00_control", kind, col)
            )
        lo, hi = ci(boot)
        rows.append({"metric": name, "dynamic_value": dyn_v, "matched_control_value": ctrl_v, "delta": obs_delta, "ci_low": lo, "ci_high": hi, "unit": "fraction" if kind == "mean" else "ADC or ADC-samples"})

    strata = (
        matched.groupby(["population", "current_group", "match_topology"], as_index=False)
...
def decision_from_metrics(metrics: pd.DataFrame, support: pd.DataFrame, matched: pd.DataFrame) -> Dict[str, object]:
    ...
    dynamic_total = int(support["dynamic_n"].sum())
    matched_total = int(support["matched_n"].sum())
    coverage = matched_total / max(dynamic_total, 1)
    pileup_supported = bool(sec.delta > 0 and sec.ci_low > 0 and abs(charge.delta) < 0.25 * max(abs(charge.matched_control_value), 1.0))
    selector_artifact = bool((baseline.delta > 0 and baseline.ci_low > 0) and (charge.delta < 0 and charge.ci_high < 0))
    verdict = "pileup_support_region" if pileup_supported and not selector_artifact else "selector_artifact_region"
    return {
        "verdict": verdict,
        "matched_coverage": coverage,
        "matched_rows": int(len(matched)),
        "matched_dynamic_rows": int((matched["population"] == "dynamic_baseline_excursion").sum()),
        "matched_control_rows": int((matched["population"] == "matched_s00_control").sum()),
        "dynamic_baseline_excursion_total": dynamic_total,
...
def cnn_scores(x_train: np.ndarray, y_train: np.ndarray, x_test: np.ndarray, config: dict) -> np.ndarray:
    ...
    try:
        ...
        filters = [
            ...
            np.asarray([1.0, -2.0, 1.0], dtype=float),
            np.asarray([-1.0, 0.0, 1.0], dtype=float),
            np.asarray([0.25, 0.5, 0.25], dtype=float),
            np.asarray([-0.5, 1.0, -0.5], dtype=float),
        ]

        def conv_features(x: np.ndarray) -> np.ndarray:
            cols = []
            for filt in filters:
                conv = np.apply_along_axis(lambda row: np.convolve(row, filt, mode="same"), 1, x)
                cols.extend([conv.max(axis=1), conv.min(axis=1), conv.mean(axis=1), conv.std(axis=1)])
            return np.column_stack(cols)

        model = HistGradientBoostingClassifier(max_iter=120, learning_rate=0.05, max_leaf_nodes=9, l2_regularization=0.05, random_state=seed)
        model.fit(conv_features(x_train), y_train)
        return model.predict_proba(conv_features(x_test))[:, 1]
...
def model_benchmark(matched_in: pd.DataFrame, waves: np.ndarray, config: dict) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    y = (sample["population"] == "dynamic_baseline_excursion").astype(int).to_numpy()
    ...
    models = {
        ...
        "gradient_boosted_trees": ("model", HistGradientBoostingClassifier(max_iter=160, learning_rate=0.06, max_leaf_nodes=15, l2_regularization=0.05, random_state=int(config["random_seed"]))),
        ...
        "new_shape_residual_fusion": ("fusion", ExtraTreesClassifier(n_estimators=240, min_samples_leaf=4, class_weight="balanced", random_state=int(config["random_seed"]), n_jobs=-1)),
        "shuffled_label_fusion_control": ("fusion_shuffle", ExtraTreesClassifier(n_estimators=160, min_samples_leaf=4, class_weight="balanced", random_state=int(config["random_seed"]) + 1, n_jobs=-1)),
    }

    score_rows = []
    pred_cols = ["run", "event_index", "stave", "population", "current_group", "dynamic_topology", "match_topology", "latent_cluster"]
    pred = sample.loc[test_mask, [col for col in pred_cols if col in sample.columns]].copy()
...
def write_report(out_dir: Path, config: dict, reproduction: pd.DataFrame, latent_summary: pd.DataFrame, support: pd.DataFrame, metrics: pd.DataFrame, strata: pd.DataFrame, bench: pd.DataFrame, leakage: pd.DataFrame, decision: Dict[str, object], result: Dict[str, object]) -> None:
    winner = result["winner"]["method"]
    report = f"""# S00g: dynamic-only topology proxy replacement with joined P02 latent labels
...
- **Latent artifact:** `{config['latent_artifact']}`

## Abstract

This study reruns the S00f dynamic-only baseline-excursion analysis after replacing the raw multiplicity topology proxy with a joined P02/S00e latent topology label for every selected row. The raw ROOT reproduction gate returns **{result['reproduction']['dynamic_only']:,}** dynamic-only pulses, exactly matching the frozen S00a/S00d value of 65,636. After exact matching to S00 controls by run, current label, stave, dynamic-amplitude bin, and joined latent topology, the physics-facing verdict is **{decision['verdict']}**. The benchmark winner recorded in `result.json` is **{winner}**.
...
\\[
\\max(x_0,x_1,x_2,x_3)-\\min(x_0,x_1,x_2,x_3) \\ge {config['baseline_excursion_adc']:.0f}\\;{{\\rm ADC}}.
\\]
...
The latent topology is constructed by joining the S00e embedding release on `(run,event_index,stave_index,dynamic_only)` and clustering the four-dimensional latent vector `z` into {config['latent_cluster_count']} unsupervised MiniBatchKMeans labels `p02_latent_cXX`. The old raw `dynamic_topology` remains in the pulse table for diagnostics, but it is not part of the S00g exact matching key. Exact matched coverage is **{decision['matched_coverage']:.3f}**: {decision['matched_dynamic_rows']:,} dynamic baseline-excursion pulses and {decision['matched_control_rows']:,} S00 controls.

Latent topology occupancy:

{markdown_table(latent_summary, ['latent_topology', 'latent_cluster', 'dynamic_only', 'rows', 'runs', 'median_dynamic_amp_adc', 'raw_dynamic_topologies'], max_rows=20)}

Top support strata:

{markdown_table(support, ['match_key', 'dynamic_n', 'control_n', 'matched_n'], max_rows=12)}
...
{markdown_table(metrics, ['metric', 'dynamic_value', 'matched_control_value', 'delta', 'ci_low', 'ci_high', 'unit'])}

Matched strata summary by joined latent topology:

{markdown_table(strata, ['population', 'current_group', 'match_topology', 'n', 'secondary_fraction', 'timing_tail_fraction', 'median_dynamic_amp_adc'], max_rows=14)}

## Model Benchmark

All models use the same train/held-out split by run; held-out runs are `{config['heldout_runs']}`. Learned features exclude run, event number, current label, median amplitude, dynamic amplitude, dynamic-minus-median, and baseline-excursion ADC. The traditional fixed-secondary score is included as a non-learned reference. The ML/NN panel contains ridge, histogram gradient-boosted trees, MLP, 1D-CNN, and a new shape-residual fusion ExtraTrees architecture using train-only PCA waveform coordinates plus non-selector shape summaries. The `cnn_1d` implementation recorded in `result.json` is `{result['cnn_1d_implementation']}`. The target is dynamic baseline-excursion membership versus exact matched S00 controls.
...
def plot_summary(out_dir: Path, counts: pd.DataFrame, metrics: pd.DataFrame, bench: pd.DataFrame) -> None:
    ...
    axes[1].set_ylabel("dynamic - matched control")
    axes[1].set_title("Matched pile-up proxy deltas")
    fig.tight_layout()
    ...
    ax.set_title("Dynamic baseline-excursion discriminator")
    fig.tight_layout()
...
def main() -> None:
    ...
    counts, features, waves, inputs = scan_raw(config)
    features = features.reset_index(drop=True)
    features["source_index"] = np.arange(len(features), dtype=int)
    print("joining S00e/P02 latent topology labels", flush=True)
    features, latent_summary = join_latent_topology(features, config)
    reproduction = reproduction_table(counts, config)
    if not bool(reproduction["pass"].all()):
        raise RuntimeError("raw selector reproduction failed")

    print("matching dynamic baseline-excursion pulses to exact S00 controls", flush=True)
    matched, support = make_matched_support(features, config)
    if matched.empty:
        raise RuntimeError("no exact matched support")
    print(f"matched rows={len(matched)} dynamic={int((matched['population'] == 'dynamic_baseline_excursion').sum())}", flush=True)
    ...
    latent_summary.to_csv(out_dir / "latent_topology_summary.csv", index=False)
    ...
    result = {
        ...
        "latent_artifact": config["latent_artifact"],
        "latent_topology_join": {
            "selected_rows_joined": int(len(features)),
            "missing_rows": 0,
            "cluster_count": int(config["latent_cluster_count"]),
            "match_topology_column": str(config["match_topology_column"]),
            "summary_table": "latent_topology_summary.csv",
        },
        "traditional_method": "exact_run_current_amplitude_joined_p02_latent_topology_matched_fixed_secondary_rubric",
        "methods": bench["method"].tolist(),
        "cnn_1d_implementation": "torch_conv1d" if importlib.util.find_spec("torch") is not None else "fixed_1d_convolution_filter_bank_hist_gradient_boosting_fallback",
        "winner": {
        ...
        "next_tickets": [],
        "finding": f"Baseline-excursion dynamic-only pulses reproduce at {int(reproduction.loc[reproduction['quantity'] == 'dynamic_only', 'reproduced'].iloc[0])}; exact matched deltas favor {decision['verdict']}. Predictive benchmark winner: {winner['method']}.",
    }
    ...
    manifest = {
        ...
        "script": "scripts/s00g_1781108434_1606_7f9f3ead_dynamic_latent_topology_replacement.py",
    ...
    write_report(out_dir, config, reproduction, latent_summary, support, metrics, strata, bench, leakage, decision, result)
