#!/usr/bin/env python3
"""P03h train-fold external pedestal-proxy validation before P03f ablation.
...
fitting residual correctors. Samples 0-3 are linearly residualized against
blinded pedestal/baseline proxies using only the training runs in each fold; the
residualizer is externally validated on baseline-proxy strata held out from the
training fold before it is applied to the LORO timing benchmark.
The ML features exclude event ids, run ids, event order, other-stave times, and
pairwise residuals.
"""
...
os.environ.setdefault("MPLCONFIGDIR", "/tmp/matplotlib-p03h-1783548269")
...
def pedestal_proxy_block(pulses: pd.DataFrame, config: dict) -> Tuple[np.ndarray, List[str]]:
    wf = np.vstack(pulses["waveform"].to_numpy()).astype(np.float32)
    amp = pulses["amplitude_adc"].to_numpy(dtype=np.float32)
    norm = wf / np.maximum(amp[:, None], 1.0)
    samples = [int(i) for i in config["residualization"]["samples"]]
    early = norm[:, samples]
    x = np.asarray(samples, dtype=np.float32)
    xc = x - float(np.mean(x))
    denom = float(np.sum(xc**2)) if float(np.sum(xc**2)) > 0 else 1.0
    slope = ((early - early.mean(axis=1, keepdims=True)) @ xc) / denom
    amp64 = pulses["amplitude_adc"].to_numpy(dtype=float)
    proxy_parts = [
        early.mean(axis=1),
        slope,
        early.std(axis=1),
        early.mean(axis=1) - norm[:, 4:8].mean(axis=1),
        np.log1p(amp64),
    ...
    names = [
        ...
        "baseline_slope_0_3",
        "baseline_rms_0_3",
        "early_late_contrast_0_3_vs_4_7",
        "log_amp",
        "peak_sample",
        "area_over_amp",
    ]
    stave, stave_names = stave_block(pulses, config)
    return np.hstack([np.column_stack(proxy_parts), stave]).astype(np.float32), names + stave_names


def residualize_samples_by_train(
    wave: np.ndarray,
    pulses: pd.DataFrame,
    train_idx: np.ndarray,
    config: dict,
) -> Tuple[np.ndarray, List[dict], List[str]]:
    proxies, proxy_names = pedestal_proxy_block(pulses, config)
    scaler = StandardScaler()
    Z_train = scaler.fit_transform(proxies[train_idx])
    Z_all = scaler.transform(proxies)
    samples = [int(i) for i in config["residualization"]["samples"]]
    alpha = float(config["residualization"].get("ridge_alpha", 0.001))
    out = wave.copy()
    rows = []
    for sample in samples:
        y = wave[train_idx, sample].astype(float)
        model = Ridge(alpha=alpha)
        model.fit(Z_train, y)
        pred_train = model.predict(Z_train)
        pred_all = model.predict(Z_all)
        train_mean = float(np.mean(y))
        out[:, sample] = (wave[:, sample].astype(float) - pred_all + train_mean).astype(np.float32)
        ss_tot = float(np.sum((y - train_mean) ** 2))
        ss_res = float(np.sum((y - pred_train) ** 2))
        rows.append(
            {
                "table": "pedestal_residualization",
                "sample": int(sample),
                "ridge_alpha": alpha,
                "train_r2": float(1.0 - ss_res / ss_tot) if ss_tot > 0 else float("nan"),
                "train_original_std": float(np.std(y)),
                "train_residualized_std": float(np.std(out[train_idx, sample])),
                "proxy_feature_set_sha256": hashlib.sha256("|".join(proxy_names).encode("utf-8")).hexdigest(),
            }
        )
    return out.astype(np.float32), rows, proxy_names


def external_residualizer_validation(
    wave: np.ndarray,
    pulses: pd.DataFrame,
    train_idx: np.ndarray,
    config: dict,
    ...
    samples = [int(i) for i in config["residualization"]["samples"]]
    alpha = float(config["residualization"].get("ridge_alpha", 0.001))
    n_strata = int(config["residualization"].get("external_validation_strata", 4))
    train_idx = np.asarray(train_idx, dtype=int)
    if len(train_idx) < n_strata * 2:
        return pd.DataFrame()
    score = proxies[train_idx, 0] + 0.25 * proxies[train_idx, 1] + 0.10 * proxies[train_idx, 2]
    ranks = pd.Series(score).rank(method="first").to_numpy(dtype=float)
    strata = np.minimum(n_strata - 1, np.floor(n_strata * (ranks - 1) / max(len(ranks), 1)).astype(int))
    rows = []
    for hold in range(n_strata):
        cal_idx = train_idx[strata != hold]
        ext_idx = train_idx[strata == hold]
        if len(cal_idx) < 2 or len(ext_idx) < 2:
            continue
        scaler = StandardScaler()
        z_cal = scaler.fit_transform(proxies[cal_idx])
        z_ext = scaler.transform(proxies[ext_idx])
        for sample in samples:
            y_cal = wave[cal_idx, sample].astype(float)
            y_ext = wave[ext_idx, sample].astype(float)
            model = Ridge(alpha=alpha)
            model.fit(z_cal, y_cal)
            pred = model.predict(z_ext)
            ext_mean = float(np.mean(y_ext))
            ss_tot = float(np.sum((y_ext - ext_mean) ** 2))
            ss_res = float(np.sum((y_ext - pred) ** 2))
            rows.append(
                {
                    "table": "external_pedestal_validation",
                    "stratum": int(hold),
                    "sample": int(sample),
                    "n_calibration": int(len(cal_idx)),
                    "n_external": int(len(ext_idx)),
                    "external_r2": float(1.0 - ss_res / ss_tot) if ss_tot > 0 else float("nan"),
                    "external_original_std": float(np.std(y_ext)),
                    "external_residual_std": float(np.std(y_ext - pred + float(np.mean(y_cal)))),
                    "proxy_feature_set_sha256": hashlib.sha256("|".join(proxy_names).encode("utf-8")).hexdigest(),
                }
            )
...
def run_one_fold(pulses_all: pd.DataFrame, config: dict, heldout_run: int, rng: np.random.Generator) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for variant_i, variant in enumerate(variants):
        ...
        policy = (
            policy
            + "; samples 0-3 are fold-local Ridge residuals after projecting out blinded pedestal/baseline proxies"
        )
        feature_policies[variant] = policy
        train_mask = train_base & finite_mask(X, target, runs)
        train_idx = np.flatnonzero(train_mask)
        external_validation = external_residualizer_validation(wave, work, train_idx, cfg)
        if len(external_validation):
            external_validation["heldout_run"] = int(heldout_run)
            external_validation["variant"] = variant
            model_rows.extend(external_validation.to_dict(orient="records"))
        wave, resid_rows, proxy_names = residualize_samples_by_train(wave, work, train_idx, cfg)
        X = np.hstack([wave, aux]).astype(np.float32)
        feature_names = [f"resid_{name}" if i in set(int(j) for j in cfg["residualization"]["samples"]) else name for i, name in enumerate(feature_names)]
        train_mask = train_base & finite_mask(X, target, runs)
        train_idx = np.flatnonzero(train_mask)
...
def write_report(out_dir: Path, config: dict, result: dict, match: pd.DataFrame, pooled: pd.DataFrame, per_run: pd.DataFrame, leakage: pd.DataFrame) -> None:
    ...
    resid = pd.read_csv(out_dir / "model_diagnostics.csv")
    resid = resid[resid["table"] == "pedestal_residualization"].copy()
    resid_summary = (
        resid.groupby("sample")[["train_r2", "train_original_std", "train_residualized_std"]].mean().reset_index()
        if len(resid)
        else pd.DataFrame(columns=["sample", "train_r2", "train_original_std", "train_residualized_std"])
    )
    ext = pd.read_csv(out_dir / "external_pedestal_validation.csv")
    ext_summary = (
        ext.groupby("sample")[["external_r2", "external_original_std", "external_residual_std"]].mean().reset_index()
        if len(ext)
        else pd.DataFrame(columns=["sample", "external_r2", "external_original_std", "external_residual_std"])
    )
    text = f"""# P03h: external pedestal-proxy validation for residualized early waveform samples
...
- **External pedestal-proxy validation:** `{config['residualization'].get('external_validation_strata', 4)}` train-fold strata per held-out run

## Question and preregistered estimand

The ticket asks whether the P03g pedestal-proxy residualizer is externally valid inside each training fold before the residualized P03f multimodel ablation is applied to a held-out run. Operationally, each leave-one-run-out training set is split into blinded baseline-proxy strata; the residualizer is calibrated on three strata and scored on the excluded stratum. Only after this train-fold external check is recorded is the production residualizer refit on the full training fold and used in the timing benchmark. The estimand is the B4/B6/B8 event-paired timing width after a fold-local S03a analytic timewalk correction:
...
{markdown_table(resid_summary, ['sample', 'train_r2', 'train_original_std', 'train_residualized_std'])}

### External pedestal-proxy stratum validation

The table below is not the production fit used for the held-out run. It is an internal validation performed only within the six training runs of each LORO fold. Let `S_k` be a quantile stratum of the blinded proxy score. For each sample `s`, the audit fits

`x_s = beta_s^T z + epsilon_s`

on `train \\ S_k` and scores `R^2_ext(s,k)` plus the residual standard deviation on `S_k`. This tests whether the proxy relation generalizes across baseline-proxy strata rather than merely fitting all train pulses at once.

{markdown_table(ext_summary, ['sample', 'external_r2', 'external_original_std', 'external_residual_std'])}
...
The external validation gate reports mean `R^2_ext = {result['external_pedestal_validation']['mean_external_r2']:.4f}` over `{result['external_pedestal_validation']['n_rows']}` sample/stratum/fold/variant rows. This does **not** validate a globally stable pedestal-proxy projection: samples 0 and 3 transport well enough across proxy strata, but samples 1 and 2 have negative external `R^2`. The timing benchmark is therefore a predictive closure result for the production residualized feature set, not proof that the pedestal residualizer is causal or stratum-stable.
...
```bash
/home/billy/anaconda3/bin/python scripts/p03h_1783548269_15632_5bd03538_external_pedestal_proxy_validation.py --config configs/p03h_1783548269_15632_5bd03538_external_pedestal_proxy_validation.json
```

Artifacts include `reproduction_match_table.csv`, `external_pedestal_validation.csv`, `heldout_run_summary.csv`, `pooled_run_block_summary.csv`, `pairwise_residuals.csv`, `leakage_checks.csv`, `model_diagnostics.csv`, figures, `input_sha256.csv`, `result.json`, and `manifest.json`.
"""
...
def plot_outputs(out_dir: Path, pooled: pd.DataFrame, all_pairs: pd.DataFrame) -> None:
    ...
    ax.set_title("P03h externally validated pedestal-residualized ablation")
    fig.tight_layout()
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/p03h_1783548269_15632_5bd03538_external_pedestal_proxy_validation.json")
    ...
    external = diagnostics[diagnostics["table"] == "external_pedestal_validation"].copy()
    external.to_csv(out_dir / "external_pedestal_validation.csv", index=False)
    ...
    if ext_mean < 0:
        verdict += " The train-fold external pedestal-proxy validation fails as a global stability claim because mean external R2 is negative; the residualizer remains a predictive preprocessing step, not a validated causal baseline projection."

    ext_mean = float(external["external_r2"].mean()) if len(external) else float("nan")
    ext_min = float(external["external_r2"].min()) if len(external) else float("nan")
    result = {
        "study": "P03h",
        "ticket": config["ticket_id"],
        "worker": config["worker"],
        "title": config["title"],
        "residualized_samples": [int(i) for i in config["residualization"]["samples"]],
        ...
        "external_pedestal_validation": {
            "n_rows": int(len(external)),
            "mean_external_r2": ext_mean,
            "min_external_r2": ext_min,
            "mean_external_residual_std": float(external["external_residual_std"].mean()) if len(external) else float("nan"),
            "mean_external_original_std": float(external["external_original_std"].mean()) if len(external) else float("nan"),
        },
        "verdict": verdict,
        "next_tickets": [
            "P03i: replace linear early-sample residualization with stratum-stable monotone/robust pedestal projections and require positive external proxy-stratum R2 before timing promotion"
        ],
