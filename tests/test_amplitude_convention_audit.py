...
def test_absolute_classification_records_provenance(tmp_path: Path) -> None:
    ...
    result = MODULE.audit(path, 40000, 3500.0, 5000.0)
...
def test_net_and_ambiguous_are_distinct(tmp_path: Path) -> None:
    ...
    assert MODULE.audit(net, 40000, 3500.0, 5000.0)["convention"] == "NET"
    assert MODULE.audit(ambiguous, 40000, 3500.0, 5000.0)["convention"] == "AMBIGUOUS"
