#!/usr/bin/env python3
"""P02f shape-atom veto test on S03/S04-style pair residual tail tables."""
...
TRAD_FEATURES = [
    ...
    "q_downstream_max",
    "hand_tail_score",
    "hand_peak_edge_score",
    "hand_saturation_score",
    "hand_secondary_score",
    "hand_shape_score",
]
ML_FEATURES = [
    "ae_pair_max",
    "ae_pair_mean",
    "autoencoder_mse_pair_max",
    "template_mse_pair_max",
    "shape_late_frac_max",
    "shape_late_frac_mean",
    "shape_late_frac_absdiff",
    "shape_pre_frac_max",
    "shape_width20_max",
    "shape_width50_max",
    "shape_saturation_count_max",
    "shape_secondary_peak_max",
    "shape_post_peak_min_min",
    "shape_area_over_amp_mean",
    "shape_peak_edge_score",
    "pair_B4_B6",
    "pair_B4_B8",
    "pair_B6_B8",
]
TOPOLOGY_FEATURES = ["pair_B4_B6", "pair_B4_B8", "pair_B6_B8"]
...
def annotate_q_template(pulses: pd.DataFrame, q_table: pd.DataFrame) -> pd.DataFrame:
    ...
    norm = wf / amp[:, None]
    pos = np.clip(norm, 0.0, None)
    pos_sum = np.maximum(pos.sum(axis=1), 1.0e-9)
    out["shape_late_frac"] = pos[:, 12:].sum(axis=1) / pos_sum
    out["shape_pre_frac"] = pos[:, :4].sum(axis=1) / pos_sum
    out["shape_width20"] = (norm > 0.20).sum(axis=1)
    out["shape_width50"] = (norm > 0.50).sum(axis=1)
    out["shape_saturation_count"] = (norm > 0.98).sum(axis=1)
    out["shape_area_over_amp"] = out["area_adc_samples"].to_numpy(dtype=float) / amp
    peak = out["peak_sample"].to_numpy(dtype=float)
    out["shape_peak_edge"] = np.maximum(np.abs(peak - 6.0), np.abs(peak - 7.0))
    secondary = np.zeros(len(out), dtype=float)
    post_min = np.zeros(len(out), dtype=float)
    for i, pk in enumerate(out["peak_sample"].to_numpy(dtype=int)):
        masked = pos[i].copy()
        lo, hi = max(0, pk - 1), min(pos.shape[1], pk + 2)
        masked[lo:hi] = 0.0
        secondary[i] = float(masked.max())
        tail = norm[i, min(norm.shape[1] - 1, pk + 1) :]
        post_min[i] = float(tail.min()) if len(tail) else 0.0
    out["shape_secondary_peak"] = secondary
    out["shape_post_peak_min"] = post_min
    return out
...
def pair_table(pulses: pd.DataFrame, method: str, label: str, config: dict, runs: List[int]) -> pd.DataFrame:
    ...
    ae_wide = sub.pivot(index="event_id", columns="stave", values="q_autoencoder_rmse")
    template_mse_wide = sub.pivot(index="event_id", columns="stave", values="template_mse")
    autoencoder_mse_wide = sub.pivot(index="event_id", columns="stave", values="autoencoder_mse")
    shape_wide = {
        name: sub.pivot(index="event_id", columns="stave", values=name)
        for name in [
            "shape_late_frac",
            "shape_pre_frac",
            "shape_width20",
            "shape_width50",
            "shape_saturation_count",
            "shape_area_over_amp",
            "shape_peak_edge",
            "shape_secondary_peak",
            "shape_post_peak_min",
        ]
    }
    rows = []
    for left, right in PAIRS:
        ...
        frame = pd.DataFrame(
            {
                ...
                "ae_left": ae_wide[left].to_numpy(dtype=float),
                "ae_right": ae_wide[right].to_numpy(dtype=float),
                "template_mse_left": template_mse_wide[left].to_numpy(dtype=float),
                "template_mse_right": template_mse_wide[right].to_numpy(dtype=float),
                "autoencoder_mse_left": autoencoder_mse_wide[left].to_numpy(dtype=float),
                "autoencoder_mse_right": autoencoder_mse_wide[right].to_numpy(dtype=float),
        ...
        frame["ae_pair_max"] = frame[["ae_left", "ae_right"]].max(axis=1)
        frame["ae_pair_mean"] = frame[["ae_left", "ae_right"]].mean(axis=1)
        frame["template_mse_pair_max"] = frame[["template_mse_left", "template_mse_right"]].max(axis=1)
        frame["autoencoder_mse_pair_max"] = frame[["autoencoder_mse_left", "autoencoder_mse_right"]].max(axis=1)
        ...
        for atom, wide in shape_wide.items():
            left_values = wide[left].reindex(frame["event_id"]).to_numpy(dtype=float)
            right_values = wide[right].reindex(frame["event_id"]).to_numpy(dtype=float)
            out_name = atom.replace("shape_", "shape_")
            frame[f"{out_name}_max"] = np.maximum(left_values, right_values)
            frame[f"{out_name}_mean"] = 0.5 * (left_values + right_values)
            frame[f"{out_name}_absdiff"] = np.abs(left_values - right_values)
            frame[f"{out_name}_min"] = np.minimum(left_values, right_values)
        frame["hand_tail_score"] = frame[["shape_late_frac_max", "shape_width20_max"]].max(axis=1)
        frame["hand_peak_edge_score"] = frame["shape_peak_edge_max"]
        frame["hand_saturation_score"] = frame["shape_saturation_count_max"]
        frame["hand_secondary_score"] = frame["shape_secondary_peak_max"]
        frame["hand_shape_score"] = np.nanmax(
            np.column_stack(
                [
                    frame["q_pair_max"].to_numpy(dtype=float),
                    frame["ae_pair_max"].to_numpy(dtype=float),
                    frame["shape_late_frac_max"].to_numpy(dtype=float),
                    frame["shape_width20_max"].to_numpy(dtype=float) / 18.0,
                    frame["shape_saturation_count_max"].to_numpy(dtype=float) / 18.0,
                    frame["shape_secondary_peak_max"].to_numpy(dtype=float),
                ]
            ),
            axis=1,
        )
        frame["shape_peak_edge_score"] = frame["shape_peak_edge_max"]
...
def feature_matrix(train: pd.DataFrame, frame: pd.DataFrame, features: List[str]) -> np.ndarray:
    med = train[features].median(axis=0, skipna=True).fillna(0.0)
    return frame[features].fillna(med).to_numpy(dtype=float)
...
def choose_ml_veto(
    ...
    X = feature_matrix(train, train, features)
    ...
    final.fit(X, y_fit)
    held_scores = final.predict_proba(feature_matrix(train, heldout, features))[:, 1]
    return policy, held_scores


def choose_iforest_veto(train: pd.DataFrame, heldout: pd.DataFrame, config: dict, seed: int) -> Tuple[dict, np.ndarray]:
    y_tail = tail_labels(train)
    X_train = feature_matrix(train, train, ML_FEATURES)
    X_held = feature_matrix(train, heldout, ML_FEATURES)
    ...
    model = IsolationForest(
        n_estimators=int(config["q_veto"].get("iforest_n_estimators", 240)),
        contamination=float(config["q_veto"].get("iforest_contamination", 0.10)),
        random_state=int(seed),
        n_jobs=1,
    )
    model.fit(Xs[finite])
    score = -model.score_samples(Xs)
    held_score = -model.score_samples(Xh)
    auc = float(roc_auc_score(y_tail[finite], score[finite])) if len(np.unique(y_tail[finite])) == 2 else math.nan
    min_keep = float(config["q_veto"]["min_train_keep_fraction"])
    best = None
    for q in [float(v) for v in config["q_veto"]["threshold_quantiles"]]:
        threshold = float(np.nanquantile(score[finite], q))
...
def evaluate_vetoes(
    ...
    for method in RESIDUAL_METHODS:
        ...
        variants["traditional_q_hand_threshold"] = trad_keep
        policy_rows.append({"heldout_run": heldout_run, "residual_method": method, "veto_method": "traditional_q_hand_threshold", **trad})
        ...
        held["ml_shape_rf_tail_score"] = ml_score
        variants["ml_shape_rf"] = ml_score <= float(ml_policy["threshold"])
        policy_rows.append({"heldout_run": heldout_run, "residual_method": method, "veto_method": "ml_shape_rf", **ml_policy})

        if_policy, if_score = choose_iforest_veto(train, held, config, seed + 303)
        held["ml_shape_iforest_score"] = if_score
        variants["ml_shape_iforest"] = if_score <= float(if_policy["threshold"])
        policy_rows.append({"heldout_run": heldout_run, "residual_method": method, "veto_method": "ml_shape_iforest", **if_policy})

        shuf_policy, shuf_score = choose_ml_veto(train, held, config, seed, shuffle=True)
        variants["shuffled_ml_shape_rf_control"] = shuf_score <= float(shuf_policy["threshold"])
        policy_rows.append({"heldout_run": heldout_run, "residual_method": method, "veto_method": "shuffled_ml_shape_rf_control", **shuf_policy})

        topo_policy, topo_score = choose_ml_veto(train, held, config, seed + 211, shuffle=False, features=TOPOLOGY_FEATURES)
        variants["topology_only_rf_control"] = topo_score <= float(topo_policy["threshold"])
        policy_rows.append({"heldout_run": heldout_run, "residual_method": method, "veto_method": "topology_only_rf_control", **topo_policy})
        ...
        leakage_rows.append(
            {
                "heldout_run": heldout_run,
                "residual_method": method,
                "check": "ml_shape_iforest_auc",
                "value": float(if_policy["oof_auc"]),
                "flag": bool(np.isfinite(if_policy["oof_auc"]) and if_policy["oof_auc"] > 0.90),
            }
        )
        leakage_rows.append(
            {
                "heldout_run": heldout_run,
                "residual_method": method,
                "check": "shuffled_ml_shape_rf_oof_auc",
        ...
        leakage_rows.append(
            {
                "heldout_run": heldout_run,
                "residual_method": method,
                "check": "topology_only_rf_oof_auc",
                "value": float(topo_policy["oof_auc"]),
                "flag": bool(np.isfinite(topo_policy["oof_auc"]) and topo_policy["oof_auc"] > 0.65),
            }
        )
                ...
                residual_rows.append(
                    {
                        ...
                        "ml_shape_rf_tail_score": float(getattr(row, "ml_shape_rf_tail_score", math.nan)),
                        "ml_shape_iforest_score": float(getattr(row, "ml_shape_iforest_score", math.nan)),
                    }
                )
...
def plot_outputs(out_dir: Path, pooled: pd.DataFrame, delta: pd.DataFrame) -> None:
    ...
    order = [
        ...
        ("s03b_monotone_binned", "traditional_q_hand_threshold"),
        ("s03b_monotone_binned", "ml_shape_rf"),
        ("s03b_monotone_binned", "ml_shape_iforest"),
    ]
    ...
    ax.set_title("Shape-atom vetoes on pair residual tables")
    fig.tight_layout()
    fig.savefig(out_dir / "fig_shape_veto_sigma68.png", dpi=140)
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(7.2, 3.8))
    show = delta[delta["veto_method"].isin(["traditional_q_hand_threshold", "ml_shape_rf", "ml_shape_iforest"])].copy()
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "fig_shape_veto_delta.png", dpi=140)
    plt.close(fig)
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/p02f_1781028505_1268_682a3cf2_shape_atom_veto.yaml")
    ...
    q_per_run.to_csv(out_dir / "shape_veto_per_run_metrics.csv", index=False)
    q_residuals.to_csv(out_dir / "heldout_pair_shape_veto_residuals.csv", index=False)
    policies.to_csv(out_dir / "shape_veto_policy_by_fold.csv", index=False)
    ...
    pooled.to_csv(out_dir / "shape_veto_pooled_run_bootstrap.csv", index=False)
    delta.to_csv(out_dir / "shape_veto_delta_bootstrap.csv", index=False)
    ...
    s03b_trad = d_idx.loc[("s03b_monotone_binned", "traditional_q_hand_threshold")]
    s03b_ml = d_idx.loc[("s03b_monotone_binned", "ml_shape_rf")]
    s03b_iforest = d_idx.loc[("s03b_monotone_binned", "ml_shape_iforest")]
    leakage_flag = bool(leakage["flag"].fillna(False).any())
    best_delta = float(min(s03b_trad["value"], s03b_ml["value"], s03b_iforest["value"]))
    best_delta_high = float(delta.loc[delta["value"].idxmin(), "ci_high"])
    if best_delta_high < 0.0:
        verdict = "shape_atom_veto_improves_pair_residual_width"
    elif best_delta < 0.0:
        verdict = "shape_atom_veto_has_weak_nonrobust_narrowing"
    else:
        verdict = "shape_atom_veto_does_not_improve_pair_residual_width"
    conclusion = (
        f"The pair-level learned shape-atom veto benchmark does not produce a statistically secure residual-tail improvement. "
        f"S03b no-veto sigma68 is {s03b_base['value']:.3f} ns; traditional q/hand-shape delta is "
        f"{s03b_trad['value']:.3f} ns [{s03b_trad['ci_low']:.3f}, {s03b_trad['ci_high']:.3f}], "
        f"RF shape delta is {s03b_ml['value']:.3f} ns [{s03b_ml['ci_low']:.3f}, {s03b_ml['ci_high']:.3f}], "
        f"and IsolationForest delta is {s03b_iforest['value']:.3f} ns [{s03b_iforest['ci_low']:.3f}, {s03b_iforest['ci_high']:.3f}]. "
    ...
    result = {
        ...
        "traditional": {
            ...
            "traditional_q_hand_threshold_delta_ns": [float(s03b_trad["value"]), float(s03b_trad["ci_low"]), float(s03b_trad["ci_high"])],
            "ml_shape_rf_delta_ns": [float(s03b_ml["value"]), float(s03b_ml["ci_low"]), float(s03b_ml["ci_high"])],
            "ml_shape_iforest_delta_ns": [float(s03b_iforest["value"]), float(s03b_iforest["ci_low"]), float(s03b_iforest["ci_high"])],
        },
        "ml": {
            "method": "RandomForest and IsolationForest learned shape-atom veto scores on S03b pair residual tails",
            "delta_ns": [float(s03b_ml["value"]), float(s03b_ml["ci_low"]), float(s03b_ml["ci_high"])],
        },
        "leakage": {
            "features_exclude_run_event_timing_residuals": True,
            "features_exclude_scalar_q_template": True,
            "split_by_run": True,
            "shuffled_ml_shape_rf_control_included": True,
            "topology_only_rf_control_included": True,
            "flag": leakage_flag,
        },
        "verdict": verdict,
