#!/usr/bin/env python3
"""S14: PSTAR/GEANT4 Birks energy calibration benchmark.
...
def make_report(
    ...
    lines = [
        "# S14: PSTAR/GEANT4 and Birks Energy Calibration Benchmark",
        "",
        "## Abstract",
        "",
        (
            "This ticket replaces the empirical range-energy model with a PSTAR/GEANT4 stopping-power "
            "anchor, fits Birks quenching on duplicate readout, and tests whether learned waveform "
            "regressors improve the resulting MeV-scale closure in raw B-stack data. The analysis "
            "reads the raw ROOT waveform branch, subtracts per-event pretrigger pedestals, benchmarks "
            "a strong traditional dE/dx/Birks method against ridge, gradient-boosted trees, MLP, "
            "1D-CNN, and a new range-gated residual MLP, and estimates 95% confidence intervals by "
            "held-out run bootstrap. The raw ROOT reproduction gate passes "
            f"exactly at {result['raw_reproduction']['reproduced_selected_pulses']:,} selected B-stave pulses. "
            f"The energy-closure winner named in `result.json` is **{winner}**, with fractional "
            f"energy res68={result['winner']['res68_frac']:.5f} "
            f"(95% CI [{ci[0]:.5f}, {ci[1]:.5f}])."
        ),
        "",
        "## Data and Reproduction Gate",
        "",
        f"The analysis reads `TRIGGER`, `EVENTNO`, `EVT`, `NO`, `HRD`, `HRDI`, and `HRDv` from raw B-stack `hrdb_run_*.root` files under `{config['raw_root_dir']}`. Baseline is the median of samples 0--3 for each channel. A selected pulse is an even B-stave channel with peak amplitude above 1000 ADC after baseline subtraction. This gate is rerun directly from raw ROOT before any model fitting.",
        "",
        ...
        "## Composite Ranking",
        "",
        "The ticket winner is selected by the energy calibration metric \\(R^{68}_{E,m}\\). The composite diagnostic \\(L_m=R^{68}_{E,m}+(1-\\mathrm{AUC}_{PID,m})\\) is reported only as a secondary robustness check for methods with both energy and weak-label PID outputs.",
        "",
        ...
        "",
        "```bash",
        "/home/billy/anaconda3/bin/python scripts/ticket_2390_s14_energy_calibration_benchmark.py --config configs/ticket_2390_s14_energy_calibration_benchmark.yaml",
        "```",
    ]
...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/ticket_2390_s14_energy_calibration_benchmark.yaml")
    ...
    winner_row = metrics.sort_values("res68_frac").iloc[0].to_dict()
    winner_pid_auc = float(trad_pid["roc_auc"]) if winner_row["method"] == "geant4_birks_lookup" else float(
        pid_summary.loc[pid_summary["method"] == winner_row["method"], "roc_auc"].iloc[0]
    )
    ...
    result = {
        ...
        "winner": {
            ...
            "selection_metric": "minimum held-out fractional energy res68 with run-block bootstrap CIs",
            "composite_loss": float(
                comparable.loc[comparable["method"].isin([winner_row["method"], "traditional_dedx_birks_likelihood"]), "composite_loss"].iloc[0]
            ),
            "res68_frac": float(winner_row["res68_frac"]),
            "res68_ci95": winner_row["res68_ci95"],
            "weak_label_pid_roc_auc": winner_pid_auc,
        ...
        "finding": (
            ...
            f"Across the traditional and ML/NN energy methods, the held-out energy-closure winner is {winner_row['method']} "
            f"with energy res68={float(winner_row['res68_frac']):.5f}; its associated weak-label PID ROC AUC diagnostic is {winner_pid_auc:.5f}. "
    ...
    make_report(out_dir, config, result, metrics, prior, byrun, leakage, counts, pid_summary, pid_byrun, comparable)
    ...
    manifest = {
        ...
        "command": "/home/billy/anaconda3/bin/python scripts/ticket_2390_s14_energy_calibration_benchmark.py --config configs/ticket_2390_s14_energy_calibration_benchmark.yaml",
