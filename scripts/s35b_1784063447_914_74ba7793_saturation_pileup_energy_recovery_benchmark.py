#!/usr/bin/env python3
"""S35b saturation and pile-up energy recovery benchmark."""
...
TICKET = "1784063447.914.74ba7793"
TITLE = "S35b saturation pile-up energy recovery benchmark"
WORKER = "testbeam-laptop-1"
SLUG = "s35b_saturation_pileup_energy_recovery_benchmark"
...
def winner_table(overall: pd.DataFrame, endpoints: pd.DataFrame) -> pd.DataFrame:
    out = overall.merge(endpoints, on="method", how="left")
    out["winner_score"] = (
...
def write_report(
    ...
    text = f"""# S35b: Saturation Pile-Up Energy Recovery Benchmark
...
`result.json` names **{winner}** as the S35b winner.  The traditional clipped
...
def main() -> None:
    started = time.time()
    os.environ.setdefault("MPLCONFIGDIR", "/tmp/matplotlib-s35b")
    ...
    result = {
        ...
        "claimed_ticket_text": "S35b saturation pile-up energy recovery benchmark",
        ...
        "winner": {
            ...
            "criterion": "minimum registered S35b held-out energy-plus-pileup composite score with run-block bootstrap CIs",
