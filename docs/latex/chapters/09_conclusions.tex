...
combined \(R_{\max}\) near \(3.05\) MHz rather than \(4.22\) MHz (a scaling exercise
using the beam duty factor; see \S\ref{sec:pileup_verdict} for claim-status caveats).
Scanning the tail threshold changes the numerical live-time but does not restore
a 90 ns measured window.
...
The following items remain open after the MC validation programme:

\begin{enumerate}
\item \textbf{MC geometry (MV3 FAIL).} The Geant4 stopping-depth profile
  disagrees catastrophically with data ($\chi^2/\text{ndf} = 68\,269$)
  due to missing upstream material budget in the simulation geometry.
  The estimated deficit is $\approx 10\,\text{g\,cm}^{-2}$ of inter-stave
  dead material. Correction requires a new MC production run.

\item \textbf{Digitizer timewalk model (MV4 tension).} The corrected timing
  $\sigma_{68}$ is in $2.7\sigma$ tension with data due to an unphysical
  toy timewalk ($B/\sqrt{A}$ with $B<0$). Replacing it with the physical
  leading-edge form ($\tau_{\rm rise}V_{\rm th}/A$) is expected to resolve
  the tension; this requires a re-run of the MC digitizer.

\item \textbf{Absolute energy scale and particle identity.} The gain
  calibration carries $\pm 30\%$ uncertainty (dominant: absence of a
  forced-trigger zero-signal reference sample). A future beam run with
  forced/random triggers would fix this.

\item \textbf{Two-ended readout cross-correlation.} Single-ended readout means
  correlated timing and amplitude terms in the two-ended case cannot be
  separated from the current dataset. This requires the full dual-readout
