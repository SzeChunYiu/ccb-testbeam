...
support-gated CNN architecture.  The raw ROOT reproduction and train/evaluation
split are inherited from the reviewed P04g loader, but all model fitting is
performed here from the claimed P04r config.
"""
...
def prediction_metrics(meta: pd.DataFrame, pred: dict, idx: np.ndarray, baseline: dict, config: dict) -> dict:
...
def baseline_arrays(meta: pd.DataFrame, raw_pred: dict, config: dict) -> dict:
    y_charge = meta["clean_charge"].to_numpy()
    y_time = meta["true_time_sample"].to_numpy()
    y_tail = meta["true_tail_frac"].to_numpy()
...
def run_block_bootstrap(meta: pd.DataFrame, predictions: Dict[str, dict], heldout_mask: np.ndarray, baseline: dict, config: dict) -> pd.DataFrame:
    ...
    for method, pred in predictions.items():
        point_rows[method] = prediction_metrics(meta, pred, held_idx, baseline, config)
    for _ in range(int(config["bootstrap_reps"])):
        ...
        for method, pred in predictions.items():
            m = prediction_metrics(meta, pred, idx, baseline, config)
...
def utility(summary: pd.DataFrame) -> pd.Series:
    return (
        ...
        + 1.50 * summary["net_harm_label_rate"]
...
def utility_from_metrics(metrics: dict) -> float:
    return float(
        ...
        + 1.50 * metrics["net_harm_label_rate"]
...
def bootstrap_method_deltas(
    ...
    metrics = ["charge_res68_abs_frac", "time_abs68_samples", "net_harm_label_rate", "utility"]
    samples = {
    ...
    for _ in range(int(config["bootstrap_reps"])):
        chosen = rng.integers(0, len(block_indices), size=len(block_indices))
        idx = np.concatenate([block_indices[i] for i in chosen])
        ref_metrics = prediction_metrics(meta, predictions[reference_method], idx, baseline, config)
        ref_metrics["utility"] = utility_from_metrics(ref_metrics)
        for method in samples:
            m = prediction_metrics(meta, predictions[method], idx, baseline, config)
...
def support_atom_table(meta: pd.DataFrame, predictions: Dict[str, dict], heldout_mask: np.ndarray, baseline: dict, config: dict) -> pd.DataFrame:
    rows = []
    ...
    for keys, group in work.groupby(atom_columns(), observed=True):
        ...
        raw_m = prediction_metrics(meta, predictions["no_recovery"], idx, baseline, config)
        for method, pred in predictions.items():
...
def leakage_audit(meta: pd.DataFrame, train_mask: np.ndarray, heldout_mask: np.ndarray, corrupt: np.ndarray, predictions: Dict[str, dict], config: dict) -> dict:
    ...
    return {
        ...
        "too_good_triggered": bool(summary_min_charge(predictions, meta, heldout_mask, config) < 0.005),
    }


def summary_min_charge(predictions: Dict[str, dict], meta: pd.DataFrame, heldout_mask: np.ndarray, config: dict) -> float:
    baseline = baseline_arrays(meta, predictions["no_recovery"], config)
    idx = np.where(heldout_mask)[0]
    vals = []
    for name, pred in predictions.items():
        if name != "no_recovery":
            vals.append(prediction_metrics(meta, pred, idx, baseline, config)["charge_res68_abs_frac"])
...
def write_report(out_dir: Path, config: dict, result: dict, summary: pd.DataFrame, support: pd.DataFrame, deltas: pd.DataFrame) -> None:
    support_focus = support.sort_values(["net_harm_label_rate", "charge_gain_vs_no_recovery"], ascending=[False, False]).head(18) if not support.empty else support
    lines = [
        "# P04r: dropout recovery timing-harm support map",
        "",
        ...
        "",
        (
            "This analysis asks where sample-dropout recovery improves amplitude or charge while damaging timing phase, "
            "tail shape, or downstream support. The raw reproduction gate exactly recovers the canonical S00 selected-pulse "
            "count, then deterministic dropouts are injected into real selected pulses. A strong rising-edge Huber recovery "
            "is compared to ridge, gradient-boosted trees, MLP, 1D-CNN inpainting, and a support-gated CNN. The primary "
            "selection criterion is a pre-registered utility combining charge resolution, amplitude resolution, timing "
            "phase error, catastrophic failures, and net timing/tail harm."
        ),
        "",
        ...
        "",
        markdown_table(deltas, ["comparison", "metric", "delta", "delta_ci95"], max_rows=24),
        "",
        "## Support atoms",
        "",
...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/p04r_1781051221_550_4fc60093_dropout_timing_harm_support_map.json")
    ...
    print("P04r: reading raw ROOT and reproducing S00 selected-pulse count ...")
    ...
    print("P04r: injecting dropouts into {} clean pulses ...".format(len(selected_idx)))
    ...
    print("P04r: fitting traditional comparator and tabular ML models ...")
    predictions: Dict[str, dict] = {
    ...
    nn_diagnostics = {}
    if torch is not None:
        print("P04r: fitting CNN inpainting and support-gated CNN ...")
        ...
        baseline_for_harm = baseline_arrays(inj, predictions["no_recovery"], config)
        cnn_metrics_all = prediction_metrics(inj, predictions["cnn_inpaint"], np.arange(len(inj)), baseline_for_harm, config)
        del cnn_metrics_all
        ...
        predictions["support_gated_cnn"] = {
            "method_family": "new_support_gated_cnn",
            "amp": gated_amp,
            "charge": gated_charge,
            "wave": gated_wave,
        ...
        nn_diagnostics = {"cnn_inpaint": cnn_diag, "support_gated_cnn": gate_diag}
    else:
        print("P04r: torch unavailable; CNN methods skipped.")

    print("P04r: computing run-block bootstrap confidence intervals ...")
    baseline = baseline_arrays(inj, predictions["no_recovery"], config)
    summary = run_block_bootstrap(inj, predictions, heldout_mask, baseline, config)
    summary["utility"] = utility(summary)
    summary = summary.sort_values("utility").reset_index(drop=True)
    support = support_atom_table(inj, predictions, heldout_mask, baseline, config)

    best_trad = summary[summary["method_family"] == "traditional"].sort_values("utility").iloc[0]
    winner = summary.iloc[0]
    deltas = bootstrap_method_deltas(inj, predictions, heldout_mask, baseline, config, str(best_trad["method"]), summary)
    ...
    leakage = leakage_audit(inj, train_mask, heldout_mask, corrupt, predictions, config)
    finding = (
        "Winner by support-aware utility is {winner} (U={utility:.5g}); best traditional comparator is "
        "{trad} (U={trad_u:.5g}). The support map identifies atoms with charge gain but nonzero timing/tail harm, "
    ...
    write_report(out_dir, config, result, summary, support, deltas)

    output_names = ["REPORT.md", "result.json", "counts_by_run.csv", "heldout_summary.csv", "support_atoms.csv", "method_deltas.csv"]
    manifest = {
        "study": config["study"],
        "ticket_id": config["ticket_id"],
        "command": "/home/billy/anaconda3/bin/python scripts/p04r_1781051221_550_4fc60093_dropout_timing_harm_support_map.py --config configs/p04r_1781051221_550_4fc60093_dropout_timing_harm_support_map.json",
