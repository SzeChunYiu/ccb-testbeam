...
TICKET = "1784062062.819.0cd45327"
WORKER = "testbeam-laptop-4"
SLUG = "s34a_pileup_timing_separation_architecture_bakeoff"
    ...
    cfg.update(
        {
            "study_id": "S34a",
            "ticket_id": TICKET,
            "title": "S34a pulse timing and pile-up separation architecture bakeoff",
            "worker": WORKER,
...
def pileup_mask_transformer_prediction(events: pd.DataFrame, waveforms: np.ndarray, cfg: dict) -> pd.DataFrame:
    if torch is None:
        raise RuntimeError("torch is required for the S34a pile-up mask transformer benchmark")
...
def write_report(
    ...
    text = f"""# S34a: pulse timing and pile-up separation architecture bakeoff
...
def main() -> None:
    ...
    result = {
        ...
        "evaluation_design": {
            ...
            "winner_score": "registered S34a composite endpoint score",
        },
        ...
        "winner": {
            "name": winner,
            "criterion": "minimum registered S34a composite endpoint score with run-block bootstrap CIs",
