...
B-stack pulse count and adds the S56c-specific PID-boundary, pedestal, pile-up,
...
SLUG = "s56c_likelihood_pid_templates_vs_multitask_waveform_networks"
...
def write_report(
    ...
    report = f"""# S56c/#2507 Likelihood PID Templates vs Multitask Waveform Networks
...
Use **`{winner}`** as the S56c benchmark winner.  The result favors a hybrid
...
def main() -> None:
    ...
    result = {
        "ticket_id": TICKET,
        "issue_number": 2507,
        "issue_url": "https://github.com/SzeChunYiu/factory-tickets/issues/2507",
        "project": "testbeam",
        "worker": WORKER,
        "status": "complete",
        "claimed_once": True,
        "claim_command": "tn-ticket claim testbeam-laptop-2 --project testbeam",
        "claim_note": "The single permitted tn-ticket claim invocation returned the known null pseudo-ticket; issue #2507 was then label-swapped manually without rerunning claim.",
        "title": "S56c likelihood PID templates vs multitask waveform networks under pedestal and pile-up",
        ...
        "winner": {
            ...
            "selection_rule": "minimum S56c composite loss",
        ...
        "done_command": "tn-ticket done 2507",
    }

    (OUT / "result.json").write_text(json.dumps(result, indent=2) + "\n", encoding="utf-8")
    write_report(result, reproduction, counts, summary, run_metrics, confusion, boundary, shortcuts)
