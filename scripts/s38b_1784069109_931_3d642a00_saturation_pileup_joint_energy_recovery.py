#!/usr/bin/env python3
"""S38b saturation and pile-up joint energy recovery.
...
but writes a new artifact directory for the claimed sparse ticket and adds an
injection-source bootstrap CI table.  The claimed queue item had title
``S38b: saturation and pile-up joint energy recovery with censored waveform targets`` and no body, so the pre-registered target is the generic
fleet gate: reproduce the raw selected-pulse number, then benchmark a strong
traditional method against ridge, gradient-boosted trees, MLP, 1D-CNN, a small
sequence transformer, and a physics-residual boosted stack under run-heldout
...
TICKET = "1784069109.931.3d642a00"
SLUG = "s38b_saturation_pileup_joint_energy_recovery"
WORKER = "testbeam-laptop-4"
OUT = ROOT / "reports" / f"{TICKET}__{SLUG}"
RAW_ROOT_DIR = Path("/home/billy/ccb-data/extracted/root/root")
CLAIMED_TICKET_BODY = (
    "Question: can models recover charge/energy proxies from saturated or overlapping pulses better "
    "than conservative censored traditional reconstruction? Compare a traditional clipped-template "
    "integral plus two-pulse bounded fit against ridge, gradient-boosted trees, MLP, 1D-CNN, and a "
    "masked-token waveform transformer on duplicate-readout, injected pile-up, and run-heldout "
    "saturation slices. Require bootstrap CIs for charge residual, energy-proxy bias, timing shift, "
    "pile-up false split/merge rate, saturation-onset calibration, and PID-proxy migration. Expected "
    "information gain: maps the boundary where pulse-shape learning adds usable energy information "
    "versus absorbing pile-up or saturation artifacts."
)
...
def load_config() -> dict:
    ...
    cfg.update(
        {
            "ticket_id": TICKET,
            "title": "Saturation and pile-up joint energy recovery with censored waveform targets",
            "worker": WORKER,
            "output_dir": str(OUT),
...
The claimed local-queue item was `{TICKET}` with title `S38b: saturation and pile-up joint energy recovery with censored waveform targets` and an
...
def main() -> None:
    ...
    result = {
        ...
        "worker": WORKER,
        "title": cfg["title"],
        "claimed_ticket_title": "S38b: saturation and pile-up joint energy recovery with censored waveform targets",
