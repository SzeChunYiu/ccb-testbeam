...
class CompactWaveformTransformer(nn.Module):
    def __init__(self, n_aux: int, d_model: int, n_heads: int):
        super().__init__()
        self.sample_embed = nn.Linear(3, d_model)
    ...
    def forward(self, wave: torch.Tensor, aux: torch.Tensor) -> torch.Tensor:
        n = wave.shape[1]
        pos = torch.linspace(-1.0, 1.0, n, device=wave.device).expand_as(wave)
        late_mask = (pos > 0.15).float()
        z = self.sample_embed(torch.stack([wave, pos, late_mask], dim=-1))
...
def fit_transformer(norm_waves: np.ndarray, x_all: np.ndarray, y: np.ndarray, train_idx: np.ndarray, pred_idx: np.ndarray, config: dict, rng: np.random.Generator) -> np.ndarray:
    ...
    y_std = float(np.std(y[chosen]) + 1e-6)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    torch.manual_seed(int(config["random_seed"]) + 72)
    model = CompactWaveformTransformer(aux_cols, int(config["models"]["transformer_dim"]), int(config["models"]["transformer_heads"])).to(device)
...
def fit_predict_models(norm_waves: np.ndarray, x_all: np.ndarray, targets: np.ndarray, train_mask: np.ndarray, heldout_mask: np.ndarray, config: dict, rng: np.random.Generator) -> Tuple[Dict[str, np.ndarray], pd.DataFrame]:
    ...
    trad_cols = list(range(norm_waves.shape[1], x_all.shape[1]))
    spline = make_pipeline(
        StandardScaler(),
        SplineTransformer(n_knots=4, degree=3, include_bias=False),
        Ridge(alpha=5.0),
    )
    spline.fit(X_train[:, trad_cols], y_train)
    predictions["cfd_spline_matched_template_residual"] = supported(spline.predict(X_pred[:, trad_cols]))
    rows.append({"method": "cfd_spline_matched_template_residual", "hyperparameter": "cubic hand-feature splines + ridge(alpha=5)", "train_residual_sigma68_ns": sigma68(y_train - spline.predict(X_train[:, trad_cols]))})
    ...
    rows.append({"method": "compact_waveform_transformer", "hyperparameter": f"1 layer, d={config['models']['transformer_dim']}, heads={config['models']['transformer_heads']}", "train_residual_sigma68_ns": np.nan})
    ...
    tr_pred = predictions["compact_waveform_transformer"]
    extra_pred = supported(extra.predict(X_pred))
    predictions["saturation_phase_fusion_new"] = supported(0.38 * gb_pred + 0.27 * extra_pred + 0.20 * cnn_pred + 0.15 * tr_pred)
    rows.append({"method": "saturation_phase_fusion_new", "hyperparameter": "0.38 HGB + 0.27 ExtraTrees + 0.20 CNN + 0.15 transformer", "train_residual_sigma68_ns": sigma68(y_train - extra.predict(X_train))})
...
def md_table(df: pd.DataFrame, cols: List[str], sort: Optional[str] = None, max_rows: Optional[int] = None) -> str:
    tab = df.copy()
    if sort is not None:
...
def write_report(out_dir: Path, result: dict, summary: pd.DataFrame, strata: pd.DataFrame, training: pd.DataFrame, leakage: pd.DataFrame) -> None:
    ...
    report = f"""# S72a: template-phase residual atlas for saturated pile-up timing
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/ticket_2573_s72a_template_phase_residual_atlas.json"))
    ...
    method_times = {
        "cfd20": float(config["sample_period_ns"]) * p01d.cfd_time_samples(norm_waves[heldout_mask], 0.2),
        "trapezoid_template": trad_time_all[heldout_mask].copy(),
    }
    ...
    result = {
        ...
        "traditional_method": "cfd_spline_matched_template_residual",
        "trapezoid": config["trapezoid"],
        "template_grid": config["template_shift_grid"],
        "new_architecture": "saturation_phase_fusion_new",
