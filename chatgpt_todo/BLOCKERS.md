...
- **State:** OPEN
- **Resolved engineering defects:** the PR #890 default reference path and masking inline fallback were corrected under `AUD-G4-004`; silent endpoint clamping was corrected under `AUD-G4-006`; silent malformed-row skipping, row reordering, duplicate energies, and nonfinite/nonphysical reference values were corrected under `AUD-G4-007`; quenched visible-energy fallback into raw PSTAR acceptance was corrected under `AUD-G4-008`; and a standalone fail-closed event-CSV preflight was validated under `AUD-G4-009`. Current tooling can select and validate the committed reference, reject unsupported lookup energies, distinguish raw and quenched deposits, and validate every simulation CSV row with exact input provenance.
- **Remaining integration defect:** `compare_stopping_power.py` does not yet invoke the `AUD-G4-009` preflight automatically and its legacy `read_sim` path can still skip malformed rows. Until integration is validated, every scientific run must execute `tools/audit/validate_stopping_power_sim_table.py` first and preserve its JSON result.
...
- **Validated boundary:** current output is explicitly `DIAGNOSTIC_ONLY`; synthetic tests establish arithmetic, reference wiring, reference integrity, domain handling, energy-deposit convention gating, and standalone simulation-input validation only.
- **Resolution:** integrate the preflight into the canonical CLI, validate real exported event tables, then run a clean provenance-retained proton closure using `G4EmCalculator::ComputeTotalDEDX` or primary entry/exit kinetic energy plus path-length/reference integration; quantify escaping secondary energy and production-cut dependence; preserve exact material, density, physics list, cuts, versions, commands, seeds, event counts, hashes, uncertainty, and overlay/ratio/diagnostic plots. Treat deuterons separately with a documented approximation or authoritative reference.
