...
def audit(ledger_path: Path, dashboard_path: Path, summary_path: Path, table_path: Path, provenance_path: Path, mv3_path: Path) -> dict[str, Any]:
    ...
    expected = {
        "CL-013": {"current_value": "92", "unit": "ADC/MeV", "syst_unc": "28", "truth_type": "data_mc_calibration_proxy", "status": "GATED"},
        "CL-021": {"current_value": "68269.40598948313", "truth_type": "legacy_data_mc_profile_diagnostic", "status": "FLAWED"},
        "CL-022": {"current_value": "0.003232254011764034", "numerator": "283", "denominator": "87555", "truth_type": "mc_truth_only", "status": "TRUTH_LEVEL_MC_ONLY"},
    }
    for claim, fields in expected.items():
        ...
        for field, value in fields.items():
            if row.get(field) != value:
                findings.append(finding("CLAIM_FIELD_MISMATCH", "ledger", f"{claim}.{field}"))
    for artifact, text in {"dashboard": dashboard, "summary": summary}.items():
        required = ("92 ADC/MeV", "28 ADC/MeV", "68269.40598948313", "283/87555")
        for token in required:
            if token not in text:
                findings.append(finding("CANONICAL_TOKEN_MISSING", artifact, token))
        if re.search(r"CL-013[^\n]{0,120}\b110(?:\.0)?\b", text):
            findings.append(finding("CL013_CANONICAL_VALUE_MISMATCH", artifact))
        if re.search(r"CL-021[^\n]{0,140}(?:86135|8\.6e4|6\.8e4)", text, re.I):
            findings.append(finding("CL021_CLUSTERD_RERUN_CONFLATED", artifact))
        if "25/38 toy early-peak C12" in text:
            findings.append(finding("CL022_TOY_COUNTS_SUBSTITUTED", artifact))
        if not any(x in text for x in ("does not supersede CL-021", "does **not supersede CL-021**")):
            findings.append(finding("DISTINCT_DIAGNOSTIC_CAVEAT_MISSING", artifact))
    ...
    table_checks = {
        ...
        "Stopping-depth data/MC closure": ("68269.40598948313", "FLAWED"),
        "Anomaly / C12 identity": ("283/87555", "TRUTH_LEVEL_MC_ONLY"),
    }
    for name, (token, status) in table_checks.items():
        row = by_claim.get(name)
        if not row or token not in row.get("headline", "") or row.get("status") != status:
            findings.append(finding("CLAIMS_TABLE_MISMATCH", "claims_table", name))
    ...
    if not isinstance(identities, dict):
        ...
        for path in sorted(REQUIRED_IDENTITIES):
            ...
            ok = isinstance(item, dict) and item.get("algorithm") == "git_blob_sha1"
            ok = ok and bool(re.fullmatch(r"[0-9a-f]{40}", str(item.get("digest", ""))))
            ok = ok and bool(re.fullmatch(r"[0-9a-f]{64}", str(item.get("sha256", ""))))
            ok = ok and isinstance(item.get("bytes"), int) and item["bytes"] > 0
            if not ok:
                findings.append(finding("PROVENANCE_INPUT_UNBOUND", "provenance", path))
    ...
    return {
        ...
        "scientific_boundary": "Claim/provenance binding only; no accepted closure or detector performance.",
    }
...
def main() -> int:
    ...
    for option in ("ledger", "dashboard", "summary", "claims-table", "provenance", "cluster-d-mv3", "output"):
