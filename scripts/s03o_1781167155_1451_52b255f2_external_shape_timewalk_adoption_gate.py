#!/usr/bin/env python3
"""S03o external-shape constrained timewalk adoption benchmark.
...
def plot_outputs(out_dir: Path, pooled: pd.DataFrame, per_run: pd.DataFrame) -> None:
    ...
    ax.set_title("S03o external-shape constrained support benchmark")
    fig.tight_layout()
    ...
    ax.set_ylabel("excluded-support sigma68 (ns)")
    ax.set_title("Run-split transfer under external shape constraint")
...
def write_report(
    ...
    report = f"""# Study report: S03o - External-shape constrained timewalk adoption gate
...
```bash
.venv/bin/python scripts/s03o_1781167155_1451_52b255f2_external_shape_timewalk_adoption_gate.py --config {config_path}
```
...
def run_analysis(config_path: Path) -> None:
    ...
    result = {
        ...
        "verdict": (
            f"{winner}_wins_external_shape_support"
            if winner != "traditional_hier_amp"
            else "traditional_hier_amp_wins_external_shape_support"
        ),
        "critic": "pending",
        "next_tickets": [],
