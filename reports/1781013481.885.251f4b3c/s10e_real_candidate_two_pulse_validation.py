...
This ticket reruns the S10/S10b/S10d raw-ROOT gates first, then scores real
high-current candidate windows with source-run-held-out traditional template
fits and ML regressors trained only on low-current/template-overlay injections.
"""
...
def load_s10d_module():
    module_path = ROOT / "scripts" / "s10d_two_pulse_resolvability_livetime.py"
    spec = importlib.util.spec_from_file_location("s10d_two_pulse_resolvability_livetime", module_path)
...
def reproduce_s10b_s10d_headlines() -> dict[str, pd.DataFrame]:
    """Rerun the S10b and S10d headline gates from raw ROOT for this ticket."""
    s10d = load_s10d_module()
    config = s10d.load_config(S10D_CONFIG)
    config["raw_root_dir"] = str(RAW)
    rng = np.random.default_rng(int(config["random_seed"]))

    s00_counts = s10d.reproduce_counts(config)
    s10_ml = s10d.reproduce_s10_ml(config)
    s10b, s10b_heldout, s10b_rmax = s10d.reproduce_s10b_live_time(config)
    if not bool(s00_counts["pass"].all()):
        raise RuntimeError("S10d S00 selected-pulse reproduction failed")
    if len(s10_ml) and not bool(s10_ml["pass"].all()):
        raise RuntimeError("S10 injection AP reproduction failed")
    if not bool(s10b["pass"].all()):
        raise RuntimeError("S10b live-time reproduction failed")
    ...
    injected_events = pd.concat([train_events, held_events], ignore_index=True)
    injected_waveforms = np.vstack([train_wave, held_wave])
    trad = s10d.run_template_fits(injected_events, injected_waveforms, templates, config)
    ml, ml_cv = s10d.run_ml(injected_events, injected_waveforms, config)
    combined = injected_events.merge(trad, on="event_id").merge(ml, on="event_id")
    overall = s10d.summarize_methods(combined, rng, config)
    delay_rows = s10d.resolvability_by_delay(combined, config)
    delay_overall, delay_ci, run_delay = s10d.delay_summary(delay_rows, combined, config, rng)
    leakage = s10d.leakage_checks(injected_events, injected_waveforms, ml, config)

    expected = {
        "constrained_template_fit": 60.0,
        "compact_mlp_classifier_regressor": 20.0,
    }
    rows = []
    for method, report_value in expected.items():
        reproduced = float(delay_ci[delay_ci["method"] == method]["value"].iloc[0])
        rows.append(
            {
                "quantity": f"S10d {method} resolvable delay ns",
                "report_value": report_value,
                "reproduced": reproduced,
                "delta": reproduced - report_value,
                "tolerance": 1.0e-9,
                "pass": bool(abs(reproduced - report_value) <= 1.0e-9),
            }
        )
    s10d_headlines = pd.DataFrame(rows)
    if not bool(s10d_headlines["pass"].all()):
        raise RuntimeError("S10d headline delay reproduction failed")

    return {
        "s10d_s00_reproduction": s00_counts,
        "s10_ml_reproduction": s10_ml,
        "s10b_reproduction": s10b,
        "s10b_heldout_live_time": s10b_heldout,
        "s10b_poisson_rmax_table": s10b_rmax,
        "s10d_template_summary": template_summary,
        "s10d_head_to_head_overall": overall,
        "s10d_resolvability_by_delay": delay_rows,
        "s10d_resolvability_bootstrap_ci": delay_ci,
        "s10d_run_heldout_resolvability": run_delay,
        "s10d_leakage_checks": leakage,
        "s10d_headline_reproduction": s10d_headlines,
        "s10d_ml_group_cv": ml_cv,
    }
...
def make_synthetic_training(
    ...
    injected, frac, ratio, delay = inject_waveforms(base, base_amp, sec, sec_amp, rng)
        ...
        rows.append(
            {
                ...
                "trad_delay_sample": float(two["t2"] - two["t1"]) if not bool(one["failed"] or two["failed"]) else float("nan"),
                "trad_delay_ns": float(10.0 * (two["t2"] - two["t1"])) if not bool(one["failed"] or two["failed"]) else float("nan"),
                "trad_amp1_adc": float(two["amp1"]),
                "trad_amp2_adc": float(two["amp2"]),
                "trad_total_fit_amp_adc": float(two["amp1"] + two["amp2"]) if not bool(one["failed"] or two["failed"]) else float("nan"),
            }
        )
    return pd.DataFrame(rows)
...
def heldout_predictions(events: pd.DataFrame, waves: np.ndarray, sample: pd.DataFrame, rng: np.random.Generator) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for heldout_run in sorted(sample["run"].unique()):
        ...
        x_train, y_class, y_frac, y_delay, train_meta = make_synthetic_training(train, waves, templates, rng, SYNTHETIC_TRAIN_PER_FOLD)
        ...
        reg.fit(x_train[feature_cols], np.column_stack([y_frac, y_delay]))

        x_test = ml_features(test_waves, test["ref_stave"].to_numpy(), templates)
        ml_score = clf.predict_proba(x_test[feature_cols])[:, 1]
        ml_pred = reg.predict(x_test[feature_cols])
        ml_frac = np.clip(ml_pred[:, 0], 0.0, 0.8)
        ml_delay = np.clip(ml_pred[:, 1], 0.0, max(FIT_SEPARATIONS))

        x_cal, y_cal, y_frac_cal, y_delay_cal, _cal_meta = make_synthetic_training(test, waves, templates, rng, SYNTHETIC_CAL_PER_FOLD)
        cal_score = clf.predict_proba(x_cal[feature_cols])[:, 1]
        cal_pred = reg.predict(x_cal[feature_cols])
        cal_frac = np.clip(cal_pred[:, 0], 0.0, 0.8)
        cal_delay = np.clip(cal_pred[:, 1], 0.0, max(FIT_SEPARATIONS))
        ...
        fold_rows.append(
            {
                ...
                "synthetic_delay_mae_sample": float(np.mean(np.abs(cal_delay - y_delay_cal))),
                "synthetic_delay_mae_ns": float(10.0 * np.mean(np.abs(cal_delay - y_delay_cal))),
        ...
        frame["ml_delay_sample"] = ml_delay
        frame["ml_delay_ns"] = 10.0 * ml_delay
...
def _bootstrap_mean_ci(values: np.ndarray, rng: np.random.Generator, n_boot: int = 400) -> tuple[float, float, float]:
    values = np.asarray(values, dtype=float)
    values = values[np.isfinite(values)]
    if len(values) == 0:
        return float("nan"), float("nan"), float("nan")
    idx = np.arange(len(values))
    boot = [float(np.mean(values[rng.choice(idx, size=len(idx), replace=True)])) for _ in range(n_boot)]
    return float(np.mean(values)), float(np.quantile(boot, 0.025)), float(np.quantile(boot, 0.975))


def run_stability_summary(scores: pd.DataFrame, rng: np.random.Generator) -> pd.DataFrame:
    rows = []
    metrics = [
        ("trad_candidate_rate_score_gt_0p015", (scores["trad_score_sse_improvement"] > 0.015).astype(float)),
        ("trad_secondary_fraction", scores["trad_secondary_fraction"]),
        ("trad_recovered_delay_ns", scores["trad_delay_ns"]),
        ("ml_candidate_rate_score_gt_0p5", (scores["ml_overlap_score"] > 0.5).astype(float)),
        ("ml_secondary_fraction", scores["ml_secondary_fraction"]),
        ("ml_recovered_delay_ns", scores["ml_delay_ns"]),
    ]
    for (run, group), sub in scores.groupby(["run", "group"], sort=True):
        for name, series in metrics:
            aligned = series.loc[sub.index].to_numpy(dtype=float)
            value, ci_low, ci_high = _bootstrap_mean_ci(aligned, rng)
            rows.append(
                {
                    "run": int(run),
                    "group": str(group),
                    "n_scored_events": int(len(sub)),
                    "metric": name,
                    "value": value,
                    "ci_low": ci_low,
                    "ci_high": ci_high,
                    "bootstrap_unit": "events_within_heldout_source_run",
                }
            )
    return pd.DataFrame(rows)


def threshold_validation_summary(scores: pd.DataFrame, s10d_delay_ci: pd.DataFrame, rng: np.random.Generator) -> pd.DataFrame:
    thresholds = {
        "trad": float(s10d_delay_ci[s10d_delay_ci["method"] == "constrained_template_fit"]["value"].iloc[0]),
        "ml": float(s10d_delay_ci[s10d_delay_ci["method"] == "compact_mlp_classifier_regressor"]["value"].iloc[0]),
    }
    specs = [
        ("traditional", "trad", "trad_score_sse_improvement", "trad_delay_ns", "trad_secondary_fraction", 0.015),
        ("ml", "ml", "ml_overlap_score", "ml_delay_ns", "ml_secondary_fraction", 0.5),
    ]
    rows = []
    for label, key, score_col, delay_col, area_col, score_threshold in specs:
        for group, sub in scores.groupby("group", sort=True):
            candidate = sub[score_col].to_numpy(dtype=float) > score_threshold
            delay = sub[delay_col].to_numpy(dtype=float)
            area = sub[area_col].to_numpy(dtype=float)
            candidate_rate, rate_lo, rate_hi = _bootstrap_mean_ci(candidate.astype(float), rng)
            stable = np.where(candidate & np.isfinite(delay), delay >= thresholds[key], np.nan)
            stable_frac, stable_lo, stable_hi = _bootstrap_mean_ci(stable, rng)
            area_candidate = np.where(candidate, area, np.nan)
            area_mean, area_lo, area_hi = _bootstrap_mean_ci(area_candidate, rng)
            delay_candidate = np.where(candidate, delay, np.nan)
            delay_mean, delay_lo, delay_hi = _bootstrap_mean_ci(delay_candidate, rng)
            rows.extend(
                [
                    {
                        "method": label,
                        ...
                        "ci_low": rate_lo,
                        "ci_high": rate_hi,
                        "s10d_threshold_ns": thresholds[key],
                    },
                    {
                        "method": label,
                        "group": str(group),
                        "metric": "candidate_fraction_delay_ge_s10d_threshold",
                        "value": stable_frac,
                        "ci_low": stable_lo,
                        "ci_high": stable_hi,
                        "s10d_threshold_ns": thresholds[key],
                    },
                    {
                        "method": label,
                        "group": str(group),
                        "metric": "candidate_mean_recovered_delay_ns",
                        "value": delay_mean,
                        "ci_low": delay_lo,
                        "ci_high": delay_hi,
                        "s10d_threshold_ns": thresholds[key],
                    },
                    {
                        "method": label,
                        "group": str(group),
                        "metric": "candidate_mean_secondary_fraction",
                        "value": area_mean,
                        "ci_low": area_lo,
                        "ci_high": area_hi,
                        "s10d_threshold_ns": thresholds[key],
...
def write_report(
    ...
    s10b_repro = s10d_tables["s10b_reproduction"]
    s10d_repro = s10d_tables["s10d_headline_reproduction"]
    run_focus = run_stability[
        run_stability["metric"].isin(
            [
    ...
    run_focus_summary = (
        run_focus.groupby(["group", "metric"], sort=True)
        .agg(
            n_runs=("run", "nunique"),
            mean_value=("value", "mean"),
            min_value=("value", "min"),
            max_value=("value", "max"),
            median_ci_width=("ci_high", lambda s: float(np.nanmedian(s.to_numpy() - run_focus.loc[s.index, "ci_low"].to_numpy()))),
        )
        .reset_index()
    )
    lines = [
        ...
        "",
        (
            "The S10 raw-ROOT topology gate was reproduced before the real-candidate study. "
        ...
        "",
        s10b_repro.to_markdown(index=False),
        "",
        s10d_repro.to_markdown(index=False),
        "",
        ...
        "",
        (
            "For each held-out run, median empirical templates are built from all other runs. A bounded two-pulse "
        ...
        "",
        (
            "The ML method is a run-held-out random-forest classifier/regressor trained on synthetic two-pulse "
            "overlays made only from training-run raw pulses. Features are normalized waveform samples and "
        ...
        (
            ...
            f"{folds['shuffled_label_synthetic_auc'].mean():.3f}; delay MAE is "
            f"{folds['synthetic_delay_mae_ns'].mean():.2f} ns."
        ),
        "",
        "## Threshold validation",
        "",
        (
            "Each event is scored only by models/templates with that source run held out. The table below summarizes "
            "real-candidate rate and recovered delay/area behavior against the reproduced S10d resolvability thresholds."
        ),
        "",
        threshold_validation.to_markdown(index=False),
        "",
        "Run-level held-out stability is in `run_stability_summary.csv`; grouped ranges for the core delay/area rows are:",
        "",
        run_focus_summary.to_markdown(index=False),
        "",
        "## Leakage review",
        "",
...
def main() -> int:
    ...
    s10d_tables = reproduce_s10b_s10d_headlines()
    ...
    run_stability = run_stability_summary(scores, rng)
    threshold_validation = threshold_validation_summary(scores, s10d_tables["s10d_resolvability_bootstrap_ci"], rng)

    s10d_input_runs = [31, 32, 33, 34, 35, 36, 37, 39, 40, 41, 42, 58, 59, 60, 61, 62, 63, 64, 65]
    input_files = [raw_file(run) for run in sorted(set(run_to_group()) | set(s10d_input_runs))]
    ...
    threshold_validation.to_csv(OUT / "threshold_validation_summary.csv", index=False)
    ...
    conclusion = (
        f"Replacing the S10c binary downstream label with a constrained two-pulse amplitude estimate gives a "
        ...
        "Run-held-out threshold validation is reported by current group and source run. Leakage probes do not flag "
        "identifier or source-run leakage, but the estimate remains a pulse-shape diagnostic rather than a truth-labelled pile-up decomposition."
    )
    trad_delay_gate = s10d_tables["s10d_headline_reproduction"][
        s10d_tables["s10d_headline_reproduction"]["quantity"] == "S10d constrained_template_fit resolvable delay ns"
    ].iloc[0]
    ml_delay_gate = s10d_tables["s10d_headline_reproduction"][
        s10d_tables["s10d_headline_reproduction"]["quantity"] == "S10d compact_mlp_classifier_regressor resolvable delay ns"
    ].iloc[0]
    result = {
        "study": STUDY,
        "ticket": TICKET,
        "worker": WORKER,
        "title": "real-candidate two-pulse validation",
        "reproduced": bool(
            repro["pass"].all()
            and s10d_tables["s10b_reproduction"]["pass"].all()
            and s10d_tables["s10d_headline_reproduction"]["pass"].all()
        ),
        "reproduction_gate": "S10 topology, S10b live-time, and S10d resolvability headlines reproduced from raw B-stack ROOT",
        ...
        "traditional": {
            "method": "bounded_two_pulse_template_fit",
            "metric": "matched_stratified_secondary_fraction_high_minus_low",
            "value": float(trad["value"]),
            "ci": [float(trad["ci_low"]), float(trad["ci_high"])],
            "s10d_resolvable_delay_threshold_ns": float(trad_delay_gate["reproduced"]),
        },
        "ml": {
            "method": "run_heldout_random_forest_residual_diagnostic",
            ...
            "mean_delay_mae_on_synthetic_holdout_ns": float(folds["synthetic_delay_mae_ns"].mean()),
            "s10d_resolvable_delay_threshold_ns": float(ml_delay_gate["reproduced"]),
        },
        "threshold_validation": threshold_validation.to_dict(orient="records"),
        "run_stability": run_stability.to_dict(orient="records"),
        "leakage_flags": int(leakage["flag"].sum()),
        "leakage_checks_pass": bool(~leakage["flag"].any()),
        "conclusion": conclusion,
        "next_tickets": [
            "S10f: hand-scan waveform gallery for high-current candidates above the S10d real-candidate threshold",
            "S10g: validate real-candidate two-pulse estimates with independent A-stack timing coincidences",
        ],
    ...
    write_report(topology, repro, s10d_tables, stratum_table, method_summary, stratum_summary, run_stability, threshold_validation, folds, leakage, result)
    manifest = {
        "study": STUDY,
        "ticket": TICKET,
        "worker": WORKER,
