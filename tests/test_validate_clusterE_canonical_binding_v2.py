...
def test_v2_identity_contract_validates(tmp_path: Path) -> None:
    ...
    with ledger.open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow(row("CL-013", current_value="92", unit="ADC/MeV", syst_unc="28", truth_type="data_mc_calibration_proxy", status="GATED"))
        writer.writerow(row("CL-021", current_value="68269.40598948313", truth_type="legacy_data_mc_profile_diagnostic", status="FLAWED"))
        writer.writerow(row("CL-022", current_value="0.003232254011764034", numerator="283", denominator="87555", truth_type="mc_truth_only", status="TRUTH_LEVEL_MC_ONLY"))
    text = "CL-013 92 ADC/MeV 28 ADC/MeV\nCL-021 68269.40598948313; rerun does not supersede CL-021\nCL-022 283/87555\n"
    ...
    with table.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.writer(handle)
        writer.writerow(["claim", "headline", "evidence_class", "status", "source", "figure", "claim_id"])
        writer.writerow(["ADC gain (data/MC proxy, MV0)", "92 ADC/MeV; 28 ADC/MeV", "x", "GATED", "x", "x", "CL-013"])
