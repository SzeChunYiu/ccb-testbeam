...
event keys as model features.  It reuses the G4-05C modeling utilities so the
...
METHODS = [
    ...
    ("mlp", "mlp", "nn"),
    ("cnn1d", "cnn1d", "nn"),
    ("shape_gated_cnn", "waveform_morphology_gated_cnn_new", "nn"),
]
...
def make_plots(out_dir: Path, per_run: pd.DataFrame, pooled: pd.DataFrame, shift: pd.DataFrame) -> None:
    ...
    ax.set_title("G4-05D frozen Sample-I model transfer")
    ax.legend(fontsize=7, ncol=2)
    fig.tight_layout()
    fig.savefig(out_dir / "fig_g4_05d_per_run_transfer.png", dpi=130)
    plt.close(fig)
    ...
    fig.tight_layout()
    fig.savefig(out_dir / "fig_g4_05d_pooled_winner.png", dpi=130)
    plt.close(fig)
    ...
    if len(target):
        ...
        ax.legend()
        fig.tight_layout()
        fig.savefig(out_dir / "fig_g4_05d_target_shift.png", dpi=130)
        plt.close(fig)
...
def write_report(
    ...
    lines = [
        ...
        "",
        "```bash",
        f"{sys.executable} scripts/g4_05d_1783775641_21314_24a82b5b_adjacent_s02_s03_transfer_guard.py --config {config_path}",
        "```",
        "",
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/g4_05d_1783775641_21314_24a82b5b_adjacent_s02_s03_transfer_guard.yaml")
