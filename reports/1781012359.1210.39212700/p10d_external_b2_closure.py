#!/usr/bin/env python3
"""P10d external B2/B4/B6/B8 timing closure from raw ROOT."""
...
def evaluate_methods(config: dict, pulses: pd.DataFrame, predictions: Dict[str, np.ndarray]) -> pd.DataFrame:
    work = pulses.copy()
    for name, pred in predictions.items():
        work[f"t_{name}_ns"] = work["t_cfd_ns"].to_numpy(dtype=float) - pred
    rows = []
...
def write_report(
    ...
    lines = [
        ...
        "",
        "P10b reference explicit downstream timing: "
        f"{config['p10b_reference']['traditional_timing_sigma68_ns']:.6g} ns "
        f"[{config['p10b_reference']['traditional_timing_ci_ns'][0]:.6g}, {config['p10b_reference']['traditional_timing_ci_ns'][1]:.6g}]. "
        "P10d does not reuse that fitted model; it retrains from raw all-hit events and evaluates an external B2-including closure.",
        "",
        "## Methods",
        "",
...
def main() -> int:
    ...
    parser.add_argument("--config", default="reports/1781012359.1210.39212700/p10d_config.json")
    ...
    run_df = evaluate_methods(config, pulses, predictions)
    ...
    result = {
        ...
        "next_tickets": [
            "P10e: fit an explicit correction with B2 included as a train target and compare B2-held-out versus B2-trained external closure.",
            "P10f: repeat P10d with forced equal all-hit event counts per held-out run to separate run weighting from high-current topology effects.",
        ],
    }
    (out_dir / "result.json").write_text(json.dumps(result, indent=2), encoding="utf-8")
    write_report(out_dir, config, repro, run_df, summary, trad_best, ml_best, leakage, result)
