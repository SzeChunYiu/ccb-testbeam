...
- **Adversarial hardening:** review of the first implementation found a post-hash gap for an in-place same-inode/same-size file mutation. The current branch retains `mtime_ns`/`ctime_ns` in the opened-file record and requires final path metadata to match the hashed object, with an injected mutation regression.
- **Executed local falsifier:** `python -m pytest -q tests/test_geant4_runtime_dependency_attestation.py` -> `9 passed in 0.07s`, no RNG. Includes a real Linux `/proc` child-process round trip in addition to synthetic hostile fixtures.
...
- **Status:** `ACTIVE / PR_1204_OPEN / LOCAL_DETERMINISTIC_FALSIFIERS_PASS_9 / EXACT_HEAD_CI_PENDING / REAL_HIBEAM_RUNTIME_BLOCKED / DETECTOR_INFERENCE_BLOCKED`
