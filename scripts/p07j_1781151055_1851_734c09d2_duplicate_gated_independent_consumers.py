...
P07f count/family anchors, forms the duplicate-readout gate before hiding the odd channel,
and propagates the resulting correction to independent q_template and timing consumers.
It benchmarks a transparent per-run duplicate-knee policy against ridge, gradient-boosted
trees, MLP, a 1D CNN, and a residual gated CNN. All supervised methods are leave-one-run-out
and exclude run/event identifiers plus all odd-channel variables.
"""
...
DEFAULT_CONFIG = ROOT / "configs/p07j_1781151055_1851_734c09d2_duplicate_gated_independent_consumers.json"
...
def markdown_table(df: pd.DataFrame, index: bool = False) -> str:
    """Render a compact GitHub markdown table without pandas' optional tabulate dependency."""
    view = df.copy()
    if index:
    ...
    for _, row in view.iterrows():
        lines.append("| " + " | ".join(esc(row[c]) for c in view.columns) + " |")
    return "\n".join(lines)
...
def write_report(
    ...
    lines = [
        ...
        f"**Date (UTC):** {time.strftime('%Y-%m-%d', time.gmtime())}  ",
        ...
        "",
        markdown_table(reproduction, index=False),
        "",
        ...
        "",
        markdown_table(family_summary, index=False),
        "",
        ...
        "",
        markdown_table(calib_summary, index=False),
        "",
        ...
        "",
        markdown_table(summary[bench_cols], index=False),
        "",
        "ML/NN minus traditional deltas on the same run-bootstrap point estimates:",
        "",
        markdown_table(delta_table, index=False),
        "",
        f"Winner by side-effect-gated utility is **{winner['method']}**. The transparent traditional action-band policy has support fraction {trad['action_support_fraction']:.4f}, correction fraction {trad['correct_fraction']:.4f}, veto fraction {trad['veto_fraction']:.4f}, and harm rate {trad['harm_rate_vs_no_correction']:.4f}; the winner has support fraction {winner['action_support_fraction']:.4f}, correction fraction {winner['correct_fraction']:.4f}, and harm rate {winner['harm_rate_vs_no_correction']:.4f}.",
        "",
        "## 5. Falsification",
        "",
        ...
        "",
        markdown_table(leakage.describe(include="all").transpose(), index=True),
        "",
        ...
        "",
        result["next_tickets"][0]["title"] + " -- " + result["next_tickets"][0]["body"],
        "",
        "## 9. Reproducibility",
        "",
        "```bash",
...
def main() -> int:
    ...
    finding = (
        "The configured side-effect-gated utility ranks {} first, with action support "
        "{:.4f} and correction fraction {:.4f}; accepted/corrected fraction "
        "{:.4f} [{:.4f}, {:.4f}], charge res68 {:.4f} [{:.4f}, {:.4f}], "
        "timing-tail delta {:+.4f}, q_template median shift {:+.4f}, and harm rate {:.4f}. "
        "That ranking is not accepted as an automatic production recommendation because label purity "
        "and side-effect risk matter: precision is {:.4f}. "
        "The transparent duplicate-run-family action policy is the conservative deployment recommendation "
        "because it has much lower harm ({:.4f}) and directly enforces odd-readout knee support, "
        "although its correction fraction is smaller ({:.4f}). Waveform-only ML therefore does not "
        "justify replacing the duplicate-readout gate for production natural B2 deployment."
    ).format(
        winner["method"],
        winner["action_support_fraction"],
        winner["correct_fraction"],
        winner["accepted_fraction"],
        winner["accepted_fraction_ci_low"],
        winner["accepted_fraction_ci_high"],
        winner["charge_res68"],
    ...
    result = {
        ...
        "next_tickets": [
            {
                "title": "P07l blinded downstream-energy closure for duplicate-gated B2 corrections",
                "body": "Freeze the P07j/P07k duplicate-gated correction from this independent-consumer benchmark and apply it to a fully blinded energy/PID summary table with no odd-channel or duplicate-residual columns available after gate formation. Expected information gain: separates waveform/timing/q_template side-effect safety from final physics-energy stability before production adoption."
            }
        ],
        "runtime_sec": None,
    }
    ...
    write_report(out, result, reproduction, knees, summary, calibration, leakage)
    manifest = {
        "ticket": cfg["ticket"],
        "study": cfg["study"],
