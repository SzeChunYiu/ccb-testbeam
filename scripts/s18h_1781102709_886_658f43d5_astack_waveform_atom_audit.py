#!/usr/bin/env python3
"""S18h: audit which A-stack waveform atom drives late/mixed ML transfer."""
...
os.environ.setdefault("MPLCONFIGDIR", "reports/1781102709.886.658f43d5__s18h_astack_waveform_atom_audit/.mplconfig")
...
def atom_feature_matrix(df: pd.DataFrame, mode: str, rng: np.random.Generator) -> np.ndarray:
    """Return an S18h atom-ablation feature matrix using the S18g feature order."""
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/s18h_1781102709_886_658f43d5_astack_waveform_atom_audit.json"))
    ...
    result = {
        ...
        "atom_audit": {
            ...
            "modes": ATOM_MODES,
            "interpretation": "Compare full, leading-edge-dropped, tail-dropped, amplitude-only, and waveform-shuffled controls on the same Sample III to Sample IV split.",
        },
        "input_sha256": str(out_dir / "input_sha256.csv"),
        "git_commit": git_head(),
        "next_tickets": [
            "S18k: fit a fixed-efficiency A-stack amplitude gate with per-run quantiles to separate pulse-selection support loss from CFD interpolation noise in late/mixed transfer."
        ],
    }
    ...
    with (out_dir / "REPORT.md").open("a", encoding="utf-8") as handle:
        handle.write(
            ...
            "This ticket-specific audit asks which waveform atom makes late/mixed Sample-III to Sample-IV A1-A3 transfer look narrow. "
