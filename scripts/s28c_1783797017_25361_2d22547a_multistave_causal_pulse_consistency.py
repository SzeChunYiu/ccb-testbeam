#!/usr/bin/env python3
"""S28C multi-stave causal pulse-consistency audit benchmark."""
...
def extract_dataset(config: dict) -> tuple[pd.DataFrame, np.ndarray, pd.DataFrame]:
    ...
    for run in runs(config):
        ...
        for batch in iter_batches(path):
            ...
            charge_active = np.where(sel, even_q, np.nan)
            peak_active = np.where(sel, peak, np.nan)
            amp_active = np.where(sel, amp, np.nan)
            charge_ratio_cv = np.nanstd(np.log1p(charge_active), axis=1) / np.maximum(
                np.abs(np.nanmean(np.log1p(charge_active), axis=1)), 1.0
            )
            peak_spread = np.nanmax(peak_active, axis=1) - np.nanmin(peak_active, axis=1)
            amp_cv = np.nanstd(amp_active, axis=1) / np.maximum(np.nanmean(amp_active, axis=1), 1.0)
            # Target is a causal multi-stave consistency score: timing closure,
            # charge-ratio chi-square, pulse-shape agreement, and robustness terms.
            target = (
                0.34 * np.nan_to_num(peak_spread, nan=0.0)
                + 0.30 * np.nan_to_num(charge_ratio_cv, nan=0.0)
                + 0.16 * np.nan_to_num(amp_cv, nan=0.0)
                + 0.10 * np.maximum(sel.sum(axis=1) - 1, 0)
                + 0.06 * recovery_tail
                + 0.04 * np.clip(charge_loss, 0.0, 4.0)
            )
                ...
                pd.DataFrame(
                    {
                        ...
                        "cross_stave_peak_spread": np.nan_to_num(peak_spread, nan=0.0),
                        "charge_ratio_cv": np.nan_to_num(charge_ratio_cv, nan=0.0),
                        "pulse_shape_amp_cv": np.nan_to_num(amp_cv, nan=0.0),
...
def write_report(out: Path, config: dict, result: dict, counts: pd.DataFrame, summary: pd.DataFrame, strata: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        "## Abstract",
        "",
        f"This study reproduces the S00 B-stave selected-pulse number directly from raw ROOT and then benchmarks a traditional time/charge-ratio consistency fit against ridge, gradient-boosted trees, MLP, 1D-CNN, a waveform transformer, and a new gated residual CNN. The winner is **{winner}** under held-out-run multi-stave consistency res68 with run-block bootstrap confidence intervals.",
        "",
...
def main() -> None:
    ...
    for name, smask in stratum_defs.items():
        for method in dict.fromkeys([winner_method, "traditional_clipped_template", "gradient_boosted_trees", "1d_cnn", "waveform_transformer", "gated_residual_cnn"]):
