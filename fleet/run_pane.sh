...
if [ "$arg" = planner ]; then
  ...
  GOAL="/goal You are $label, an autonomous data-driven ccb-testbeam worker (190 MeV p on CD2, HRD scintillator stacks, no Monte Carlo). Do ONE ticket end to end: run \"tn-ticket claim $label --project testbeam\" (if it says queue empty, stop). The printed body is your study. Reproduce its number from the raw ROOT under the data folder FIRST, then do a strong traditional method AND an ML method, split BY RUN with held-out bootstrap CIs, and hunt for leakage if a result looks too good. Write a concise REPORT.md plus result.json and a manifest (input sha256) into a reports subfolder named by the ticket id. Run \"tn-ticket done <id>\". Then make a git branch, commit ONLY your report files and any name-spaced s or p scripts and configs, push, and open a PR with gh. Append 1-2 follow-up tickets with tn-ticket. Then stop."
fi
