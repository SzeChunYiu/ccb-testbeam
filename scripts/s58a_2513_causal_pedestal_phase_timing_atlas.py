#!/usr/bin/env python3
"""S58a/#2513 causal pedestal-phase timing benchmark."""
...
CONFIG_DEFAULT = "configs/s58a_2513_causal_pedestal_phase_timing_atlas.json"
...
S16M = load_module("s16m_for_s58a", S16M_PATH)
...
def add_shape_features(pairs: pd.DataFrame, waves: np.ndarray) -> pd.DataFrame:
    ...
    pedestal_score = out["pre_rms4_max"] + 0.05 * out["pre_ptp4_max"] + 0.01 * out["nuisance_abs_max_adc"]
    pedestal_edges = np.unique(np.nanquantile(pedestal_score.to_numpy(float), [1.0 / 3.0, 2.0 / 3.0]))
    ped_bin = np.searchsorted(pedestal_edges, pedestal_score.to_numpy(float), side="right")
    out["pedestal_state"] = np.asarray(["quiet_pedestal", "moderate_pedestal", "active_pedestal"], dtype=object)[np.clip(ped_bin, 0, 2)]
    pileup_score = np.maximum(out["late_tail_fraction_mean"].to_numpy(float), out["half_width_samples_mean"].to_numpy(float) / 18.0)
    pileup_score += 0.5 * np.maximum(out["late_tail_fraction_diff"].abs().to_numpy(float), 0.0)
    out["pileup_state"] = np.where(pileup_score >= np.nanquantile(pileup_score, 0.75), "mild_pileup", "single_like")
    phase_score = np.abs(out["delta_peak"].to_numpy(float)) + 2.0 * np.abs(out["leading_curvature_diff"].to_numpy(float))
    out["phase_state"] = np.where(phase_score >= np.nanquantile(phase_score, 0.75), "phase_shifted", "phase_nominal")
...
def fit_torch(train: pd.DataFrame, test: pd.DataFrame, waves: np.ndarray, cols: Sequence[str], config: dict, kind: str) -> np.ndarray:
    ...
    elif kind == "causal_pedestal_phase_cnn_new":
        gate_index = [
            i
            for i, c in enumerate(cols)
            if "rise" in c or "tail" in c or "curvature" in c or "pre_" in c or "nuisance" in c or "peak" in c
        ]
...
def fit_fold_models(pairs: pd.DataFrame, waves: np.ndarray, config: dict) -> pd.DataFrame:
    ...
    for run in sorted(pairs["run"].unique()):
        ...
        for method in ["one_dimensional_cnn", "compact_pair_transformer", "causal_pedestal_phase_cnn_new"]:
...
def summarize_metrics(scored: pd.DataFrame) -> pd.DataFrame:
    ...
    for method, group in scored.groupby("method"):
        residual = group["corrected_residual_ns"].to_numpy(dtype=float)
        finite = np.isfinite(residual)
        phase_means = group.assign(_finite_residual=np.where(finite, residual, np.nan)).groupby("phase_state")["_finite_residual"].mean()
        phase_gap = float(abs(phase_means.max() - phase_means.min())) if len(phase_means.dropna()) >= 2 else float("nan")
        rows.append(
            {
                "method": method,
                **S16M.metric_dict(residual),
                "failure_rate": float(np.mean(~finite)) if len(finite) else float("nan"),
                "phase_shape_residual_gap_ns": phase_gap,
            }
        )
...
def bootstrap_summary(scored: pd.DataFrame, reps: int, seed: int) -> pd.DataFrame:
    ...
    samples: Dict[str, Dict[str, List[float]]] = {
        method: {
            ...
            "bias_ns": [],
            "failure_rate": [],
            "phase_shape_residual_gap_ns": [],
            "delta_sigma68_vs_traditional_ns": [],
        }
    ...
    for _ in range(int(reps)):
        ...
        for method in methods:
            ...
            residual = sample["corrected_residual_ns"].to_numpy(dtype=float)
            samples[method]["failure_rate"].append(float(np.mean(~np.isfinite(residual))) if len(residual) else float("nan"))
            finite = np.isfinite(residual)
            phase_means = sample.assign(_finite_residual=np.where(finite, residual, np.nan)).groupby("phase_state")["_finite_residual"].mean()
            phase_gap = float(abs(phase_means.max() - phase_means.min())) if len(phase_means.dropna()) >= 2 else float("nan")
            samples[method]["phase_shape_residual_gap_ns"].append(phase_gap)
...
def stratified_metrics(scored: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for strat in ["pid_proxy", "near_threshold_energy", "pedestal_state", "pileup_state", "phase_state"]:
...
def make_plots(metrics: pd.DataFrame, scored: pd.DataFrame, out_dir: Path) -> None:
    ...
    ax.set_title("#2513 run-held-out timing benchmark")
    fig.tight_layout()
...
def write_report(config: dict, result: dict, reproduction: pd.DataFrame, metrics: pd.DataFrame, per_run: pd.DataFrame, stratified: pd.DataFrame, out_dir: Path) -> None:
    ...
    report = f"""# S58a/#2513: Causal Pedestal-Phase Pulse-Shape Timing Atlas
...
{md_table(metrics, ["method", "n_pairs", "sigma68_ns", "sigma68_ns_ci_low", "sigma68_ns_ci_high", "tail_abs_gt_0p5_ns", "tail_abs_gt_0p5_ns_ci_low", "tail_abs_gt_0p5_ns_ci_high", "bias_ns", "bias_ns_ci_low", "bias_ns_ci_high", "failure_rate", "failure_rate_ci_low", "failure_rate_ci_high", "phase_shape_residual_gap_ns", "phase_shape_residual_gap_ns_ci_low", "phase_shape_residual_gap_ns_ci_high", "delta_sigma68_vs_traditional_ns_ci_low", "delta_sigma68_vs_traditional_ns_ci_high"])}

## Per-Run Stability

{md_table(per_run, ["method", "run", "n_pairs", "sigma68_ns", "tail_abs_gt_0p5_ns", "bias_ns", "failure_rate", "phase_shape_residual_gap_ns"])}
...
The winner named in `result.json` is `{winner}` by the registered rule: lowest held-out `sigma68_ns` among correction methods, with tail fraction and absolute bias as tie breakers. No more than one novel follow-up ticket is proposed: external trigger validation for this same causal pedestal-phase method panel.
"""
...
def main() -> int:
    ...
    claimed_text = (
        ...
        "claim_helper_stderr:\nnull\n"
        "claim_helper_stdout:\n# null\n\nnull\n"
        "manual_claim_issue: 2513\n"
        "manual_claim_command: gh issue edit 2513 --repo SzeChunYiu/factory-tickets --add-label factory:claimed --add-label worker:testbeam-laptop-3 --remove-label factory:open\n"
        "#2513 NEW S58a causal pedestal-phase pulse-shape timing atlas\n"
        "Question: Can a causal pedestal-state plus phase model separate pulse-shape drift from timing bias under mild pile-up?\n\n"
        "Compare traditional generalized least-squares matched-template/CFD fits against ridge, gradient-boosted trees, MLP, 1D-CNN, and a small waveform transformer. Use run-heldout splits, pedestal strata, pile-up strata, and bootstrap CIs for timing bias, sigma68, shape residuals, and failure rates.\n\n"
        "Deliverable: a compact scoreboard plus calibration plots identifying where pedestal memory changes pulse shape versus timing pickoff.\n"
    )
    ...
    result = {
        ...
        "done_command": "tn-ticket done 2513",
    }
