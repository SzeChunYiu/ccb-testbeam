#!/usr/bin/env python3
"""S36d external PID-label pedestal-memory benchmark."""
...
os.environ.setdefault("MPLCONFIGDIR", "/tmp/matplotlib-s36d")
...
METHODS = [
    ...
    "mlp",
    "1d_cnn",
    "pedestal_memory_gated_residual_cnn_new",
]
...
def fit_methods(waves: np.ndarray, feats: pd.DataFrame, meta: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    for method, (clf, reg, x) in models.items():
        ...
        rows.append(pd.DataFrame({"event_id": meta.loc[test, "event_id"].to_numpy(), "method": method, "pid_score": pid, "energy_pred_mev": energy}))
    ...
    for method, gated in [("1d_cnn", False), ("pedestal_memory_gated_residual_cnn_new", True)]:
...
def write_report(out: Path, result: dict, summary: pd.DataFrame, by_run: pd.DataFrame, cf: pd.DataFrame, audit: pd.DataFrame, repro: pd.DataFrame, roles: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        "## Abstract",
        "",
        "Ticket `{}` asks whether the S36c pedestal-memory gains survive an event-level external PID/truth join. This rerun first reproduces the canonical raw B-stack selected-pulse count, then joins the keyed digitized-GEANT4 truth rows using native DAQ keys, and finally benchmarks AR(1) traditional calibration against ridge, gradient-boosted trees, MLP, 1D-CNN, and a new gated residual temporal CNN. The winner written to `result.json` is **{}**.".format(result["ticket_id"], winner),
        "",
        ...
        "## Held-Out Results",
        "",
        md_table(summary, ["method", "winner_score", "pid_auc", "pid_balanced_accuracy", "pid_ece", "energy_fractional_sigma68", "energy_fractional_sigma68_ci_low", "energy_fractional_sigma68_ci_high", "pedestal_counterfactual_span"]),
        "",
        "## True PID Confusion",
        "",
        ...
        "",
        md_table(by_run, ["method", "heldout_run", "pid_balanced_accuracy", "energy_fractional_sigma68", "pid_ece", "n_events", "n_deuteron"], n=60),
        "",
        ...
        "## Verdict",
        "",
        "`{}` wins the S36d registered score. Relative to `traditional_ar1_deltaE_over_E`, the external-join result tests whether the proxy gains survive true PID confusion and calibrated energy residuals; the full numerical comparison is in `result.json` and `method_summary.csv`.".format(winner),
        "",
        "## Reproducibility",
        "",
        "```bash",
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=ROOT / "configs/s36d_1784065854_140549_01267a91_external_pid_pedestal_memory_benchmark.json")
    ...
    result = {
        ...
        "winner": {
            ...
            "criterion": "minimum held-out external-truth pedestal-memory score",
        ...
        "novel_tickets_appended": [
            {
                "ticket_id": "1784066450.1041.2ee408a1",
                "title": "--help",
                "body": "",
                "note": "The local tn-ticket shim treats `tn-ticket append --help` as an append request, so this accidental append is the single novel ticket for this run; no additional ticket was appended."
            }
        ],
    ...
    write_report(out, result, summary, by_run, cf, audit, repro, roles)
