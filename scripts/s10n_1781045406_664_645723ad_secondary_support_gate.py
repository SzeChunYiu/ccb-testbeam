...
This ticket extends the S11b real-current two-pulse diagnostic.  It keeps the
same raw ROOT reproduction and bounded two-pulse traditional method, then asks
whether the high-current secondary-fraction excess is stable under threshold
and support choices and whether several run-held-out ML/NN methods explain the
secondary-fraction versus overlap-score disagreement.
"""
...
OUT = ROOT / "reports" / "1781045406.664.645723ad__s10n_high_stat_secondary_support_stability_gate"
...
RNG_SEED = 1005406664
BOOTSTRAPS = 700
SYNTHETIC_TRAIN_PER_FOLD = 1800
SYNTHETIC_CAL_PER_FOLD = 600
...
def write_report(
    ...
    lines = [
        ...
        "",
        (
            "The S10n question is whether the S10e/S11b high-current two-pulse-like excess is a stable "
            "measured waveform support effect or a threshold/model artifact. The raw ROOT reproduction gate passes, "
...
def main() -> int:
    ...
    result = {
        ...
        "ticket": TICKET,
        "worker": WORKER,
        "title": "S10n high-stat secondary support stability gate",
    ...
    write_report(topology, repro, stratum_table, method_summary, deltas, stability, stability_diag, folds, leakage, winner, result)
    manifest = {
        "study": STUDY,
        "ticket": TICKET,
        "worker": WORKER,
