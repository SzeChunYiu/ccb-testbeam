#!/usr/bin/env python3
"""S43a: trapezoid/template timing versus ML and neural pulse-shape models."""
...
def write_report(out_dir: Path, result: dict, summary: pd.DataFrame, strata: pd.DataFrame, training: pd.DataFrame, leakage: pd.DataFrame) -> None:
    ...
    report = f"""# S43a: trapezoid-template versus neural pulse-shape timing phase study
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/s43a_1784183456_689_45cf43e9_trapezoid_neural_timing_phase.json"))
    ...
    result = {
        ...
        "new_architecture": "shape_residual_fusion",
