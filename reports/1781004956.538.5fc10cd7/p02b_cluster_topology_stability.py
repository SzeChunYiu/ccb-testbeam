#!/usr/bin/env python3
"""P02b: cluster topology stability across runs and staves.
...
held-out folds.  No Monte Carlo inputs are used.
"""
...
os.environ.setdefault("MPLCONFIGDIR", "/tmp/ccb-testbeam-p02b-mpl")
...
TICKET_ID = "1781004956.538.5fc10cd7"
STUDY_ID = "P02b"
TITLE = "cluster topology stability across runs and staves"
        ...
        for batch in iter_tree(path):
            ...
            if len(event_idx):
                ...
                meta_parts.append(
                    pd.DataFrame(
                        {
                            "row_id": np.arange(len(event_idx), dtype=np.int64) + global_event,
...
def q_bins(train_q: np.ndarray, test_q: np.ndarray) -> np.ndarray:
    edges = np.quantile(train_q, [0.25, 0.50, 0.75])
    labels = np.asarray(["q_best", "q_midgood", "q_midpoor", "q_poor"], dtype=object)
    return labels[np.digitize(test_q, edges)]
...
def fold_predictions(
    ...
    for fold, (train_idx, test_idx) in enumerate(splits):
        ...
        train_q, test_q = q_template_scores(
            waves[train_idx],
            meta.iloc[train_idx]["amplitude_adc"].to_numpy(),
            meta.iloc[train_idx]["stave"].to_numpy(),
            waves[test_idx],
            meta.iloc[test_idx]["amplitude_adc"].to_numpy(),
            meta.iloc[test_idx]["stave"].to_numpy(),
        )
            ...
            pd.DataFrame(
                {
                    ...
                    "ml_cluster": ml_pred,
                    "q_template_bin": q_bins(train_q, test_q),
...
def metric_rows(pred: pd.DataFrame, split: str) -> pd.DataFrame:
    ...
    targets = ["q_template_bin", "peak_group", "downstream_topology", "dt_label", "manual_flag"]
...
def leakage_checks(pred: pd.DataFrame, sampled_meta: pd.DataFrame) -> pd.DataFrame:
    rows: List[dict] = []
    for method, col in [("traditional", "traditional_cluster"), ("ml_autoencoder", "ml_cluster")]:
        rows.append({"check": "cluster_vs_run", "method": method, "ami": safe_ami(sampled_meta["run"].astype(str).to_numpy(), pred[col].astype(str).to_numpy()), "purity": purity_score(sampled_meta["run"].astype(str).to_numpy(), pred[col].astype(str).to_numpy())})
        rows.append({"check": "cluster_vs_stave", "method": method, "ami": safe_ami(sampled_meta["stave"].astype(str).to_numpy(), pred[col].astype(str).to_numpy()), "purity": purity_score(sampled_meta["stave"].astype(str).to_numpy(), pred[col].astype(str).to_numpy())})
    return pd.DataFrame(rows)
...
def write_report(result: dict, run_metrics: pd.DataFrame, stave_metrics: pd.DataFrame, ci: pd.DataFrame, leak: pd.DataFrame) -> None:
    ...
    delta = ci[(ci["split"] == "run") & (ci["method"] == "delta") & (ci["target"] == "manual_flag") & (ci["metric"] == "ami")].iloc[0]
    lines = [
        f"# Study report: {STUDY_ID} - {TITLE}",
        "",
        f"**Ticket:** {TICKET_ID}",
        f"**Command:** `/home/billy/anaconda3/bin/python reports/{TICKET_ID}/p02b_cluster_topology_stability.py`",
        "",
        "## Reproduction first",
        ...
        "",
        "## Methods",
        f"All modeling used a run/stave-balanced subsample of **{result['analysis_sample_rows']:,}** selected pulses from the reproduced raw population. The primary split is GroupKFold by run; a leave-one-stave-held-out stress test checks stave sampling artifacts. Hyperparameters (`n_clusters`) are selected by training-fold BIC only.",
        "",
    ...
    for split in ["run", "stave"]:
        for method in ["traditional", "ml_autoencoder"]:
            lines.append(
                f"| {split} | {method} | {metric_line(split, method, 'q_template_bin')} | {metric_line(split, method, 'peak_group')} | {metric_line(split, method, 'downstream_topology')} | {metric_line(split, method, 'dt_label')} | {metric_line(split, method, 'manual_flag')} |"
            )
    lines.extend(
        [
            "",
            ...
            f"The paired run-bootstrap CI for ML minus traditional on manual-flag AMI is **[{delta['ci_low']:.3f}, {delta['ci_high']:.3f}]**. A large positive result would have triggered a run/stave leakage alarm; the observed deltas are treated as morphology diagnostics, not truth labels.",
            "",
            "## Verdict",
            "",
...
def main() -> None:
    ...
    fig, ax = plt.subplots(figsize=(8, 4))
    plot_data = run_metrics[run_metrics["target"].isin(["manual_flag", "q_template_bin", "downstream_topology"])]
    ...
    result = {
        ...
        "follow_up_tickets": [
            "P02c: replace diagnostic q-template bins with the full S01 q_template_per_pulse table and repeat run/stave-held-out cluster stability.",
    ...
    manifest = {
        "ticket_id": TICKET_ID,
        "command": f"/home/billy/anaconda3/bin/python reports/{TICKET_ID}/p02b_cluster_topology_stability.py",
        "git_commit": git_commit(),
        "python": platform.python_version(),
        "platform": platform.platform(),
        "random_seed": RANDOM_SEED,
        "input_sha256": raw_hashes,
