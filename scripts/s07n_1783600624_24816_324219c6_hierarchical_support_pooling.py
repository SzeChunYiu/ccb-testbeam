...
the label is injected truth rather than a D_t threshold. The S07n question is
whether operating thresholds can be calibrated inside matched clean support
strata while retaining the injected-morphology discrimination win.
"""
...
def support_bin_frame(data: pd.DataFrame, y: np.ndarray, runs: np.ndarray, config: dict) -> pd.DataFrame:
    amp_cols = [c for c in data.columns if c.endswith("_log_amp")]
    out = pd.DataFrame(index=data.index)
    out["run"] = runs
    out["label"] = y
    out["amp_proxy"] = data[amp_cols].to_numpy(dtype=float).mean(axis=1)
    out["topology_bin"] = data["n_downstream"].to_numpy(dtype=int)
    out["baseline_proxy"] = data["ds_shape_mean_final_fraction"].to_numpy(dtype=float)
    out["amp_bin"] = -1
    out["baseline_bin"] = -1
    bins = int(config["match_quantile_bins"])
    for held_run in sorted(np.unique(runs)):
        train_clean = (runs != held_run) & (y == 0)
        test = runs == held_run
        for col, bcol in [("amp_proxy", "amp_bin"), ("baseline_proxy", "baseline_bin")]:
            values = out.loc[train_clean, col].to_numpy(dtype=float)
            edges = np.unique(np.nanquantile(values[np.isfinite(values)], np.linspace(0.0, 1.0, bins + 1)))
            if len(edges) <= 2:
                out.loc[test, bcol] = 0
                out.loc[train_clean, bcol] = 0
            else:
                out.loc[test, bcol] = np.clip(np.searchsorted(edges[1:-1], out.loc[test, col], side="right"), 0, bins - 1)
                out.loc[train_clean, bcol] = np.clip(np.searchsorted(edges[1:-1], out.loc[train_clean, col], side="right"), 0, bins - 1)
    return out


def hierarchical_pool_for_row(bins: pd.DataFrame, train_clean: np.ndarray, held_run: int, row_idx: int, config: dict) -> Tuple[np.ndarray, str]:
    min_clean = int(config["hierarchical_min_train_clean"])
    run_window = int(config["hierarchical_adjacent_run_window"])
    row = bins.loc[row_idx]
    candidates = [
        (
            "adjacent_run_amp_topology_baseline",
            train_clean
            & (np.abs(bins["run"].to_numpy(dtype=int) - held_run) <= run_window)
            & (bins["amp_bin"].to_numpy(dtype=int) == int(row["amp_bin"]))
            & (bins["topology_bin"].to_numpy(dtype=int) == int(row["topology_bin"]))
            & (bins["baseline_bin"].to_numpy(dtype=int) == int(row["baseline_bin"])),
        ),
        (
            "any_run_amp_topology_baseline",
            train_clean
            & (bins["amp_bin"].to_numpy(dtype=int) == int(row["amp_bin"]))
            & (bins["topology_bin"].to_numpy(dtype=int) == int(row["topology_bin"]))
            & (bins["baseline_bin"].to_numpy(dtype=int) == int(row["baseline_bin"])),
        ...
        ("amp_only", train_clean & (bins["amp_bin"].to_numpy(dtype=int) == int(row["amp_bin"]))),
        ("global_clean", train_clean),
    ]
    for level, mask in candidates:
        if int(mask.sum()) >= min_clean or level == "global_clean":
            return mask, level
    return train_clean, "global_clean"


def hierarchical_support_pooling_rows(data: pd.DataFrame, y: np.ndarray, runs: np.ndarray, score_map: Dict[str, np.ndarray], config: dict) -> Tuple[pd.DataFrame, pd.DataFrame]:
    bins = support_bin_frame(data, y, runs, config)
    q = float(config["fixed_clean_efficiency"])
    row_records = []
    summary_records = []
    for method, score in score_map.items():
        decisions = np.zeros(len(data), dtype=bool)
        thresholds = np.full(len(data), np.nan, dtype=float)
        levels: List[str] = [""] * len(data)
        pool_sizes = np.zeros(len(data), dtype=int)
        exact_possible = np.zeros(len(data), dtype=bool)
        for held_run in sorted(np.unique(runs)):
            ...
            for idx in test_idx:
                exact = (
                    train_clean
                    & (bins["amp_bin"].to_numpy(dtype=int) == int(bins.loc[idx, "amp_bin"]))
                    & (bins["topology_bin"].to_numpy(dtype=int) == int(bins.loc[idx, "topology_bin"]))
                    & (bins["baseline_bin"].to_numpy(dtype=int) == int(bins.loc[idx, "baseline_bin"]))
                )
                exact_possible[idx] = int(exact.sum()) >= int(config["hierarchical_min_train_clean"])
                pool, level = hierarchical_pool_for_row(bins, train_clean, int(held_run), int(idx), config)
                threshold = float(np.quantile(score[pool], q))
                thresholds[idx] = threshold
                levels[idx] = level
                pool_sizes[idx] = int(pool.sum())
                decisions[idx] = bool(score[idx] > threshold) if math.isfinite(score[idx]) else False

        for level, group_idx in pd.Series(np.arange(len(data))).groupby(levels):
            idx = group_idx.to_numpy(dtype=int)
            if len(idx) == 0:
                continue
            clean = idx[y[idx] == 0]
            injected = idx[y[idx] == 1]
            summary_records.append(
                {
                    ...
                    "mean_pool_clean": float(pool_sizes[idx].mean()),
                    "clean_false_positive_rate": float(decisions[clean].mean()) if len(clean) else float("nan"),
                    "injected_rejection": float(decisions[injected].mean()) if len(injected) else float("nan"),
                    "exact_stratum_available_fraction": float(exact_possible[idx].mean()),
                }
            )
        all_clean = y == 0
        all_inj = y == 1
        summary_records.append(
            {
                "method": method,
                "pool_level": "all_hierarchical",
                "n_rows": int(len(data)),
                "n_clean": int(all_clean.sum()),
                "n_injected": int(all_inj.sum()),
                "mean_pool_clean": float(pool_sizes.mean()),
                "clean_false_positive_rate": float(decisions[all_clean].mean()),
                "injected_rejection": float(decisions[all_inj].mean()),
                "exact_stratum_available_fraction": float(exact_possible.mean()),
            }
        )
        for i in range(len(data)):
            row_records.append(
                {
                    "row_id": data.iloc[i]["row_id"],
                    "method": method,
                    "run": int(runs[i]),
                    "label_injected": int(y[i]),
                    "score": float(score[i]) if math.isfinite(score[i]) else float("nan"),
                    "threshold": float(thresholds[i]) if math.isfinite(thresholds[i]) else float("nan"),
                    "veto": bool(decisions[i]),
                    "pool_level": levels[i],
                    "pool_clean": int(pool_sizes[i]),
                    "exact_stratum_available": bool(exact_possible[i]),
                }
            )
    return pd.DataFrame(row_records), pd.DataFrame(summary_records)
...
def write_report(
    ...
    text = f"""# S07n: hierarchical support-pooling calibration for injected morphology
...
{markdown_table(pooling_summary)}
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/s07n_1783600624_24816_324219c6_hierarchical_support_pooling.json")
    ...
    pooling_rows, pooling_summary = hierarchical_support_pooling_rows(data, y, runs, score_map, config)
    ...
    pooling_rows.to_csv(out_dir / "hierarchical_pooling_by_row.csv", index=False)
    pooling_summary.to_csv(out_dir / "hierarchical_pooling_summary.csv", index=False)
    ...
    result = {
        ...
        "hierarchical_support_pooling": pooling_summary.to_dict(orient="records"),
        "falsification": {
        ...
        "critic": "pending",
        "next_tickets": [
            "S07o: validate the S07n hierarchical support-pooling gate on non-injected real-current control windows with blinded manual waveform adjudication."
        ],
    }
    (out_dir / "result.json").write_text(json.dumps(json_ready(result), indent=2), encoding="utf-8")
    write_report(out_dir, config, reproduction, counts, scoreboard, op_summary, support, pooling_summary, leakage, pd.concat(cv_frames, ignore_index=True), result)
    ...
    manifest = {
        ...
        "command": f"scripts/s07n_1783600624_24816_324219c6_hierarchical_support_pooling.py --config {config_path}",
