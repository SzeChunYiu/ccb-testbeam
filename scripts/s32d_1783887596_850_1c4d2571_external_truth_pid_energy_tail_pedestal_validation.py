...
This runner repeats the S32c model panel, but replaces waveform-proxy PID and
energy targets with keyed digitized GEANT4 truth: dominant Sci_bar proton vs
deuteron PID and total B-stack deposited energy.  It independently reproduces
...
def write_report(out: Path, result: dict, summary: pd.DataFrame, joint: pd.DataFrame, calibration: pd.DataFrame, paired: pd.DataFrame, strata: pd.DataFrame, leakage: pd.DataFrame, feature_roles: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        "## Abstract",
        "",
        f"S32d repeats the S32c method panel with independent GEANT4 truth labels for particle species and calibrated deposited energy. The raw B-stack selected-pulse reproduction gate is **{result['reproduction']['selected_pulses']:,}**, matching the registered **{result['reproduction']['expected_selected_pulses']:,}** pulses exactly. Across the run-held-out and external particle-family-held-out splits, `result.json` names **{winner}** as the minimum mean joint-loss method.",
        "",
        ...
        "",
        f"Requested S32c held-out runs were `{result['split']['requested_heldout_runs']}`; the keyed G4-08 bridge contains `{result['split']['available_runs']}`, so the run-held-out test uses the available intersection `{result['split']['run_heldout_runs']}`. The particle-held-out split removes `{result['split']['particle_holdout_family']}` from training. Bootstrap CIs resample held-out run blocks with replacement using `{result['split']['bootstrap_replicates']}` replicates.",
        "",
        ...
        "## Verdict",
        "",
        f"`result.json` names **{winner}** as the winner. Relative to S32c, the key scientific change is that PID and energy targets are externally supplied by GEANT4 rather than defined from waveform thresholds; this substantially reduces self-referential proxy leakage while retaining explicit tail and pedestal stress tests.",
        "",
        "## Reproducibility",
        "",
        "```bash",
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=ROOT / "configs/s32d_1783887596_850_1c4d2571_external_truth_pid_energy_tail_pedestal_validation.json")
    ...
    result = {
        ...
        "winner": {
            ...
            "selection_rule": "minimum mean registered joint loss across available S32c-style run-heldout and external particle-family-heldout splits",
        },
        "winner_details": winner_row,
        "artifacts": {
