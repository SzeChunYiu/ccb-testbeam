#!/usr/bin/env python3
"""S07d injected timing-corruption App.I benchmark.

This report-local script first reproduces the App.I raw-ROOT D_t tail count from
S07b, then replaces the self-referential D_t label with a known injected
two-pulse corruption target. The benchmark is run-held-out throughout.
"""
...
def write_report(
    ...
    trad = scoreboard[scoreboard["method"] == "traditional fold-selected timing/template"].iloc[0]
    ...
    text = f"""# Study report: S07d - injected two-pulse timing-corruption target
...
## Verdict
The independent injected target removes the S07b self-reference problem. On this target, the fold-selected traditional timing/template score reaches ROC AUC {trad['roc_auc']:.3f} [{trad['roc_auc_ci_low']:.3f}, {trad['roc_auc_ci_high']:.3f}], while the shape-only RF reaches ROC AUC {rf['roc_auc']:.3f} [{rf['roc_auc_ci_low']:.3f}, {rf['roc_auc_ci_high']:.3f}]. The RF is therefore a sensitive waveform-shape detector for this injected two-pulse corruption, but the result should be interpreted as injection-recovery evidence, not as a measured beam pile-up rate.

## Reproducibility
Regenerate with:

```bash
uv run --with uproot --with numpy --with pandas --with scikit-learn --with matplotlib python {out_dir / 's07d_injected_timing_corruption.py'} --config {out_dir / 's07d_config.json'}
```
...
def main() -> int:
    ...
    parser.add_argument("--config", default="reports/1781006037.1502.693f2c73__s07d_injected_timing_corruption/s07d_config.json")
    ...
    trad_score, trad_fold, trad_choices, trad_candidates = traditional_oof(data, y, config)
    ...
    scoreboard = pd.DataFrame(
        [
            summarize_method(
                ...
                seed,
                n_boot,
                "Fold-local best signed score from D_t, |C_t|, downstream shape summaries, and matched-template residual.",
            ),
            summarize_method(
    ...
    result = {
        ...
        "traditional_auc": float(scoreboard.loc[scoreboard["method"] == "traditional fold-selected timing/template", "roc_auc"].iloc[0]),
        ...
        "rf_minus_traditional_auc": float(
            ...
            - scoreboard.loc[scoreboard["method"] == "traditional fold-selected timing/template", "roc_auc"].iloc[0]
        ),
    ...
    manifest = {
        ...
        "input_sha256": {str(raw_file(config, int(run))): sha256_file(raw_file(config, int(run))) for run in config["runs"]},
