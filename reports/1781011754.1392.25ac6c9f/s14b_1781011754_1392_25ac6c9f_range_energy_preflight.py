#!/usr/bin/env python3
"""S14b: range-energy calibration preflight from raw B-stack ROOT.

This is deliberately a preflight. It uses PSTAR/depth lookup and duplicate-readout
charge closure to audit whether an energy-scale path is internally consistent enough
to justify a later Birks/GEANT4 study. It does not create particle truth labels.
"""
...
def feature_matrix(events: pd.DataFrame, staves: List[str], anchors: np.ndarray, charge_scale: float = 1.0) -> Tuple[np.ndarray, List[str], List[int]]:
...
def run_block_bootstrap(
    ...
    for rep in range(reps):
        ...
        pieces = [np.flatnonzero(heldout_mask & (events["run"].to_numpy() == run)) for run in chosen]
        idx = np.concatenate(pieces)
        tmp_mask = np.zeros(len(events), dtype=bool)
        tmp_mask[idx] = True
        trad_res = res68(y[idx], pred_trad[idx])
        ml_res = res68(y[idx], pred_ml[idx])
        rows.append(
            {
                "rep": rep,
                ...
                "traditional_depth_violation_rate": depth_violation_rate(events, pred_trad, tmp_mask),
                "ml_depth_violation_rate": depth_violation_rate(events, pred_ml, tmp_mask),
            }
        )
...
def energy_sensitivity_res68(base: np.ndarray, low: np.ndarray, high: np.ndarray) -> float:
    denom = np.maximum(np.abs(base), 1.0)
    frac = np.maximum(np.abs((low - base) / denom), np.abs((high - base) / denom))
    return float(np.percentile(frac, 68))
...
def propagation_summary(
    ...
    held_idx = np.flatnonzero(heldout_mask)
    model_res = res68(y[held_idx], pred[held_idx])
    prop_res = energy_sensitivity_res68(pred[held_idx], pred_low[held_idx], pred_high[held_idx])
    combined = float(np.sqrt(model_res * model_res + prop_res * prop_res))

    rng = np.random.default_rng(seed)
    runs = np.asarray(sorted(events.loc[heldout_mask, "run"].unique()), dtype=int)
    model_vals = np.empty(reps, dtype=float)
    prop_vals = np.empty(reps, dtype=float)
    combined_vals = np.empty(reps, dtype=float)
    run_arr = events["run"].to_numpy()
    for rep in range(reps):
        chosen = rng.choice(runs, size=len(runs), replace=True)
        idx = np.concatenate([np.flatnonzero(heldout_mask & (run_arr == run)) for run in chosen])
        model_vals[rep] = res68(y[idx], pred[idx])
        prop_vals[rep] = energy_sensitivity_res68(pred[idx], pred_low[idx], pred_high[idx])
        combined_vals[rep] = float(np.sqrt(model_vals[rep] * model_vals[rep] + prop_vals[rep] * prop_vals[rep]))

    return {
        "geometry": geometry,
        "method": method,
        "n": int(heldout_mask.sum()),
        "p04b_external_charge_res68": charge_res68,
        "model_energy_proxy_res68": model_res,
        "model_energy_proxy_res68_ci95": str(ci(model_vals)),
        "p04b_charge_propagated_energy_res68": prop_res,
        "p04b_charge_propagated_energy_res68_ci95": str(ci(prop_vals)),
        "combined_energy_proxy_res68": combined,
        "combined_energy_proxy_res68_ci95": str(ci(combined_vals)),
        "acceptance_res68": acceptance,
        "acceptable_for_s14_preflight": bool(combined <= acceptance),
    }


def fit_evaluate_geometry(config: dict, events: pd.DataFrame, variant: str, train_mask: np.ndarray, heldout_mask: np.ndarray) -> dict:
    staves = list(config["staves"].keys())
    ...
    charge_res68 = float(config["p04b_external_charge_proxy"]["res68_abs_frac"])
    charge_low = max(0.0, 1.0 - charge_res68)
    charge_high = 1.0 + charge_res68
    pred_trad_low = trad_cal.predict(even_charge * charge_low, depth_idx)
    pred_trad_high = trad_cal.predict(even_charge * charge_high, depth_idx)
    X_low, _, _ = feature_matrix(events, staves, anchors, charge_scale=charge_low)
    X_high, _, _ = feature_matrix(events, staves, anchors, charge_scale=charge_high)
    pred_ml_low = model.predict(X_low)
    pred_ml_high = model.predict(X_high)
    metrics = pd.DataFrame(
        [
            metric_row(events, y, pred_depth_only, heldout_mask, "pstar_depth_only", variant),
            metric_row(events, y, pred_trad, heldout_mask, "traditional_depth_charge_lookup", variant),
            metric_row(events, y, pred_ml, heldout_mask, "ml_monotonic_hgb", variant),
        ]
    )
    by_run = []
    for run, run_df in events.loc[heldout_mask].groupby("run"):
        ...
        for method, pred in [
            ...
            row = metric_row(events, y, pred, mask, method, variant)
            row["run"] = int(run)
            by_run.append(row)
    ...
    propagation = pd.DataFrame(
        [
            propagation_summary(
                ...
                pred_trad_high,
                heldout_mask,
                "traditional_depth_charge_lookup",
                variant,
                charge_res68,
                float(config["energy_uncertainty_acceptance_res68"]),
                int(config["bootstrap_reps"]),
                int(config["random_seed"]) + 401,
            ),
            propagation_summary(
                events,
                y,
...
def make_report(
    ...
    command = f"/home/billy/anaconda3/bin/python {Path(__file__).as_posix()} --config {config['_config_path']}"
    ...
    prop_display = propagation[propagation["geometry"] == nominal][prop_cols]
    ...
    lines = [
        ...
        f"- **Upstream P04b reproduction:** `{config['p04b_external_charge_proxy']['source_result']}` gives external charge-proxy res68 `{config['p04b_external_charge_proxy']['res68_abs_frac']:.5f}` with CI `{config['p04b_external_charge_proxy']['res68_ci95']}`.",
        ...
        "",
        prop_display.to_markdown(index=False),
        "",
        "## 6. Run-split checks",
        "",
...
def main() -> None:
    ...
    parser.add_argument("--config", default="configs/s14b_range_energy_preflight.yaml")
    ...
    for variant in config["geometry_variants"]:
        print(f"fitting geometry {variant} ...", flush=True)
        ...
        all_propagation.append(fit["propagation"])
        boot_by_geom[variant] = boot
        fit_outputs[variant] = fit

    metrics_df = pd.concat(all_metrics, ignore_index=True)
    by_run_df = pd.concat(all_by_run, ignore_index=True)
    propagation_df = pd.concat(all_propagation, ignore_index=True)
    ...
    nominal_prop = propagation_df[propagation_df["geometry"] == config["nominal_geometry"]].set_index("method")
    ...
    trad_combined = float(nominal_prop.loc["traditional_depth_charge_lookup", "combined_energy_proxy_res68"])
    ml_combined = float(nominal_prop.loc["ml_monotonic_hgb", "combined_energy_proxy_res68"])
    acceptance = float(config["energy_uncertainty_acceptance_res68"])
    delta = ml_res - trad_res
    delta_ci = ci(nominal_boot["ml_minus_traditional_res68"].to_numpy())
    finding = (
        f"The nominal 4 cm geometry preflight reproduces S00 exactly and gives held-out odd-readout energy-proxy "
        ...
        f"Propagating the ticket-local raw-root P04b external charge-proxy res68 {config['p04b_external_charge_proxy']['res68_abs_frac']:.4f} "
        f"raises the combined nominal range-energy proxy res68 to {trad_combined:.4f} for traditional and {ml_combined:.4f} for ML "
        f"against the preflight acceptance threshold {acceptance:.2f}. "
        f"Across explicit geometry variants, traditional res68 spans {envelope['traditional_res68_min']:.4f}-{envelope['traditional_res68_max']:.4f} "
        f"and ML spans {envelope['ml_res68_min']:.4f}-{envelope['ml_res68_max']:.4f}. The propagated uncertainty fails the "
        f"{acceptance:.2f} S14-style preflight threshold for a per-event energy claim; the table remains useful as an internal "
        "charge/depth audit, but Birks quenching, material budget, and external particle truth remain unresolved."
    )
    ...
    propagation_df.to_csv(out_dir / "p04b_uncertainty_propagation.csv", index=False)
    ...
    result = {
        ...
        "p04b_external_charge_proxy": config["p04b_external_charge_proxy"],
        "p04b_uncertainty_propagation": json.loads(propagation_df.to_json(orient="records")),
    ...
    make_report(out_dir, config, counts, metrics_df, by_run_df, nominal_boot, geom_summary, propagation_df, leak, result)
    ...
    output_names = [
        ...
        "p04b_uncertainty_propagation.csv",
    ...
    manifest = {
        ...
        "command": f"/home/billy/anaconda3/bin/python {Path(__file__).as_posix()} --config {config_path}",
