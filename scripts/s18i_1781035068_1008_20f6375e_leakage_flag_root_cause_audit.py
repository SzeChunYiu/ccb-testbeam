#!/usr/bin/env python3
"""S18i: A-stack leakage-flag root cause audit.

This script extends the S18e/S18f/S18h A-stack studies with explicit leakage
controls. It reads raw A-stack ROOT files, reconstructs A1-A3 CFD20 pairs,
benchmarks traditional and learned residual corrections on Sample IV held-out
runs, and adds feature-knockout, shuffled-pool, run-only, and waveform-only
sentinels for the ticket-owned directory.
"""
...
os.environ.setdefault(
    "MPLCONFIGDIR",
    "reports/1781035068.1008.20f6375e__s18i_astack_leakage_flag_root_cause_audit/.mplconfig",
)
...
def engineered_features(df: pd.DataFrame, mode: str = "safe", samples: int = 18) -> np.ndarray:
    if mode == "waveform_only":
        return wave_tensor(df, samples).reshape(len(df), -1)
    if mode == "run_only":
        return np.column_stack(
            [
                df["run"].to_numpy(dtype=float),
    ...
    if mode == "safe":
        cols.extend([df["tail_left"].to_numpy(), df["tail_right"].to_numpy()])
    elif mode != "safe_no_tail":
...
def tune_ridge(train: pd.DataFrame, alphas: Sequence[float], feature_mode: str, samples: int) -> Tuple[Any, pd.DataFrame]:
    x = engineered_features(train, feature_mode, samples)
    ...
    for alpha in alphas:
        ...
        rows.append({"method": "ridge", "feature_set": feature_mode, "params": json.dumps({"alpha": float(alpha)}), "cv_rmse_ns": float(np.mean(rmses))})
...
def tune_gbr(train: pd.DataFrame, grid: Sequence[Dict[str, Any]], seed: int, feature_mode: str, samples: int) -> Tuple[Any, pd.DataFrame]:
    x = engineered_features(train, feature_mode, samples)
    y = train["raw_residual_ns"].to_numpy()
    groups = train["run"].to_numpy()
    rows = []
    for params in grid:
        ...
        for tr_idx, va_idx in group_cv_splits(groups):
            ...
            model.fit(x[tr_idx], y[tr_idx])
            rmses.append(math.sqrt(mean_squared_error(y[va_idx], model.predict(x[va_idx]))))
        rows.append({"method": "gradient_boosted_trees", "feature_set": feature_mode, "params": json.dumps(params, sort_keys=True), "cv_rmse_ns": float(np.mean(rmses))})
    table = pd.DataFrame(rows).sort_values(["cv_rmse_ns", "params"]).reset_index(drop=True)
    params = json.loads(table.iloc[0]["params"])
    model = GradientBoostingRegressor(random_state=seed, loss="ls", **params)
    model.fit(x, y)
...
def tune_mlp(train: pd.DataFrame, grid: Sequence[Dict[str, Any]], seed: int, feature_mode: str, samples: int) -> Tuple[Any, pd.DataFrame]:
    x = engineered_features(train, feature_mode, samples)
    y = train["raw_residual_ns"].to_numpy()
    groups = train["run"].to_numpy()
    rows = []
    for params in grid:
        ...
        for tr_idx, va_idx in group_cv_splits(groups):
            ...
            model.fit(x[tr_idx], y[tr_idx])
            rmses.append(math.sqrt(mean_squared_error(y[va_idx], model.predict(x[va_idx]))))
        rows.append({"method": "mlp", "feature_set": feature_mode, "params": json.dumps(params, sort_keys=True), "cv_rmse_ns": float(np.mean(rmses))})
    table = pd.DataFrame(rows).sort_values(["cv_rmse_ns", "params"]).reset_index(drop=True)
    params = json.loads(table.iloc[0]["params"])
    model = make_pipeline(
        StandardScaler(),
...
def torch_train_predict(
    ...
    if len(train) < min_train or len(np.unique(train["run"].to_numpy())) < 2:
        model, cv = tune_ridge(train, config["ridge_alphas"], feature_mode, samples)
        pred = model.predict(engineered_features(test, feature_mode, samples))
        return pred, {"status": "fallback_ridge_small_pool", "cv_rmse_ns": float(cv.iloc[0]["cv_rmse_ns"])}

    x_feat = engineered_features(train, feature_mode, samples).astype(np.float32)
    ...
    feat_test = ((engineered_features(test, feature_mode, samples).astype(np.float32) - feat_mean) / feat_std).astype(np.float32)
...
def train_models_for_pool(train: pd.DataFrame, config: Dict[str, Any], pool: str, seed: int, feature_mode: str) -> Tuple[Dict[str, Any], pd.DataFrame]:
    samples = int(config["samples_per_channel"])
    models: Dict[str, Any] = {}
    cv_rows: List[pd.DataFrame] = []
    ridge, ridge_cv = tune_ridge(train, config["ridge_alphas"], feature_mode, samples)
    ridge_cv["pool"] = pool
    cv_rows.append(ridge_cv)
    models["ridge"] = ridge

    if len(train) >= 80 and len(np.unique(train["run"].to_numpy())) >= 2:
        gbr, gbr_cv = tune_gbr(train, config["gbr_grid"], seed, feature_mode, samples)
        gbr_cv["pool"] = pool
        cv_rows.append(gbr_cv)
        models["gradient_boosted_trees"] = gbr

        mlp, mlp_cv = tune_mlp(train, config["mlp_grid"], seed, feature_mode, samples)
        mlp_cv["pool"] = pool
        ...
        models["mlp"] = ridge
        cv_rows.append(pd.DataFrame([{"pool": pool, "method": "gradient_boosted_trees", "feature_set": feature_mode, "params": "fallback_ridge_small_pool", "cv_rmse_ns": np.nan}]))
        cv_rows.append(pd.DataFrame([{"pool": pool, "method": "mlp", "feature_set": feature_mode, "params": "fallback_ridge_small_pool", "cv_rmse_ns": np.nan}]))
...
def pair_bootstrap_ci(values: np.ndarray, rng: np.random.Generator, n_resamples: int, metric: Callable[[np.ndarray], float]) -> Tuple[float, float]:
    values = np.asarray(values)
    stats = []
    for _ in range(n_resamples):
        sampled = rng.choice(values, size=len(values), replace=True)
        stats.append(metric(sampled))
    return tuple(float(x) for x in np.quantile(stats, [0.025, 0.975]))
...
def paired_delta(
    ...
    pair_stats = []
    base_all = df[col_base].to_numpy()
    test_all = df[col_test].to_numpy()
    for _ in range(n_resamples):
        idx = rng.choice(np.arange(len(df)), size=len(df), replace=True)
        pair_stats.append(robust_width(test_all[idx]) - robust_width(base_all[idx]))
    arr = np.asarray(stats)
    lo, hi = np.quantile(arr, [0.025, 0.975])
    pair_lo, pair_hi = np.quantile(np.asarray(pair_stats), [0.025, 0.975])
    p_value = 2.0 * min(float(np.mean(arr <= 0.0)), float(np.mean(arr >= 0.0)))
    return float(np.median(arr)), float(lo), float(hi), min(p_value, 1.0), float(pair_lo), float(pair_hi)


def leakage_control_gaps(metrics: pd.DataFrame, deltas: pd.DataFrame) -> pd.DataFrame:
    rows = []
    indexed = metrics.set_index(["pool", "method"])
    for pool in sorted(metrics["pool"].unique()):
        for method in ["ridge", "gradient_boosted_trees", "mlp", "cnn1d", "leakage_gated_cnn_new"]:
            if (pool, method) not in indexed.index:
                continue
            safe_width = float(indexed.loc[(pool, method), "robust_width_ns"])
            for control in [f"{method}__safe_no_tail", f"{method}__waveform_only", f"{method}__run_only"]:
                if (pool, control) not in indexed.index:
                    continue
                control_width = float(indexed.loc[(pool, control), "robust_width_ns"])
                rows.append(
                    {
                        "pool": pool,
                        "method": method,
                        "control": control,
                        "safe_width_ns": safe_width,
                        "control_width_ns": control_width,
                        "control_minus_safe_ns": control_width - safe_width,
                        "interpretation": "large negative means the control is suspiciously stronger than the accepted safe feature set",
                    }
                )
    out = pd.DataFrame(rows)
    if len(out):
        out = out.merge(
            deltas[["pool", "comparison", "ci_low_ns", "ci_high_ns", "p_value"]],
            how="left",
            left_on=["pool", "control"],
            right_on=["pool", "comparison"],
        ).drop(columns=["comparison"])
    return out


def pool_transfer_deltas(metrics: pd.DataFrame) -> pd.DataFrame:
    rows = []
    indexed = metrics.set_index(["pool", "method"])
    comparisons = [("sample_iii_late", "sample_iii_early"), ("sample_iii_mixed", "sample_iii_late"), ("sample_iii_late", "run64_only")]
    for method in sorted(metrics["method"].unique()):
        for pool_a, pool_b in comparisons:
            if (pool_a, method) in indexed.index and (pool_b, method) in indexed.index:
                rows.append(
                    {
                        "method": method,
                        "pool_a": pool_a,
                        "pool_b": pool_b,
                        "pool_a_width_ns": float(indexed.loc[(pool_a, method), "robust_width_ns"]),
                        "pool_b_width_ns": float(indexed.loc[(pool_b, method), "robust_width_ns"]),
                        "pool_a_minus_pool_b_ns": float(indexed.loc[(pool_a, method), "robust_width_ns"] - indexed.loc[(pool_b, method), "robust_width_ns"]),
                    }
                )
    return pd.DataFrame(rows)


def shuffled_pool_controls(
    all_pairs: pd.DataFrame,
    heldout_iv: pd.DataFrame,
    ...
    repeats = int(config.get("controls", {}).get("shuffled_pool_repeats", 0))
    if repeats <= 0 or sample_iii.empty:
        return pd.DataFrame()
    rows = []
    samples = int(config["samples_per_channel"])
    for pool, original in train_cache.items():
        if pool == "run64_only":
            continue
        n_train = len(original)
        base_resid = fit_traditional(original, heldout_iv)
        base_width = robust_width(base_resid)
        for rep in range(repeats):
            idx = rng.choice(sample_iii.index.to_numpy(), size=n_train, replace=n_train > len(sample_iii))
            shuffled_train = sample_iii.loc[idx].copy()
            model, cv = tune_ridge(shuffled_train, config["ridge_alphas"], "safe", samples)
            pred = model.predict(engineered_features(heldout_iv, "safe", samples))
            resid = heldout_iv["raw_residual_ns"].to_numpy() - pred
            rows.append(
                {
                    "pool": pool,
                    "repeat": rep,
                    "method": "ridge_safe_shuffled_pool",
                    "n_train": int(n_train),
                    "cv_rmse_ns": float(cv.iloc[0]["cv_rmse_ns"]),
                    "robust_width_ns": robust_width(resid),
                    "full_rms_ns": full_rms(resid),
                    "tail_fraction_abs_gt_5ns": float(np.mean(np.abs(resid - np.median(resid)) > 5.0)),
                    "delta_vs_original_traditional_ns": robust_width(resid) - base_width,
                }
            )
    return pd.DataFrame(rows)
...
def support_mask(train: pd.DataFrame, test: pd.DataFrame) -> np.ndarray:
    train_x = engineered_features(train, "safe", 18)
    test_x = engineered_features(test, "safe", 18)
...
def leakage_checks(train: pd.DataFrame, config: Dict[str, Any], seed: int) -> pd.DataFrame:
    samples = int(config["samples_per_channel"])
    x = engineered_features(train, "safe", samples)
    y = train["raw_residual_ns"].to_numpy()
    groups = train["run"].to_numpy()
    rows = [
...
def evaluate(config: Dict[str, Any]) -> Dict[str, pd.DataFrame]:
    ...
    samples = int(config["samples_per_channel"])
    feature_sets = list(config.get("controls", {}).get("feature_sets", ["safe"]))
    primary_feature_set = str(config.get("controls", {}).get("primary_feature_set", "safe"))
    ...
    for pool, pool_cfg in config["calibration_pools"].items():
        ...
        for feature_mode in feature_sets:
            suffix = "" if feature_mode == primary_feature_set else f"__{feature_mode}"
            if feature_mode == primary_feature_set:
                models, cv_table = train_models_for_pool(train, config, pool, seed + len(feature_mode), feature_mode)
                cv_rows.append(cv_table)
                for method in ["ridge", "gradient_boosted_trees", "mlp"]:
                    pred = models[method].predict(engineered_features(test, feature_mode, samples))
                    frame[f"{method}{suffix}_residual_ns"] = test["raw_residual_ns"].to_numpy() - pred
            else:
                ridge_model, ridge_cv = tune_ridge(train, config["ridge_alphas"], feature_mode, samples)
                ridge_cv["pool"] = pool
                cv_rows.append(ridge_cv)
                pred = ridge_model.predict(engineered_features(test, feature_mode, samples))
                frame[f"ridge{suffix}_residual_ns"] = test["raw_residual_ns"].to_numpy() - pred
            if feature_mode in {primary_feature_set, "waveform_only"}:
                for method, gated in [("cnn1d", False), ("leakage_gated_cnn_new", True)]:
                    label = f"{method}{suffix}"
                    pred, info = torch_train_predict(
                        train,
                        test,
                        config,
                        gated=gated,
                        seed=seed + len(pool) + len(feature_mode) + (1 if gated else 0),
                        feature_mode=feature_mode,
                    )
                    frame[f"{label}_residual_ns"] = test["raw_residual_ns"].to_numpy() - pred
                    cv_rows.append(
                        pd.DataFrame(
                            [
                                {
    ...
    residual_cols = [col for col in heldout.columns if col.endswith("_residual_ns") and col != "raw_residual_ns"]
    methods = [col.replace("_residual_ns", "") for col in residual_cols]
    ...
    for pool in config["calibration_pools"]:
        ...
        for method in methods:
            ...
            row["pool"] = pool
            row["pair_ci_low_ns"], row["pair_ci_high_ns"] = pair_bootstrap_ci(sub[col].to_numpy(), rng, int(config["bootstrap_resamples"]), robust_width)
            ...
            if method != "traditional":
                med, lo, hi, p, pair_lo, pair_hi = paired_delta(sub, "traditional_residual_ns", col, rng, int(config["bootstrap_resamples"]))
                delta_rows.append(
                    {
                        "pool": pool,
                        ...
                        "ci_low_ns": lo,
                        "ci_high_ns": hi,
                        "pair_ci_low_ns": pair_lo,
                        "pair_ci_high_ns": pair_hi,
                        "p_value": p,
                    }
                )
    ...
    leakage_control_gap_rows = leakage_control_gaps(metrics, deltas)
    pool_transfer_rows = pool_transfer_deltas(metrics)
    shuffled_rows = shuffled_pool_controls(all_pairs, heldout_iv, train_cache, config, seed)
    ...
    return {
        ...
        "leakage_control_gaps": leakage_control_gap_rows,
        "pool_transfer_deltas": pool_transfer_rows,
        "shuffled_pool_controls": shuffled_rows,
...
def atom_audit(
    ...
    rows = []
    accepted = metrics[~metrics["method"].str.contains("__", regex=False)].copy()
    late = accepted[accepted["pool"].eq("sample_iii_late")].set_index("method")
    early = accepted[accepted["pool"].eq("sample_iii_early")].set_index("method")
        ...
        late_gap=lambda d: np.where(
            ...
            d[
                [
                    c
                    for c in d.columns
                    if c.endswith("_robust_width_ns")
                    and not c.startswith("traditional")
                    and "__" not in c
                ]
            ].min(axis=1)
...
def write_result_json(out_dir: Path, config: Dict[str, Any], artifacts: Dict[str, pd.DataFrame]) -> None:
    ...
    accepted = metrics[~metrics["method"].str.contains("__", regex=False)].copy()
    best = accepted.sort_values("robust_width_ns").iloc[0]
    trad_best = accepted[accepted["method"].eq("traditional")].sort_values("robust_width_ns").iloc[0]
    late = accepted[accepted["pool"].eq("sample_iii_late")].sort_values("robust_width_ns")
    late_best_ml = late[late["method"].ne("traditional")].iloc[0]
    leakage_gaps = artifacts["leakage_control_gaps"]
    suspicious_controls = leakage_gaps[leakage_gaps["control_minus_safe_ns"] < -0.10] if len(leakage_gaps) else leakage_gaps
    shuffled = artifacts["shuffled_pool_controls"]
    result = {
        "study": config["study"],
        "ticket": config["ticket"],
        "worker": config["worker"],
        "title": "A-stack residual-correction leakage-flag root cause audit",
        ...
        "traditional": {
            ...
            "pair_bootstrap_ci": [float(trad_best["pair_ci_low_ns"]), float(trad_best["pair_ci_high_ns"])],
        },
        "ml": {
            "metric": "sample_iv_A1_A3_sigma68_ns",
            ...
            "pair_bootstrap_ci": [float(best["pair_ci_low_ns"]), float(best["pair_ci_high_ns"])],
        },
        "winner": f"{best['pool']}::{best['method']}",
        "winner_scope": "accepted_safe_methods_only",
        ...
        "falsification": {
            ...
            "n_tries": int((accepted["method"].nunique() - 1) * accepted["pool"].nunique()),
        ...
        "leakage_control_summary": {
            "n_suspicious_control_gaps": int(len(suspicious_controls)),
            "most_negative_control_minus_safe_ns": float(leakage_gaps["control_minus_safe_ns"].min()) if len(leakage_gaps) else None,
            "criterion": "control_minus_safe_ns < -0.10 flags a control that outperforms the accepted feature set enough to question leakage/control definition",
        },
        "shuffled_pool_summary": {
            "n_repeats": int(len(shuffled)),
            "median_delta_vs_original_traditional_ns": float(shuffled["delta_vs_original_traditional_ns"].median()) if len(shuffled) else None,
            "min_delta_vs_original_traditional_ns": float(shuffled["delta_vs_original_traditional_ns"].min()) if len(shuffled) else None,
        },
        ...
        "critic": "pending",
        "next_tickets": [
            "S18j: A-stack monotone support-matched timewalk transfer with forbidden-control freeze"
        ],
    }
...
def write_report(out_dir: Path, config_path: Path, config: Dict[str, Any], artifacts: Dict[str, pd.DataFrame]) -> None:
    ...
    leakage_gaps = artifacts["leakage_control_gaps"].copy()
    pool_transfer = artifacts["pool_transfer_deltas"].copy()
    shuffled = artifacts["shuffled_pool_controls"].copy()

    accepted = metrics[~metrics["method"].str.contains("__", regex=False)].copy()
    best = accepted.sort_values("robust_width_ns").iloc[0]
    best_trad = accepted[accepted["method"].eq("traditional")].sort_values("robust_width_ns").iloc[0]
    late = accepted[accepted["pool"].eq("sample_iii_late")].sort_values("robust_width_ns")
    late_best_ml = late[late["method"].ne("traditional")].iloc[0]
    ...
    report = f"""# Study report: S18i - A-stack residual-correction leakage-flag root cause audit
...
{md_table(metrics[metrics['method'].eq('traditional')], ['pool', 'n_pairs', 'robust_width_ns', 'pair_ci_low_ns', 'pair_ci_high_ns', 'robust_ci_low_ns', 'robust_ci_high_ns', 'full_rms_ns', 'core_sigma_ns', 'chi2_ndf', 'tail_fraction_abs_gt_5ns'])}
...
{md_table(artifacts['model_cv_scan'], ['pool', 'method', 'feature_set', 'params', 'cv_rmse_ns'])}
...
{md_table(metrics, ['pool', 'method', 'n_pairs', 'robust_width_ns', 'pair_ci_low_ns', 'pair_ci_high_ns', 'robust_ci_low_ns', 'robust_ci_high_ns', 'full_rms_ns', 'core_sigma_ns', 'chi2_ndf', 'tail_fraction_abs_gt_5ns'])}

Paired deltas versus the traditional method:

{md_table(deltas, ['pool', 'comparison', 'delta_median_ns', 'pair_ci_low_ns', 'pair_ci_high_ns', 'ci_low_ns', 'ci_high_ns', 'p_value'])}

Accepted winner by robust width is **{best['pool']}::{best['method']}** at **{best['robust_width_ns']:.3f} ns**. The strongest traditional comparator is **{best_trad['pool']}::traditional** at **{best_trad['robust_width_ns']:.3f} ns**. The late-pool best accepted ML method is **{late_best_ml['method']}** at **{late_best_ml['robust_width_ns']:.3f} ns**, versus late-pool traditional **{late_trad['robust_width_ns']:.3f} ns**.

Verdict: the accepted comparison tests whether learned residual corrections survive leakage-oriented controls. The best accepted method differs from the best traditional comparator by **{(best['robust_width_ns'] - best_trad['robust_width_ns']):.3f} ns** in point estimate. The late-pool best accepted learned method differs from the late-pool traditional comparator by **{(late_best_ml['robust_width_ns'] - late_trad['robust_width_ns']):.3f} ns**. Bootstrap intervals determine whether this is a secure improvement or only an exploratory point-estimate closure.
...
{leakage_gaps.to_markdown(index=False) if len(leakage_gaps) else 'No leakage-control gap rows were produced.'}

Pool-transfer deltas:

{pool_transfer.to_markdown(index=False) if len(pool_transfer) else 'No pool-transfer delta rows were produced.'}

Shuffled-pool residual-correction null:

{shuffled.groupby('pool')[['robust_width_ns', 'delta_vs_original_traditional_ns']].agg(['median', 'min', 'max']).to_markdown() if len(shuffled) else 'No shuffled-pool controls were requested.'}
...
**Data leakage.** Acceptance metrics are split by run. Safe features exclude run id, event id, raw residual, sample-period indicator, and timing columns. Row-split and run-only diagnostics are reported only as sentinels, not as evidence of performance. Leakage flags: **{leakage_flags}**.
...
Quantitative conclusion: the accepted winner is **{best['pool']}::{best['method']}** with robust width **{best['robust_width_ns']:.3f} ns** and run-block CI [{best['robust_ci_low_ns']:.3f}, {best['robust_ci_high_ns']:.3f}] ns. The best traditional comparator is **{best_trad['pool']}::traditional** with **{best_trad['robust_width_ns']:.3f} ns** and run-block CI [{best_trad['robust_ci_low_ns']:.3f}, {best_trad['robust_ci_high_ns']:.3f}] ns. The leakage-control tables decide whether the learned point-estimate gains are credible or likely control-definition artifacts.
...
def main() -> None:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/s18i_1781035068_1008_20f6375e.json"))
