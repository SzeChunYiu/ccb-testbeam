#!/usr/bin/env python3
"""P03g pedestal-proxy residualization of samples 0-3 before P03f ablation.
...
blinded pedestal/baseline proxies using only the training runs in each fold.
...
os.environ.setdefault("MPLCONFIGDIR", "/tmp/matplotlib-p03g-1781097097")
...
def write_report(out_dir: Path, config: dict, result: dict, match: pd.DataFrame, pooled: pd.DataFrame, per_run: pd.DataFrame, leakage: pd.DataFrame) -> None:
    ...
    text = f"""# P03g: pedestal-residualized samples 0-3 before P03f multimodel ablation
...
def plot_outputs(out_dir: Path, pooled: pd.DataFrame, all_pairs: pd.DataFrame) -> None:
    ...
    ax.set_title("P03g pedestal-residualized multimodel ablation")
    fig.tight_layout()
...
def main() -> int:
    ...
    parser.add_argument("--config", default="configs/p03g_1781097097_1022_02db7832_pedestal_residualized_samples.json")
    ...
    result = {
        ...
        "verdict": verdict,
        "next_tickets": [
            "P03h: train-fold external pedestal-proxy validation for residualized early waveform samples"
        ],
