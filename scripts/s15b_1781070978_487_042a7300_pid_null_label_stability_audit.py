...
This study is a weak-label stability and falsification test, not a truth PID
measurement. It starts from raw B-stack ROOT, rebuilds a transparent
range/depth charge proxy, and asks whether waveform pulse atoms add PID-like
information beyond charge, penetration depth, topology, saturation, dropout,
baseline, anomaly, and run-family support. The benchmark panel is the
preregistered traditional method plus ridge, gradient-boosted trees, MLP,
1D-CNN, and a support-residual hybrid MLP.
"""
...
HEAD_TO_HEAD_METHODS = [
    "traditional_deltae_depth_ridge",
    "ridge",
    "gradient_boosted_trees",
    "mlp",
    "cnn_1d",
    "support_residual_hybrid_mlp_new",
]
...
def topology_block_ci(
    ...
    rng = np.random.default_rng(seed)
    unique_topology = np.unique(topology)
    rows = {"auc": [], "ap": [], "purity": []}
    for _ in range(n_boot):
        sampled = rng.choice(unique_topology, size=len(unique_topology), replace=True)
        idx = np.concatenate([np.where(topology == item)[0] for item in sampled])
        if len(np.unique(y[idx])) < 2:
            continue
        rows["auc"].append(float(roc_auc_score(y[idx], score[idx])))
        rows["ap"].append(float(average_precision_score(y[idx], score[idx])))
        rows["purity"].append(purity_at_efficiency(y[idx], score[idx], fixed_efficiency))
    out = {"topology_bootstrap_valid": int(len(rows["auc"]))}
    for key, values in rows.items():
        clean = [v for v in values if np.isfinite(v)]
        out["topology_" + key + "_ci"] = [float(x) for x in np.quantile(clean, [0.025, 0.975])] if clean else [None, None]
    return out
...
def _quantile_bins(values: np.ndarray, n_bins: int) -> np.ndarray:
    ...
    values[~np.isfinite(values)] = np.nanmedian(values[np.isfinite(values)]) if np.isfinite(values).any() else 0.0
    edges = np.unique(np.quantile(values, np.linspace(0.0, 1.0, int(n_bins) + 1)))
    if len(edges) <= 2:
        return np.zeros(len(values), dtype=int)
    return np.clip(np.digitize(values, edges[1:-1], right=True), 0, len(edges) - 2).astype(int)


def add_support_audit_columns(meta: pd.DataFrame, config: dict) -> pd.DataFrame:
    """Add nuisance-support cells for matched stability checks.

    The raw scan does not expose all historical support flags as named columns.
    This audit therefore uses transparent proxies derived before any label fit:
    saturation from ADC limits, dropout from missing/low downstream charge,
    baseline from pre-pulse normalized samples, and anomaly from broad or sharp
    normalized B2 shapes.
    """

    out = meta.copy()
    audit_cfg = config.get("null_label_audit", {})
    n_amp = int(audit_cfg.get("amplitude_quantile_bins", 4))
    baseline_q = float(audit_cfg.get("baseline_abs_quantile", 0.9))
    anomaly_q = float(audit_cfg.get("anomaly_abs_quantile", 0.9))
    dropout_q = float(audit_cfg.get("dropout_downstream_fraction_quantile", 0.1))

    out["support_amp_bin"] = _quantile_bins(np.log1p(np.maximum(out["b2_amp"].to_numpy(dtype=float), 0.0)), n_amp)
    out["support_saturation_flag"] = ((out["saturated_count"].to_numpy(dtype=int) > 0) | (out["b2_saturated"].to_numpy(dtype=int) > 0)).astype(int)
    downstream_fraction = out["downstream_charge_fraction"].to_numpy(dtype=float)
    downstream_cut = float(np.nanquantile(downstream_fraction[np.isfinite(downstream_fraction)], dropout_q))
    out["support_dropout_flag"] = (
        (out["downstream_selected"].to_numpy(dtype=int) == 0)
        | (downstream_fraction <= downstream_cut)
    ).astype(int)

    sample_cols = [col for col in SAMPLE_COLS if col in out.columns]
    if sample_cols:
        early = out[sample_cols[:4]].to_numpy(dtype=float)
        baseline_proxy = np.nanmean(early, axis=1)
    else:
        baseline_proxy = out["b2_early_fraction"].to_numpy(dtype=float)
    out["support_baseline_abs_proxy"] = np.abs(baseline_proxy)
    baseline_cut = float(np.nanquantile(out["support_baseline_abs_proxy"], baseline_q))
    out["support_baseline_flag"] = (out["support_baseline_abs_proxy"] >= baseline_cut).astype(int)

    anomaly_proxy = np.maximum.reduce(
        [
            np.abs(out["b2_max_down_step"].to_numpy(dtype=float)),
            np.abs(out["b2_tail_fraction"].to_numpy(dtype=float)),
            np.abs(out["b2_final_fraction"].to_numpy(dtype=float)),
        ]
    )
    out["support_anomaly_abs_proxy"] = anomaly_proxy
    anomaly_cut = float(np.nanquantile(anomaly_proxy, anomaly_q))
    out["support_anomaly_flag"] = (anomaly_proxy >= anomaly_cut).astype(int)

    cell_cols = [
        "group",
        "depth_idx",
        "support_amp_bin",
        "support_saturation_flag",
        "support_dropout_flag",
        "support_baseline_flag",
        "support_anomaly_flag",
    ]
    out["support_cell"] = out[cell_cols].astype(str).agg("|".join, axis=1)
    return out


def support_collapse_tables(meta_eval: pd.DataFrame, config: dict) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    min_class = int(config.get("null_label_audit", {}).get("support_min_class_rows", 3))
    cell_counts = (
        meta_eval.groupby(["support_cell", "weak_label"], sort=True)
        .size()
        .unstack(fill_value=0)
        .rename(columns={0: "negative_rows", 1: "positive_rows"})
        .reset_index()
    )
    if "negative_rows" not in cell_counts:
        cell_counts["negative_rows"] = 0
    if "positive_rows" not in cell_counts:
        cell_counts["positive_rows"] = 0
    cell_counts["min_class_rows"] = cell_counts[["negative_rows", "positive_rows"]].min(axis=1)
    cell_counts["matched_cell"] = cell_counts["min_class_rows"] >= min_class
    matched_cells = set(cell_counts.loc[cell_counts["matched_cell"], "support_cell"])
    matched = meta_eval["support_cell"].isin(matched_cells).to_numpy()
    summary = pd.DataFrame(
        [
            {
                "quantity": "evaluated_rows_before_support_matching",
                "rows": int(len(meta_eval)),
                "fraction_of_evaluated": 1.0,
            },
            {
                "quantity": "evaluated_rows_in_matched_support_cells",
                "rows": int(matched.sum()),
                "fraction_of_evaluated": float(matched.mean()) if len(matched) else float("nan"),
            },
            {
                "quantity": "support_collapse_fraction",
                "rows": int((~matched).sum()),
                "fraction_of_evaluated": float((~matched).mean()) if len(matched) else float("nan"),
            },
            {
                "quantity": "matched_support_cells",
                "rows": int(cell_counts["matched_cell"].sum()),
                "fraction_of_evaluated": float(cell_counts["matched_cell"].mean()) if len(cell_counts) else float("nan"),
            },
        ]
    )
    annotated = meta_eval[["run", "event_index", "weak_label", "support_cell"]].copy()
    annotated["matched_support_cell"] = matched.astype(int)
    return summary, cell_counts, annotated


def null_label_variants(y: np.ndarray, meta_eval: pd.DataFrame, seed: int) -> Dict[str, np.ndarray]:
    rng = np.random.default_rng(seed)
    variants: Dict[str, np.ndarray] = {}
    shuffled = y.copy()
    rng.shuffle(shuffled)
    variants["global_shuffle"] = shuffled

    within_run = y.copy()
    for run in np.unique(meta_eval["run"].to_numpy(dtype=int)):
        idx = np.flatnonzero(meta_eval["run"].to_numpy(dtype=int) == run)
        within_run[idx] = rng.permutation(within_run[idx])
    variants["within_run_shuffle"] = within_run

    within_depth = y.copy()
    for (_, _), group in meta_eval.groupby(["run", "depth_idx"], sort=True):
        idx = group.index.to_numpy(dtype=int)
        within_depth[idx] = rng.permutation(within_depth[idx])
    variants["within_run_depth_shuffle"] = within_depth

    within_support = y.copy()
    for _, group in meta_eval.groupby("support_cell", sort=True):
        idx = group.index.to_numpy(dtype=int)
        within_support[idx] = rng.permutation(within_support[idx])
    variants["within_support_cell_shuffle"] = within_support
    return variants


def score_audit_rows(
    meta_eval: pd.DataFrame,
    method_names: Sequence[str],
    config: dict,
    seed_offset: int,
) -> Tuple[pd.DataFrame, pd.DataFrame]:
    y = meta_eval["weak_label"].to_numpy(dtype=int)
    runs = meta_eval["run"].to_numpy(dtype=int)
    fixed_eff = float(config["benchmark"]["fixed_efficiency"])
    n_boot = int(config["benchmark"]["bootstrap_replicates"])

    null_rows = []
    variants = null_label_variants(y, meta_eval, int(config["benchmark"]["random_seed"]) + seed_offset)
    for variant_i, (variant, y_null) in enumerate(variants.items()):
        for method_i, method in enumerate(method_names):
            score = meta_eval[method].to_numpy(dtype=float)
            metrics = run_block_metrics(
                y_null,
                score,
                runs,
                int(config["benchmark"]["random_seed"]) + seed_offset + 100 * variant_i + method_i,
                n_boot,
                fixed_eff,
            )
            null_rows.append(
                {
                    ...
                    "method": method,
                    "null_positive_fraction": float(y_null.mean()),
                    "roc_auc": metrics["roc_auc"],
                    "roc_auc_ci_low": metrics["auc_ci"][0],
                    "roc_auc_ci_high": metrics["auc_ci"][1],
                    "average_precision": metrics["average_precision"],
                    "ap_ci_low": metrics["ap_ci"][0],
                    "ap_ci_high": metrics["ap_ci"][1],
                    "ece": metrics["ece"],
                    "bootstrap_valid": metrics["bootstrap_valid"],
                }
            )

    support_summary, _, _ = support_collapse_tables(meta_eval, config)
    matched_cells = set(
        meta_eval.groupby("support_cell")["weak_label"]
        .agg(lambda s: min((s == 0).sum(), (s == 1).sum()))
        .loc[lambda s: s >= int(config.get("null_label_audit", {}).get("support_min_class_rows", 3))]
        .index
    )
    matched_mask = meta_eval["support_cell"].isin(matched_cells).to_numpy()
    support_rows = []
    for method_i, method in enumerate(method_names):
        score = meta_eval.loc[matched_mask, method].to_numpy(dtype=float)
        metrics = run_block_metrics(
            y[matched_mask],
            score,
            runs[matched_mask],
            int(config["benchmark"]["random_seed"]) + seed_offset + 700 + method_i,
            n_boot,
            fixed_eff,
        )
        support_rows.append(
            {
                ...
                "support_collapse_fraction": float(support_summary.loc[support_summary["quantity"] == "support_collapse_fraction", "fraction_of_evaluated"].iloc[0]),
                "roc_auc": metrics["roc_auc"],
                "roc_auc_ci_low": metrics["auc_ci"][0],
                "roc_auc_ci_high": metrics["auc_ci"][1],
                "average_precision": metrics["average_precision"],
                "ap_ci_low": metrics["ap_ci"][0],
                "ap_ci_high": metrics["ap_ci"][1],
                "purity_at_fixed_efficiency": metrics["purity_at_fixed_efficiency"],
                "purity_ci_low": metrics["purity_ci"][0],
                "purity_ci_high": metrics["purity_ci"][1],
                "bootstrap_valid": metrics["bootstrap_valid"],
            }
        )
    return pd.DataFrame(null_rows), pd.DataFrame(support_rows)
...
def build_benchmark(waves_all: np.ndarray, meta_all: pd.DataFrame, config: dict, out_dir: Path) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, dict]:
    ...
    scores: Dict[str, np.ndarray] = {
        ...
        + [
            ...
            "sentinel_depth_only_ridge",
            "sentinel_topology_only_ridge",
            "sentinel_group_event_order_ridge",
            "sentinel_shuffled_label_gbt",
            "sentinel_target_permutation_gbt",
            "knockout_no_charge_gbt",
            "knockout_no_depth_topology_gbt",
            "knockout_no_waveform_gbt",
        ]
    }
    ...
    for fold_number, run in enumerate(np.unique(runs), start=1):
        ...
        scores["traditional_deltae_depth_ridge"][test] = fit_ridge_score(finite_matrix(trad_train), train_y, finite_matrix(trad_test))
        scores["ridge"][test] = fit_ridge_score(wave_train, train_y, wave_test)
        scores["gradient_boosted_trees"][test] = fit_hgb_score(wave_train, train_y, wave_test, hgb_params, seed + fold_number)
        scores["mlp"][test] = fit_mlp_score(wave_train, train_y, wave_test, seed + 100 + fold_number, mlp_max_iter)
        scores["cnn_1d"][test] = fit_cnn_score(train_waves, train_y, test_waves, seed + 200 + fold_number, cnn_epochs)
        scores["support_residual_hybrid_mlp_new"][test] = fit_mlp_score(resid_train, train_y, resid_test, seed + 300 + fold_number, mlp_max_iter)
        ...
        depth_cols = ["depth_idx", "downstream_selected", "downstream_charge_fraction"]
        scores["sentinel_depth_only_ridge"][test] = fit_ridge_score(
            finite_matrix(train_df[depth_cols].to_numpy(dtype=float)),
            train_y,
            finite_matrix(test_df[depth_cols].to_numpy(dtype=float)),
        )
        topology_cols = ["multiplicity", "topology_code", "b2_hit", "b4_hit", "b6_hit", "b8_hit", "saturated_count", "b2_saturated"]
        scores["sentinel_topology_only_ridge"][test] = fit_ridge_score(
            finite_matrix(train_df[topology_cols].to_numpy(dtype=float)),
            train_y,
            finite_matrix(test_df[topology_cols].to_numpy(dtype=float)),
        )
        ...
        permuted_y = train_y.copy()
        rng_perm = np.random.default_rng(seed + 9500 + fold_number)
        train_depth = train_df["depth_idx"].to_numpy(dtype=int)
        for depth_value in np.unique(train_depth):
            idx_depth = np.flatnonzero(train_depth == depth_value)
            permuted_y[idx_depth] = rng_perm.permutation(permuted_y[idx_depth])
        scores["sentinel_target_permutation_gbt"][test] = fit_hgb_score(wave_train, permuted_y, wave_test, hgb_params, seed + 450 + fold_number)

        no_charge_cols = [c for c in train_df[HAND_COLS].columns if "area" not in c] + SAMPLE_COLS
        scores["knockout_no_charge_gbt"][test] = fit_hgb_score(
            finite_matrix(train_df[no_charge_cols].to_numpy(dtype=float)),
            train_y,
            finite_matrix(test_df[no_charge_cols].to_numpy(dtype=float)),
            hgb_params,
            seed + 500 + fold_number,
        )
        no_depth_topology_cols = SAMPLE_COLS + HAND_COLS + ["b2_amp", "b2_area", "even_total_charge"]
        scores["knockout_no_depth_topology_gbt"][test] = fit_hgb_score(
            finite_matrix(train_df[no_depth_topology_cols].to_numpy(dtype=float)),
            train_y,
            finite_matrix(test_df[no_depth_topology_cols].to_numpy(dtype=float)),
            hgb_params,
            seed + 550 + fold_number,
        )
        no_waveform_cols = [
            "depth_idx",
            "multiplicity",
        ...
        no_wave_train = train_df[no_waveform_cols].copy()
        no_wave_test = test_df[no_waveform_cols].copy()
        for col in ["calibrated_energy_mev_even", "pstar_depth_anchor_mev", "b2_amp", "b2_area", "even_total_charge", "b4_area", "b6_area", "b8_area"]:
            no_wave_train[col] = np.log1p(np.maximum(no_wave_train[col].to_numpy(dtype=float), 0.0))
            no_wave_test[col] = np.log1p(np.maximum(no_wave_test[col].to_numpy(dtype=float), 0.0))
        scores["knockout_no_waveform_gbt"][test] = fit_hgb_score(
            finite_matrix(no_wave_train.to_numpy(dtype=float)),
            train_y,
            finite_matrix(no_wave_test.to_numpy(dtype=float)),
            hgb_params,
    ...
    topology_eval = meta.loc[valid, "topology_code"].to_numpy(dtype=int)
    meta_eval = meta.loc[valid].copy().reset_index(drop=True)
    ...
    for i, (method, score_all) in enumerate(scores.items()):
        ...
        metrics.update(
            topology_block_ci(
                ...
                topology_eval,
                seed + 3000 + i,
                int(config["benchmark"].get("topology_bootstrap_replicates", n_boot)),
                fixed_eff,
            )
        )
    ...
    pairwise_rows = []
    trad_score = scores["traditional_deltae_depth_ridge"][valid]
    for method in HEAD_TO_HEAD_METHODS:
        if method == "traditional_deltae_depth_ridge":
            continue
        diff = paired_auc_diff(y_eval, trad_score, scores[method][valid], runs_eval, seed + 1000 + len(pairwise_rows), n_boot)
        pairwise_rows.append({"left": "traditional_deltae_depth_ridge", "right": method, **diff})
    ...
    meta_eval = add_support_audit_columns(meta_eval, config)
    support_summary, support_cells, support_annotations = support_collapse_tables(meta_eval, config)
    null_audit, matched_support_metrics = score_audit_rows(
        meta_eval,
        HEAD_TO_HEAD_METHODS,
        config,
        seed_offset=12000,
    )
    support_summary.to_csv(out_dir / "support_collapse_summary.csv", index=False)
    support_cells.to_csv(out_dir / "support_cells.csv", index=False)
    support_annotations.to_csv(out_dir / "support_cell_annotations.csv.gz", index=False)
    null_audit.to_csv(out_dir / "null_label_stability.csv", index=False)
    matched_support_metrics.to_csv(out_dir / "matched_support_method_metrics.csv", index=False)
    details["support_collapse_fraction"] = float(
        support_summary.loc[support_summary["quantity"] == "support_collapse_fraction", "fraction_of_evaluated"].iloc[0]
    )
    details["matched_support_rows"] = int(
        support_summary.loc[support_summary["quantity"] == "evaluated_rows_in_matched_support_cells", "rows"].iloc[0]
    )
    return metrics_df, pairwise, fold_counts, meta, support_summary, support_cells, null_audit, matched_support_metrics, details
...
def write_report(
    ...
    for _, row in support_fraction.iterrows():
        support_rows.append("| {quantity} | {rows:d} | {frac:.3f} |".format(quantity=row["quantity"], rows=int(row["rows"]), frac=float(row["fraction_of_raw_b2"])))
    support_collapse_rows = []
    for _, row in support_collapse.iterrows():
        support_collapse_rows.append(
            "| {quantity} | {rows:d} | {frac:.3f} |".format(
                quantity=row["quantity"],
                rows=int(row["rows"]),
                frac=float(row["fraction_of_evaluated"]),
            )
        )
    null_rows = []
    null_subset = null_audit[null_audit["method"].isin(["traditional_deltae_depth_ridge", result["winner"]["method"], "cnn_1d"])].copy()
    for _, row in null_subset.sort_values(["null_variant", "method"]).iterrows():
        null_rows.append(
            "| {variant} | {method} | {auc:.3f} [{lo:.3f}, {hi:.3f}] | {ap:.3f} | {ece:.3f} |".format(
                variant=row["null_variant"],
                method=row["method"],
                auc=row["roc_auc"],
    ...
    matched_rows = []
    for _, row in matched_support.sort_values("roc_auc", ascending=False).iterrows():
        matched_rows.append(
            "| {method} | {auc:.3f} [{lo:.3f}, {hi:.3f}] | {ap:.3f} | {pur:.3f} [{plo:.3f}, {phi:.3f}] | {collapse:.3f} |".format(
                method=row["method"],
                auc=row["roc_auc"],
                lo=row["roc_auc_ci_low"],
                hi=row["roc_auc_ci_high"],
                ap=row["average_precision"],
                pur=row["purity_at_fixed_efficiency"],
                plo=row["purity_ci_low"],
                phi=row["purity_ci_high"],
                collapse=row["support_collapse_fraction"],
            )
        )

    report = """# S15b: pulse-shape PID null-label stability audit
...
**{winner_method}** with ROC AUC {winner_auc:.3f} [{winner_lo:.3f},
{winner_hi:.3f}], but this is explicitly a weak-label winner, not a
...
{n_boot} run-block replicates. `method_metrics.csv` also includes topology-block
...
at fixed {eff:.0f}% positive-label efficiency using the global out-of-fold score
threshold.
...
| method | matched-support ROC AUC | AP | purity at {eff:.0f}% efficiency | support-collapse fraction |
|---|---:|---:|---:|---:|
{matched_table}
...
The point-estimate head-to-head winner is `{winner_method}`, but the
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/s15b_1781070978_487_042a7300_pid_null_label_stability_audit.json"))
    ...
    metrics, pairwise, fold_counts, benchmark_meta, support_collapse, support_cells, null_audit, matched_support, details = build_benchmark(waves, labeled, config, out_dir)
    ...
    valid_b2_rows = int(len(meta))
    support_fraction = pd.DataFrame(
        [
            {
                "quantity": "raw_b2_selected_rows",
                "rows": valid_b2_rows,
                "fraction_of_raw_b2": 1.0,
            },
            {
                "quantity": "weak_label_labeled_rows",
                "rows": int(len(labeled)),
                "fraction_of_raw_b2": float(len(labeled) / max(valid_b2_rows, 1)),
            },
            {
                "quantity": "balanced_head_to_head_rows",
                "rows": int(details["benchmark_rows_after_balancing"]),
                "fraction_of_raw_b2": float(details["benchmark_rows_after_balancing"] / max(valid_b2_rows, 1)),
            },
            {
                "quantity": "evaluated_oof_rows",
                "rows": int(details["evaluated_rows"]),
                "fraction_of_raw_b2": float(details["evaluated_rows"] / max(valid_b2_rows, 1)),
            },
        ]
    )
    support_fraction.to_csv(out_dir / "support_fraction_ledger.csv", index=False)
    ...
    for _, row in sent.iterrows():
        ...
        elif row["method"] == "sentinel_depth_only_ridge":
            interp = "Penetration-depth-only probe; high AUC means range support explains the weak axis."
        elif row["method"] == "sentinel_topology_only_ridge":
            interp = "Topology/saturation-only probe; high AUC means terminal topology or saturation explains the weak axis."
        ...
        elif row["method"] == "sentinel_target_permutation_gbt":
            interp = "Within-depth target permutation guard; should be near chance if label-source ordering is not leaked."
        elif row["method"] == "knockout_no_charge_gbt":
            interp = "Feature-family knockout removing charge summary terms; residual AUC estimates waveform/support-only separability."
        elif row["method"] == "knockout_no_depth_topology_gbt":
            interp = "Feature-family knockout removing explicit depth/topology terms; residual AUC estimates charge/waveform separability."
        elif row["method"] == "knockout_no_waveform_gbt":
            interp = "Feature-family knockout removing waveform samples; high AUC means conventional support variables are sufficient."
        else:
    ...
    result = {
        ...
        "winner": winner,
        "traditional": metrics[metrics["method"] == "traditional_deltae_depth_ridge"].iloc[0].to_dict(),
        "ml": metrics[metrics["method"] == winner["method"]].iloc[0].to_dict(),
        "ml_beats_baseline": bool(winner["method"] != "traditional_deltae_depth_ridge"),
        ...
        "accepted_support_fraction": support_fraction.to_dict(orient="records"),
        "support_collapse": {
            "summary": support_collapse.to_dict(orient="records"),
            "matched_support_rows": int(details["matched_support_rows"]),
            "support_collapse_fraction": float(details["support_collapse_fraction"]),
            "cell_count": int(len(support_cells)),
        },
        "null_label_stability": null_audit.to_dict(orient="records"),
        "matched_support_method_metrics": matched_support.to_dict(orient="records"),
        "primary_interpretation": (
            "This is a raw-HRD PID weak-label stability audit. The named winner is a weak-label "
            "benchmark winner only; adoption is blocked without particle truth labels and by the "
            "null-label, matched-support, and support/nuisance sentinel pattern documented in REPORT.md."
        ),
        "benchmark": details,
        "next_tickets": [
            {
                "title": "S15c: external PID truth join feasibility gate",
                "body": "Question: can any beamline, GEANT4, or external detector metadata provide event-level PID truth for the S15 weak-label rows? Expected information gain: separates real proton/deuteron PID validation from support-proxy closure before PID scores are reused."
            }
        ],
    ...
    write_report(out_dir, config, result, reproduction, metrics, pairwise, leakage, support_fraction, support_collapse, null_audit, matched_support)

    manifest = {
        "ticket_id": config["ticket_id"],
        "script": "scripts/s15b_1781070978_487_042a7300_pid_null_label_stability_audit.py",
