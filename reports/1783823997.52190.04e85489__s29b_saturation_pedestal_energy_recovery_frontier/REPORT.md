...
Ticket `1783823997.52190.04e85489` asks whether raw B-stack HRD waveforms support a stronger
...
Runtime was `341.5` s on `Linux-5.15.0-139-generic-x86_64-with-debian-bullseye-sid`.
...
The working hypothesis from this benchmark is that the useful information is not
generic waveform capacity but a low-dimensional residual correction around the
traditional saturation-knee/template fit.  The new residual stack wins because it
keeps the physics-facing fit as an inductive bias while allowing run-local
pedestal and overlap residuals to move the energy and timing estimates.  The
failure mode is also clear: the current truth is controlled injection truth, so a
dedicated electronics-metadata validation is needed before treating the method as
a production PID-boundary correction.

One follow-up was appended to the `testbeam` queue: `1783883266.48798.542b7156`,
`S29c: hardware pedestal-saturation validation with PID-boundary labels`.  It
asks whether the S29b gain survives against hardware saturation/pedestal metadata
and explicit downstream PID-boundary labels rather than amplitude-ceiling and
stave proxies.
