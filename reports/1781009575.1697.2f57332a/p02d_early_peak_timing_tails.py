...
score.  D_t, C_t, run id, event id, absolute amplitudes, and selected-stave
flags are excluded from the main ML feature matrix.
"""
...
## Reproducibility
```bash
uv run --with uproot --with numpy --with pandas --with scikit-learn python {out_dir / 'p02d_early_peak_timing_tails.py'} --config {out_dir / 'p02d_config.json'}
```
...
def main() -> int:
    ...
    parser.add_argument("--config", default="reports/1781009575.1697.2f57332a/p02d_config.json")
    ...
    result = {
        ...
        "traditional": {
            ...
            "metric": "leave-one-run-out ROC AUC on S07 D_t extreme timing-tail labels",
        ...
        "next_tickets": [
            ...
            "S07h: define a non-D_t timing-tail target for P02 morphology using injected or independent timing corruption labels."
        ],
    }
