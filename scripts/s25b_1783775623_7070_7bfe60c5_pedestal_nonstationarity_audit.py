...
against a supervised ML/NN panel under run-heldout splitting.  The new compact
sequence encoder uses transformer-style token mixing over the 18 waveform
samples and stave context.
"""
...
def fit_torch_methods(waves, meta, y, runs, train_mask, test_mask, config) -> List[pd.DataFrame]:
    ...
    methods = [
        ...
        ("NN_transformer_sequence_encoder_new", ResidualSqueezeCNN(staves.shape[1]), 3102),
    ]
    out = []
...
def main() -> int:
    ...
    parser.add_argument("--config", type=Path, default=Path("configs/s25b_1783775623_7070_7bfe60c5_pedestal_nonstationarity_audit.json"))
        ...
        "next_tickets": [
            {
                ...
                "body": "Validate S25b by deriving independent pedestal-drift labels from random-trigger/no-pulse windows and testing whether the winning waveform-shape discriminator still predicts them under run-heldout and time-blocked bootstrap CIs.",
            }
        ],
    }
