#!/usr/bin/env python3
"""P07j: duplicate-readout run-family action bands for natural B2 saturation.

The analysis reads raw B-stack ROOT files via the P07f duplicate extractor, reproduces the
P07f count/family anchors, converts per-run knee support into pass/correct/abstain/veto action
bands, and benchmarks a transparent per-run duplicate-knee policy against ridge, gradient-
boosted trees, MLP, a 1D CNN, and a residual gated CNN.  All supervised methods are
leave-one-run-out and exclude run/event identifiers plus all odd-channel variables.
"""
...
DEFAULT_CONFIG = ROOT / "configs/p07j_1781052602_655_4be6114e_saturation_knee_family_action_bands.json"
