...
CONFIG = ROOT / "configs/s43b_1784349946_602_171c4316_waveform_derivative_pulse_shape_timing_benchmark.json"
...
def load_base():
    spec = importlib.util.spec_from_file_location("s32a_base_for_s43b", BASE_SCRIPT)
    ...
    def feature_columns_with_derivatives(df: pd.DataFrame) -> list[str]:
        ...
        return cols + [c for c in DERIVATIVE_COLUMNS if c in df.columns]
...
def fit_derivative_gate_transformer(df: pd.DataFrame, config: dict, seed: int) -> np.ndarray:
    try:
        ...
        raise RuntimeError("torch is required for derivative_gate_transformer_new") from exc
...
def summarize_s43b(predictions: pd.DataFrame, config: dict, rng: np.random.Generator, base) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    delta_rows = []
    reference = "traditional_cfd_template_derivative"
...
def write_report(
    ...
    trad = metrics[metrics["method"].astype(str).eq("traditional_cfd_template_derivative")].iloc[0]
    ...
    text = f"""# S43b Waveform Derivative Pulse-Shape Timing Benchmark

## Abstract

Ticket `{config['ticket_id']}` asks whether waveform derivative and curvature
...
traditional derivative comparator obtains `{trad['sigma68_ns']:.4g} ns`
...
{md_table(strata, ['stratum', 'level', 'method', 'n', 'bias_ns', 'sigma68_ns', 'tail_fraction_abs_gt_5ns'], max_rows=220)}
...
def main() -> None:
    ...
    preds = {"traditional_cfd_template_derivative": traditional_derivative_prediction(data, base)}
    ...
    preds["derivative_gate_transformer_new"] = fit_derivative_gate_transformer(data, config, seed=int(config["random_seed"]) + 3)
    ...
    metrics, by_run, strata, deltas = summarize_s43b(predictions, config, rng, base)
