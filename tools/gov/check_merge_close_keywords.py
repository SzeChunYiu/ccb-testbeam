...
SAFE_RE = re.compile(r"(?i)\b(refs?|related|does not close)\s+#(\d+)")


def scan_text(text: str, policy: dict) -> list[dict]:
    findings = []
    for m in CLOSE_RE.finditer(text):
        # Skip if the match is part of an explicit "does not close"
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--text-file", type=Path, default=None, help="PR body / commit message file")
    p.add_argument("--text", default=None, help="Inline text to scan")
    p.add_argument("--allow-close", action="append", default=[], help="Issue numbers explicitly allowed to close")
    args = p.parse_args(argv)
    policy = json.loads((args.repo_root / "docs/contracts/SCIENTIFIC_ISSUE_COMPLETION_GATES.json").read_text(encoding="utf-8"))
    text = args.text or ""
    if args.text_file:
