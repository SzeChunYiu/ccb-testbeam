...
OUT = ROOT / "reports" / "1781078146.939.6c7536da__s10p_quiet_tau_method_disagreement_closure"
...
def quiet_tau_closure_panel(
    scores: pd.DataFrame,
    folds: pd.DataFrame,
    rng: np.random.Generator,
) -> pd.DataFrame:
    """Ticket-local S10p closure of traditional-quiet versus ML-positive tau scores.

    The traditional quiet-tau gate is intentionally conservative: it requires
    small fitted secondary fraction and little SSE improvement from a two-pulse
    model.  Learned methods are thresholded at their low-current 90th
    percentile, so candidate-rate deltas ask whether a method marks more
    high-current than low-current events after a low-current calibration.
    """
    low_runs = np.array(base.RUN_GROUPS["low_2nA"]["runs"], dtype=int)
    high_runs = np.array(base.RUN_GROUPS["high_20nA"]["runs"], dtype=int)
    trad_positive = (
        (scores["trad_secondary_fraction"].to_numpy(dtype=float) >= 0.03)
        | (scores["trad_score_sse_improvement"].to_numpy(dtype=float) >= 0.015)
    )
    trad_quiet = ~trad_positive

    def rate_delta(frame: pd.DataFrame, candidate_col: str) -> float:
        low = frame[frame["group"] == "low_2nA"][candidate_col]
        high = frame[frame["group"] == "high_20nA"][candidate_col]
        if len(low) == 0 or len(high) == 0:
            return float("nan")
        return float(high.mean() - low.mean())

    rows = []
    for method in ["traditional", "ridge", "gradient_boosted_trees", "mlp", "cnn1d", "residual_tcn"]:
        if method == "traditional":
            score_col = "trad_score_sse_improvement"
            frac_col = "trad_secondary_fraction"
            threshold = 0.015
            candidate = trad_positive
        else:
            score_col = f"{method}_overlap_score"
            frac_col = f"{method}_secondary_fraction"
            threshold = float(np.quantile(scores.loc[scores["group"] == "low_2nA", score_col], 0.90))
            candidate = scores[score_col].to_numpy(dtype=float) >= threshold
        temp = scores[["run", "group", score_col, frac_col]].copy()
        temp["candidate"] = candidate.astype(float)
        temp["quiet_disagreement"] = (candidate & trad_quiet).astype(float)
        point = rate_delta(temp, "candidate")
        quiet_point = rate_delta(temp, "quiet_disagreement")
        j_den = float(np.logical_or(candidate, trad_positive).sum())
        jaccard = float(np.logical_and(candidate, trad_positive).sum() / j_den) if j_den else float("nan")
        low_score = temp.loc[temp["group"] == "low_2nA", score_col].to_numpy(dtype=float)
        high_score = temp.loc[temp["group"] == "high_20nA", score_col].to_numpy(dtype=float)
        edist = float(energy_distance(low_score, high_score)) if len(low_score) and len(high_score) else float("nan")
        boot_rate = []
        boot_quiet = []
        boot_edist = []
        for _ in range(BOOTSTRAPS):
            ...
            for run in np.r_[
                ...
                part = temp[temp["run"] == int(run)]
                if len(part):
                    parts.append(part)
            if not parts:
                continue
            sample = pd.concat(parts, ignore_index=True)
            boot_rate.append(rate_delta(sample, "candidate"))
            boot_quiet.append(rate_delta(sample, "quiet_disagreement"))
            slo = sample.loc[sample["group"] == "low_2nA", score_col].to_numpy(dtype=float)
            shi = sample.loc[sample["group"] == "high_20nA", score_col].to_numpy(dtype=float)
            if len(slo) and len(shi):
                boot_edist.append(float(energy_distance(slo, shi)))
        if method == "traditional":
            brier = float("nan")
            brier_improvement = float("nan")
        else:
            brier = float(folds.loc[folds["method"] == method, "synthetic_holdout_brier"].mean())
            brier_improvement = 0.25 - brier
        rows.append(
            {
                "method": method,
                "tau_score": score_col,
                "candidate_rule": "traditional score>=0.015 or fraction>=0.03" if method == "traditional" else "method score >= low-current p90",
                "score_threshold": threshold,
                "candidate_rate_delta": point,
                "candidate_rate_ci_low": float(np.quantile(boot_rate, 0.025)) if boot_rate else float("nan"),
                "candidate_rate_ci_high": float(np.quantile(boot_rate, 0.975)) if boot_rate else float("nan"),
                "quiet_disagreement_rate_delta": quiet_point,
                "quiet_disagreement_ci_low": float(np.quantile(boot_quiet, 0.025)) if boot_quiet else float("nan"),
                "quiet_disagreement_ci_high": float(np.quantile(boot_quiet, 0.975)) if boot_quiet else float("nan"),
                "method_traditional_jaccard": jaccard,
                "support_shift_energy_distance": edist,
                "support_shift_ci_low": float(np.quantile(boot_edist, 0.025)) if boot_edist else float("nan"),
                "support_shift_ci_high": float(np.quantile(boot_edist, 0.975)) if boot_edist else float("nan"),
                "synthetic_brier": brier,
                "brier_improvement_vs_025": brier_improvement,
            }
        )
...
def add_truth_split_atoms(scores: pd.DataFrame) -> pd.DataFrame:
    """Attach transparent S10p atom labels without using run/current identifiers."""
    out = scores.copy()
...
def write_report(
    ...
    lines = [
        ...
        "",
        (
            "The S10p question is whether the S10e/S10f quiet-tau method disagreement is better explained by "
        ...
        "",
        (
            "The ticket-specific closure panel compares the bounded-template quiet-tau gate against each learned tau-like "
            "overlap score. The traditional positive gate is `secondary_fraction >= 0.03` or SSE-improvement `>= 0.015`; "
            "learned scores are thresholded at their low-current 90th percentile. The reported quiet-disagreement rate is "
            "the high-minus-low rate of events where the learned method is positive while the traditional gate is quiet."
        ),
        "",
        "## Results",
        "",
        ...
        table_md(
            ...
            [
                ...
                "candidate_rate_ci_low",
                "candidate_rate_ci_high",
                "quiet_disagreement_rate_delta",
                "quiet_disagreement_ci_low",
                "quiet_disagreement_ci_high",
                "method_traditional_jaccard",
                "support_shift_energy_distance",
                "brier_improvement_vs_025",
            ],
        ),
        "",
...
def main() -> int:
    ...
    quiet_tau = quiet_tau_closure_panel(scores, folds, rng)
    ...
    quiet_tau.to_csv(OUT / "quiet_tau_closure.csv", index=False)
    ...
    conclusion = (
        ...
        f"The largest learned quiet-tau disagreement rate delta is "
        f"{quiet_tau[quiet_tau['method'] != 'traditional'].sort_values('quiet_disagreement_rate_delta', ascending=False).iloc[0]['method']} at "
        f"{quiet_tau[quiet_tau['method'] != 'traditional'].sort_values('quiet_disagreement_rate_delta', ascending=False).iloc[0]['quiet_disagreement_rate_delta']:.5f}."
    )
    result = {
        "study": STUDY,
        "ticket": TICKET,
        "worker": WORKER,
        "title": "S10p quiet-tau method-disagreement closure",
        ...
        "quiet_tau_closure": quiet_tau.to_dict(orient="records"),
