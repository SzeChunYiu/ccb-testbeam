#!/usr/bin/env python3
"""S29A pedestal-shape timing frontier benchmark."""
...
def extract_dataset(config: dict) -> tuple[pd.DataFrame, np.ndarray, pd.DataFrame]:
    ...
    for run in runs(config):
        ...
        for batch in iter_batches(path):
            ...
            # Target combines duplicate-readout charge loss, delayed recovery tail, and onset timing bias.
            target = charge_loss + 0.18 * recovery_tail + 0.015 * timing_proxy
...
def add_frontier_diagnostics(summary: pd.DataFrame, events: pd.DataFrame, y: np.ndarray, preds: dict[str, np.ndarray], held: np.ndarray, config: dict) -> pd.DataFrame:
    rows = []
    residual_inputs = {method: pred - y for method, pred in preds.items()}
    sat_all = events["saturated_count"].to_numpy() > 0
    pid_all = events["pid_label"].to_numpy(dtype=int) > 0
    for _, row in summary.iterrows():
        method = str(row["method"])
        pred = preds[method]
        residual = residual_inputs[method]
        m = held & np.isfinite(pred)
        sat = sat_all & m
        unsat = (~sat_all) & m
        pid = pid_all & m
        ...
        diag = {
            "method": method,
            "timing_residual_res68": float(row["res68"]),
            "timing_residual_res68_ci95": row["res68_ci95"],
            "shape_error_mae": float(row["mae"]),
            "shape_error_mae_ci95": row["mae_ci95"],
            "energy_proxy_bias": float(row["bias"]),
            "energy_proxy_bias_ci95": row["bias_ci95"],
            "saturation_sensitivity": float(res68(y[sat], pred[sat]) - res68(y[unsat], pred[unsat])) if sat.sum() and unsat.sum() else float("nan"),
            "pid_leakage_abs_delta": abs(float(np.mean(residual[pid]) - np.mean(residual[nonpid]))) if pid.sum() and nonpid.sum() else float("nan"),
        }
        diag.update(frontier_diagnostic_bootstrap(events, y, pred, m, int(config["bootstrap_reps"]), int(config["random_seed"]) + 300 + len(method)))
        rows.append(diag)
    diagnostics = pd.DataFrame(rows)
    return summary.merge(diagnostics, on="method", how="left").sort_values(["timing_residual_res68", "shape_error_mae"]).reset_index(drop=True)


def frontier_diagnostic_bootstrap(events: pd.DataFrame, y: np.ndarray, pred: np.ndarray, mask: np.ndarray, reps: int, seed: int) -> dict:
    rng = np.random.default_rng(seed)
    idx0 = np.flatnonzero(mask)
    frame = events.iloc[idx0][["run"]].copy()
    frame["idx"] = idx0
    blocks = [g["idx"].to_numpy(dtype=int) for _, g in frame.groupby("run")]
    sat_sens = []
    pid_leak = []
    sat_all = events["saturated_count"].to_numpy() > 0
    pid_all = events["pid_label"].to_numpy(dtype=int) > 0
    ...
    for _ in range(reps):
        ...
        if sat.any() and (~sat).any():
            sat_sens.append(res68(y[idx[sat]], pred[idx[sat]]) - res68(y[idx[~sat]], pred[idx[~sat]]))
        if pid.any() and (~pid).any():
            pid_leak.append(abs(float(np.mean(residual[idx[pid]]) - np.mean(residual[idx[~pid]]))))
    return {
        "saturation_sensitivity_ci95": percentile_ci(sat_sens),
        "pid_leakage_abs_delta_ci95": percentile_ci(pid_leak),
    }
...
def write_report(out: Path, config: dict, result: dict, counts: pd.DataFrame, summary: pd.DataFrame, strata: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        "## Abstract",
        "",
        f"This study reproduces the {config['study_id']} B-stave selected-pulse number directly from raw ROOT and then benchmarks a traditional pedestal-subtracted constant-fraction/template timing baseline against ridge, gradient-boosted trees, MLP, 1D-CNN, a waveform transformer, and a new gated residual CNN. The winner is **{winner}** under held-out-run timing-residual res68 with run-block bootstrap confidence intervals.",
        "",
        ...
        "",
        markdown_table(summary, ["method", "n", "timing_residual_res68", "timing_residual_res68_ci95", "shape_error_mae", "shape_error_mae_ci95", "energy_proxy_bias", "energy_proxy_bias_ci95", "saturation_sensitivity", "saturation_sensitivity_ci95", "pid_leakage_abs_delta", "pid_leakage_abs_delta_ci95"]),
        "",
        ...
        "## Recommendation",
        "",
        f"The selected winner for `result.json` is `{winner}`. Saturated pulses should remain included only with a run-heldout pedestal/saturation correction and with explicit uncertainty inflation for high-recovery-tail and high-pedestal-drift strata; uncorrected saturated pulses should not be promoted into precision timing or energy closure tables.",
    ]
...
def main() -> None:
    ...
    preds["traditional_clipped_template"] = bounded_predict(trad, trad_x, y_cal)
    tail_x = x[:, [5, 6]]
    ...
    base_summary = score_rows(events, y, preds, held, config)
    summary = add_frontier_diagnostics(base_summary, events, y, preds, held, config)
    ...
    for name, smask in stratum_defs.items():
        for method in [winner_method, "traditional_clipped_template", "gradient_boosted_trees", "1d_cnn", "waveform_transformer", "gated_residual_cnn"]:
